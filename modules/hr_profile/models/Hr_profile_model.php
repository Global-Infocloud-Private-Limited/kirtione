...
class Hr_profile_model extends App_Model
{
	...
	public function staff_chart_by_departments_main_dashboard()
	{
		$chart = [];
		$color_data = ['#a48a9e', '#c6e1e8', '#648177' ,'#0d5ac1','#00FF7F', '#0cffe95c','#80da22','#f37b15','#da1818','#176cea','#5be4f0', '#57c4d8', '#a4d17a', '#225b8', '#be608b', '#96b00c', '#088baf',
		'#63b598', '#ce7d78', '#ea9e70' ,
		...
		$this->db->select('count('.db_prefix().'staff.staffid) as total_staff,'.db_prefix().'departments.name as department_name');
		$this->db->join(db_prefix() . 'departments', db_prefix() . 'departments.departmentid = ' . db_prefix() . 'staff.job_department', 'left');
		$this->db->group_by(db_prefix() . 'staff.job_department');
...
public function get_position_by_department($department_id = '', $status)
{
	...
	public function get_staff($id = '', $where = [])
	{
		...
		$select_str = db_prefix() . 'staff.*,CONCAT(firstname," ",lastname) as full_name,'.db_prefix() . 'accountbalances.BAL1';
...
//end file
   public function headquarter_table_data($data){
    ...
    public function get_AccountDetails($AccountID)
    {
        $selected_company = $this->session->userdata('root_company');
        $FY = $this->session->userdata('finacial_year');
        $this->db->select('staff.*,'.db_prefix() . 'accountbalances.BAL1');
        $this->db->from(db_prefix() . 'staff');
        //$this->db->where(db_prefix() . 'staff.PlantID', $selected_company);
        $this->db->where(db_prefix() . 'staff.AccountID', $AccountID);
        $this->db->join(db_prefix() . 'accountbalances', db_prefix() . 'accountbalances.AccountID = '.db_prefix() . 'staff.AccountID AND tblaccountbalances.PlantID = '.$selected_company.' AND tblaccountbalances.FY = '.$FY,'LEFT');
        $this->db->join('tblxx_citylist', 'tblxx_citylist.id = tblstaff.city ','LEFT');
        $this->db->join('tblstaff_wise_center', 'tblstaff_wise_center.AccountID = tblstaff.AccountID ','LEFT');
        $this->db->order_by(db_prefix() . 'staff.AccountID');
        $Data = $this->db->get()->row();
        if($Data){
            $CityList = $this->getCityList($Data->state);
            $Data->CityList = $CityList;
            $Data->Type = "staff";
            $DesignationList = $this->job_position_by_id($Data->job_department);
            $Data->DesignationList = $DesignationList;
            
            $AllCenters = $this->GetCenterList();
            $Data->AllCenters = $AllCenters;
            $CenterIDs = array();
            foreach($AllCenters as $key=>$val){
                array_push($CenterIDs,$val['CenterID']);
            }
            
            $AllItems = $this->GetCommodityListCenterWise($CenterIDs);
            $Data->AllItems = $AllItems;
            
            $AssignedCenterList = $this->GetStaffWiseCenterList($Data->AccountID);
            $Data->AssignedCenterList=$AssignedCenterList;
            
            $AssignedItemList = $this->GetStaffWiseItemList($Data->AccountID);
            $Data->AssignedItemList=$AssignedItemList;
            ...
            $this->db->order_by(db_prefix() . 'clients.AccountID');
            $Data2 = $this->db->get()->row();
            if($Data2){
                $Data->Type = "otherAccountID";
            }else{
                $this->db->select(db_prefix() . 'clients.*');
                $this->db->from(db_prefix() . 'clients');
                $this->db->where(db_prefix() . 'clients.AccountID', $AccountID);
    ...
    public function GetStaffWiseCenterList($AccountID){
        
        $this->db->select('tblstaff_wise_center.CenterID');
        $this->db->where('tblstaff_wise_center.AccountID', $AccountID);
        //print_r($this->db->get('tblstaff_wise_center')->result_array()); 
        return $this->db->get('tblstaff_wise_center')->result_array();
    }
    
    public function GetStaffWiseItemList($AccountID){
        
        $this->db->select('tblstaff_wise_items.ItemID');
        $this->db->where('tblstaff_wise_items.AccountID', $AccountID);
        //print_r($this->db->get('tblstaff_wise_center')->result_array()); 
        return $this->db->get('tblstaff_wise_items')->result_array();
    }
    ...
    public function GetAccountList($subGroupID)
    {
        $selected_company = $this->session->userdata('root_company');
        $this->db->select(db_prefix() . 'staff.*,tblroles.name AS RoleName,tblxx_citylist.city_name,tblhr_job_position.position_name,tbldepartments.name  AS DeptName');
        $this->db->from(db_prefix() . 'staff');
        $this->db->where(db_prefix() . 'staff.PlantID', $selected_company);
        $this->db->where('tbldepartments.departmentid !=', '11');
        $this->db->join('tblxx_citylist', 'tblxx_citylist.id = tblstaff.city ','LEFT');
        ...
        $this->db->join('tbldepartments', 'tbldepartments.departmentid = tblstaff.job_department ','LEFT');
        $this->db->order_by(db_prefix() . 'staff.AccountID');
        $Data = $this->db->get()->result_array();
        return $Data;
    }
    
    public function GetCenterWiseItems($CenterID)
    {
        $this->db->select(db_prefix() . 'Center_wise_item.ItemID,tblitems.ItemName');
        ...
        $this->db->group_by(db_prefix() . 'Center_wise_item.ItemID');
        $this->db->order_by(db_prefix() . 'items.ItemName');
        $Data = $this->db->get()->result_array();
        return $Data;
    }
    public function GetSubAdminList($subGroupID)
    {
        $selected_company = $this->session->userdata('root_company');
        $this->db->select(db_prefix() . 'staff.*,tblxx_citylist.city_name,tblhr_job_position.position_name,tbldepartments.name  AS DeptName');
        $this->db->from(db_prefix() . 'staff');
        $this->db->where(db_prefix() . 'staff.PlantID', $selected_company);
        $this->db->where('tbldepartments.departmentid', '11');
        $this->db->join('tblxx_citylist', 'tblxx_citylist.id = tblstaff.city ','LEFT');
...
//====================================================================
    ...
    public function SaveAccountDetails($Data)
    {
        ...
        $ItemListString = $Data["ItemID"];
        $password        = app_hash_password($Data['app_password']);
        ...
        $roleWisePermissionAssign= $role->permissions;
        $unset_array = unserialize($roleWisePermissionAssign);
        
        foreach ($unset_array as $group => $permissions) {
            foreach ($permissions as $permission) {
                $Assignrole = [
                    'feature' => $group,
                    'capability' => $permission,
                    'plant_id' =>$selected_company,
                    'staff_id' =>$LastId,
                    'year' =>$FY,
                    'days' =>0,
                    'Lupdate'=>date('Y-m-d H:i:s'),
                ...
                $this->db->insert(db_prefix() . 'staff_permissions', $Assignrole);
            }
        }
        ...
        if($LastId){
            ...
            // add multiple center staff wise
            foreach($Data["CenterID"] as $val)
            {
                $CenterData = array(
        		...
        		$this->db->insert(db_prefix() . 'staff_wise_center', $CenterData);
            }
            // Add multiple Item staff wise
    		foreach($ItemListString as $val){
    		    $ItemData = array(
    		        'TransDate'=>date('Y-m-d H:i:s'),
    		        'AccountID'=>strtoupper($AccountID),
    		        'ItemID'=>$val,
        		...
        		$this->db->insert(db_prefix() . 'staff_wise_items', $ItemData);
    		}
            return true;
        }else{
            return false;
    ...
    public function UpdateAccountDetails($Data)
    {
        ...
        /*$commodities = explode(",",$Data["ItemID"]);
        $Centers = explode(",",$Data["CenterID"]);
        $new = array(
       ...
       /* echo json_encode($StaffArray);
        ...
        if($this->db->affected_rows() > 0){
           $updatted++; 
           $this->db->where('AccountID', $AccountID);
           $this->db->delete(db_prefix().'staff_wise_center');
           
           $this->db->where('AccountID', $AccountID);
           $this->db->delete(db_prefix().'staff_wise_items');
           
           
            foreach($Data["CenterID"] as $val){
                $Assigned_Center_array = array(
                    'AccountID'=>strtoupper($Data["AccountID"]),
                    'CenterID'=>$val,
                    'TransDate' =>date('Y-m-d H:i:s'),
                    'UserID' =>$LogID,
                );
                $this->db->insert(db_prefix() . 'staff_wise_center',$Assigned_Center_array);
            }
            
            
            foreach($Data["ItemID"] as $val){
                $Assigned_Item_array = array(
                    'AccountID'=>strtoupper($Data["AccountID"]),
                    'ItemID'=>$val,
                    'TransDate' =>date('Y-m-d H:i:s'),
                    'UserID' =>$LogID,
                );
                $this->db->insert(db_prefix() . 'staff_wise_items',$Assigned_Item_array);
            }
        }
    ...
    public function SaveSubAdmin($Data)
    {
        $AccountID = $Data["AccountID"];
        $selected_company = $this->session->userdata('root_company');
        $FY = $this->session->userdata('finacial_year');
        ...
        $StaffArray = array(
            ...
            'active' =>$Data["active"],
            'app_access' =>$Data["app_access"],
            'StartDate' =>to_sql_date($Data["date_of_joining"]),
            'DeviceID' =>$Data["DeviceID"],
            'ifsc' =>$Data["ifsc"],
            'datecreated' =>date('Y-m-d H:i:s'),
            'UserID' =>$LogID,
        ...
        if($LastId){
            ...
            foreach($Centers as $val){
                ...
                $this->db->insert(db_prefix() . 'staff_wise_center',$Center_array);
            }
            
            $commodities = $Data["ItemID"];
            foreach($commodities as $val){
                ...
                $this->db->insert(db_prefix() . 'staff_wise_items',$Item_array);
            }
            $balArray = array(
                'PlantID' =>$selected_company,
                'AccountID' =>strtoupper($AccountID),
    ...
    public function GetCommodityListCenterWise($CenterIDs)
    {
        $this->db->select('tblitems.ItemName,tblCenter_wise_item.ItemID');
        $this->db->join('tblitems','tblitems.ItemID = tblCenter_wise_item.ItemID');
        $this->db->where('tblitems.isactive', 'Y');
        $this->db->where_in('tblCenter_wise_item.CenterID', $CenterIDs);
        $this->db->group_by('tblCenter_wise_item.ItemID');
        return $this->db->get('tblCenter_wise_item')->result_array();
    }
    
    public function GetCommodityList()
    {
        $this->db->select('tblitems.ItemName,tblitems.ItemID');
        $this->db->where('tblitems.isactive', 'Y');
        $this->db->group_by('tblitems.ItemID');
        return $this->db->get('tblitems')->result_array();
    }
    
    public function get_job_department($id = false)
	{
		if (is_numeric($id)) {
		...
		if ($id == false) {
			return $this->db->query('select * from '.db_prefix().'departments ORDER BY name ASC')->result_array();
		}
	}
	...
	public function GetCommodityByCenterID($CenterIDs)
    {
        $array = explode(',', $CenterIDs);
        $this->db->select(db_prefix() . 'CenterMaster.*');
        $this->db->where('CenterID', $array);
