...
class Hr_profile extends AdminController
{
	...
	public function setting()
	{
		...
		if(!has_permission_new('hrm_salary_type', '', 'view') ) {
			access_denied('hr_profile');
		}
...
public function update_staff_profile(){
  ...
  if ($data['id'] == '') {
	...
	public function AddEditStaff($id = '')
	{
		...
		$data['center'] = $this->hr_profile_model->getAllMandiDb();
		$data['state'] = $this->staff_model->get_state();
		$data['rootcompany'] = $this->clients_model->get_rootcompany();
        $data['job_department'] = $this->hr_profile_model->get_job_department();
        $data['role_assign'] = $this->hr_profile_model->roleAssign();
    ...
    /* Get Item list by CenterID / ajax */
    public function GetCenterWiseItems()
    {
        $CenterID = $this->input->post('CenterID');
        $ItemList = $this->hr_profile_model->GetCenterWiseItems($CenterID);
        echo json_encode($ItemList);
    }
    public function StaffListPopUp()
    {
        $subGroupID = $this->input->post('SUBAccountID');
        $account_subgroup_table = $this->hr_profile_model->GetAccountList($subGroupID);
        $html = "";
...
//====================================================================
    ...
    {
        $AccountDetails = $this->input->post();
        $AccountDetails = $this->hr_profile_model->SaveSubAdmin($AccountDetails);
        echo json_encode($AccountDetails);
    }
	...
	public function GetCommodityByCenterID()
	{
	    $CenterIDs = $this->input->post('CenterIDs');
	    $job_positions =$this->hr_profile_model->GetCommodityByCenterID($CenterIDs);
	    echo json_encode($job_positions);
	}
	
	public function AddEditSubAdmin($id = '')
	{
		...
		$data['list_staff'] = $this->staff_model->get();
		$data['table_data'] = $this->hr_profile_model->staff_table_dataNew();
		$data['CenterList'] = $this->hr_profile_model->GetCenterList();
		//$data['Staff_Wise_CenterList'] = $this->hr_profile_model->GetStaffWiseCenterList();
		$data['CommodityList'] = $this->hr_profile_model->GetCommodityList();
		$data['state'] = $this->staff_model->get_state();
		$data['rootcompany'] = $this->clients_model->get_rootcompany();
        $data['job_department'] = $this->hr_profile_model->get_job_department();
		$this->load->view('hr_record/AddEditSubAdmin', $data);
	}
	
	public function SubAdminListPopUp()
    {
        $subGroupID = $this->input->post('SUBAccountID');
        $account_subgroup_table = $this->hr_profile_model->GetSubAdminList($subGroupID);
        $html = "";
        foreach ($account_subgroup_table as $key => $value) {
            $html .= '<tr class="get_AccountID" data-id="'.$value["AccountID"].'">';
            $html .= '<td>'.$value["AccountID"].'</td>';
