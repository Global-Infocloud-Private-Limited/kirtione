        ...
        <div class="panel-body">
                        <nav aria-label="breadcrumb" >
    						<ol class="breadcrumb custombreadcrumb" style="background-color:#fff !important; margin-Bottom:0px !important;">
    							...
    							<li class="breadcrumb-item active" aria-current="page"><b>Contra List</b></li>
    						</ol>
    					</nav>
    					<hr class="hr_style">
          <?php $arrAtt = array();
...
$(document).ready(function(){
    <?= isset($contra_entry) ? '' : 'fetchNextVoucherNumber();';?>
  function load_data(from_date,to_date)
  {
    ...
    $('input[name="contra_date"]').on('change input', function () {
        <?= isset($contra_entry) ? '' : 'fetchNextVoucherNumber();';?>
    })
    function fetchNextVoucherNumber() {
        let date = $('input[name="contra_date"]').val();
        $('input[name="contra_number"]').val('Loading...');

        $.ajax({
            url:      '<?= admin_url('accounting/get_next_voucher_number'); ?>',
            type:     'POST',
            dataType: 'JSON',
            data:     { selected_date: date, passage_from: 'CONTRA' },
            success: function (r) {
                if (r && r.success) {
                    $('input[name="contra_number"]').val(r.voucher_number);
                } else {
                    var msg = (r && r.message) ? r.message : 'Unknown error';
                    $('input[name="contra_number"]').val('ERROR: ' + msg);
                }
            },
            error: function () {
                $('input[name="contra_number"]').val('AJAX ERROR');
            }
        });
    }
