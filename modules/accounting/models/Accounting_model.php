...
//=============================== Add new Journal ENtry Page ===================
    public function add_journal_entry($data)
    {
        ...
        $journal_date = to_sql_date($data['journal_date'])." ".date('H:i:s');
        $date = to_sql_date($data['journal_date']);
        $LastUniqueID = $this->generateNextVoucherIDNew($date, $selected_company, 'JOURNAL');
        $i = 1;
        foreach ($journal_entry as $key => $value) {
            if(empty($value[7])){
                $ttype = "C";
    ...
    // Get Accounts to show in journal page
    ...
    // Get All Clients table accounts to show in journal entry page
    ...
    // Get All staff to show in journal entry page
    ...
    public function searchAccounts($search = '', $limit = 20)
    {
        $selected_company = $this->session->userdata('root_company');
        $subgroup = array('1000017');
        ...
        if (!empty($search)) {
            ...
            $this->db->or_like('AccountID', $search);
            $this->db->group_end();
        }
    ...
    public function add_receipts_entry($data)
    {
		...
		foreach ($receipts_entry as $key => $value) {
		    if($value[0] != '' && $value[7] != ''){
    		    ...
    		    if(!empty($find)){
    		        return ['status' => false, 'message' => 'Batch No Present', 'data' => $value[7]];
    		        die;
    		    }
    ...
    // Get Receipts Entry ledger details
    ...
    /*public function add_contra_entry($data)
    {
        ...
        if(empty($get_result_to_cur_date)){
            $selected_company = $this->session->userdata('root_company');
            if($selected_company == 1){
                $new_contraNumber = get_option('next_contra_number_for_kirti');
            }
...
//==========================Update Contra entry ================================
    ...
    // Get Account ledger fpr contra pager
    ...
    public function fetchAccountsDatawithCrDr($filter_data = "")
    {
        $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        $this->db->select('tblaccountgroups.ActGroupID,tblaccountgroups.ActGroupName,tblaccountledger.TType,SUM(tblaccountledger.Amount) AS SumAmt');
        $this->db->join('tblclients', 'tblclients.ActGroupID = tblaccountgroups.ActGroupID');
        $this->db->join('tblaccountledger', 'tblaccountledger.AccountID = tblclients.AccountID AND tblaccountledger.PlantID = tblclients.PlantID');
        if($filter_data){
            $this->db->where_in('tblaccountgroups.ActGroupID', $filter_data["MainGroup"]);
        }
        ...
        $this->db->group_by('tblaccountgroups.ActGroupID,tblaccountledger.TType');
        return $this->db->get('tblaccountgroups')->result_array();
    }

    public function fetchAccountsDatawithOpnBal($filter_data = "")
    {
        $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        $this->db->select('tblaccountgroups.ActGroupID,tblaccountgroups.ActGroupName,SUM(tblaccountbalances.BAL1) AS SumAmt');
        $this->db->join('tblclients', 'tblclients.ActGroupID = tblaccountgroups.ActGroupID');
        $this->db->join('tblaccountbalances', 'tblaccountbalances.AccountID = tblclients.AccountID AND tblaccountbalances.PlantID = tblclients.PlantID');
        if($filter_data){
            $this->db->where_in('tblaccountgroups.ActGroupID', $filter_data["MainGroup"]);
        }
        ...
        $this->db->group_by('tblaccountgroups.ActGroupID');
        return $this->db->get('tblaccountgroups')->result_array();
    }

    public function fetchActDataSubGroup1ByMainGroup($filter_data = "")
    {
        $this->db->select('tblaccountgroupssub1.SubActGroupName,tblaccountgroupssub1.SubActGroupID1,tblaccountgroupssub1.ActGroupID');
        if($filter_data){
            $this->db->where_in('tblaccountgroupssub1.ActGroupID', $filter_data["MainGroup"]);
            if($filter_data['MainSubGroup']){
                $this->db->where_in('tblaccountgroupssub1.SubActGroupID1', $filter_data['MainSubGroup']);
            }
	...
	public function fetchActDataSubGroup1ByMainGroupwithCrDr($filter_data = "")
    {
	    $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        $this->db->select('tblaccountgroupssub1.SubActGroupName,tblaccountgroupssub1.SubActGroupID1,tblaccountledger.TType,SUM(tblaccountledger.Amount) AS SumAmt');
        $this->db->join('tblclients', 'tblclients.SubActGroupID1 = tblaccountgroupssub1.SubActGroupID1');
		$this->db->join('tblaccountledger', 'tblaccountledger.AccountID = tblclients.AccountID AND tblaccountledger.PlantID = tblclients.PlantID');
		if($filter_data){
            $this->db->where_in('tblaccountgroupssub1.ActGroupID', $filter_data["MainGroup"]);
            if($filter_data['MainSubGroup']){
		...
		$this->db->where('tblaccountledger.FY', $fy);
		$this->db->where('tblaccountledger.PartyID', "KASPL");
        $this->db->group_by('tblaccountgroupssub1.SubActGroupID1,tblaccountledger.TType');
        return $this->db->get('tblaccountgroupssub1')->result_array();
    }

    public function fetchActDataSubGroup1ByMainGroupwithOpnBal($filter_data = "")
    {
	    $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        $this->db->select('tblaccountgroupssub1.SubActGroupName,tblaccountgroupssub1.SubActGroupID1,SUM(tblaccountbalances.BAL1) AS SumAmt');
        $this->db->join('tblclients', 'tblclients.SubActGroupID1 = tblaccountgroupssub1.SubActGroupID1');
        $this->db->join('tblaccountbalances', 'tblaccountbalances.AccountID = tblclients.AccountID AND tblaccountbalances.PlantID = tblclients.PlantID');
		if($filter_data){
            $this->db->where_in('tblaccountgroupssub1.ActGroupID', $filter_data["MainGroup"]);
		...
		$this->db->where('tblaccountbalances.FY', $fy);
		$this->db->where('tblaccountbalances.PartyID', "KASPL");
        $this->db->group_by('tblaccountgroupssub1.SubActGroupID1');
        return $this->db->get('tblaccountgroupssub1')->result_array();
    }

    public function fetchActDataSubGroupByActSubGroup1($filter_data = "")
    {
        $this->db->select('tblaccountgroupssub.SubActGroupName,tblaccountgroupssub.SubActGroupID,tblaccountgroupssub.SubActGroupID1');
        if($filter_data["MainSubGroup"]){
            $this->db->where_in('tblaccountgroupssub.SubActGroupID1', $filter_data["MainSubGroup"]);
            if($filter_data['AccountSubGroupID2']){
                $this->db->where_in('tblaccountgroupssub.SubActGroupID', $filter_data['AccountSubGroupID2']);
            }
	...
	public function fetchActDataSubGroupByActSubGroup1withCrDr($filter_data = "")
    {
	    $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        $this->db->select('tblaccountgroupssub.SubActGroupName,tblaccountgroupssub.SubActGroupID,tblaccountgroupssub.SubActGroupID,tblaccountgroupssub.SubActGroupID,tblaccountledger.TType,SUM(tblaccountledger.Amount) AS SumAmt');
        $this->db->join('tblclients', 'tblclients.SubActGroupID = tblaccountgroupssub.SubActGroupID');
		$this->db->join('tblaccountledger', 'tblaccountledger.AccountID = tblclients.AccountID AND tblaccountledger.PlantID = tblclients.PlantID');
	   if($filter_data["MainSubGroup"]){
            $this->db->where_in('tblaccountgroupssub.SubActGroupID1', $filter_data["MainSubGroup"]);
            if($filter_data['AccountSubGroupID2']){
		...
		$this->db->where('tblaccountledger.FY', $fy);
		$this->db->where('tblaccountledger.PartyID', "KASPL");
        $this->db->group_by('tblaccountgroupssub.SubActGroupID,tblaccountledger.TType');
        return $this->db->get('tblaccountgroupssub')->result_array();
    }

	public function fetchActDataSubGroupByActSubGroup1withOpnBal($filter_data = "")
    {
	    $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        $this->db->select('tblaccountgroupssub.SubActGroupName,tblaccountgroupssub.SubActGroupID,SUM(tblaccountbalances.BAL1) AS SumAmt');
        $this->db->join('tblclients', 'tblclients.SubActGroupID = tblaccountgroupssub.SubActGroupID');
        $this->db->join('tblaccountbalances', 'tblaccountbalances.AccountID = tblclients.AccountID AND tblaccountbalances.PlantID = tblclients.PlantID');
	   if($filter_data["MainSubGroup"]){
            $this->db->where_in('tblaccountgroupssub.SubActGroupID1', $filter_data["MainSubGroup"]);
		...
		$this->db->where('tblaccountbalances.FY', $fy);
		$this->db->where('tblaccountbalances.PartyID', "KASPL");
        $this->db->group_by('tblaccountgroupssub.SubActGroupID');
        return $this->db->get('tblaccountgroupssub')->result_array();
    }

    public function fetchAccountledgerdata($filter_data = "")
    {
	    $fy = $this->session->userdata('finacial_year');
        $this->db->select('tblclients.SubActGroupID,tblclients.company,tblclients.AccountID');
        if($filter_data["AccountSubGroupID2"]){
            $this->db->where_in('tblclients.SubActGroupID', $filter_data["AccountSubGroupID2"]);
        }

        //$this->db->where('SubActGroupID', $ledActGroupID);
        $ledgerMasterData = $this->db->get('tblclients')->result_array();
		// Get CR DR data
		$this->db->select('tblaccountledger.AccountID,tblaccountledger.TType,sum(tblaccountledger.Amount) AS SumAmt');
		$this->db->from('tblaccountledger');
		$this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID', 'inner');
		$this->db->where('tblaccountledger.FY', $fy);
		$this->db->where('tblaccountledger.PartyID', "KASPL");
		if ($filter_data["AccountSubGroupID2"]) {
             $this->db->where_in('tblclients.SubActGroupID', $filter_data["AccountSubGroupID2"]);
		...
		$this->db->group_by('tblaccountledger.AccountID,tblaccountledger.TType');
		$this->db->order_by('tblaccountledger.AccountID');
		$CDData = $this->db->get()->result_array();
		// Get CR DR data
		$this->db->select('tblaccountbalances.AccountID,tblaccountbalances.BAL1');
		$this->db->from('tblaccountbalances');
		$this->db->join('tblclients', 'tblclients.AccountID = tblaccountbalances.AccountID AND tblclients.PlantID = tblaccountbalances.PlantID', 'inner');
		$this->db->where('tblaccountbalances.FY', $fy);
		$this->db->where('tblaccountbalances.PartyID', "KASPL");
		if ($filter_data["AccountSubGroupID2"]) {
             $this->db->where_in('tblclients.SubActGroupID', $filter_data["AccountSubGroupID2"]);
		...
		$this->db->order_by('tblaccountbalances.AccountID');
		$OpnBalData = $this->db->get()->result_array();
		$i = 0;
		foreach ($ledgerMasterData as $key=>$val) {
			$cr = 0;
			$dr = 0;
			$opn = 0;
			foreach($CDData as $cdKey=>$cdVal){
				if($val["AccountID"]==$cdVal["AccountID"] && $cdVal["TType"]=="C"){
					$cr = $cdVal["SumAmt"];
				}

				if($val["AccountID"]==$cdVal["AccountID"] && $cdVal["TType"]=="D"){
					$dr = $cdVal["SumAmt"];
				}

			}

			foreach($OpnBalData as $oKey=>$oVal){
			...
			$balance = $opn + $cr - $dr;
			$ledgerMasterData[$i]["balance"] = $balance;
			$i++;
		}
    ...
    public function fetchsubAccountsData()
    {
        $this->db->select('tblaccountgroupssub.SubActGroupName,tblaccountgroupssub.SubActGroupID');
        return $this->db->get('tblaccountgroupssub')->result_array();
    }

    /*public function GetSubGroup1ByMainGroup($mainGroupID)
    {
        $this->db->select('tblaccountgroupssub1.SubActGroupID1,tblaccountgroupssub1.SubActGroupName');
        $this->db->where_in('ActGroupID', $mainGroupID);
        return $this->db->get('tblaccountgroupssub1')->result_array();
    }*/
    public function GetActSubGroupID2ByActSubGroup1($subGroupID)
    {
        if($subGroupID){
            $this->db->select('tblaccountgroupssub.SubActGroupName,tblaccountgroupssub.SubActGroupID');
            $this->db->where_in('SubActGroupID1', $subGroupID);
            return $this->db->get('tblaccountgroupssub')->result_array();
        }
    ...
    ////Trial balance sheet
    public function fetchAccountsDatatrail($BalanceSheet_head)
    {
        $this->db->select('tblaccountgroups.ActGroupName,tblaccountgroups.ActGroupID');
    ...
    public function fetchAccountsDataSubGrouptrail($mainGroupID)
    {
    ...
    public function fetchAccountsDataSubGrouptrail1($subGroupID)
    {
        $this->db->select('tblaccountgroupssub.SubActGroupName,tblaccountgroupssub.SubActGroupID');
    ...
    public function GetCommodityWiseLedgerData($BalanceSheet_head)
    {
        $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        // Excluding Trade Payables - Vendor and Trade Receivables - Party
        $Trade_pay_rec = array("1000006","1000016");
        //$Ledger_data = array();
        $this->db->select('SUM(tblaccountledger.Amount) AS SUMAmt,tblaccountledger.TType,tblclients.SubActGroupID,tblaccountledger.FY,tblitems.subgroup_id');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID');
        $this->db->join('tblitems', 'tblitems.ItemID = tblaccountledger.CommodityID',"LEFT");
        $this->db->where_in('tblclients.ActGroupID', $BalanceSheet_head["MainGroup"]);
        $this->db->where_not_in('tblclients.SubActGroupID', $Trade_pay_rec);
        $this->db->where('tblaccountledger.FY', $fy);
        $this->db->where('tblaccountledger.PlantID', $selected_company);
        $this->db->group_by('tblaccountledger.TType,tblclients.SubActGroupID,tblitems.subgroup_id');
        $CurrentYrLedger_data = $this->db->get('tblaccountledger')->result_array();
        return $CurrentYrLedger_data;
    }

    public function GetPay_Rec_CommodityWiseledgerDate()
    {
        $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        // Get Commodity Group
        $mainGroupID = array('1','2','3');
        $this->db->select('tblitems_sub_groups.id,tblitems_sub_groups.name,tblitems_sub_groups.ShortCode');
        ...
        // Only Trade Payables - Vendor and Trade Receivables - Party
        $Trade_pay_rec = array("1000006","1000016");
        $this->db->select('SUM(tblaccountledger.Amount) AS SUMAmt,tblaccountledger.TType,tblclients.SubActGroupID,tblaccountledger.AccountID,tblaccountledger.FY,tblitems.subgroup_id');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->join('tblitems', 'tblitems.ItemID = tblaccountledger.CommodityID',"LEFT");
        $this->db->where_in('tblclients.SubActGroupID', $Trade_pay_rec);
        $this->db->where('tblaccountledger.FY', $fy);
        $this->db->where('tblaccountledger.PlantID', $selected_company);
        $this->db->group_by('tblaccountledger.TType,tblclients.AccountID,tblitems.subgroup_id');
        $CurrentYr_p_r_Ledger_data = $this->db->get('tblaccountledger')->result_array();
        $i = 0;
        foreach($ItemGroup as $key=>$val){
            $ItemGroup[$i]["TradePayable"] = 0;
            $ItemGroup[$i]["TradeReceivable"] = 0;
            $i++;
        }
        ...
        foreach($ItemGroup as $key=>$val){
            ...
            $D = 0;
            $bal = 0;
            foreach($CurrentYr_p_r_Ledger_data as $LKey=>$LVal){
                if($val["id"]==$LVal["subgroup_id"] && $LVal["TType"] == "C"){
                    $C += $LVal["SUMAmt"];
                }

                if($val["id"]==$LVal["subgroup_id"] && $LVal["TType"] == "D"){
                    $D += $LVal["SUMAmt"];
                }
            ...
            $bal = $D - $C;
            if($bal > 0){
                $ItemGroup[$j]['TradeReceivable'] = $bal;
            }else{
                $ItemGroup[$j]['TradePayable'] = $bal;
            }
            ...
            $bal = 0;
            foreach($CurrentYr_p_r_Ledger_data as $LKey=>$LVal){
                if($LVal["subgroup_id"] == "" && $LVal["TType"] == "C"){
                    $C += $LVal["SUMAmt"];
                }

                if($LVal["subgroup_id"] == "" && $LVal["TType"] == "D"){
                    $D += $LVal["SUMAmt"];
                }
            ...
            $bal = $D - $C;
        $pay_rec_data['ItemGroupWise'] = $ItemGroup;
        $pay_rec_data['Unallocated'] = $bal;
        return $pay_rec_data;
    }

    public function GetPay_Rec_ledgerDate()
    {
        $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        // Only Trade Payables - Vendor and Trade Receivables - Party
        $Trade_pay_rec = array("1000006","1000016");
        $this->db->select('SUM(tblaccountledger.Amount) AS SUMAmt,tblaccountledger.TType,tblclients.SubActGroupID,tblaccountledger.AccountID,tblaccountledger.FY');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where_in('tblclients.SubActGroupID', $Trade_pay_rec);
        $this->db->where('tblaccountledger.FY', $fy);
        $this->db->where('tblaccountledger.PlantID', $selected_company);
        ...
        $this->db->group_by('tblaccountledger.TType,tblclients.AccountID');
        $CurrentYr_p_r_Ledger_data = $this->db->get('tblaccountledger')->result_array();
        $P_R_Ledger_data->Cur_yr_p_r_ledger = $CurrentYr_p_r_Ledger_data;
        // Privius year ledger
        $last_fy = $fy - 1;
        $this->db->select('SUM(tblaccountledger.Amount) AS SUMAmt,tblaccountledger.TType,tblclients.SubActGroupID,tblaccountledger.AccountID,tblaccountledger.FY');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID');
        ...
        $lastYr_p_r_Ledger_data = $this->db->get('tblaccountledger')->result_array();
        $P_R_Ledger_data->Last_yr_p_r_ledger = $lastYr_p_r_Ledger_data;
        return $P_R_Ledger_data;
    }

    public function GetPay_Rec_OpnBalData()
    {
        $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        // Only Trade Payables - Vendor and Trade Receivables - Party
        $Trade_pay_rec = array("1000006","1000016");
        $this->db->select('SUM(tblaccountbalances.BAL1) AS SUMAmt,tblclients.SubActGroupID,tblaccountbalances.AccountID,tblaccountbalances.FY');
        ...
        $this->db->group_by('tblaccountbalances.AccountID');
        $CurrentYr_P_R_OpnBal = $this->db->get('tblaccountbalances')->result_array();
        $P_R_OpnBal_data->Cur_yr_P_R_OpnBal = $CurrentYr_P_R_OpnBal;
        // Privius year ledger
        $last_fy = $fy - 1;
        $this->db->select('SUM(tblaccountbalances.BAL1) AS SUMAmt,tblclients.SubActGroupID,tblaccountbalances.AccountID,tblaccountbalances.FY');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountbalances.AccountID');
        ...
        $this->db->group_by('tblaccountbalances.AccountID');
        $CurrentYr_P_R_OpnBal = $this->db->get('tblaccountbalances')->result_array();
        $P_R_OpnBal_data->Last_yr_P_R_OpnBal = $CurrentYr_P_R_OpnBal;
        return $P_R_OpnBal_data;
    }

    public function GetPay_Rec_Accounts()
    {
        $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        // Only Trade Payables - Vendor and Trade Receivables - Party
        $Trade_pay_rec = array("1000006","1000016");
        $this->db->select('tblclients.AccountID,tblclients.CustomerType');
        $this->db->where_in('tblclients.SubActGroupID', $Trade_pay_rec);
        $this->db->where('tblclients.PlantID', $selected_company);
        $this->db->group_by('tblclients.AccountID');
        $AccountsList = $this->db->get('tblclients')->result_array();
        return $AccountsList;
    }
    ...
    public function GetStaffAccounts()
    {
        ...
        $StaffAccountsList = $this->db->get('tblstaff')->result_array();
    ...
    public function GetCurrentTax()
    {
        ...
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where(db_prefix() . 'clients.SubActGroupID', "1000008");
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        $this->db->where_not_in(db_prefix() . 'accountledger.PassedFrom', "PAYMENTS");
        $this->db->group_by(db_prefix() . 'accountledger.TType');
        $data =  $this->db->get(db_prefix() . 'accountledger')->result_array();
        return $data;
    }
	...
	public function GetTaxExpLedgerWiseCommodityWise($CenterID = "")
	{
		$fy = $this->session->userdata('finacial_year');
		$selected_company = $this->session->userdata('root_company');
		$last_fy = $fy - 1;
		$years = array($fy,$last_fy);
		$DefTaxGroup = array("100042");
		// Ledger Entry
		$this->db->select('SUM(tblaccountledger.Amount) AS SumAmt,tblaccountledger.TType,tblaccountledger.FY,
		tblclients.AccountID,tblclients.company,tblitems.subgroup_id,tblclients.SubActGroupID1,tblaccountgroupssub1.SubActGroupName AS SubActGroupName1,
		tblaccountgroupssub.SubActGroupName AS SubActGroupName2,tblclients.SubActGroupID');
		$this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
		$this->db->join('tblaccountgroupssub1', 'tblaccountgroupssub1.SubActGroupID1 = tblclients.SubActGroupID1');
		$this->db->join('tblaccountgroupssub', 'tblaccountgroupssub.SubActGroupID = tblclients.SubActGroupID');
		$this->db->join('tblitems', 'tblitems.ItemID = tblaccountledger.CommodityID AND tblitems.PlantID = tblaccountledger.PlantID',"LEFT");
		$this->db->where_in(db_prefix() . 'accountledger.FY', $years);
		$this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
		$this->db->where(db_prefix() . 'accountledger.PartyID', "KASPL");
		if($CenterID){
		    $this->db->where(db_prefix() . 'accountledger.CenterID', $CenterID);
		}

		$this->db->where_in(db_prefix() . 'clients.SubActGroupID1', $DefTaxGroup);
		$this->db->group_by('tblaccountledger.TType,tblaccountledger.AccountID,tblitems.subgroup_id,tblaccountledger.FY');
		$CRDRActWise =  $this->db->get(db_prefix() . 'accountledger')->result_array();
		return $CRDRActWise;
	}
    ...
    public function GetDirectExpLedgerWiseCommodityWise($CenterID = "")
	{
		$fy = $this->session->userdata('finacial_year');
		$selected_company = $this->session->userdata('root_company');
		$last_fy = $fy - 1;
	...
	public function GetOthIncomeLedgerWiseCommodityWise($CenterID = "")
	{
		$fy = $this->session->userdata('finacial_year');
		$selected_company = $this->session->userdata('root_company');
		$last_fy = $fy - 1;
	...
	public function GetIndExpLedgerWiseCommodityWise($CenterID = "")
	{
		$fy = $this->session->userdata('finacial_year');
		$selected_company = $this->session->userdata('root_company');
		$last_fy = $fy - 1;
		...
		$this->db->where_not_in(db_prefix() . 'clients.SubActGroupID1', $ActSubGroup1);
		$this->db->group_by('tblaccountledger.TType,tblaccountledger.AccountID,tblitems.subgroup_id,tblaccountledger.FY');
		$CRDRActWise =  $this->db->get(db_prefix() . 'accountledger')->result_array();
		return $CRDRActWise;
	}

	public function GetFinCostLedgerWiseCommodityWise($CenterID = "")
	{
		$fy = $this->session->userdata('finacial_year');
		$selected_company = $this->session->userdata('root_company');
		$last_fy = $fy - 1;
	...
	public function GetEmpBenLedgerWiseCommodityWise($CenterID = "")
	{
		$fy = $this->session->userdata('finacial_year');
		$selected_company = $this->session->userdata('root_company');
		$last_fy = $fy - 1;
	...
	public function GetAssetsLedgerWiseCommodityWise($CenterID = "")
	{
		$fy = $this->session->userdata('finacial_year');
		$selected_company = $this->session->userdata('root_company');
		$last_fy = $fy - 1;
		...
		$this->db->where(db_prefix() . 'accountledger.PartyID', "KASPL");
		if($CenterID){
		    $this->db->where(db_prefix() . 'accountledger.PartyID', $CenterID);
		}

		$this->db->where_in(db_prefix() . 'clients.ActGroupID', "10035");
		$this->db->group_by('tblaccountledger.TType,tblaccountledger.AccountID,tblitems.subgroup_id,tblaccountledger.FY');
		$CRDRActWise =  $this->db->get(db_prefix() . 'accountledger')->result_array();
		return $CRDRActWise;
	}

	public function GetLiaLedgerWiseCommodityWise($CenterID = "")
	{
		$fy = $this->session->userdata('finacial_year');
		$selected_company = $this->session->userdata('root_company');
		$last_fy = $fy - 1;
		...
		$this->db->where_in(db_prefix() . 'clients.ActGroupID', "10000");
		$this->db->group_by('tblaccountledger.TType,tblaccountledger.AccountID,tblitems.subgroup_id,tblaccountledger.FY');
		$CRDRActWise =  $this->db->get(db_prefix() . 'accountledger')->result_array();
		return $CRDRActWise;
	}
	...
	public function GetDepreCostLedgerWiseCommodityWise()
	{
		$fy = $this->session->userdata('finacial_year');
		$selected_company = $this->session->userdata('root_company');
		$last_fy = $fy - 1;
		...
		$this->db->where_in(db_prefix() . 'clients.SubActGroupID1', "100026");
		$this->db->group_by('tblaccountledger.TType,tblaccountledger.AccountID,tblitems.subgroup_id,tblaccountledger.FY');
		$CRDRActWise =  $this->db->get(db_prefix() . 'accountledger')->result_array();
		return $CRDRActWise;
	}
    ...
    public function GetCalCOGS()
    {
        ...
        // Beginning Inventory
        $this->db->select('SUM(tblstockmaster.OQty * tblstockmaster.AvgRate) AS OpnCost');
        ...
        $OpnStock =  $this->db->get(db_prefix() . 'stockmaster')->row();
        // Purchase and Purchase return
        $this->db->select('SUM(tblhistory.BilledQty * tblhistory.final_rate) AS TotalPurchAmt');
        ...
        $Purchase =  $this->db->get(db_prefix() . 'history')->row();
        $TotalPurchase = $Purchase->TotalPurchAmt ;
        // Direct Expenses
        $this->db->select('SUM(tblaccountledger.Amount) AS SumAmt,tblaccountledger.TType');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        ...
        $this->db->select('tblhistory.ItemID,tblhistory.final_rate,tblhistory.BilledQty,tblhistory.cgst,tblhistory.sgst,tblhistory.igst');
        $this->db->where(db_prefix() . 'history.PlantID', $selected_company);
        $this->db->where(db_prefix() . 'history.FY', $fy);
        $this->db->where(db_prefix() . 'history.TType', "P");
        $this->db->where(db_prefix() . 'history.TType2', "Purchase");
        ...
        $this->db->select('tblhistory.ItemID,SUM(tblhistory.BilledQty) AS QtySum');
        $this->db->where(db_prefix() . 'history.PlantID', $selected_company);
        $this->db->where(db_prefix() . 'history.FY', $fy);
                ...
                if($PVal["ItemID"] == $SVal["ItemID"]){
                    ...
                    if($SVal["QtySum"] > 0){
                        ...
                        if($SaleQty <= $PurchQty){
                            ...
                            $GstPer = $PVal["cgst"] + $PVal["sgst"] + $PVal["igst"];
                            if($GstPer == null){
                                $GstPer = 0;
                            }

                            $GstAmt = ($Amt * $GstPer)/100;
                            $NetAmt = $Amt;
                            /*echo $Amt;
                            echo "<br>";*/
                            $PurchaseAmt += $NetAmt;
        ...
        $OQtyAmt = $OpnStock->OpnCost;
        $ClsAmt = $OQtyAmt + $PurchaseAmt;
        $COGS = $OQtyAmt + $TotalPurchase + $DirectExpbal - $ClsAmt;
        /*echo "<pre>";
        echo "opn -".$OQtyAmt;
        echo "<br>";
        echo "purchase amt ". $TotalPurchase;
        echo "<br>";
        echo "direct exp - ".$DirectExpbal;
        echo "<br>";
        echo " cls amt - ".$ClsAmt;
        echo "<br>";
        echo "<br>";
    ...
    // Get Sale Center List
        $this->db->select('tblsalesmaster.CenterID,tblCenterMaster.CenterName');
        $this->db->join(' tblCenterMaster', ' tblCenterMaster.CenterID = tblsalesmaster.CenterID');
        $this->db->where_in(db_prefix() . 'salesmaster.FY', $fy);
        $this->db->where_in(db_prefix() . 'salesmaster.PlantID', $selected_company);
        $CenterList =  $this->db->get(db_prefix() . 'salesmaster')->result_array();
    }
    ...
    public function GetOperatingActivity()
    {
        $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        // Cash Sales
        $OperatingActivity->CashSell = 0;
        // Reciept from Debitors and Direct Income
        // Trade Payables - Vendor and Trade Receivables - Party
        $Trade_pay_rec = array("1000006","1000016");
        $this->db->select('SUM(' .db_prefix() .'accountledger.Amount) as AMT');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID', $Trade_pay_rec);
        $this->db->where(db_prefix() .'accountledger.PassedFrom', "RECEIPTS");
        $this->db->where(db_prefix() .'accountledger.TType', "C");
        $Pay_rec_ReceiptsAmt = $this->db->get(db_prefix() . 'accountledger')->row();
        // Get Direct Income
        $this->db->select('SUM(' .db_prefix() .'accountledger.Amount) as AMT');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->where(db_prefix() . 'clients.ActGroupID', "10011");
        $this->db->where(db_prefix() .'accountledger.PassedFrom', "RECEIPTS");
        $this->db->where(db_prefix() .'accountledger.TType', "C");
        $DirectIncome = $this->db->get(db_prefix() . 'accountledger')->row();
        $recFromDebitors = $Pay_rec_ReceiptsAmt->AMT + $DirectIncome->AMT;
        $OperatingActivity->ReceiptFromDeb = $recFromDebitors;
        // Cash Purchase
        $OperatingActivity->CashPurchase = 0;
        // Paid to creditors and direct expenses
        // Trade Payables - Vendor and Trade Receivables - Party
        $this->db->select('IFNULL(SUM(' .db_prefix() .'accountledger.Amount),0) as AMT');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID', $Trade_pay_rec);
        ...
        $Pay_rec_PaymentsAmt = $this->db->get(db_prefix() . 'accountledger')->row();
        // Get Direct Expenses
        $this->db->select('IFNULL(SUM(' .db_prefix() .'accountledger.Amount),0) as AMT');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        ...
        $this->db->where(db_prefix() .'accountledger.PassedFrom', "PAYMENTS");
        $this->db->where(db_prefix() .'accountledger.TType', "D");
        $DirectExpense = $this->db->get(db_prefix() . 'accountledger')->row();
        $PaidToCreditors = $Pay_rec_PaymentsAmt->AMT + $DirectExpense->AMT;
        $OperatingActivity->PaidToCreditors = $PaidToCreditors;
        // Income Tax Paid
        $this->db->select('IFNULL(SUM(' .db_prefix() .'accountledger.Amount),0) as AMT');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        ...
        $this->db->where(db_prefix() .'accountledger.PassedFrom', "PAYMENTS");
        $this->db->where(db_prefix() .'accountledger.TType', "D");
        $TaxPaymentsAmt = $this->db->get(db_prefix() . 'accountledger')->row();
        $OperatingActivity->IncomeTaxPaid = $TaxPaymentsAmt->AMT;
        // Indirect Expenses
        // Indirect Expenses except finance cost(100030)
        $Ind_Exp = array('10018');
        $this->db->select('tblaccountgroupssub1.SubActGroupID1');
        $this->db->where_in(db_prefix() .'accountgroupssub1.ActGroupID', $Ind_Exp);
        $ind_exp_group1 = $this->db->get(db_prefix() . 'accountgroupssub1')->result_array();
        $ind_exp_group1_exp_fin_cost = array();
        foreach($ind_exp_group1 as $key=>$val){
            array_push($ind_exp_group1_exp_fin_cost,$val["SubActGroupID1"]);
        }
        ...
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID1', $ind_exp_group1_exp_fin_cost);
        $this->db->where(db_prefix() .'accountledger.PassedFrom', "PAYMENTS");
        $this->db->where(db_prefix() .'accountledger.TType', "D");
        $IndExpAmt = $this->db->get(db_prefix() . 'accountledger')->row();
        $OperatingActivity->IndExpAmt = $IndExpAmt->AMT;
        return $OperatingActivity;
    }

    public function GetInvestingActivity()
    {
        $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        // Fixed assets group
        // Tangible Assets,Intangible Assets,Capital Work in Progress, Intangible Assets under development
        $fixed_assets = array("1000010","1000011","1000012","1000013");
        // Sale of Fixed Assets
        $this->db->select('tblaccountledger.VoucherID');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID', $fixed_assets);
        $this->db->where(db_prefix() .'accountledger.TType', "C");
        $sale_fixed_assets_cr_entry = $this->db->get(db_prefix() . 'accountledger')->result_array();
        $salefixed_assets_cr_voucher = array();
        foreach($sale_fixed_assets_cr_entry as $key=>$val){
            array_push($salefixed_assets_cr_voucher,$val["VoucherID"]);
        }

        if($salefixed_assets_cr_voucher){
            $this->db->select('IFNULL(SUM(' .db_prefix() .'accountledger.Amount),0) as AMT');
            $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
            $this->db->where(db_prefix() . 'accountledger.FY', $fy);
            $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
            $this->db->where_in(db_prefix() . 'clients.SubActGroupID', '1000017');
            $this->db->where_in(db_prefix() .'accountledger.VoucherID', $salefixed_assets_cr_voucher);
            $this->db->where(db_prefix() .'accountledger.TType', "D");
            $SaleFixAssetsAmt = $this->db->get(db_prefix() . 'accountledger')->row();
            $InvestingActivity->SaleFixAssetsAmt = $SaleFixAssetsAmt->AMT;
        }else{
            $InvestingActivity->SaleFixAssetsAmt = 0.00;
        }
        ...
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID', $fixed_assets);
        $this->db->where(db_prefix() .'accountledger.TType', "D");
        $pur_fixed_assets_cr_entry = $this->db->get(db_prefix() . 'accountledger')->result_array();
        $purfixed_assets_cr_voucher = array();
        foreach($pur_fixed_assets_cr_entry as $key=>$val){
            array_push($purfixed_assets_cr_voucher,$val["VoucherID"]);
        }

        if($purfixed_assets_cr_voucher){
            $this->db->select('IFNULL(SUM(' .db_prefix() .'accountledger.Amount),0) as AMT');
            $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
            $this->db->where(db_prefix() . 'accountledger.FY', $fy);
            $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
            $this->db->where_in(db_prefix() . 'clients.SubActGroupID', '1000017');
            $this->db->where_in(db_prefix() .'accountledger.VoucherID', $purfixed_assets_cr_voucher);
            $this->db->where(db_prefix() .'accountledger.TType', "C");
            $purFixAssetsAmt = $this->db->get(db_prefix() . 'accountledger')->row();
            $InvestingActivity->purFixAssetsAmt = $purFixAssetsAmt->AMT;
        }else{
            $InvestingActivity->purFixAssetsAmt = 0.00;
        }

        // long-term Investments group
        // Long term investments
        $long_term_inv = array("1000063");
        // Sale of long-term Investments
        $this->db->select('tblaccountledger.VoucherID');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID', $long_term_inv);
        $this->db->where(db_prefix() .'accountledger.TType', "C");
        $salelong_term_inv_cr_entry = $this->db->get(db_prefix() . 'accountledger')->result_array();
        $salelong_term_inv_cr_voucher = array();
        foreach($salelong_term_inv_cr_entry as $key=>$val){
            array_push($salelong_term_inv_cr_voucher,$val["VoucherID"]);
        }

        if($salelong_term_inv_cr_voucher){
            $this->db->select('IFNULL(SUM(' .db_prefix() .'accountledger.Amount),0) as AMT');
            $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
            $this->db->where(db_prefix() . 'accountledger.FY', $fy);
            $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
            $this->db->where_in(db_prefix() . 'clients.SubActGroupID', '1000017');
            $this->db->where_in(db_prefix() .'accountledger.VoucherID', $salelong_term_inv_cr_voucher);
            $this->db->where(db_prefix() .'accountledger.TType', "D");
            $SalelongTermInvAmt = $this->db->get(db_prefix() . 'accountledger')->row();
            $InvestingActivity->SalelongTermInvAmt = $SalelongTermInvAmt->AMT;
        }else{
            $InvestingActivity->SalelongTermInvAmt = 0.00;
        }
        ...
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID', $long_term_inv);
        $this->db->where(db_prefix() .'accountledger.TType', "D");
        $purlong_term_inv_cr_entry = $this->db->get(db_prefix() . 'accountledger')->result_array();
        $purlong_term_inv_cr_voucher = array();
        foreach($purlong_term_inv_cr_entry as $key=>$val){
            array_push($purlong_term_inv_cr_voucher,$val["VoucherID"]);
        }

        if($purlong_term_inv_cr_voucher){
            $this->db->select('IFNULL(SUM(' .db_prefix() .'accountledger.Amount),0) as AMT');
            $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
            $this->db->where(db_prefix() . 'accountledger.FY', $fy);
            $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
            $this->db->where_in(db_prefix() . 'clients.SubActGroupID', '1000017');
            $this->db->where_in(db_prefix() .'accountledger.VoucherID', $purlong_term_inv_cr_voucher);
            $this->db->where(db_prefix() .'accountledger.TType', "C");
            $purlongTermInvAmt = $this->db->get(db_prefix() . 'accountledger')->row();
            $InvestingActivity->purlongTermInvAmt = $purlongTermInvAmt->AMT;
        }else{
            $InvestingActivity->purlongTermInvAmt = 0.00;
        }

        //Loan and advance
        //Loan and advance Group
        // Long-term loans and advances
        $loan_adv = array("1000064");
        $this->db->select('tblaccountledger.VoucherID');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID', $loan_adv);
        $this->db->where(db_prefix() .'accountledger.TType', "C");
        $loan_adv_cr_entry = $this->db->get(db_prefix() . 'accountledger')->result_array();
        $loan_adv_cr_voucher = array();
        foreach($loan_adv_cr_entry as $key=>$val){
            array_push($loan_adv_cr_voucher,$val["VoucherID"]);
        }

        if($loan_adv_cr_voucher){
            $this->db->select('IFNULL(SUM(' .db_prefix() .'accountledger.Amount),0) as AMT');
            $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
            $this->db->where(db_prefix() . 'accountledger.FY', $fy);
            $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
            $this->db->where_in(db_prefix() . 'clients.SubActGroupID', '1000017');
            $this->db->where_in(db_prefix() .'accountledger.VoucherID', $loan_adv_cr_voucher);
            $this->db->where(db_prefix() .'accountledger.TType', "D");
            $Loan_advAmt = $this->db->get(db_prefix() . 'accountledger')->row();
            $InvestingActivity->Loan_advAmt = $Loan_advAmt->AMT;
        }else{
            $InvestingActivity->Loan_advAmt = 0.00;
        }

        //Interest Recieved
        //Interest Recieved Group
        // Bank Interest Received
        $bnk_int = array("1000053");
        $this->db->select('tblaccountledger.VoucherID');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID', $bnk_int);
        $this->db->where(db_prefix() .'accountledger.TType', "C");
        $bnk_int_cr_entry = $this->db->get(db_prefix() . 'accountledger')->result_array();
        $bnk_int_cr_voucher = array();
        foreach($bnk_int_cr_entry as $key=>$val){
            array_push($bnk_int_cr_voucher,$val["VoucherID"]);
        }

        if($bnk_int_cr_voucher){
            $this->db->select('IFNULL(SUM(' .db_prefix() .'accountledger.Amount),0) as AMT');
            $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
            $this->db->where(db_prefix() . 'accountledger.FY', $fy);
            $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
            $this->db->where_in(db_prefix() . 'clients.SubActGroupID', '1000017');
            $this->db->where_in(db_prefix() .'accountledger.VoucherID', $bnk_int_cr_voucher);
            $this->db->where(db_prefix() .'accountledger.TType', "D");
            $BnkIntAmt = $this->db->get(db_prefix() . 'accountledger')->row();
            $InvestingActivity->BnkIntAmt = $BnkIntAmt->AMT;
        }else{
            $InvestingActivity->BnkIntAmt = 0.00;
        }

        //Dividend Recieved
        //Dividend Recieved Group
        // Dividend on Share
        $devident = array("1000055");
        $this->db->select('tblaccountledger.VoucherID');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID', $devident);
        $this->db->where(db_prefix() .'accountledger.TType', "C");
        $divident_cr_entry = $this->db->get(db_prefix() . 'accountledger')->result_array();
        $divident_cr_voucher = array();
        foreach($divident_cr_entry as $key=>$val){
            array_push($divident_cr_voucher,$val["VoucherID"]);
        }

        if($divident_cr_voucher){
            $this->db->select('IFNULL(SUM(' .db_prefix() .'accountledger.Amount),0) as AMT');
            $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
            $this->db->where(db_prefix() . 'accountledger.FY', $fy);
            $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
            $this->db->where_in(db_prefix() . 'clients.SubActGroupID', '1000017');
            $this->db->where_in(db_prefix() .'accountledger.VoucherID', $divident_cr_voucher);
            $this->db->where(db_prefix() .'accountledger.TType', "D");
            $DividentAmt = $this->db->get(db_prefix() . 'accountledger')->row();
            $InvestingActivity->DividentAmt = $DividentAmt->AMT;
        }else{
            $InvestingActivity->DividentAmt = 0.00;
        }
        ...
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID', $rent);
        $this->db->where(db_prefix() .'accountledger.TType', "C");
        $rent_cr_entry = $this->db->get(db_prefix() . 'accountledger')->result_array();
        $rent_cr_voucher = array();
        foreach($rent_cr_entry as $key=>$val){
            array_push($rent_cr_voucher,$val["VoucherID"]);
        }

        if(!empty($rent_cr_voucher)){
            $this->db->select('IFNULL(SUM(' .db_prefix() .'accountledger.Amount),0) as AMT');
            $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
            $this->db->where(db_prefix() . 'accountledger.FY', $fy);
            $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
            $this->db->where_in(db_prefix() . 'clients.SubActGroupID', '1000017');
            $this->db->where_in(db_prefix() .'accountledger.VoucherID', $rent_cr_voucher);
            $this->db->where(db_prefix() .'accountledger.TType', "D");
            $RentAmt = $this->db->get(db_prefix() . 'accountledger')->row();
            $InvestingActivity->RentAmt = $RentAmt->AMT;
        }else{
            $InvestingActivity->RentAmt = 0.00;
        }

        //Commission Recieved
        //Commission Recieved Group
        // Commission Recieved
        $Commission = array("1000052");
        $this->db->select('tblaccountledger.VoucherID');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID', $Commission);
        $this->db->where(db_prefix() .'accountledger.TType', "C");
        $commission_cr_entry = $this->db->get(db_prefix() . 'accountledger')->result_array();
        $commission_cr_voucher = array();
        foreach($commission_cr_entry as $key=>$val){
            array_push($commission_cr_voucher,$val["VoucherID"]);
        }

        if($commission_cr_voucher){
            $this->db->select('IFNULL(SUM(' .db_prefix() .'accountledger.Amount),0) as AMT');
            $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
            $this->db->where(db_prefix() . 'accountledger.FY', $fy);
            $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
            $this->db->where_in(db_prefix() . 'clients.SubActGroupID', '1000017');
            $this->db->where_in(db_prefix() .'accountledger.VoucherID', $commission_cr_voucher);
            $this->db->where(db_prefix() .'accountledger.TType', "D");
            $CommissionAmt = $this->db->get(db_prefix() . 'accountledger')->row();
            $InvestingActivity->CommissionAmt = $CommissionAmt->AMT;
        }else{
            $InvestingActivity->CommissionAmt = 0.00;
        }

        //Discount Recieved
        //Discount Recieved Group
        // Discount Recieved
        $Discount = array("1000051");
        $this->db->select('tblaccountledger.VoucherID');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID', $Discount);
        $this->db->where(db_prefix() .'accountledger.TType', "C");
        $discount_cr_entry = $this->db->get(db_prefix() . 'accountledger')->result_array();
        $discount_cr_voucher = array();
        foreach($discount_cr_entry as $key=>$val){
            array_push($discount_cr_voucher,$val["VoucherID"]);
        }

        if(!empty($discount_cr_voucher)){
            $this->db->select('IFNULL(SUM(' .db_prefix() .'accountledger.Amount),0) as AMT');
            $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
            $this->db->where(db_prefix() . 'accountledger.FY', $fy);
            $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
            $this->db->where_in(db_prefix() . 'clients.SubActGroupID', '1000017');
            $this->db->where_in(db_prefix() .'accountledger.VoucherID', $discount_cr_voucher);
            $this->db->where(db_prefix() .'accountledger.TType', "D");
            $DiscountAmt = $this->db->get(db_prefix() . 'accountledger')->row();
            $InvestingActivity->DiscountAmt = $DiscountAmt->AMT;
        }else{
            $InvestingActivity->DiscountAmt = 0.00;
        }

        return $InvestingActivity;
    }

    public function GetFinancingActivity()
    {
        $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        // Share Capital
        $shares = array("1000000");
        //Procceeds from Issue of Shares
        //Procceeds from Issue of Shares Group
        // Share Capital
        // Share Capital CR Entry with corresponds entry in cash and cash equivalents
        $this->db->select('tblaccountledger.VoucherID');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID', $shares);
        $this->db->where(db_prefix() .'accountledger.TType', "C");
        $shares_cr_entry = $this->db->get(db_prefix() . 'accountledger')->result_array();
        $shares_cr_voucher = array();
        foreach($shares_cr_entry as $key=>$val){
            array_push($shares_cr_voucher,$val["VoucherID"]);
        }

        if($shares_cr_voucher){
            $this->db->select('IFNULL(SUM(' .db_prefix() .'accountledger.Amount),0) as AMT');
            $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
            $this->db->where(db_prefix() . 'accountledger.FY', $fy);
            $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
            $this->db->where_in(db_prefix() . 'clients.SubActGroupID', '1000017');
            $this->db->where_in(db_prefix() .'accountledger.VoucherID', $shares_cr_voucher);
            $this->db->where(db_prefix() .'accountledger.TType', "D");
            $SharesAmt = $this->db->get(db_prefix() . 'accountledger')->row();
            $FinancingActivity->SharesAmt = $SharesAmt->AMT;
        }else{
            $FinancingActivity->SharesAmt = 0.00;
        }

        //Redemption of Preference Shares Group
        // Share Capital
        // Share Capital DR Entry with corresponds entry in cash and cash equivalents
        $this->db->select('tblaccountledger.VoucherID');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID', $shares);
        $this->db->where(db_prefix() .'accountledger.TType', "D");
        $sharesNeg_cr_entry = $this->db->get(db_prefix() . 'accountledger')->result_array();
        $sharesNeg_cr_voucher = array();
        foreach($sharesNeg_cr_entry as $key=>$val){
            array_push($sharesNeg_cr_voucher,$val["VoucherID"]);
        }

        if($sharesNeg_cr_voucher){
            $this->db->select('IFNULL(SUM(' .db_prefix() .'accountledger.Amount),0) as AMT');
            $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
            $this->db->where(db_prefix() . 'accountledger.FY', $fy);
            $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
            $this->db->where_in(db_prefix() . 'clients.SubActGroupID', '1000017');
            $this->db->where_in(db_prefix() .'accountledger.VoucherID', $sharesNeg_cr_voucher);
            $this->db->where(db_prefix() .'accountledger.TType', "C");
            $SharesNegAmt = $this->db->get(db_prefix() . 'accountledger')->row();
            $FinancingActivity->SharesNegAmt = $SharesNegAmt->AMT;
        }else{
            $FinancingActivity->SharesNegAmt = 0.00;
        }

        // Non Current Liabilities,Interest eared in Debenture Group
        $non_curr_lia = array("100001","100036");
        // Procceeds from Issue of Debentures and Other Long-term Borrowing
        // Non Current Liabilities and Interest eared in Debenture  CR entry Corresponds entry in cash and cash equivalent
        $this->db->select('tblaccountledger.VoucherID');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID1', $non_curr_lia);
        $this->db->where(db_prefix() .'accountledger.TType', "C");
        $non_curr_lia_pos_cr_entry = $this->db->get(db_prefix() . 'accountledger')->result_array();
        $non_curr_lia_pos_cr_voucher = array();
        foreach($non_curr_lia_pos_cr_entry as $key=>$val){
            array_push($non_curr_lia_pos_cr_voucher,$val["VoucherID"]);
        }

        if($non_curr_lia_pos_cr_voucher){
            $this->db->select('IFNULL(SUM(' .db_prefix() .'accountledger.Amount),0) as AMT');
            $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
            $this->db->where(db_prefix() . 'accountledger.FY', $fy);
            $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
            $this->db->where_in(db_prefix() . 'clients.SubActGroupID', '1000017');
            $this->db->where_in(db_prefix() .'accountledger.VoucherID', $non_curr_lia_pos_cr_voucher);
            $this->db->where(db_prefix() .'accountledger.TType', "D");
            $NonCurrLiaPosAmt = $this->db->get(db_prefix() . 'accountledger')->row();
            $FinancingActivity->NonCurrLiaPosAmt = $NonCurrLiaPosAmt->AMT;
        }else{
            $FinancingActivity->NonCurrLiaPosAmt = 0.00;
        }

        //Repayment of Debentures and Other Long-term Borrowing
        // Non Current Liabilities and Interest eared in Debenture  DR entry Corresponds entry in cash and cash equivalent
        $this->db->select('tblaccountledger.VoucherID');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID1', $non_curr_lia);
        $this->db->where(db_prefix() .'accountledger.TType', "D");
        $non_curr_lia_neg_cr_entry = $this->db->get(db_prefix() . 'accountledger')->result_array();
        $non_curr_lia_neg_cr_voucher = array();
        foreach($non_curr_lia_neg_cr_entry as $key=>$val){
            array_push($non_curr_lia_neg_cr_voucher,$val["VoucherID"]);
        }

        if($non_curr_lia_neg_cr_voucher){
            $this->db->select('IFNULL(SUM(' .db_prefix() .'accountledger.Amount),0) as AMT');
            $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
            $this->db->where(db_prefix() . 'accountledger.FY', $fy);
            $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
            $this->db->where_in(db_prefix() . 'clients.SubActGroupID', '1000017');
            $this->db->where_in(db_prefix() .'accountledger.VoucherID', $non_curr_lia_neg_cr_voucher);
            $this->db->where(db_prefix() .'accountledger.TType', "C");
            $NonCurrLiaNegAmt = $this->db->get(db_prefix() . 'accountledger')->row();
            $FinancingActivity->NonCurrLiaNegAmt = $NonCurrLiaNegAmt->AMT;
        }else{
            $FinancingActivity->NonCurrLiaNegAmt = 0.00;
        }

        //Interest Paid
        // Finance Cost Group
        $intPaid = array("100030");
        // Finance Cost  DR entry Corresponds entry in cash and cash equivalent
        $this->db->select('tblaccountledger.VoucherID');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID1', $intPaid);
        $this->db->where(db_prefix() .'accountledger.TType', "D");
        $int_paid_neg_cr_entry = $this->db->get(db_prefix() . 'accountledger')->result_array();
        $int_paid_neg_cr_voucher = array();
        foreach($int_paid_neg_cr_entry as $key=>$val){
            array_push($int_paid_neg_cr_voucher,$val["VoucherID"]);
        }

        if($int_paid_neg_cr_voucher){
            $this->db->select('IFNULL(SUM(' .db_prefix() .'accountledger.Amount),0) as AMT');
            $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
            $this->db->where(db_prefix() . 'accountledger.FY', $fy);
            $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
            $this->db->where_in(db_prefix() . 'clients.SubActGroupID', '1000017');
            $this->db->where_in(db_prefix() .'accountledger.VoucherID', $int_paid_neg_cr_voucher);
            $this->db->where(db_prefix() .'accountledger.TType', "C");
            $IntPaidNegAmt = $this->db->get(db_prefix() . 'accountledger')->row();
            $FinancingActivity->IntPaidNegAmt = $IntPaidNegAmt->AMT;
        }else{
            $FinancingActivity->IntPaidNegAmt = 0.00;
        }
        ...
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID', $devident);
        $this->db->where(db_prefix() .'accountledger.TType', "D");
        $divident_dr_entry = $this->db->get(db_prefix() . 'accountledger')->result_array();
        $divident_dr_voucher = array();
        foreach($divident_dr_entry as $key=>$val){
            array_push($divident_dr_voucher,$val["VoucherID"]);
        }

        if($divident_dr_voucher){
            $this->db->select('IFNULL(SUM(' .db_prefix() .'accountledger.Amount),0) as AMT');
            $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
            $this->db->where(db_prefix() . 'accountledger.FY', $fy);
            $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
            $this->db->where_in(db_prefix() . 'clients.SubActGroupID', '1000017');
            $this->db->where_in(db_prefix() .'accountledger.VoucherID', $divident_dr_voucher);
            $this->db->where(db_prefix() .'accountledger.TType', "C");
            $DividentPaidAmt = $this->db->get(db_prefix() . 'accountledger')->row();
            $InvestingActivity->DividentPaidAmt = $DividentPaidAmt->AMT;
        }else{
            $InvestingActivity->DividentPaidAmt = 0.00;
        }

    }

    public function GetCash_CashEquvalents()
    {
        $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        // Cash and Cash Equivalents
        $Cash_and_bank = array("1000017");
        // Opening balances
        $this->db->select('IFNULL(SUM(' .db_prefix() .'accountbalances.BAL1),0) as OpnAMT');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountbalances.AccountID AND tblclients.PlantID = tblaccountbalances.PlantID');
        $this->db->where(db_prefix() . 'accountbalances.FY', $fy);
        $this->db->where(db_prefix() . 'accountbalances.PlantID', $selected_company);
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID', $Cash_and_bank);
        $Cash_Bank_opn = $this->db->get(db_prefix() . 'accountbalances')->row();
        // ledger data
        /*$this->db->select('IFNULL(SUM(' .db_prefix() .'accountledger.Amount),0) as AMT,tblaccountledger.TType');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID', $Cash_and_bank);
        $this->db->group_by(db_prefix() .'accountledger.TType');
        $Cash_Bank_cr_dr = $this->db->get(db_prefix() . 'accountledger')->result_array();
        $Cash_Bank_bal = 0;
        $cr = 0;
        $dr = 0;
        foreach($Cash_Bank_cr_dr as $key=>$val){
            if($val["TType"]=="C"){
                $cr += $val["AMT"];
            }
        ...
        $Cash_Bank_bal = $Cash_Bank_opn->OpnAMT + $dr - $cr;*/
        return $Cash_Bank_opn->OpnAMT;
    }
    ...
    // Segment report  data
    public function GetSegmentData($data_post)
    {
        $FY = $this->session->userdata('finacial_year');
        $Pre_fy = $FY - 1;
        $fy_year = array($FY,$Pre_fy);
        $selected_company = $this->session->userdata('root_company');
        $CenterID = $data_post['CenterID'];
        $this->db->select('tblitems_sub_groups.id,tblitems_sub_groups.name');
        $this->db->where(db_prefix() . 'items_sub_groups.main_group_id', '3');
        $CommodityList = $this->db->get(db_prefix() . 'items_sub_groups')->result_array();
        $data_array['CommodityList'] = $CommodityList;
        return $data_array;
    }
    ...
    /*public function GetSegmentHead_revenue_value_old()
    {
        $FY = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        $this->db->select('tblhistory.TransDate2,SUM((tblhistory.BilledQty * 10) * tblhistory.final_rate) AS billAmt,
        tblitems.subgroup_id AS GroupID');
        $this->db->join('tblitems', 'tblitems.ItemID = tblhistory.ItemID AND tblitems.PlantID = tblhistory.PlantID');
        $this->db->where(db_prefix() . 'history.TType', "S");
        $this->db->where(db_prefix() . 'history.TType2', 'Sale');
        $this->db->where(db_prefix() . 'history.FY', $FY);
        $this->db->where(db_prefix() . 'history.PlantID', $selected_company);
        $this->db->group_by(db_prefix() . 'items.subgroup_id,DAY(tblhistory.TransDate2)');
        return $this->db->get(db_prefix() . 'history')->result_array();
    }*/
    public function GetSegmentHead_revenue_value()
    {
        $FY = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        $this->db->select('tblhistory.TransDate2,SUM((tblhistory.BilledQty * 10) * tblhistory.final_rate) AS billAmt,
        ...
        $this->db->where(db_prefix() . 'history.FY', $FY);
        $this->db->where(db_prefix() . 'history.PlantID', $selected_company);
        $this->db->group_by(db_prefix() . 'items.subgroup_id');
        return $this->db->get(db_prefix() . 'history')->result_array();
    }

    public function GetCalCOGSGroupWise()
    {
        $FY = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        $this->db->select('tblitems_sub_groups.id AS subgroup_id,tblitems_sub_groups.name');
        $this->db->where(db_prefix() . 'items_sub_groups.main_group_id', '3');
        $CommodityList = $this->db->get(db_prefix() . 'items_sub_groups')->result_array();
        // P = Purchases during the period
        // Beginning Inventory
        $this->db->select('SUM(tblstockmaster.OQty * tblstockmaster.AvgRate) AS OpnCost,tblitems.subgroup_id AS GroupID');
        $this->db->join('tblitems', 'tblitems.ItemID = tblstockmaster.ItemID AND tblitems.PlantID = tblstockmaster.PlantID',"LEFT");
        $this->db->where(db_prefix() . 'stockmaster.PlantID', $selected_company);
        $this->db->where(db_prefix() . 'stockmaster.TypeID', "SP");
        $this->db->where(db_prefix() . 'stockmaster.PartyID', "KASPL");
        $this->db->where(db_prefix() . 'stockmaster.FY', $FY);
        ...
        $OpnStock =  $this->db->get(db_prefix() . 'stockmaster')->result_array();
        // Purchase and Purchase return
        $this->db->select('SUM(tblhistory.BilledQty * tblhistory.final_rate) AS TotalPurchAmt,tblitems.subgroup_id AS GroupID');
        $this->db->join('tblitems', 'tblitems.ItemID = tblhistory.ItemID AND tblitems.PlantID = tblhistory.PlantID',"LEFT");
        $this->db->where(db_prefix() . 'history.PlantID', $selected_company);
        $this->db->where(db_prefix() . 'history.FY', $FY);
        $this->db->where(db_prefix() . 'history.TType', 'P');
        $this->db->where(db_prefix() . 'history.TType2', "Purchase");
        $this->db->where(db_prefix() . 'history.PartyID', "KASPL");
        $this->db->group_by(db_prefix() . 'items.subgroup_id');
        $Purchase =  $this->db->get(db_prefix() . 'history')->result_array();
        // Direct Expenses
        $this->db->select('SUM(tblaccountledger.Amount) AS SumAmt,tblaccountledger.TType,tblitems.subgroup_id AS GroupID');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->join('tblitems', 'tblitems.ItemID = tblaccountledger.CommodityID AND tblitems.PlantID = tblaccountledger.PlantID',"LEFT");
        $this->db->where(db_prefix() . 'accountledger.FY', $FY);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->where(db_prefix() . 'clients.ActGroupID', '10010');
        $this->db->where(db_prefix() . 'accountledger.PartyID', 'KASPL');
        $this->db->group_by('tblaccountledger.TType,tblitems.subgroup_id');
        $DirectExpCR_DR =  $this->db->get(db_prefix() . 'accountledger')->result_array();
        $dirExp = array();
        foreach($CommodityList as $Key=>$val){
            ...
            $dr = 0;
            $bal = 0;
            foreach($DirectExpCR_DR as $key1=>$val1){
                if($val["subgroup_id"]==$val1["GroupID"] && $val1["TType"] == "C"){
                    $cr += $val1["SumAmt"];
                }

                if($val["subgroup_id"]==$val1["GroupID"] && $val1["TType"] == "D"){
                    $dr += $val1["SumAmt"];
                }
            ...
            $bal = $dr - $cr;
            $array = array(
                "GroupID"=>$val["subgroup_id"],
                "GroupBal"=>$bal
            );

            array_push($dirExp,$array);
        }

    // Ending Inventory
        // Get Purchase entry Item wise , PO wise FIFO Base
        $this->db->select('tblhistory.ItemID,tblhistory.final_rate,tblhistory.BilledQty,tblitems.subgroup_id AS GroupID,tblhistory.TransDate2');
        $this->db->join('tblitems', 'tblitems.ItemID = tblhistory.ItemID AND tblitems.PlantID = tblhistory.PlantID');
        $this->db->where(db_prefix() . 'history.PlantID', $selected_company);
        $this->db->where(db_prefix() . 'history.FY', $FY);
        $this->db->where(db_prefix() . 'history.TType', "P");
            ...
            foreach($Saledata as $SKey=>$SVal){
                if($PVal["ItemID"] == $SVal["ItemID"]){
                    if($SVal["QtySum"] > 0){
                        ...
                        if($SaleQty <= $PurchQty){
                            $balQty = $PurchQty - $SaleQty;
                            $Purhasedata[$j]["BilledQty"] = $balQty;
                            $Saledata[$i]["QtySum"] = 0;
                        }else{
                            $balQty = $SaleQty - $PurchQty;
                            $Saledata[$i]["QtySum"] = $balQty;
                            $Purhasedata[$j]["BilledQty"] = 0;
                        }
        ...
        // COGS = Beginning Inventory + P − Ending Inventory
        $COGSArray = array();
        foreach($CommodityList as $Key2=>$val2){
            $Total_cogs = 0;
            $opnbal = 0;
            $AllPurchase = 0;
            $DirectExp = 0;
            $ClsInvAmt = 0;
            // opn balance
            foreach($OpnStock as $key3=>$val3){
                if($val2["subgroup_id"] == $val3["GroupID"]){
                    $opnbal += $val3["OpnCost"];
                }
            ...
            // purchase amount
            foreach($Purchase as $key4=>$val4){
                if($val2["subgroup_id"] == $val4["GroupID"]){
                    $AllPurchase += $val4["TotalPurchAmt"];
                }

            }

            // Direct Exp
            foreach($dirExp as $key5=>$val5){
                if($val2["subgroup_id"] == $val5["GroupID"]){
                    $DirectExp += $val5["GroupBal"];
                }

            }

            // Ending Inventory
            foreach($Purhasedata as $key6=>$val6){
                if($val2["subgroup_id"] == $val6["GroupID"]){
                    $ClsInvAmt += $val6["BilledQty"] * $val6["final_rate"];
                }

            }

            $Total_cogs = $opnbal + $AllPurchase + $DirectExp - $ClsInvAmt;
            $array = array(
                "GroupID"=>$val2["subgroup_id"],
                "COGSBal"=>$Total_cogs
            );

            array_push($COGSArray,$array);
        }

        return $COGSArray;
    }

    public function GetIndExpGroupWise()
    {
        $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        $this->db->select('tblitems_sub_groups.id AS GroupID,tblitems_sub_groups.name');
        $this->db->where(db_prefix() . 'items_sub_groups.main_group_id', '3');
        $CommodityList = $this->db->get(db_prefix() . 'items_sub_groups')->result_array();
        // Get All Indirect Expenses
        $this->db->select('SUM(tblaccountledger.Amount) AS SumAmt,tblaccountledger.TType,tblitems.subgroup_id AS GroupID');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->join('tblitems', 'tblitems.ItemID = tblaccountledger.CommodityID AND tblitems.PlantID = tblaccountledger.PlantID',"LEFT");
        $this->db->where(db_prefix() . 'accountledger.FY', $fy);
        ...
        $this->db->where_in(db_prefix() . 'clients.ActGroupID', '10018');
        $this->db->group_by('tblaccountledger.TType,tblitems.subgroup_id');
        $IndExpGroupWise =  $this->db->get(db_prefix() . 'accountledger')->result_array();
        $i = 0;
        foreach($CommodityList as $key=>$val){
            $bal2 = 0;
            $cr2 = 0;
            $dr2 = 0;
            foreach($IndExpGroupWise as $key2=>$val2){
                if($val["GroupID"] == $val2["GroupID"] && $val2["TType"]=="C"){
                    $cr2 += $val2["SumAmt"];
                }

                if($val["GroupID"] == $val2["GroupID"] && $val2["TType"]=="D"){
                    $dr2 += $val2["SumAmt"];
                }
    ...
    /*public function Getassets_value_form_ledger_old()
    {
        $FY = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        $this->db->select('tblaccountledger.Transdate,tblaccountledger.TType,SUM(tblaccountledger.Amount) AS Amt,tblitems.subgroup_id');
        $this->db->join('tblitems', 'tblitems.ItemID = tblaccountledger.CommodityID AND tblitems.PlantID = tblaccountledger.PlantID',"LEFT");
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where(db_prefix() . 'clients.ActGroupID', "10035");
        $this->db->where(db_prefix() . 'accountledger.FY', $FY);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->group_by(db_prefix() . 'items.subgroup_id,DAY(tblaccountledger.TransDate2),tblaccountledger.TType');
        return $this->db->get(db_prefix() . 'accountledger')->result_array();
    }*/
    public function Getassets_value_form_ledger()
    {
        $FY = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        $this->db->select('tblaccountledger.TType,SUM(tblaccountledger.Amount) AS Amt,tblitems.subgroup_id');
        $this->db->join('tblitems', 'tblitems.ItemID = tblaccountledger.CommodityID AND tblitems.PlantID = tblaccountledger.PlantID',"LEFT");
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where(db_prefix() . 'clients.ActGroupID', "10035");
        $this->db->where(db_prefix() . 'accountledger.FY', $FY);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->group_by(db_prefix() . 'items.subgroup_id,tblaccountledger.TType');
        return $this->db->get(db_prefix() . 'accountledger')->result_array();
    }

    /*public function Getassets_value_for_pay_rec_old()
    {
        $FY = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        // Trade Payable, Trade receivable
        $array = array("1000006","1000016");
        $this->db->select('tblaccountledger.Transdate,tblaccountledger.TType,SUM(tblaccountledger.Amount) AS Amt,tblitems.subgroup_id');
        $this->db->join('tblitems', 'tblitems.ItemID = tblaccountledger.CommodityID AND tblitems.PlantID = tblaccountledger.PlantID',"LEFT");
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID', $array);
        $this->db->where(db_prefix() . 'accountledger.FY', $FY);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->group_by(db_prefix() . 'items.subgroup_id,DAY(tblaccountledger.TransDate2),tblaccountledger.TType');
        return $this->db->get(db_prefix() . 'accountledger')->result_array();
    }*/
    public function GetCurrent_liability()
    {
        $FY = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        $this->db->select('tblitems_sub_groups.id AS subgroup_id,tblitems_sub_groups.name');
        ...
        // Current Liability
        $Short_code = "100002";
        $this->db->select('tblaccountgroupssub.SubActGroupID,tblaccountgroupssub.SubActGroupName');
        $this->db->where(db_prefix() . 'accountgroupssub.SubActGroupID1', $Short_code);
        $CurrLiability_SubGrp = $this->db->get(db_prefix() . 'accountgroupssub')->result_array();
        $CurrLiabilitySubGrpList = array();
        foreach($CurrLiability_SubGrp as $val){
            if($val["SubActGroupID"] != "1000006"){
                array_push($CurrLiabilitySubGrpList,$val["SubActGroupID"]);
            }

        }

        $this->db->select('tblaccountledger.AccountID');
        $this->db->join('tblitems', 'tblitems.ItemID = tblaccountledger.CommodityID AND tblitems.PlantID = tblaccountledger.PlantID',"LEFT");
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID', $CurrLiabilitySubGrpList);
        $this->db->where(db_prefix() . 'accountledger.FY', $FY);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->group_by(db_prefix() . 'accountledger.AccountID');
        $AccountList = $this->db->get(db_prefix() . 'accountledger')->result_array();
        $this->db->select('tblaccountledger.Transdate,tblaccountledger.TType,SUM(tblaccountledger.Amount) AS Amt,tblitems.subgroup_id,tblaccountledger.AccountID');
        $this->db->join('tblitems', 'tblitems.ItemID = tblaccountledger.CommodityID AND tblitems.PlantID = tblaccountledger.PlantID',"LEFT");
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID', $CurrLiabilitySubGrpList);
        $this->db->where(db_prefix() . 'accountledger.FY', $FY);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->group_by(db_prefix() . 'items.subgroup_id,tblaccountledger.AccountID,tblaccountledger.TType');
        $Ledger = $this->db->get(db_prefix() . 'accountledger')->result_array();
        $i = 0;
        foreach($CommodityList as $Key=>$val){
            $total = 0;
            foreach($AccountList as $Key1=>$val1){
                $bal = 0;
                $cr = 0;
                $dr = 0;
                foreach($Ledger as $key2=>$val3){
                    if($val["subgroup_id"]==$val3['subgroup_id'] && $val1["AccountID"] == $val3["AccountID"] && $val3["TType"]=="C"){
                        $cr += $val3["Amt"];
                    }

                    if($val["subgroup_id"]==$val3['subgroup_id'] && $val1["AccountID"] == $val3["AccountID"] && $val3["TType"]=="D"){
                        $dr += $val3["Amt"];
                    }
    ...
    public function GetNonCurrent_liability()
    {
        $FY = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        // Non Current Liability
        $arr = array("100001");
        $this->db->select('tblaccountledger.TType,SUM(tblaccountledger.Amount) AS Amt');
        $this->db->join('tblclients', 'tblclients.AccountID = tblaccountledger.AccountID AND tblclients.PlantID = tblaccountledger.PlantID');
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID1', $arr);
        $this->db->where(db_prefix() . 'accountledger.FY', $FY);
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
        $this->db->group_by(db_prefix() . 'accountledger.TType');
        $Ledger = $this->db->get(db_prefix() . 'accountledger')->result_array();
    ...
    public function Getassets_value_for_pay_rec()
    {
        $FY = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        // Trade Payable, Trade receivable
        ...
        foreach($CommodityList as $Key=>$val){
            ...
            $CommodityList[$i]["assets_bal"] = abs($assets_total);
            $CommodityList[$i]["liability_bal"] = abs($liability_total);
            $i++;
        }
    ...
    public function Getassets_value_for_Opninv()
    {
        $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        // Beginning Inventory
        $this->db->select('SUM(tblstockmaster.OQty * tblstockmaster.AvgRate) AS OpnCost,tblitems.subgroup_id');
        $this->db->join('tblitems', 'tblitems.ItemID = tblstockmaster.ItemID AND tblitems.PlantID = tblstockmaster.PlantID');
        $this->db->where(db_prefix() . 'stockmaster.PlantID', $selected_company);
        $this->db->where(db_prefix() . 'stockmaster.TypeID', "SP");
        $this->db->where(db_prefix() . 'stockmaster.PartyID', "KASPL");
    ...
    public function GetSegmentHead()
    {
        $this->db->select('tblSegmentHead.ShortCode,tblSegmentHead.code,tblSegmentHead.Name');
        return $this->db->get(db_prefix() . 'SegmentHead')->result_array();
    }
...
//====================== Add Payment Voucher entry =============================
    ...
    public function get_data_general_ledger2($data_filter)
    {
        ...
        if($accounts_details->no_show == "1" && !is_admin() && $permission_details->AccountID !== $data_filter['accounting_method']){
            ...
            if(isset($data_filter['plant_id'])){
                if($data_filter['plant_id'] != 'All'){
                    $this->db->WHERE(db_prefix().'accountledger.PartyID', $data_filter['plant_id']);
                }
    ...
    */
    ...
    public function get_contra_entry($id){
        ...
        $this->db->where('FY', $fy);
        $this->db->where('VoucherID', $id);
        $this->db->where('PassedFrom', "CONTRA");
...
//======================= Delete Journal Entry =================================
    public function delete_journal_entry($id)
    {
        ...
        $this->db->where('PlantID', $selected_company);
        $this->db->where('FY', $fy);
        $this->db->where('PassedFrom', "JOURNAL");
        $this->db->where('VoucherID', $id);
        $this->db->delete(db_prefix() . 'accountledger');
        if ($this->db->affected_rows() > 0) {
            return true;
    ...
    public function get_voucher_data($filterdata)
     {
        ...
        $to_date = to_sql_date($filterdata["to_date"]);
        $voucher_type = $filterdata["voucher_type"];
        $is_reconciled = $filterdata["is_reconciled"];
        $selected_company = $this->session->userdata('root_company');
        $regExp ="'.*;s:[0-9]+:'".$selected_company."'.*'";
        $regExp1 ="'.*;s:[0-9]+:";
        $regExp2 =".*'";
  ...
  public function get_account_details_by_AccountID($id)
    ...
    public function clientData($data){
        ...
        $selected_company = $this->session->userdata('root_company');
            ...
            $this->db->select(db_prefix() . 'clients.company,'.db_prefix() . 'clients.AccountID,'.db_prefix() . 'clients.address,'.db_prefix() . 'clients.zip,'.db_prefix() .'accountbalances.BAL1,'.db_prefix().'accountgroupssub.SubActGroupName');
    ...
    public function staffData($data){
        ...
        $selected_company = $this->session->userdata('root_company');
            ...
            $from_date_new = '20'.$fy.'-'.$m.'01';
            $regExp ='.*;s:[0-9]+:"'.$selected_company.'".*';
            $this->db->select(db_prefix() . 'staff.firstname,'.db_prefix() . 'staff.AccountID,'.db_prefix() . 'staff.firstname,'.db_prefix() . 'staff.lastname,'.db_prefix() . 'staff.current_address,'.db_prefix() . 'staff.pincode,'.db_prefix() . 'staff.home_town,'.db_prefix() .'accountbalances.BAL1,'.db_prefix().'accountgroupssub.SubActGroupName');
...
//==============================================================================
    // Get Account Sub Group 2
    public function GetAccountSubGroup2()
    {
        $SQL = 'select tblaccountgroupssub.SubActGroupID,tblaccountgroupssub.SubActGroupName,tblaccountgroupssub1.SubActGroupName AS SubActGroupName1,
        tblaccountgroups.ActGroupName AS ActMainGroupName
        from tblaccountgroupssub
        INNER JOIN tblaccountgroupssub1 ON tblaccountgroupssub1.SubActGroupID1 = tblaccountgroupssub.SubActGroupID1
        INNER JOIN tblaccountgroups ON tblaccountgroups.ActGroupID = tblaccountgroupssub1.ActGroupID
         ORDER BY tblaccountgroups.ActGroupID';
        $query = $this->db->query($SQL);
...
//=========================TDS Page Code =======================================
    ...
    public function GetTDSList()
    {
        ...
        $TDSMasterList = $this->db->get()->result_array();
        return $TDSMasterList;
    }

    // Get TDS Deduction
    public function LoadTDSData($data)
    {
        $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        $from_date = to_sql_date( $data['from_date']);
        $to_date = to_sql_date($data['to_date']);
        $tdsID = $data['tdsID'];
        $this->db->select('tblTDSMaster.TDSCode,tblTDSMaster.TDSName,tblaccountledger.Transdate,tblaccountledger.VoucherID,tblaccountledger.PassedFrom,
        tblpurchasemaster.tds_per,tblpurchasemaster.AccountID,tblpurchasemaster.Invamt,tblpurchasemaster.tdsAmt,
        tblclients.company,tblcontacts.Pan');
        $this->db->from(db_prefix() . 'TDSMaster');
        $this->db->join(db_prefix() .'accountledger', db_prefix() .'accountledger.AccountID = '.db_prefix() .'TDSMaster.TDSCode');
        $this->db->join(db_prefix() .'purchasemaster', db_prefix() .'purchasemaster.TDS = '.db_prefix() .'accountledger.AccountID AND tblpurchasemaster.PurchID = tblaccountledger.VoucherID',"LEFT");
        $this->db->join(db_prefix() .'clients', db_prefix() .'clients.AccountID = '.db_prefix() .'purchasemaster.AccountID AND tblclients.PlantID = tblpurchasemaster.PlantID',"LEFT");
        $this->db->join(db_prefix() .'contacts', db_prefix() .'contacts.AccountID = '.db_prefix() .'clients.AccountID AND tblcontacts.PlantID = tblclients.PlantID',"LEFT");
        $this->db->where(db_prefix() . 'accountledger.PlantID', $selected_company);
		$this->db->where(db_prefix() . 'accountledger.FY', $fy);
		if($tdsID){
		    $this->db->where(db_prefix() . 'accountledger.AccountID', $tdsID);
		}
    ...
    public function PaymentReqtableData($data)
	{
		$fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
		$from_date = to_sql_date( $data['from_date']);
        $to_date = to_sql_date($data['to_date']);
        $this->db->select(db_prefix() . 'PaymentRequest.*,'.db_prefix() .'clients.company,');
	    $this->db->from(db_prefix() . 'PaymentRequest');
		$this->db->join(db_prefix() .'clients', db_prefix() .'clients.AccountID = '.db_prefix() .'PaymentRequest.AccountID ');
		$this->db->where( db_prefix() . 'PaymentRequest.TransDate BETWEEN "'.$from_date.' 00:00:00" AND "'.$to_date.' 23:59:59"');
		 if($data['status'] != ''){
			  $this->db->where(db_prefix() . 'PaymentRequest.Status', $data['status']);
		 }

		 $this->db->order_by( db_prefix() .'PaymentRequest.id','ASC');
		 return $this->db->get()->result_array();
	}
	...
	public function ApproveRequest($id)
	{
		...
		$this->db->where('id', $id);
		if ($this->db->update(db_prefix() . 'PaymentRequest', $approvestatus)) {
			return true;
		} else {
			return false;
		}
	...
	public function RejectRequest($id)
	{
		$rejectstatus = array(
			"Status"=>4,
			"RejectUserID"=>$this->session->userdata('username')
		);

		$this->db->where('id', $id);
		if ($this->db->update(db_prefix() . 'PaymentRequest', $rejectstatus)) {
			return true;
		} else {
			return false;
		}
	...
	public function PaymentupdateStatus($id)
	{
		$Paystatus = array(
			"Status"=>3,
			"PaymentUserID"=>$this->session->userdata('username')
		);

		$this->db->where('id', $id);
		if ($this->db->update(db_prefix() . 'PaymentRequest', $Paystatus)) {
			return true;
		} else {
			return false;
		}
	...
	function get_payment_data_by_id($id)
	{
	    ...
	    $this->db->from(db_prefix() . 'PaymentRequest');
		$this->db->join(db_prefix() .'clients', db_prefix() .'clients.AccountID = '.db_prefix() .'PaymentRequest.AccountID ');
		  $this->db->where(db_prefix() . 'PaymentRequest.id', $id);
