...
function accounting_add_head_component() {
	...
	if (!(strpos($viewuri, 'admin/accounting/trial_balance_summary1') === false) || !(strpos($viewuri, 'admin/accounting/trial_balance_summary1') === false)) {
	...
	if (!(strpos($viewuri, 'admin/accounting/SegmentWiseBalancesheet') === false) || !(strpos($viewuri, 'admin/accounting/SegmentWiseBalancesheet') === false)) {
	...
	if (!(strpos($viewuri, 'admin/accounting/trialbalance1') === false) || !(strpos($viewuri, 'admin/accounting/trialbalance1') === false)) {
	...
	if (!(strpos($viewuri, 'admin/accounting/trialbalance') === false) || !(strpos($viewuri, 'admin/accounting/trialbalance') === false)) {
	...
	if (!(strpos($viewuri, 'admin/accounting/SegmentReports') === false) || !(strpos($viewuri, 'admin/accounting/SegmentReports') === false)) {
...
function accounting_load_js() {
	...
	if (!(strpos($viewuri, 'admin/accounting/trial_balance_summary1') === false) || !(strpos($viewuri, 'admin/accounting/trial_balance_summary1') === false)) {
		...
		echo '<script src="https://cdn.jsdelivr.net/npm/xlsx/dist/xlsx.full.min.js"></script>';
	}
...
function accounting_module_init_menu_items() {
	...
	if (has_permission_new('accounting_ledger_entry', '', 'view') || has_permission_new('profitlossreport', '', 'view')

	|| has_permission_new('payment_request', '', 'view') 

	|| has_permission_new('accounting_cashflow', '', 'view') || has_permission_new('balancesheet', '', 'view') 

	|| has_permission_new('trial_balance_summary', '', 'view') || has_permission_new('accounting_contra_entry', '', 'view') 

	|| has_permission_new('accounting_tds_report', '', 'view') || has_permission_new('accounting_journal_entry', '', 'view') 

	|| has_permission_new('accounting_receipt_entry', '', 'view') || has_permission_new('accounting_payment_entry', '', 'view') 

	|| has_permission_new('accounting_voucher_register_entry', '', 'view') || has_permission_new('accounting_tcs_report', '', 'view') 

	|| has_permission_new('accounting_cd_report', '', 'view') || has_permission_new('accounting_report', '', 'view') 

	|| has_permission_new('accounting_trial_balance', '', 'view') || has_permission_new('accounting_trial_balance', '', 'view')) {
        ...
        if (has_permission_new('account_groups', '', 'view')) {
        ...
        if (has_permission_new('SubGroup1', '', 'view')) {

            $CI->app_menu->add_sidebar_children_item('accounting', [

                    'slug'     => 'account-groups',

                    'name'     => 'Account SubGroups 1',
        ...
        if (has_permission_new('payment_request', '', 'view')) {
			...
			$CI->app_menu->add_sidebar_children_item('accounting', [
				...
				'href' => admin_url('accounting/PaymentRequest'),
		...
		if (has_permission_new('accounting_receipt_entry', '', 'view')) {
			...
			$CI->app_menu->add_sidebar_children_item('accounting', [
				...
				'href' => admin_url('accounting/receipt_entry'),
...
// 		}
		...
		if (has_permission_new('accounting_trial_balance', '', 'view')) {
			...
			$CI->app_menu->add_sidebar_children_item('accounting', [
				...
				// 'href' => admin_url('accounting/trialbalance1'),
				'href' => admin_url('accounting/trialbalance'),
		...
		if (has_permission_new('trial_balance_summary', '', 'view')) {
			...
			$CI->app_menu->add_sidebar_children_item('accounting', [
				...
				'href' => admin_url('accounting/trial_balance_summary1'),
		...
		if (has_permission_new('accounting_cashflow', '', 'view')) {
			...
			$CI->app_menu->add_sidebar_children_item('accounting', [
				...
				'href' => admin_url('accounting/CashFlow'),
		...
		if (has_permission('accounting_report', '', 'view')) {
			...
			$CI->app_menu->add_sidebar_children_item('accounting', [
				...
				'href' => admin_url('accounting/SegmentReports'),
		...
		if (has_permission('ImportStatement', '', 'view')) {

			$CI->app_menu->add_sidebar_children_item('accounting', [

				'slug' => 'ImportStatement',

				'name' => "Import Statement",

				'icon' => 'fa fa-area-chart',

				'href' => admin_url('rate_master/ImportStatement'),
...
function accounting_permissions() {
	...
	$view_edit_print_export['capabilities']=[
	...
	$view_print_export['capabilities'] = [
	...
	register_staff_capabilities('account_groups', $all_access, _l('Account Groups'),'Accounts');

	register_staff_capabilities('SubGroup1', $all_access, _l('Account SubGroups 1'),'Accounts');

	register_staff_capabilities('account_subgroups', $all_access, _l('Account SubGroups 2'),'Accounts');

	register_staff_capabilities('account_head', $all_access, _l('Account Head'),'Accounts');

	register_staff_capabilities('accounting_contra_entry', $all_access, _l('accounting_contra_entry'),'Accounts');

	register_staff_capabilities('accounting_journal_entry', $all_access, _l('accounting_journal_entry'),'Accounts');

	register_staff_capabilities('accounting_receipt_entry', $all_access, _l('accounting_receipts_entry'),'Accounts');

	register_staff_capabilities('accounting_payment_entry', $all_access, _l('accounting_payments_entry'),'Accounts');

	register_staff_capabilities('payment_request', $view_edit_print_export, _l('payment_request'),'Accounts');

    register_staff_capabilities('accounting_ledger_entry', $view_print_export, 'Account Ledger','Accounts');

	register_staff_capabilities('accounting_voucher_register_entry', $view_print_export, 'Voucher Register','Accounts');

	register_staff_capabilities('accounting_cd_report', $view_print_export, 'CD Report','Accounts');

	register_staff_capabilities('accounting_tcs_report', $view_print_export, 'TCS Report','Accounts');

	register_staff_capabilities('accounting_tds_report', $view_print_export, 'TDS Report','Accounts');

	register_staff_capabilities('account_monitor_report', $view_print_export, 'Account Monitor','Accounts');

	register_staff_capabilities('accounting_trial_balance', $view_print_export, 'Trial Balance','Accounts');

	register_staff_capabilities('profitlossreport', $view_print, 'Profit Loss Report','Accounts');

	register_staff_capabilities('balancesheet', $view_print, 'Balance Sheet','Accounts');

	register_staff_capabilities('trial_balance_summary', $view_print, 'Trial Balance Summary','Accounts');

	register_staff_capabilities('accounting_cashflow', $view_print_export, 'CashFlow','Accounts');

	register_staff_capabilities('accounting_report', $view_print, 'SegmentReports','Accounts');

	register_staff_capabilities('ImportStatement', $all_access, 'Import Statement','Accounts');
