...
//============================== JOurnal Entry page ============================
  public function new_journal_entry($id = '')
  {
    ...
    $data['all_centers'] = $this->accounting_model->all_centers();

    $data['all_commodities'] = $this->accounting_model->all_commodities();

    $data['all_parties'] = $this->accounting_model->all_parties();
  ...
  public function payment_entry($id = '')
  {
    ...
    if ($this->input->post()) {
      ...
      if ($id == '') {
        ...
        if (is_array($success)) {
          ...
          set_alert('warning', 'Duplicate cheque number found ' . $success['data']); // condition added 22apr26

        } elseif ($success === 'close_the_book') {
    ...
    $data['all_centers'] = $this->paymententry_model->all_centers();

    $data['all_commodities'] = $this->paymententry_model->all_commodities();

    $data['all_parties'] = $this->paymententry_model->all_parties();
...

  {
    ...
    $data = $this->accounting_model->searchAccounts($search);
...

  {
    ...
    $data['all_centers'] = $this->receipt_model->all_centers();

    $data['all_commodities'] = $this->receipt_model->all_commodities();

    $data['all_parties'] = $this->receipt_model->all_parties();
...

  {
    ...
    $revenue_from_opn = $this->accounting_model->Getrevenue_from_opn();

    //$data['TotalSaleGroupWise'] = $revenue_from_opn->TotalSaleGroupWise;;

    $Count = 1;

    // I. Revenue from Operation

    $rev_from_opt->CurrentYear = $revenue_from_opn->CurrentYear;

    $rev_from_opt->PriviousYear = $revenue_from_opn->PriviousYear;

    $rev_from_opt->isPrimary = 'Y';
    ...
    $OtherIncomeSubgroup2Wise = $this->accounting_model->GetOtherIncomeSubgroup2Wise();

    //$data['OtherIncomeSubgroup2Wise'] = $OtherIncomeSubgroup2Wise;
    ...
    $total_rev->CurrentYear = $rev_from_opt->CurrentYear + $OtherIncome->CurrentYear;

    $total_rev->PriviousYear = $rev_from_opt->PriviousYear + $OtherIncome->PriviousYear;

    $total_rev->isPrimary = 'Y';
    ...
    $DirectExpSubgroup2Wise = $this->accounting_model->GetDirectExpSubgroup2Wise();

    $data['DirectExpSubgroup2Wise'] = $DirectExpSubgroup2Wise;



    // Inventory

    $CurrentInventoryValue = $this->accounting_model->GetCurrentInventoryValue();

    $data['CurrentInventoryValue'] = $CurrentInventoryValue["AllInventoryAmt"];

    $data['CurrentInventoryItemWiseValue'] = $CurrentInventoryValue["CommodityWiseInventoryAmt"];



    $CalCOGS = $CurrentInventoryValue["TotalOpnValue"] + $CurrentInventoryValue["TotalPurchValue"] + $DirectExp->CurrentYear - $CurrentInventoryValue["AllInventoryAmt"];

    $CalCOGSPre = $CurrentInventoryValue["TotalOpnValuePre"] + $CurrentInventoryValue["TotalPurchValuePre"] + $DirectExp->PriviousYear - $CurrentInventoryValue["AllInventoryAmtPre"];
    ...
    $EMPBenSubgroup2Wise = $this->accounting_model->GetEMPBenSubgroup2Wise();

    //$data['EMPBenSubgroup2Wise'] = $EMPBenSubgroup2Wise;
    ...
    $FinCostSubgroup2Wise = $this->accounting_model->GetFinCostSubgroup2Wise();

    //$data['FinCostSubgroup2Wise'] = $FinCostSubgroup2Wise;
    ...
    $IndirectExpSubgroup2Wise = $this->accounting_model->GetIndirectExpSubgroup2Wise();

    //$data['IndirectExpSubgroup2Wise'] = $IndirectExpSubgroup2Wise;



    // Depreciation And Amortization Expense

    $DeprecData = $this->accounting_model->GetDeprecData();

    // Deferred tax

    $DeferredTax = $this->accounting_model->GetDeferredTax();



    // IV. Expenses

    $Expensess->isPrimary = "N";
    ...
    $new_array = array(
      ...
      "name" => "1. Cost of Goods Sold (COGS)",

      "CurrentYear" => $CalCOGS,

      "PriviousYear" => $CalCOGSPre

    );

    array_push($Exp_array, $new_array);

    $new_array = array(

      "name" => "2. Employee benefits expense",

      "CurrentYear" => $EMPBenData->CurrentYear,

      "PriviousYear" => $EMPBenData->PriviousYear
    ...
    $new_array = array(
      ...
      "name" => "3. Finance Costs",

      "CurrentYear" => $FinanceCostData->CurrentYear,

      "PriviousYear" => $FinanceCostData->PriviousYear
    ...
    $new_array = array(
      ...
      "name" => "4. Depreciation And Amortization Expense",

      "CurrentYear" => $DeprecData->CurrentYear,

      "PriviousYear" => $DeprecData->PriviousYear
    ...
    $new_array = array(
      ...
      "name" => "5. Other Expenses",

      "CurrentYear" => $OtherExpensesData->CurrentYear,

      "PriviousYear" => $OtherExpensesData->PriviousYear
    ...
    $new_array = array(
      ...
      "name" => "6. Other Purchase",

      "CurrentYear" => $DirectPurchase->CurrentYear,

      "PriviousYear" => $DirectPurchase->PriviousYear

    );

    array_push($Exp_array, $new_array);

    $Expensess->details = $Exp_array;

    // V.Profit before Exceptional and extraordinary items and tax(III- IV)

    $Expenses = $CalCOGS + $EMPBenData->CurrentYear + $FinanceCostData->CurrentYear + $DeprecData->CurrentYear + $OtherExpensesData->CurrentYear + $DirectPurchase->CurrentYear;

    $ExpensesPre = $CalCOGSPre + $EMPBenData->PriviousYear + $FinanceCostData->PriviousYear + $DeprecData->PriviousYear + $OtherExpensesData->PriviousYear + $DirectPurchase->PriviousYear;



    $Profit->CurrentYear = ($rev_from_opt->CurrentYear + $OtherIncome->CurrentYear) - $Expenses;

    $Profit->PriviousYear = ($rev_from_opt->PriviousYear + $OtherIncome->PriviousYear) - $ExpensesPre;

    $Profit->isPrimary = 'Y';

    // VI.Exceptional Items

    $ExceptionalItem->CurrentYear = 0.00;

    $ExceptionalItem->PriviousYear = 0.00;

    $ExceptionalItem->isPrimary = 'Y';

    // VII.Profit before extraordinary items and tax (V - VII)

    $profit_before_ext_item_tax->CurrentYear = (($rev_from_opt->CurrentYear + $OtherIncome->CurrentYear) - $Expenses) - $ExceptionalItem->CurrentYear;

    $profit_before_ext_item_tax->PriviousYear = (($rev_from_opt->PriviousYear + $OtherIncome->PriviousYear) - $ExpensesPre) - $ExceptionalItem->PriviousYear;

    $profit_before_ext_item_tax->isPrimary = 'Y';

    // VIII.Extraordinary Items

    $ExtraordinaryItem->CurrentYear = 0.00;

    $ExtraordinaryItem->PriviousYear = 0.00;

    $ExtraordinaryItem->isPrimary = 'Y';

    // IX.Profit before tax(VII - VIII)

    $profit_before_tax->CurrentYear = $profit_before_ext_item_tax->CurrentYear - $ExtraordinaryItem->CurrentYear;

    $profit_before_tax->PriviousYear = $profit_before_ext_item_tax->PriviousYear - $ExtraordinaryItem->PriviousYear;

    $profit_before_tax->isPrimary = 'Y';
    ...
    array_push($TaxExp_array, $new_array);

    $new_array = array(

      "name" => "2. Deferred tax",

      "CurrentYear" => $DeferredTax->CurrentYear,

      "PriviousYear" => $DeferredTax->PriviousYear

    );

    array_push($TaxExp_array, $new_array);

    $TaxExp->details = $TaxExp_array;

    // XI.Profit(Loss) for the period (IX-X)

    $PL_for_the_period->CurrentYear = $profit_before_tax->CurrentYear - $TaxExp->CurrentYear;

    $PL_for_the_period->PriviousYear = $profit_before_tax->PriviousYear - $TaxExp->PriviousYear;
    ...
    foreach ($ActMainGroup as $mainGroup) {
        ...
        if ($mainGroup["ActGroupID"] == $ActsubGrp1["ActGroupID"]) {
          ...
          foreach ($ActSubGroup2 as $ActsubGrp2) {
            ...
            if ($ActsubGrp1["SubActGroupID1"] == $ActsubGrp2["SubActGroupID1"]) {
              ...
              foreach ($AccountList as $ActList) {
                ...
                if ($ActList["SubActGroupID"] == $ActsubGrp2['SubActGroupID']) {
                  ...
                  if ($ActList["AccountID"] == "PL") {
                    ...
                    $Act_opn = $PL_for_the_period->CurrentYear;

                    $Act_opnPre = $PL_for_the_period->PriviousYear;
                  }
              ...
              $subGroupData['Group2ClsBal'] = abs($ClsBalSubGrp2Wise);

              $subGroupData['Group2ClsBalPre'] = abs($ClsBalSubGrp2WisePre);
          ...
          $subGroupData1['Group1ClsBal'] = abs($ClsBalSubGrp1Wise);

          $subGroupData1['Group1ClsBalPre'] = abs($ClsBalSubGrp1WisePre);
      ...
      $mainGroupData['MainGroupClsBal'] = abs($ClsBalMainGrpWise);

      $mainGroupData['MainGroupClsBalPre'] = abs($ClsBalMainGrpWisePre);
    ...
    $this->load->view('balancesheet/balancesheetNew', $data);
  }
  ...
  public function GetAnnexure_data()

  {

    $SubgroupID = $this->input->post('SubgroupID');

    $Pay_Rec_ledger_data = $this->accounting_model->GetPay_Rec_ledgerDate();

    $Pay_Rec_opn_data = $this->accounting_model->GetPay_Rec_OpnBalData();

    $GetPay_Rec_Accounts = $this->accounting_model->GetPay_Rec_Accounts();



    $GetStaffAccounts = $this->accounting_model->GetStaffAccounts();

    $TradePayablesFarmer = 0;

    $TradePayablesTrader = 0;

    $TradePayablesBroker = 0;

    $TradePayablesCorporate = 0;

    $TradePayablesOther = 0;

    $TradeReceivablesFarmer = 0;

    $TradeReceivablesTrader = 0;

    $TradeReceivablesBroker = 0;

    $TradeReceivablesCorporate = 0;

    $TradeReceivablesOther = 0;



    foreach ($GetPay_Rec_Accounts as $Key => $val) {
      ...
      foreach ($Pay_Rec_ledger_data->Cur_yr_p_r_ledger as $LKey => $Lval) {

        if ($val["AccountID"] == $Lval["AccountID"] && $Lval["TType"] == "C") {

          $cr = $Lval["SUMAmt"];
        }

        if ($val["AccountID"] == $Lval["AccountID"] && $Lval["TType"] == "D") {

          $dr = $Lval["SUMAmt"];
        }
      }

      foreach ($Pay_Rec_opn_data->Cur_yr_P_R_OpnBal as $OKey => $Oval) {

        if ($val["AccountID"] == $Oval["AccountID"]) {
      ...
      if ($bal > 0) {
        ...
        if ($val["CustomerType"] == "1") {
          ...
          $TradeReceivablesFarmer += abs($bal);
        } elseif ($val["CustomerType"] == "2") {

          $TradeReceivablesBroker += abs($bal);
        } elseif ($val["CustomerType"] == "3") {

          $TradeReceivablesTrader += abs($bal);
        } elseif ($val["CustomerType"] == "4") {

          $TradeReceivablesCorporate += abs($bal);
        } else {

          $TradeReceivablesOther += abs($bal);
        }
      } else {

        if ($val["CustomerType"] == "1") {

          $TradePayablesFarmer += abs($bal);
        } elseif ($val["CustomerType"] == "2") {

          $TradePayablesBroker += abs($bal);
        } elseif ($val["CustomerType"] == "3") {

          $TradePayablesTrader += abs($bal);
        } elseif ($val["CustomerType"] == "4") {

          $TradePayablesCorporate += abs($bal);
        } else {

          $TradePayablesOther += abs($bal);
        }
      }
    }
    ...
    foreach ($GetStaffAccounts as $Key1 => $val1) {
      ...
      foreach ($Pay_Rec_ledger_data->Cur_yr_p_r_ledger as $LKey1 => $Lval1) {

        if ($val1["AccountID"] == $Lval1["AccountID"] && $Lval1["TType"] == "C") {

          $cr1 = $Lval1["SUMAmt"];
        }

        if ($val1["AccountID"] == $Lval1["AccountID"] && $Lval1["TType"] == "D") {

          $dr1 = $Lval1["SUMAmt"];
        }
      }

      foreach ($Pay_Rec_opn_data->Cur_yr_P_R_OpnBal as $OKey1 => $Oval1) {

        if ($val1["AccountID"] == $Oval1["AccountID"]) {
      ...
      if ($bal1 > 0) {
        ...
        $TradeReceivablesOther += abs($bal1);
      } else {

        $TradePayablesOther += abs($bal1);
      }
    }
    ...
    $payable = array(
      ...
      "Balance" => $TradePayablesFarmer

    );

    array_push($payable_data, $payable);
    ...
    $payable = array(
      ...
      "Balance" => $TradePayablesCorporate
    ...
    $receivable = array(
      ...
      "Balance" => $TradeReceivablesFarmer

    );

    array_push($receivable_data, $receivable);
    ...
    $receivable = array(
      ...
      "Balance" => $TradeReceivablesTrader
    ...
    $receivable = array(
      ...
      "Balance" => $TradeReceivablesBroker
    ...
    $receivable = array(
      ...
      "Balance" => $TradeReceivablesCorporate
    ...
    if ($SubgroupID == "1000006" || $SubgroupID == "1000016") {
  ...
  public function GetSubGroup1ByMainGroup()

  {

    $filter_data['MainGroup'] = $this->input->post('MainGroup');

    $SubGroup1 = $this->accounting_model->GetActSubGroup1ByMainGroup($filter_data);

    echo json_encode($SubGroup1);
  }



  public function GetActSubGroupID2ByAct1SubGroup1()

  {

    $filter_data['ActSubGroup1'] = $this->input->post('ActSubGroup1');

    $SubGroup = $this->accounting_model->GetActSubGroupID2ByActSubGroup1($filter_data['ActSubGroup1']);
...

  {
    ...
    if ($this->input->post()) {
      ...
      $BalanceSheet_head = $this->input->post();
    } else {
    ...
    $data['SubGroup1'] = $this->accounting_model->GetActSubGroup1ByMainGroup($BalanceSheet_head, "All");

    $data['SubGroup'] = $this->accounting_model->GetActSubGroup2ByMainGroup($BalanceSheet_head, "All");
    ...
    foreach ($ActMainGroup as $mainGroup) {
      ...
      foreach ($ActSubGroup1 as $ActsubGrp1) {
        ...
        if ($mainGroup["ActGroupID"] == $ActsubGrp1["ActGroupID"]) {
            ...
            if ($ActsubGrp1["SubActGroupID1"] == $ActsubGrp2["SubActGroupID1"]) {
              ...
              foreach ($AccountList as $ActList) {
                ...
                if ($ActList["SubActGroupID"] == $ActsubGrp2['SubActGroupID']) {
                  ...
                  $OpnBalSubGrp2Wise += $Act_opn;

                  $CrBalSubGrp2Wise += $ActCr;

                  $DrBalSubGrp2Wise += $ActDr;
              ...
              $subGroupData['Group2OpnBal'] = $OpnBalSubGrp2Wise;

              $subGroupData['Group2CrBal'] = $CrBalSubGrp2Wise;

              $subGroupData['Group2DrBal'] = $DrBalSubGrp2Wise;



              $ClsBalSubGrp1Wise += $ClsBalSubGrp2Wise;

              $OpnBalSubGrp1Wise += $OpnBalSubGrp2Wise;

              $CrBalSubGrp1Wise += $CrBalSubGrp2Wise;

              $DrBalSubGrp1Wise += $DrBalSubGrp2Wise;
          ...
          $subGroupData1['Group1OpnBal'] = $OpnBalSubGrp1Wise;

          $subGroupData1['Group1CrBal'] = $CrBalSubGrp1Wise;

          $subGroupData1['Group1DrBal'] = $DrBalSubGrp1Wise;



          $ClsBalMainGrpWise += $ClsBalSubGrp1Wise;

          $OpnBalMainGrpWise += $OpnBalSubGrp1Wise;

          $CrBalMainGrpWise += $CrBalSubGrp1Wise;

          $DrBalMainGrpWise += $DrBalSubGrp1Wise;
      ...
      $mainGroupData['MainGroupOpnBal'] = $OpnBalMainGrpWise;

      $mainGroupData['MainGroupCrBal'] = $CrBalMainGrpWise;

      $mainGroupData['MainGroupDrBal'] = $DrBalMainGrpWise;
    ...
    $this->load->view('trial_balance/TrialBalance', $data);
  ...
  public function trialbalance()
  {
    ...
    if ($this->input->post()) {
      ...
      // Safety fallback
      if (
        !isset($BalanceSheet_head['MainGroup']) ||
        empty($BalanceSheet_head['MainGroup'])
      ) {
        $BalanceSheet_head['MainGroup'] = $defaultMainGroups;
      }
    } else {
      $BalanceSheet_head = array(
        'MainGroup' => $defaultMainGroups
      );
    }
    ...
    // Removed duplicate fetchAccountsData() call
    $ActMainGroup = $this->TrialBalance_model
      ->fetchAccountsData($BalanceSheet_head);

    $data['MainGroup'] = $ActMainGroup;


    $ActSubGroup1 = $this->TrialBalance_model
      ->GetActSubGroup1ByMainGroup($BalanceSheet_head);

    $ActSubGroup2 = $this->TrialBalance_model
      ->GetActSubGroup2ByMainGroup($BalanceSheet_head);


    // Dropdown data
    $data['SubGroup1'] = $this->TrialBalance_model
      ->GetActSubGroup1ByMainGroup($BalanceSheet_head, "All");

    $data['SubGroup'] = $this->TrialBalance_model
      ->GetActSubGroup2ByMainGroup($BalanceSheet_head, "All");


    // ============================================================
    // ACCOUNT AND STAFF MASTER DATA
    // ============================================================

    $AccountList = $this->TrialBalance_model
      ->GetAccountListByMainGroup($BalanceSheet_head);

    $StaffList = $this->TrialBalance_model
      ->GetStaffList($BalanceSheet_head);


    // ============================================================
    // LEDGER AND OPENING BALANCE DATA
    // ============================================================
    ...
    $revenue_from_opn = $this->TrialBalance_model
      ->Getrevenue_from_opn();

    $rev_from_opt = new stdClass();

    $rev_from_opt->CurrentYear =
      $revenue_from_opn->CurrentYear;

    $rev_from_opt->PriviousYear =
      $revenue_from_opn->PriviousYear;
    ...
    $OtherIncome = $this->TrialBalance_model
      ->GetOtherIncome();
    ...
    // REMOVED:
    // GetOtherIncomeSubgroup2Wise()
    //
    // It was loaded but never used by TrialBalanceOptimize view.
    ...
    $total_rev = new stdClass();

    $total_rev->CurrentYear =
      $rev_from_opt->CurrentYear +
      $OtherIncome->CurrentYear;

    $total_rev->PriviousYear =
      $rev_from_opt->PriviousYear +
      $OtherIncome->PriviousYear;
    ...
    $DirectExp = $this->TrialBalance_model
      ->GetDirectExpenses();


    // REMOVED:
    // GetDirectExpSubgroup2Wise()
    //
    // Not used by TrialBalanceOptimize view.
    ...
    $CurrentInventoryValue = $this->TrialBalance_model
      ->GetCurrentInventoryValue();

    $data['CurrentInventoryValue'] =
      $CurrentInventoryValue["AllInventoryAmt"];

    $data['CurrentInventoryItemWiseValue'] =
      $CurrentInventoryValue["CommodityWiseInventoryAmt"];


    $CalCOGS =
      $CurrentInventoryValue["TotalOpnValue"] +
      $CurrentInventoryValue["TotalPurchValue"] +
      $DirectExp->CurrentYear -
      $CurrentInventoryValue["AllInventoryAmt"];


    $CalCOGSPre =
      $CurrentInventoryValue["TotalOpnValuePre"] +
      $CurrentInventoryValue["TotalPurchValuePre"] +
      $DirectExp->PriviousYear -
      $CurrentInventoryValue["AllInventoryAmtPre"];
    ...
    $EMPBenData = $this->TrialBalance_model
      ->GetEMPBen();


    // REMOVED:
    // GetEMPBenSubgroup2Wise()
    ...
    $FinanceCostData = $this->TrialBalance_model
      ->GetFinanceCostData();


    // REMOVED:
    // GetFinCostSubgroup2Wise()
    ...
    $OtherExpensesData = $this->TrialBalance_model
      ->GetOtherExpensesData();


    // REMOVED:
    // GetIndirectExpSubgroup2Wise()
    ...
    $DeprecData = $this->TrialBalance_model
      ->GetDeprecData();
    ...
    $DeferredTax = $this->TrialBalance_model
      ->GetDeferredTax();
    ...
    /*
      * IMPORTANT:
      *
      * Your original controller uses $DirectPurchase but does not show
      * where it is initialized.
      *
      * To preserve your current calculation behavior and prevent
      * undefined variable errors, initialize it only if it does not
      * already exist.
      */

    if (!isset($DirectPurchase)) {
      $DirectPurchase = new stdClass();
      $DirectPurchase->CurrentYear = 0;
      $DirectPurchase->PriviousYear = 0;
    }


    $Expensess = new stdClass();
    $Expensess->isPrimary = "N";
    ...
    $Expenses =
      $CalCOGS +
      $EMPBenData->CurrentYear +
      $FinanceCostData->CurrentYear +
      $DeprecData->CurrentYear +
      $OtherExpensesData->CurrentYear +
      $DirectPurchase->CurrentYear;


    $ExpensesPre =
      $CalCOGSPre +
      $EMPBenData->PriviousYear +
      $FinanceCostData->PriviousYear +
      $DeprecData->PriviousYear +
      $OtherExpensesData->PriviousYear +
      $DirectPurchase->PriviousYear;
    ...
    $Profit->CurrentYear =
      ($rev_from_opt->CurrentYear + $OtherIncome->CurrentYear)
      - $Expenses;

    $Profit->PriviousYear =
      ($rev_from_opt->PriviousYear + $OtherIncome->PriviousYear)
      - $ExpensesPre;
    ...
    $profit_before_ext_item_tax = new stdClass();

    $profit_before_ext_item_tax->CurrentYear =
      (($rev_from_opt->CurrentYear + $OtherIncome->CurrentYear)
        - $Expenses)
      - $ExceptionalItem->CurrentYear;


    $profit_before_ext_item_tax->PriviousYear =
      (($rev_from_opt->PriviousYear + $OtherIncome->PriviousYear)
        - $ExpensesPre)
      - $ExceptionalItem->PriviousYear;
    ...
    $ExtraordinaryItem = new stdClass();
    ...
    $profit_before_tax = new stdClass();

    $profit_before_tax->CurrentYear =
      $profit_before_ext_item_tax->CurrentYear -
      $ExtraordinaryItem->CurrentYear;

    $profit_before_tax->PriviousYear =
      $profit_before_ext_item_tax->PriviousYear -
      $ExtraordinaryItem->PriviousYear;
    ...
    /*
      * Your original code later reads:
      *
      * $TaxExp->CurrentYear
      * $TaxExp->PriviousYear
      *
      * but these properties were never assigned.
      *
      * Initialize them using the same deferred tax amount.
      */

    $TaxExp->CurrentYear = $DeferredTax->CurrentYear;
    $TaxExp->PriviousYear = $DeferredTax->PriviousYear;


    // ============================================================
    // XI. PROFIT / LOSS FOR PERIOD
    // ============================================================

    $PL_for_the_period = new stdClass();

    $PL_for_the_period->CurrentYear =
      $profit_before_tax->CurrentYear -
      $TaxExp->CurrentYear;

    $PL_for_the_period->PriviousYear =
      $profit_before_tax->PriviousYear -
      $TaxExp->PriviousYear;
    ...
    //
    // Convert all flat arrays into indexed lookup arrays.
    //
    // OLD:
    // Every account scans every ledger row.
    //
    // NEW:
    // Every account performs direct array lookup.
    // ============================================================


    // ------------------------------------------------------------
    // INDEX SUB GROUP 1 BY MAIN GROUP
    // ------------------------------------------------------------

    $subGroup1ByMain = array();

    foreach ($ActSubGroup1 as $row) {

      $mainGroupID = $row['ActGroupID'];

      if (!isset($subGroup1ByMain[$mainGroupID])) {
        $subGroup1ByMain[$mainGroupID] = array();
      }

      $subGroup1ByMain[$mainGroupID][] = $row;
    }


    // ------------------------------------------------------------
    // INDEX SUB GROUP 2 BY SUB GROUP 1
    // ------------------------------------------------------------

    $subGroup2ByGroup1 = array();

    foreach ($ActSubGroup2 as $row) {

      $group1ID = $row['SubActGroupID1'];

      if (!isset($subGroup2ByGroup1[$group1ID])) {
        $subGroup2ByGroup1[$group1ID] = array();
      }

      $subGroup2ByGroup1[$group1ID][] = $row;
    }


    // ------------------------------------------------------------
    // INDEX CLIENT ACCOUNTS BY SUB GROUP 2
    // ------------------------------------------------------------

    $accountsByGroup2 = array();

    foreach ($AccountList as $row) {

      $group2ID = $row['SubActGroupID'];

      if (!isset($accountsByGroup2[$group2ID])) {
        $accountsByGroup2[$group2ID] = array();
      }

      $accountsByGroup2[$group2ID][] = $row;
    }


    // ------------------------------------------------------------
    // INDEX STAFF BY SUB GROUP 2
    // ------------------------------------------------------------
    ...
    foreach ($StaffList as $row) {
      ...
      if (!isset($staffByGroup2[$group2ID])) {
        $staffByGroup2[$group2ID] = array();
      }

      $staffByGroup2[$group2ID][] = $row;
    }


    // ------------------------------------------------------------
    // INDEX OPENING BALANCE BY ACCOUNT
    // ------------------------------------------------------------
    ...
    if (
      ...
      foreach ($opn_data->Cur_yr_OpnBal as $row) {
        ...
        if (!isset($openingByAccount[$accountID])) {
          $openingByAccount[$accountID] = 0;
        }

        /*
              * += is intentionally used instead of =
              * so calculation remains safe if duplicate account rows exist.
              */

        $openingByAccount[$accountID] +=
          (float)$row['SUMAmt'];
      }
    }
    ...
    // ------------------------------------------------------------
    // INDEX CLIENT LEDGER BY ACCOUNT + TRANSACTION TYPE
    // ------------------------------------------------------------
    ...
    if (
      ...
      foreach ($ledger_data->Cur_yr_ledger as $row) {
        ...
        if (!isset($ledgerByAccount[$accountID])) {

          $ledgerByAccount[$accountID] = array(
            'C' => 0,
            'D' => 0
          );
        }

        if (!isset($ledgerByAccount[$accountID][$type])) {
          $ledgerByAccount[$accountID][$type] = 0;
        }

        $ledgerByAccount[$accountID][$type] +=
          (float)$row['SUMAmt'];
      }
    }
    ...
    // ------------------------------------------------------------
    // INDEX STAFF LEDGER BY ACCOUNT + TRANSACTION TYPE
    // ------------------------------------------------------------
    ...
    if (
      ...
      foreach ($staffledger_data->Cur_yr_ledger as $row) {
        ...
        if (!isset($staffLedgerByAccount[$accountID])) {

          $staffLedgerByAccount[$accountID] = array(
            'C' => 0,
            'D' => 0
          );
        }

        if (!isset($staffLedgerByAccount[$accountID][$type])) {
          $staffLedgerByAccount[$accountID][$type] = 0;
        }

        $staffLedgerByAccount[$accountID][$type] +=
          (float)$row['SUMAmt'];
      }
    }
    ...
    // ============================================================
    // BUILD OPTIMIZED NESTED TRIAL BALANCE DATA
    // ============================================================
    ...
    foreach ($ActMainGroup as $mainGroup) {
      ...
      foreach ($group1Rows as $ActsubGrp1) {
        ...
        $group1ID = $ActsubGrp1['SubActGroupID1'];

        $group2Rows =
          isset($subGroup2ByGroup1[$group1ID])
          ? $subGroup2ByGroup1[$group1ID]
          : array();
        ...
        foreach ($group2Rows as $ActsubGrp2) {
          ...
          foreach ($groupAccounts as $ActList) {
            ...
            // DIRECT O(1) LOOKUP
            $Act_opn =
              isset($openingByAccount[$accountID])
              ? $openingByAccount[$accountID]
              : 0;


            $ActCr =
              isset($ledgerByAccount[$accountID]['C'])
              ? $ledgerByAccount[$accountID]['C']
              : 0;


            $ActDr =
              isset($ledgerByAccount[$accountID]['D'])
              ? $ledgerByAccount[$accountID]['D']
              : 0;


            // Original totals preserved
            $OpnBalSubGrp2Wise += $Act_opn;
            $CrBalSubGrp2Wise += $ActCr;
            $DrBalSubGrp2Wise += $ActDr;


            // Original opening balance logic preserved
            if ($Act_opn > 0) {
            ...
            // Original liability / asset direction preserved
            if ($i > 1) {
            ...
            // Original PL adjustment preserved
            if ($accountID == "PL") {
          ...
          foreach ($groupStaff as $staffList) {
            ...
            $ActCr =
              isset($staffLedgerByAccount[$accountID]['C'])
              ? $staffLedgerByAccount[$accountID]['C']
              : 0;


            $ActDr =
              isset($staffLedgerByAccount[$accountID]['D'])
              ? $staffLedgerByAccount[$accountID]['D']
              : 0;
          ...
          $subGroupData['Group2ClsBal'] =
            $ClsBalSubGrp2Wise;

          $subGroupData['Group2OpnBal'] =
            $OpnBalSubGrp2Wise;

          $subGroupData['Group2CrBal'] =
            $CrBalSubGrp2Wise;

          $subGroupData['Group2DrBal'] =
            $DrBalSubGrp2Wise;


          // Add to subgroup level 1 totals
          $ClsBalSubGrp1Wise +=
            $ClsBalSubGrp2Wise;
        ...
        $subGroupData1['Group1ClsBal'] =
          $ClsBalSubGrp1Wise;

        $subGroupData1['Group1OpnBal'] =
          $OpnBalSubGrp1Wise;

        $subGroupData1['Group1CrBal'] =
          $CrBalSubGrp1Wise;

        $subGroupData1['Group1DrBal'] =
          $DrBalSubGrp1Wise;


        // Add to main group totals
        $ClsBalMainGrpWise +=
          $ClsBalSubGrp1Wise;
      ...
      $mainGroupData['MainGroupClsBal'] =
        $ClsBalMainGrpWise;

      $mainGroupData['MainGroupOpnBal'] =
        $OpnBalMainGrpWise;

      $mainGroupData['MainGroupCrBal'] =
        $CrBalMainGrpWise;

      $mainGroupData['MainGroupDrBal'] =
        $DrBalMainGrpWise;
    ...
    $this->load->view('trial_balance/TrialBalanceOptimize', $data);
    // echo "<pre>"; print_r($data); die;
  }
  ...
  /*public function trialbalance1()

    {
        ...
        if ($this->input->post()) {
            ...
            $data['SubGroup1'] = $this->accounting_model->GetActSubGroup1ByMainGroup($filter_data);

            $result = $this->accounting_model->fetchAccountsData($filter_data);

            $resultwithbal = $this->accounting_model->fetchAccountsDatawithCrDr($filter_data);

            $resultwithOpnBal = $this->accounting_model->fetchAccountsDatawithOpnBal($filter_data);

            $subGroups1 = $this->accounting_model->fetchActDataSubGroup1ByMainGroup($filter_data);

			$subGroups1withbal = $this->accounting_model->fetchActDataSubGroup1ByMainGroupwithCrDr($filter_data);

			$subGroups1withOpnBal = $this->accounting_model->fetchActDataSubGroup1ByMainGroupwithOpnBal($filter_data);

            $subGroups = $this->accounting_model->fetchActDataSubGroupByActSubGroup1($filter_data);

			$subGroupswithbal = $this->accounting_model->fetchActDataSubGroupByActSubGroup1withCrDr($filter_data);

			$subGroupswithOpnBal = $this->accounting_model->fetchActDataSubGroupByActSubGroup1withOpnBal($filter_data);

            $data['SubGroup'] = $this->accounting_model->GetActSubGroupID2ByActSubGroup1($filter_data["MainSubGroup"]);

            $ledgroup = $this->accounting_model->fetchAccountledgerdata($filter_data['AccountSubGroupID2']);

        }else{

            $result = $this->accounting_model->fetchAccountsData($BalanceSheet_head);

            $resultwithbal = $this->accounting_model->fetchAccountsDatawithCrDr($BalanceSheet_head);

            $resultwithOpnBal = $this->accounting_model->fetchAccountsDatawithOpnBal($BalanceSheet_head);

            $subGroups1 = $this->accounting_model->fetchActDataSubGroup1ByMainGroup();

			$subGroups1withbal = $this->accounting_model->fetchActDataSubGroup1ByMainGroupwithCrDr();

			$subGroups1withOpnBal = $this->accounting_model->fetchActDataSubGroup1ByMainGroupwithOpnBal();

            $subGroups = $this->accounting_model->fetchActDataSubGroupByActSubGroup1();

			$subGroupswithbal = $this->accounting_model->fetchActDataSubGroupByActSubGroup1withCrDr();

			$subGroupswithOpnBal = $this->accounting_model->fetchActDataSubGroupByActSubGroup1withOpnBal();

            $ledgroup = $this->accounting_model->fetchAccountledgerdata();
        ...
        $data['MainGroup'] = $this->accounting_model->fetchAccountsData($BalanceSheet_head);
        ...
        foreach ($result as $mainGroup) {
            ...
            foreach($resultwithbal as $balKey=>$balVal){

                if($mainGroup["ActGroupID"] == $balVal["ActGroupID"] && $balVal["TType"] == "C"){

                    $CR = $balVal["SumAmt"];

                }else if($mainGroup["ActGroupID"] == $balVal["ActGroupID"] && $balVal["TType"] == "D"){
            ...
            foreach($resultwithOpnBal as $obalKey=>$obalVal){

                if($mainGroup["ActGroupID"] == $obalVal["ActGroupID"]){

                    $OPNBal = $obalVal["SumAmt"];

                }

            }

            $mainGroupData['OpnBal'][] = $OPNBal;

            foreach ($subGroups1 as $subGrp1) {

                if($mainGroup["ActGroupID"] == $subGrp1["ActGroupID"]){

                    $subGroupData1 = [

                        'SubGroup1Name' => $subGrp1['SubActGroupName'],

                        'SubGroup1' => $subGrp1['SubActGroupID1'],
					...
					foreach($subGroups1withbal as $balKey1=>$balVal1){

						if($subGrp1["SubActGroupID1"] == $balVal1["SubActGroupID1"] && $balVal1["TType"] == "C"){

							$CR1 = $balVal1["SumAmt"];

						}else if($subGrp1["SubActGroupID1"] == $balVal1["SubActGroupID1"] && $balVal1["TType"] == "D"){
					...
					$subGroupData1['CR1'][] = $CR1;

					$subGroupData1['DR1'][] = $DR1;

					$OPNBal1 = 0;

					foreach($subGroups1withOpnBal as $obalKey1=>$obalVal1){

						if($subGrp1["SubActGroupID1"] == $obalVal1["SubActGroupID1"]){

							$OPNBal1 = $obalVal1["SumAmt"];

						}

					}

                     $subGroupData1['OpnBal1'][] = $OPNBal1;

					foreach ($subGroups as $subGrp) {

                        if($subGrp1["SubActGroupID1"]==$subGrp["SubActGroupID1"]){

                            $subGroupData = [

                                'SubGroupName' => $subGrp['SubActGroupName'],

                                'SubGroup' => $subGrp['SubActGroupID'],
                            ...
                            foreach($subGroupswithbal as $balKey2=>$balVal2){

								if($subGrp["SubActGroupID"] == $balVal2["SubActGroupID"] && $balVal2["TType"] == "C"){

									$CR2 = $balVal1["SumAmt"];

								}else if($subGrp1["SubActGroupID"] == $balVal2["SubActGroupID"] && $balVal2["TType"] == "D"){
								...
								$subGroupData['CR2'][] = $CR2;

								$subGroupData['DR2'][] = $DR2;

								$OPNBal2 = 0;

								foreach($subGroupswithOpnBal as $obalKey2=>$obalVal2){

									if($subGrp["SubActGroupID"] == $obalVal1["SubActGroupID"]){

										$OPNBal2 = $obalVal2["SumAmt"];

									}

								}

                               $subGroupData['OpnBal2'][] = $OPNBal2;

                            foreach($ledgroup as $leKey=>$leVal){

                                if($leVal["SubActGroupID"]==$subGrp['SubActGroupID'] && $leVal["balance"] >0 ){

                                    $ledgerData = [

                                        'AccountName' => $leVal['company'],

                                        'AccountID' => $leVal['AccountID'],

                                    ];

                                    $subGroupData['ledgers'][] = $ledgerData;
        ...
        $this->load->view('trial_balance/trial_balance1', $data);

    }*/



  public function trial_balance_load_data()

  {
    ...
    $result = $this->accounting_model->fetchsubAccountsData();

    $htmlRows = [];

    foreach ($result as $key => $subGroup) {

      $ledgroup = $this->accounting_model->fetchAccountledgerdata($subGroup['SubActGroupID']);

      $result[$key]['ledgers'] = $ledgroup;
    }
    ...
    foreach ($result as $key => $val) {
      ...
      $htmlBody .= '<tr class="treegrid-' . $mainGroupCount . ' parent-node expanded"

                            style="font-size:14px;"id="maingroup">

                            <td class="parent">

                                <div class="treegrid-container" style="margin-left: 24px;">
      ...
      foreach ($val['ledgers'] as $innerkey => $innerval) {

        $htmlBody .= '<tr class="treegrid-' . $subGroupCount . ' treegrid-parent-' . $mainGroupCount . ' parent-node expanded "

                            style="font-size:13px;" id="subgroup"> 

                            <td class="parent">

                                <div class="treegrid-container" style="margin-left: 72px;">
    ...
    $htmlTable = '<div class="page" id="DivIdToPrint">
...

  {
    ...
    $data['ItemGroup'] = $this->accounting_model->GetItemGroup();

    $data['CenterList'] = $this->accounting_model->GetSaleCenterList();
    ...
    $data['revenue_from_opn'] = $revenue_from_opn;

    $data['TotalSaleGroupWise'] = $revenue_from_opn->TotalSaleGroupWise;



    $OthIncLedgerWiseCommodityWise = $this->accounting_model->GetOthIncomeLedgerWiseCommodityWise($CenterID);

    $data['OthIncLedgerWiseCommodityWise'] = $OthIncLedgerWiseCommodityWise;

    $data['CommodityList'] = $this->accounting_model->GetCommodityList();

    $CenterID = "";

    $DirectExpLedgerWiseCommodityWise = $this->accounting_model->GetDirectExpLedgerWiseCommodityWise($CenterID);

    $data['DirectExpLedgerWiseCommodityWise'] = $DirectExpLedgerWiseCommodityWise;
    ...
    //print_r($data['TotalSaleGroupWise']);
    ...
    $CurrentInventoryValue = $this->accounting_model->GetCurrentInventoryValue($CenterID);
    ...
    $EmpBenLedgerWiseCommodityWise = $this->accounting_model->GetEmpBenLedgerWiseCommodityWise($CenterID);

    $data['EmpBenLedgerWiseCommodityWise'] = $EmpBenLedgerWiseCommodityWise;
    ...
    $FinCostLedgerWiseCommodityWise = $this->accounting_model->GetFinCostLedgerWiseCommodityWise($CenterID);

    $data['FinCostLedgerWiseCommodityWise'] = $FinCostLedgerWiseCommodityWise;
    ...
    $IndExpLedgerWiseCommodityWise = $this->accounting_model->GetIndExpLedgerWiseCommodityWise($CenterID);

    $data['IndExpLedgerWiseCommodityWise'] = $IndExpLedgerWiseCommodityWise;



    /*echo "<pre>";

		print_r($data['IndExpLedgerWiseCommodityWise']);
    ...
    $DepreCostLedgerWiseCommodityWise = $this->accounting_model->GetDepreCostLedgerWiseCommodityWise($CenterID);

    $data['DepreCostLedgerWiseCommodityWise'] = $DepreCostLedgerWiseCommodityWise;

    // Deferred tax

    $DeferredTax = $this->accounting_model->GetDeferredTax();

    $TaxExpLedgerWiseCommodityWise = $this->accounting_model->GetTaxExpLedgerWiseCommodityWise($CenterID);

    $data['TaxExpLedgerWiseCommodityWise'] = $TaxExpLedgerWiseCommodityWise;
    ...
    $profitLossData = array(
      ...
      "I. Revenue from Operation"   => $rev_from_opt,

      "II. Other Income"            => $OtherIncome,

      "III. Total Revenue (I + II)" => $total_rev,

      "IV. Expenses" => $Expensess,

      "V.Profit before Exceptional and extraordinary items and tax(III- IV)" => $Profit,

      "VI.Exceptional Items" => $ExceptionalItem,

      'VII.Profit before extraordinary items and tax (V - VII)' => $profit_before_ext_item_tax,

      'VIII.Extraordinary Items' => $ExtraordinaryItem,

      'IX.Profit before tax(VII - VIII)' => $profit_before_tax,

      "X.Tax Expense" => $TaxExp,

      'XI.Profit(Loss) for the period  (IX-X)' => $PL_for_the_period,

    );



    $data['profitLossData'] = $profitLossData;



    $this->load->view('balancesheet/profitloss_report', $data);
  }
  ...
  public function export_balsheetreport()
  {
    ...
    if ($this->input->post()) {
      ...
      foreach ($result as $mainGroup) {
        ...
        $subGroups = $this->accounting_model->GetActSubGroup1ByMainGroup($mainGroup['ActGroupID']);
        ...
        foreach ($subGroups as $subGroup) {
          ...
          $subGroups1 = $this->accounting_model->GetActSubGroup2ByMainGroup($subGroup['SubActGroupID1']);
...

  {
    ...
    if (!has_permission_new('accounting_cashflow', '', 'view')) {
    ...
    $data['OperatingActivity'] = $this->accounting_model->GetOperatingActivity();

    $data['InvestingActivity'] = $this->accounting_model->GetInvestingActivity();

    $data['FinancingActivity'] = $this->accounting_model->GetFinancingActivity();

    $data['Cash_CashEquvalents'] = $this->accounting_model->GetCash_CashEquvalents();

    /*echo "<pre>";

        print_r($data['InvestingActivity']);

        die;*/

    $data['ledger_detail'] = $this->accounting_model->get_ledger_detail();

    $data['ledger_details'] = $this->accounting_model->get_ledger_details();

    $data['taxc_details'] = $this->accounting_model->get_taxc_details();

    $data['taxd_details'] = $this->accounting_model->get_taxd_details();

    // return print json_encode($data);

    $data['title'] = "Cash Flow reports";

    $this->load->view('CashFlow/CashFlowReport', $data);
  }



  public function SegmentReports()

  {

    if (!has_permission_new('accounting_report', '', 'view')) {

      access_denied('accounting report');
    }
    ...
    $data['CenterList'] = $this->accounting_model->GetCenterList();

    $data['title'] = "Segment wise reports";

    $CenterID = "";

    if ($this->input->post() && $this->input->post("CenterID") != "") {
    ...
    $data['CommodityWiseInventoryAmt'] = $CurrentInventoryValue["CommodityWiseInventoryAmt"];
    ...
    $AssetsLedgerWiseCommodityWise = $this->accounting_model->GetAssetsLedgerWiseCommodityWise($CenterID);

    $data['AssetsLedgerWiseCommodityWise'] = $AssetsLedgerWiseCommodityWise;

    $LiaLedgerWiseCommodityWise = $this->accounting_model->GetLiaLedgerWiseCommodityWise($CenterID);

    $data['LiaLedgerWiseCommodityWise'] = $LiaLedgerWiseCommodityWise;

    /*echo "<pre>";

		print_r($AssetsLedgerWiseCommodityWise);

		die;*/

    $this->load->view('Segment/SegmentReport', $data);
  }



  public function GetSegmentReport()

  {

    $data_filter = $this->input->post();

    $as_on = to_sql_date($this->input->post('as_on'));

    //$quarter = QUARTER(to_sql_date($as_on));
    ...
    $qtr_one_date = '20' . $FY . '-06-30';

    $qtr_two_date = '20' . $FY . '-09-30';

    $qtr_three_date = '20' . $FY . '-12-31';

    $qtr_four_date = '20' . $fy2 . '-03-31';



    $lyqtr_one_date = '20' . $fy3 . '-06-30';

    $lyqtr_two_date = '20' . $fy3 . '-09-30';

    $lyqtr_three_date = '20' . $fy3 . '-12-31';

    $lyqtr_four_date = '20' . $FY . '-03-31';



    if (strtotime($as_on) > strtotime($qtr_one_date)) {

      $qtr_nu = 2;
    }

    if (strtotime($as_on) > strtotime($qtr_two_date)) {

      $qtr_nu = 3;
    }

    if (strtotime($as_on) > strtotime($qtr_three_date)) {
    ...
    $data = array(
      ...
      "as_on" => strtotime($as_on),

      "1st" => strtotime($qtr_two_date)

    );

    $total_colmn = 3 + $qtr_nu;

    $CommodityList = $this->accounting_model->GetCommodityList();
    ...
    $html .= '<th style="text-align:center;" rowspan="3">Business Segment</th>';

    $html .= '<th style="text-align:center;" colspan="' . $colspan . '">Quarter ended</th>';

    //$html .= '<th colspan="2">Half Year ended</th>';

    $html .= '<th style="text-align:center;">Year ended</th>';
    ...
    $html .= '<th style="text-align:center;" colspan="' . $colspan . '"> (Reviewed)</th>';

    //$html .= '<th colspan="2"> (Reviewed)</th>';

    $html .= '<th style="text-align:center;">(Audited)</th>';
    ...
    $html .= '<td>Revenue from Commision</td>';
  ...
  public function voucher_register_report()
  {
    ...
    if ($this->input->post()) {
      ...
      if ($voucher_type == "CONTRA" || $voucher_type == "JOURNAL") {
        ...
        $set_col_tk["Is_Reconciled"] =  'Is Reconciled';
      ...
      foreach ($table_data as $k => $value) {
        ...
        if ($voucher_type == "CONTRA" || $voucher_type == "JOURNAL") {
          ...
          $list_add[] = $value["reconcile_status"];
...

  {
    ...
    foreach ($table_data as $key => $value) {
      ...
      if ($voucher_type == "CONTRA" || $voucher_type == "JOURNAL") {
        ...
        $html .= '<td>' . $value['reconcile_status'] . '</td>';
...
//======================= Delete Payment Entry =================================
  ...
  public function rp_general_ledger()
  {
    ...
    if (!has_permission_new('accounting_ledger_entry', '', 'view')) {
      ...
      access_denied('accounting_ledger_entry');
    }
    ...
    $data['all_parties']    = $this->accounting_model->get_all_parties();
      ...
      foreach ($data_report as $key => $value) {
        ...
        if ($value["Amount"] !== "0.00") {
          ...
          if ($value["PassedFrom"] == "SALE") {
            ...
            $url = admin_url() . 'accounting/payment_entry/' . $value["VoucherID"];
          } else if ($value["PassedFrom"] == "RECEIPTS") {

            $url = admin_url() . 'accounting/receipt_entry/' . $value["VoucherID"];
          ...
          $html .= '<td style="text-align:center;color:' . $CSSStatus . '">' . $value["reconcile_status"] . '</td>';
          ...
          $html .= '<td>' . substr($value["company"], 0, 50) . '' . $str2 . '</td>';

          $html .= '<td style="text-align:center;">' . $value["ref_no"] . '</td>';
...

  {
    ...
    if (!has_permission_new('accounting_tds_report', '', 'view')) {
      ...
      access_denied('accounting_tds_report');
    }
    ...
    $data['TDSList'] = $this->accounting_model->GetTDSList();

    $this->load->view('TDS/TDSReport', $data);
  }

  // get tds report

  public function load_data_for_tds()

  {
    ...
    $data = $this->accounting_model->LoadTDSData($this->input->post());
    ...
    if (count($data) > 0) {
      ...
      foreach ($data as $value) {
        ...
        $html .= '<td align="center">' . $value['Pan'] . '</td>';

        $html .= '<td align="right">' . number_format($value['Invamt'], 2, '.', '') . '</td>';

        $Total_baseAmt += $value['Invamt'];

        $html .= '<td align="center">' . number_format($value['tds_per'], 2, '.', '') . '</td>';

        $html .= '<td align="right">' . number_format($value['tdsAmt'], 2, '.', '') . '</td>';

        $Total_tdsAmt += $value['tdsAmt'];
      ...
      $html .= '<td align="left" style="font-weight:700;" colspan="7">Total</td>';

      $html .= '<td align="right" style="font-weight:700;">' . number_format($Total_baseAmt, 2, '.', '') . '</td>';

      $html .= '<td align="right"></td>';

      $html .= '<td align="right" style="font-weight:700;">' . number_format($Total_tdsAmt, 2, '.', '') . '</td>';
  ...
  public function export_tds_report()

  {
    ...
    if ($this->input->post()) {
      ...
      $msg = "TDS Report " . $this->input->post('from_date') . " To " . $this->input->post('to_date');
      ...
      $set_col_tk["TDS Description"] =  'TDS Description';

      $set_col_tk["Posting Data"] = 'Posting Data';

      $set_col_tk["From"] = 'From';

      $set_col_tk["Doc Number"] = 'Doc Number';

      $set_col_tk["Supplier Name"] = ' Supplier Name';

      $set_col_tk["Supplier PAN"] = 'Supplier PAN';

      $set_col_tk["Base Amount"] = 'Base Amount';

      $set_col_tk["TDS Rate%"] = 'TDS Rate%';

      $set_col_tk["TDS Amount"] = 'TDS Amount';
      ...
      $list_add[] = $Total_baseAmt;

      $list_add[] = "";

      $list_add[] = $Total_tdsAmt;
  ...
  public function AccountMonitor()
  {
    ...
    $data['ActGroupData'] = $this->accounting_model->GetAccountSubGroup2();
  ...
  public function export_credit_debit_report()
  {
    ...
    if ($this->input->post()) {
      ...
      foreach ($data as $k => $value) {
        ...
        $cgst_amt += $value['cgstamt'];

        $igst_amt += $value['igstamt'];

        $sgst_amt += $value['sgstamt'];
  ...
  public function load_data_cd_report()
  {
    ...
    foreach ($data as $value) {
      ...
      $html .= '<td align="center">' . $value['BT'] . '</td>';
      ...
      $html .= '<td align="right">' . $value['SaleAmt'] . '</td>';
      ...
      $html .= '<td align="right">' . $value['cgstamt'] . '</td>';

      $html .= '<td align="right">' . $value['sgst'] . '</td>';

      $html .= '<td align="right">' . $value['sgstamt'] . '</td>';

      $html .= '<td align="right">' . $value['igst'] . '</td>';

      $html .= '<td align="right">' . $value['igstamt'] . '</td>';

      $html .= '<td align="right">' . $value['BillAmt'] . '</td>';
    ...
    $html .= '<td align="right"><b>' . $cgst_amt . '</b></td>';

    $html .= '<td></td>';

    $html .= '<td align="right"><b>' . $sgst_amt . '</b></td>';

    $html .= '<td></td>';

    $html .= '<td align="right"><b>' . $igst_amt . '</b></td>';

    $html .= '<td align="right"><b>' . $total_amt . '</b></td>';
  ...
  public function trial_balance_summary1()
  {
    if (!has_permission_new('trial_balance_summary', '', 'view')) {
      access_denied('accounting_tcs_report');
    }
    ...
    $data['company_detail'] =
      $this->accounting_model->get_company_detail1($selected_company);
    ...
    $BalanceSheet_head['MainGroup'] = array(
      "10000",
      "10035",
      "10025",
      "10028",
    ...
    //
    // Old code:
    // Each group/account repeatedly scanned complete arrays.
    //
    // New code:
    ...
    //
    // IMPORTANT:
    // Key includes AccountID + FY.
    //
    // This preserves your original condition:
    //
    // AccountID == account
    // AND FY == current FY
    // ------------------------------------------------------------
    ...
    foreach ($ActMainGroup as $mainGroup) {
      ...
      foreach ($group1Rows as $ActsubGrp1) {
        ...
        // Direct indexed subgroup 2 lookup
        $group1ID =
          $ActsubGrp1['SubActGroupID1'];
        ...
        foreach ($group2Rows as $ActsubGrp2) {
          ...
          foreach ($groupStaff as $staffList) {
            ...
            //
            // The original code does NOT add negative staff
            // closing balance to $DrClsBalSubGrp2Wise.
            // =============================================
    ...
    $this->load->view('trial_balance/trial_balance_summary1', $data);
    // echo '<pre>'; print_r($data); die;
  }
...

  {
    ...
    echo json_encode($debitledgerdata);
  ...
  public function export_trial_balance_report()
  {
    ...
    if ($this->input->post()) {
      ...
      foreach ($data as $k => $value) {
        ...
        $balance = (float) $value['BAL1'] + (float) $debitAmt - (float) $creditAmt;
...

  {
    ...
    if (!has_permission_new('payment_request', '', 'view')) {

      access_denied('accounting_tcs_report');
    }

    $data['title'] = "Payment Request List";

    $this->load->model('accounts_master_model');

    $data['company_detail'] = $this->accounts_master_model->get_company_detail();

    $this->load->view('payment_request/PaymentRequest', $data);
  }
  ...
  public function load_data_Payment_request()

  {
    ...
    $data = $this->accounting_model->PaymentReqtableData($this->input->post());
    ...
    foreach ($data as $value) {
      ...
      $html .= '<td align="center">' . $value['Amount'] . '</td>';

      $html .= '<td align="center">' . $status . '</td>';

      if ($value['Status'] == 1 && has_permission_new('payment_request', '', 'edit')) {

        $html .= '<td><button type="submit" class="btn btn-success" style="padding: 2px 8px; 

                            font-size:12px;" id="approve_status" onclick="Aprrovereq(' . $value['id'] . ')">Approve</button>

                        <button type="submit" class="btn btn-danger btn-sm" style="padding: 2px 8px; font-size: 12px;" onclick="Rejectreq(' . $value['id'] . ')">Reject</button>

                      </td>';
      } else if ($value['Status'] == 2 && has_permission_new('accounting_payment_entry', '', 'edit')) {

        $html .= '<td><button type="submit" class="btn btn-info" style="padding: 2px 8px;font-size:12px;" id="generatepayment" onclick="PaymentReq(' . $value['id'] . ')">Payment</button>                        
  ...
  public function update_payment_request_status()

  {

    if (!has_permission_new('payment_request', '', 'edit')) {
    ...
    $updatestatus = $this->accounting_model->ApproveRequest($id);
  ...
  public function reject_payment_request_status()

  {
    ...
    $updatestatus = $this->accounting_model->RejectRequest($id);
  ...
  public function updateto_payment_status()

  {
    ...
    $updatestatus = $this->accounting_model->PaymentupdateStatus($id);

    $payment_data = $this->accounting_model->get_payment_data_by_id($id);

    $PaymentModes =  $this->accounting_model->get_paymentModes();



    if ($updatestatus) {

      echo json_encode(['status' => 'success', 'message' => 'Status changed to payment successfully.', 'payment_data' => $payment_data, 'PaymentModes' => $PaymentModes]);
    } else {
...

  {
    ...
    $debit_insert = $this->db->insert(db_prefix() . 'accountledger', $insert_Debitledger);

    $ordno++;



    $insert_Creditledger = array(
    ...
    $credit_insert = $this->db->insert(db_prefix() . 'accountledger', $insert_Creditledger);

    if ($debit_insert && $credit_insert) {
      ...
      echo json_encode([
        ...
        'message' => 'Failed to generate payment ledger. Please try again.'
  ...
  public function ProfitLossTFormat($from_date = "", $Todate = "") {
    if (!has_permission_new('profitlossTFormat', '', 'view')) {
      access_denied('access denied');
    }
    $fy = $this->session->userdata('finacial_year');
    $strFYStart = '20' . $fy . '-04-01';
    $intFYEndYear = (int) $fy + 1;
    $strFYEnd = '20' . $intFYEndYear . '-03-31';
    if (!empty($from_date) && !empty($Todate)) {
      if ($from_date < $strFYStart || $from_date > $strFYEnd) {
        set_alert('danger', 'From Date must be within current financial year (01-Apr-' . $fy . ' to 31-Mar-' . $intFYEndYear . ').');
        redirect(admin_url('accounting/ProfitLossTFormat'));
        return;
      }
      if ($Todate < $strFYStart || $Todate > $strFYEnd) {
        set_alert('danger', 'To Date must be within current financial year (01-Apr-' . $fy . ' to 31-Mar-' . $intFYEndYear . ').');
        redirect(admin_url('accounting/ProfitLossTFormat'));
        return;
      }
      if ($from_date > $Todate) {
        set_alert('danger', 'From Date cannot be after To Date.');
        redirect(admin_url('accounting/ProfitLossTFormat'));
        return;
      }
    }
    ...
    $data['company_detail'] = $this->misc_reports_model->get_company_detail();
    if (!empty($Todate)) {
      $formatted_date = date('d/m/Y', strtotime($Todate));
      $data['ToDate'] = $formatted_date;
    }
    if (!empty($from_date)) {
    ...
    if (!empty($from_date) && !empty($Todate)) {
      // Other Income
      $OtherIncome = $this->ProfitLoss_model->GetOtherIncome($fromdate, $todate);
      $data['OtherIncome'] = $OtherIncome;
      $TotalOpnQtyAmt = 0;
      $OpeningInventoryAmt = $this->ProfitLoss_model->GetOpeningInventoryAmt($fromdate, $todate);
      $data['OpeningInventoryAmt'] = $OpeningInventoryAmt;
      $ClosingInventoryAmt = $this->ProfitLoss_model->GetClosingInventoryAmt($fromdate, $todate);
      $data['ClosingInventoryAmt'] = $ClosingInventoryAmt;
      $TransactionAmt = $this->ProfitLoss_model->GetTransactionAmt($fromdate, $todate);
      $data['TransactionAmt'] = $TransactionAmt;
      // Direct Expenses
      $DirectExp = $this->ProfitLoss_model->GetDirectExpenses($fromdate, $todate);
      $data['DirectExp'] = $DirectExp;
      // Employee benefits expense
      $EMPBenData = $this->ProfitLoss_model->GetEMPBen($fromdate, $todate);
      $data['EMPBenData'] = $EMPBenData;
      // Finance Costs
      $FinanceCostData = $this->ProfitLoss_model->GetFinanceCostData($fromdate, $todate);
      $data['FinanceCostData'] = $FinanceCostData;
      // Depreciation And Amortization Expense
      $DeprecData = $this->ProfitLoss_model->GetDeprecAmortData($fromdate, $todate);
      $data['DeprecData'] = $DeprecData;
      // Indirect Expenses
      $OtherExpensesData = $this->ProfitLoss_model->GetOtherExpensesData($fromdate, $todate);
      $data['OtherExpensesData'] = $OtherExpensesData;
      // TAX Expense
      $TaxExpense = $this->ProfitLoss_model->GetTaxExpense($fromdate, $todate);
  ...
  public function export_ProfitLossTFormat() {
    ...
    if ($this->input->post()) {
      ...
      $intFYEndYear = (int) $fy + 1;
      $strFYEnd = '20' . $intFYEndYear . '-03-31';
      if (!empty($from_date) && ($from_date < $strFYStart || $from_date > $strFYEnd)) {
        echo json_encode(['success' => false, 'message' => 'From Date must be within current financial year (01-Apr-' . $fy . ' to 31-Mar-' . $intFYEndYear . ').']);
        return;
      }
      if (!empty($date) && ($date < $strFYStart || $date > $strFYEnd)) {
        echo json_encode(['success' => false, 'message' => 'To Date must be within current financial year (01-Apr-' . $fy . ' to 31-Mar-' . $intFYEndYear . ').']);
        return;
      }
      if (!empty($from_date) && !empty($date) && $from_date > $date) {
      ...
      $writer = new XLSXWriter();
      $sheet = 'Sheet1';
      $title_css = ['font-style' => 'bold', 'font-size' => '13px'];
      $main_group_css = ['font-size' => '12px'];
      $group1_css = ['font-size' => '11px'];
      $group2_css = ['font-size' => '10px'];
      $Item_css = ['font-size' => '9px'];

      $style_bold = ['font-style' => 'bold'];
      $style_amount = ['halign' => 'right', 'number_format' => '#,##0.00'];
      // Unicode non-breaking space for alignment
      $nbsp = "\u{00A0}";
      $indent1 = str_repeat($nbsp, 8); // for space
      ...
      $writer->writeSheetRow($sheet, ['Profit & Loss Report From ' . $fromdate . ' To ' . $todate], [$style_bold]);
      $writer->writeSheetRow($sheet, ['Particulars', '', '', '', '', '', 'Amount(₹)'], [$style_bold, $style_bold, '', $style_bold, $style_bold]);
      //$rows = [];
      ...
      $rows = ['OPENING AMOUNT', '', '', '', '', '', '', number_format($OpeningInventoryAmt->TotalinventoryAmt, 2)];
      $writer->writeSheetRow($sheet, $rows, [$title_css, '', '', '', '', '', '', $title_css]);

      foreach ($OpeningInventoryAmt->inventory as $mainGroup) {
        $MainGroupTotalAmt = (float)$mainGroup['MainGroupTotalAmt'];
        if ($MainGroupTotalAmt == 0) {
          continue;
        }
        $rows = ['', $mainGroup['MainGroupName'], '', '', '', '', number_format($mainGroup['MainGroupTotalAmt'], 2), ''];
        $writer->writeSheetRow($sheet, $rows, ['', $main_group_css, '', '', '', '', $main_group_css, '']);
        foreach ($mainGroup['SubGroup1'] as $subGroup1) {
          $SubGroup1TotalAmt = (float)$subGroup1['SubGroup1TotalAmt'];
          if ($SubGroup1TotalAmt == 0) {
            continue;
          }
          $rows = ['', '', $subGroup1['SubGroup1Name'], '', '', '', number_format($subGroup1['SubGroup1TotalAmt'], 2), ''];
          $writer->writeSheetRow($sheet, $rows, ['', '', $group1_css, '', '', '', $group1_css, '']);
          // foreach ($subGroup1['SubGroup2'] as $subGroup2) {
          //   $SubGroup2TotalAmt = (float)$subGroup2['SubGroup2TotalAmt'];
          //   if ($SubGroup2TotalAmt == 0) {
          //     continue;
          //   }
          //   $rows = ['', '', '', $subGroup2['SubGroup2Name'], '', '', number_format($subGroup2['SubGroup2TotalAmt'], 2), ''];
          //   $writer->writeSheetRow($sheet, $rows, ['', '', '', $group2_css, '', '', $group2_css, '']);
            foreach ($subGroup1['ItemDetails'] as $item) {
              $ItemTotalAmt = (float)$item['ItemTotalAmt'];
              if ($ItemTotalAmt == 0) {
                continue;
              }
              $rows = ['', '', '', $item['ItemID'] . ' - ' . $item['ItemName'], '', number_format($item['ItemTotalAmt'], 2), ''];
              $writer->writeSheetRow($sheet, $rows, ['', '', '', $Item_css, '', $Item_css, '']);
            }
          // }
        }
      }
      ...
      // NET PURCHASE
      $NetPurchase = $TransactionAmt->TotalPurchaseAmt - $TransactionAmt->TotalPurchaseRtnAmt - $TransactionAmt->TotalPurchDNAmt + $TransactionAmt->TotalPurchCNAmt;
      $rows = ['NET PURCHASE AMOUNT', '', '', '', '', '', '', number_format($NetPurchase, 2)];
      $writer->writeSheetRow($sheet, $rows, [$title_css, '', '', '', '', '', '', $title_css]);
      // PURCHASE AMOUNT
      $rows = ['', 'PURCHASE AMOUNT (Taxable) - add', '', '', '', '', '', number_format($TransactionAmt->TotalPurchaseAmt, 2)];
      $writer->writeSheetRow($sheet, $rows, ['', $main_group_css, '', '', '', '', '', $main_group_css]);

      foreach ($TransactionAmt->inventory as $mainGroup) {
        $MainGroupTotalQty = (float)$mainGroup['MainGroupPurchaseAmt'];
        if ($MainGroupTotalQty == 0) {
          continue;
        }
        $rows = ['', '', $mainGroup['MainGroupName'],  '', '', '', number_format($mainGroup['MainGroupPurchaseAmt'], 2), ''];
        $writer->writeSheetRow($sheet, $rows, ['', '', $main_group_css, '', '', '', $main_group_css, '']);
        foreach ($mainGroup['SubGroup1'] as $subGroup1) {
          ...
          $rows = ['', '', '', $subGroup1['SubGroup1Name'], '', '', number_format($subGroup1['SubGroup1PurchaseAmt'], 2), ''];
          $writer->writeSheetRow($sheet, $rows, ['', '', '', $group1_css, '', '', $group1_css, '']);
          // foreach ($subGroup1['SubGroup2'] as $subGroup2) {
          //   $SubGroup2TotalQty = (float)$subGroup2['SubGroup2PurchaseAmt'];
          //   if ($SubGroup2TotalQty == 0) {
          //     continue;
          //   }
          //   $rows = ['', '', '', '', $subGroup2['SubGroup2Name'], '', number_format($subGroup2['SubGroup2PurchaseAmt'], 2), ''];
          //   $writer->writeSheetRow($sheet, $rows, ['', '', '', '', $group2_css, '', $group2_css, '']);
            foreach ($subGroup1['ItemDetails'] as $item) {
              ...
              $rows = ['', '', '', '', $item['ItemID'] . ' - ' . $item['ItemName'], number_format($item['PurchaseAmt'], 2), ''];
              $writer->writeSheetRow($sheet, $rows, ['', '', '', '', $Item_css, $Item_css, '']);
            }
          // }
        }
      ...
      // PURCHASE RETURN
      $rows = ['', 'PURCHASE RETURN AMOUNT (Taxable) - less', '', '', '', '', '', number_format($TransactionAmt->TotalPurchaseRtnAmt, 2)];
      $writer->writeSheetRow($sheet, $rows, ['', $main_group_css, '', '', '', '', '', $main_group_css]);
      ...
      foreach ($TransactionAmt->inventory as $mainGroup) {
        ...
        $rows = ['', '', $mainGroup['MainGroupName'],  '', '', '', number_format($mainGroup['MainGroupPurchaseRtnAmt'], 2), ''];
        $writer->writeSheetRow($sheet, $rows, ['', '', $main_group_css, '', '', '', $main_group_css, '']);
        foreach ($mainGroup['SubGroup1'] as $subGroup1) {
          ...
          $rows = ['', '', '', $subGroup1['SubGroup1Name'], '', '', number_format($subGroup1['SubGroup1PurchaseRtnAmt'], 2), ''];
          $writer->writeSheetRow($sheet, $rows, ['', '', '', $group1_css, '', '', $group1_css, '']);
          // foreach ($subGroup1['SubGroup2'] as $subGroup2) {
          //   $SubGroup2TotalQty = (float)$subGroup2['SubGroup2PurchaseRtnAmt'];
          //   if ($SubGroup2TotalQty == 0) {
          //     continue;
          //   }
          //   $rows = ['', '', '', '', $subGroup2['SubGroup2Name'], '', number_format($subGroup2['SubGroup2PurchaseRtnAmt'], 2), ''];
          //   $writer->writeSheetRow($sheet, $rows, ['', '', '', '', $group2_css, '', $group2_css, '']);
            foreach ($subGroup1['ItemDetails'] as $item) {
              ...
              $rows = ['', '', '', '', $item['ItemID'] . ' - ' . $item['ItemName'], number_format($item['PurchaseRtnAmt'], 2), ''];
              $writer->writeSheetRow($sheet, $rows, ['', '', '', '', $Item_css, $Item_css, '']);
            }
          // }
        }
      ...
      $rows = ['', 'DEBIT NOTE AMOUNT (Taxable) - less', '', '', '', '', '', number_format($TransactionAmt->TotalPurchDNAmt, 2)];
      $writer->writeSheetRow($sheet, $rows, ['', $main_group_css, '', '', '', '', '', $main_group_css]);
      ...
      foreach ($TransactionAmt->inventory as $mainGroup) {
        ...
        $rows = ['', '', $mainGroup['MainGroupName'],  '', '', '', number_format($mainGroup['MainGroupPurchDNAmt'], 2), ''];
        $writer->writeSheetRow($sheet, $rows, ['', '', $main_group_css, '', '', '', $main_group_css, '']);
        foreach ($mainGroup['SubGroup1'] as $subGroup1) {
          ...
          $rows = ['', '', '', $subGroup1['SubGroup1Name'], '', '', number_format($subGroup1['SubGroup1PurchDNAmt'], 2), ''];
          $writer->writeSheetRow($sheet, $rows, ['', '', '', $group1_css, '', '', $group1_css, '']);
          // foreach ($subGroup1['SubGroup2'] as $subGroup2) {
          //   $SubGroup2TotalQty = (float)$subGroup2['SubGroup2PurchDNAmt'];
          //   if ($SubGroup2TotalQty == 0) {
          //     continue;
          //   }
          //   $rows = ['', '', '', '', $subGroup2['SubGroup2Name'], '', number_format($subGroup2['SubGroup2PurchDNAmt'], 2), ''];
          //   $writer->writeSheetRow($sheet, $rows, ['', '', '', '', $group2_css, '', $group2_css, '']);
            foreach ($subGroup1['ItemDetails'] as $item) {
              ...
              $rows = ['', '', '', '', $item['ItemID'] . ' - ' . $item['ItemName'], number_format($item['PurchDNAmt'], 2), ''];
              $writer->writeSheetRow($sheet, $rows, ['', '', '', '', $Item_css, $Item_css, '']);
            }
          // }
        }
      ...
      // DIRECT EXPENSES
      $rows = ['DIRECT EXPENSES - add', '', '', '', '', '', '', number_format($DirectExp->CurrentYear, 2)];
      $writer->writeSheetRow($sheet, $rows, [$title_css, '', '', '', '', '', '', $title_css]);
      foreach ($DirectExp->nestedData as $DEKey => $DEVal) {
        $rows = ['', $DEVal["Group1Name"], '', '', '', '', number_format($DEVal["Group1ClsBal"], 2), ''];
        $writer->writeSheetRow($sheet, $rows, ['', $group1_css, '', '', '', '', $group1_css, '']);

        foreach ($DEVal["SubGroups2"] as $DE2Key => $DE2Val) {
          $rows = ['', '', $DE2Val["SubActGroupID"], '', '', '', number_format($DE2Val["Group2ClsBal"], 2), ''];
          $writer->writeSheetRow($sheet, $rows, ['', '', $group2_css, '', '', '', $group2_css, '']);
          foreach ($DE2Val["Accounts"] as $DEActKey => $DEActVal) {
            $rows = ['', '', '', $DEActVal["AccountID"] . ' - ' . $DEActVal["AccountName"], '', '', number_format($DEActVal["AccountClsBal"], 2), ''];
            $writer->writeSheetRow($sheet, $rows, ['', '', '', $Item_css, '', '', $Item_css, '']);
          }
        }
      }
      ...
      $rows = ['GROSS PROFIT C/F', '', '', '', '', '', '', number_format($GrossProfitC_F, 2)];
      $writer->writeSheetRow($sheet, $rows, [$title_css, '', '', '', '', '', '', $title_css]);
      $Total = $GrossProfitC_F + ($OpeningInventoryAmt->TotalinventoryAmt + $NetPurchase + $DirectExp->CurrentYear);
      $rows = ['TOTAL', '', '', '', '', '', '', number_format($Total, 2)];
      $writer->writeSheetRow($sheet, $rows, [$title_css, '', '', '', '', '', '', $title_css]);
      // EMPLOYEE BENEFITS EXPENSES	
      $rows = ['EMPLOYEE BENEFITS EXPENSES', '', '', '', '', '', '', number_format($EMPBenData->CurrentYear, 2)];
      $writer->writeSheetRow($sheet, $rows, [$title_css, '', '', '', '', '', '', $title_css]);
      foreach ($EMPBenData->nestedData as $EBKey => $EBVal) {
        $rows = ['', $EBVal["SubGroupName"], '', '', '', '', number_format($EBVal["Group2ClsBal"], 2), ''];
        $writer->writeSheetRow($sheet, $rows, ['', $group1_css, '', '', '', '', $group1_css, '']);

        foreach ($EBVal["Accounts"] as $EBActKey => $EBActVal) {
          $rows = ['', '', $EBActVal["AccountID"] . ' - ' . $EBActVal["AccountName"], '', '', '', number_format($EBActVal["AccountClsBal"], 2), ''];
          $writer->writeSheetRow($sheet, $rows, ['', '', $group2_css, '', '', '', $group2_css, '']);
        }
      }
      // TAX EXPENSES
      $rows = ['TAX EXPENSES', '', '', '', '', '', '', number_format($TaxExpense->CurrentYear, 2)];
      $writer->writeSheetRow($sheet, $rows, [$title_css, '', '', '', '', '', '', $title_css]);
      foreach ($TaxExpense->nestedData as $TaxKey => $TaxVal) {
        $rows = ['', $TaxVal["SubGroupName"], '', '', '', '', number_format($TaxVal["Group2ClsBal"], 2), ''];
        $writer->writeSheetRow($sheet, $rows, ['', $group1_css, '', '', '', '', $group1_css, '']);

        foreach ($TaxVal["Accounts"] as $TaxActKey => $TaxActVal) {
          $rows = ['', '', $TaxActVal["AccountID"] . ' - ' . $TaxActVal["AccountName"], '', '', '', number_format($TaxActVal["AccountClsBal"], 2), ''];
          $writer->writeSheetRow($sheet, $rows, ['', '', $group2_css, '', '', '', $group2_css, '']);
        }
      }

      // FINANCE COST
      $rows = ['FINANCE COST', '', '', '', '', '', '', number_format($FinanceCostData->CurrentYear, 2)];
      $writer->writeSheetRow($sheet, $rows, [$title_css, '', '', '', '', '', '', $title_css]);
      foreach ($FinanceCostData->nestedData as $FCKey => $FCVal) {
        $rows = ['', $FCVal["SubGroupName"], '', '', '', '', number_format($FCVal["Group2ClsBal"], 2), ''];
        $writer->writeSheetRow($sheet, $rows, ['', $group1_css, '', '', '', '', $group1_css, '']);

        foreach ($FCVal["Accounts"] as $FCActKey => $FCActVal) {
          $rows = ['', '', $FCActVal["AccountID"] . ' - ' . $FCActVal["AccountName"], '', '', '', number_format($FCActVal["AccountClsBal"], 2), ''];
          $writer->writeSheetRow($sheet, $rows, ['', '', $group2_css, '', '', '', $group2_css, '']);
        }
      }
      // DEPRECIATION AND AMORTIZATION EXP
      $rows = ['DEPRECIATION AND AMORTIZATION EXP', '', '', '', '', '', '', number_format($DeprecData->CurrentYear, 2)];
      $writer->writeSheetRow($sheet, $rows, [$title_css, '', '', '', '', '', '', $title_css]);
      foreach ($DeprecData->nestedData as $DAKey => $DAVal) {
        $rows = ['', $DAVal["SubGroupName"], '', '', '', '', number_format($DAVal["Group2ClsBal"], 2), ''];
        $writer->writeSheetRow($sheet, $rows, ['', $group1_css, '', '', '', '', $group1_css, '']);

        foreach ($DAVal["Accounts"] as $DAActKey => $DAActVal) {
          $rows = ['', '', $DAActVal["AccountID"] . ' - ' . $DAActVal["AccountName"], '', '', '', number_format($DAActVal["AccountClsBal"], 2), ''];
          $writer->writeSheetRow($sheet, $rows, ['', '', $group2_css, '', '', '', $group2_css, '']);
        }
      }
      // INDIRECT EXPENSES
      $rows = ['INDIRECT EXPENSES - add', '', '', '', '', '', '', number_format($OtherExpensesData->CurrentYear, 2)];
      $writer->writeSheetRow($sheet, $rows, [$title_css, '', '', '', '', '', '', $title_css]);
      foreach ($OtherExpensesData->nestedData as $IExpKey => $IExpVal) {
        $rows = ['', $IExpVal["Group1Name"], '', '', '', '', number_format($IExpVal["Group1ClsBal"], 2), ''];
        $writer->writeSheetRow($sheet, $rows, ['', $group1_css, '', '', '', '', $group1_css, '']);

        foreach ($IExpVal["SubGroups2"] as $IExp2Key => $IExp2Val) {
          $rows = ['', '', $IExp2Val["SubActGroupID"], '', '', '', number_format($IExp2Val["Group2ClsBal"], 2), ''];
          $writer->writeSheetRow($sheet, $rows, ['', '', $group2_css, '', '', '', $group2_css, '']);
          foreach ($IExp2Val["Accounts"] as $IExpActKey => $IExpActVal) {
            $rows = ['', '', '', $IExpActVal["AccountID"] . ' - ' . $IExpActVal["AccountName"], '', '', number_format($IExpActVal["AccountClsBal"], 2), ''];
            $writer->writeSheetRow($sheet, $rows, ['', '', '', $Item_css, '', '', $Item_css, '']);
          }
        }
      }
      ...
      $rows = ['REVENUE FROM OPERATION', '', '', '', '', '', '', number_format($TotalRevenueIncome, 2)];
      $writer->writeSheetRow($sheet, $rows, [$title_css, '', '', '', '', '', '', $title_css]);
      // SALE AMOUNT
      $rows = ['', 'SALE AMOUNT (Taxable) - add', '', '', '', '', '', number_format($TransactionAmt->TotalSaleAmt, 2)];
      $writer->writeSheetRow($sheet, $rows, ['', $main_group_css, '', '', '', '', '', $main_group_css]);
      foreach ($TransactionAmt->inventory as $mainGroup) {
        ...
        $rows = ['', '', $mainGroup['MainGroupName'],  '', '', '', number_format($mainGroup['MainGroupSaleAmt'], 2), ''];
        $writer->writeSheetRow($sheet, $rows, ['', '', $main_group_css, '', '', '', $main_group_css, '']);
        foreach ($mainGroup['SubGroup1'] as $subGroup1) {
          ...
          $rows = ['', '', '', $subGroup1['SubGroup1Name'], '', '', number_format($subGroup1['SubGroup1SaleAmt'], 2), ''];
          $writer->writeSheetRow($sheet, $rows, ['', '', '', $group1_css, '', '', $group1_css, '']);
          // foreach ($subGroup1['SubGroup2'] as $subGroup2) {
          //   $SubGroup2TotalQty = (float)$subGroup2['SubGroup2SaleAmt'];
          //   if ($SubGroup2TotalQty == 0) {
          //     continue;
          //   }
          //   $rows = ['', '', '', '', $subGroup2['SubGroup2Name'], '', number_format($subGroup2['SubGroup2SaleAmt'], 2), ''];
          //   $writer->writeSheetRow($sheet, $rows, ['', '', '', '', $group2_css, '', $group2_css, '']);
            foreach ($subGroup1['ItemDetails'] as $item) {
              ...
              $rows = ['', '', '', '', $item['ItemID'] . ' - ' . $item['ItemName'], number_format($item['SaleAmt'], 2), ''];
              $writer->writeSheetRow($sheet, $rows, ['', '', '', '', $Item_css, $Item_css, '']);
            }
          // }
        }
      ...
      // SALE RETURN AMOUNT
      $rows = ['', 'SALE RETURN AMOUNT (Taxable) - less', '', '', '', '', '', number_format(($TransactionAmt->TotalFreshRtnAmt + $TransactionAmt->TotalDamageRtnAmt), 2)];
      $writer->writeSheetRow($sheet, $rows, ['', $main_group_css, '', '', '', '', '', $main_group_css]);
      foreach ($TransactionAmt->inventory as $mainGroup) {
        ...
        $rows = ['', '', $mainGroup['MainGroupName'],  '', '', '', number_format(($mainGroup['MainGroupFreshRtnAmt'] + $mainGroup['MainGroupDamageRtnAmt']), 2), ''];
        $writer->writeSheetRow($sheet, $rows, ['', '', $main_group_css, '', '', '', $main_group_css, '']);
        foreach ($mainGroup['SubGroup1'] as $subGroup1) {
          ...
          $rows = ['', '', '', $subGroup1['SubGroup1Name'], '', '', number_format(($subGroup1['SubGroup1FreshRtnAmt'] + $subGroup1['SubGroup1DamageRtnAmt']), 2), ''];
          $writer->writeSheetRow($sheet, $rows, ['', '', '', $group1_css, '', '', $group1_css, '']);
          // foreach ($subGroup1['SubGroup2'] as $subGroup2) {
          //   $SubGroup2TotalQty = (float)($subGroup2['SubGroup2FreshRtnAmt'] + $subGroup2['SubGroup2DamageRtnAmt']);
          //   if ($SubGroup2TotalQty == 0) {
          //     continue;
          //   }
          //   $rows = ['', '', '', '', $subGroup2['SubGroup2Name'], '', number_format(($subGroup2['SubGroup2FreshRtnAmt'] + $subGroup2['SubGroup2DamageRtnAmt']), 2), ''];
          //   $writer->writeSheetRow($sheet, $rows, ['', '', '', '', $group2_css, '', $group2_css, '']);
            foreach ($subGroup1['ItemDetails'] as $item) {
              ...
              $rows = ['', '', '', '', $item['ItemID'] . ' - ' . $item['ItemName'], number_format(($item['FreshRtnAmt'] + $item['DamageRtnAmt']), 2), ''];
              $writer->writeSheetRow($sheet, $rows, ['', '', '', '', $Item_css, $Item_css, '']);
            }
          // }
        }
      }
      // CREDIT NOTE AGAINST SALE INVOICE AMOUNT
      $rows = ['', 'CREDIT NOTE AMOUNT (Taxable) - less', '', '', '', '', '', number_format($TransactionAmt->TotalSaleCNAmt, 2)];
      $writer->writeSheetRow($sheet, $rows, ['', $main_group_css, '', '', '', '', '', $main_group_css]);
      foreach ($TransactionAmt->inventory as $mainGroup) {
        ...
        $rows = ['', '', $mainGroup['MainGroupName'],  '', '', '', number_format($mainGroup['MainGroupSaleCNAmt'], 2), ''];
        $writer->writeSheetRow($sheet, $rows, ['', '', $main_group_css, '', '', '', $main_group_css, '']);
        foreach ($mainGroup['SubGroup1'] as $subGroup1) {
          ...
          $rows = ['', '', '', $subGroup1['SubGroup1Name'], '', '', number_format($subGroup1['SubGroup1SaleCNAmt'], 2), ''];
          $writer->writeSheetRow($sheet, $rows, ['', '', '', $group1_css, '', '', $group1_css, '']);
          // foreach ($subGroup1['SubGroup2'] as $subGroup2) {
          //   $SubGroup2TotalQty = (float)$subGroup2['SubGroup2SaleCNAmt'];
          //   if ($SubGroup2TotalQty == 0) {
          //     continue;
          //   }
          //   $rows = ['', '', '', '', $subGroup2['SubGroup2Name'], '', number_format($subGroup2['SubGroup2SaleCNAmt'], 2), ''];
          //   $writer->writeSheetRow($sheet, $rows, ['', '', '', '', $group2_css, '', $group2_css, '']);
            foreach ($subGroup1['ItemDetails'] as $item) {
              ...
              $rows = ['', '', '', '', $item['ItemID'] . ' - ' . $item['ItemName'], number_format($item['SaleCNAmt'], 2), ''];
              $writer->writeSheetRow($sheet, $rows, ['', '', '', '', $Item_css, $Item_css, '']);
            }
          // }
        }
      ...
      // CLOSING AMOUNT
      $rows = ['', 'CLOSING AMOUNT', '', '', '', '', '', number_format($ClosingInventoryAmt->TotalinventoryAmt, 2)];
      $writer->writeSheetRow($sheet, $rows, ['', $main_group_css, '', '', '', '', '', $main_group_css]);
      foreach ($ClosingInventoryAmt->inventory as $mainGroup) {
        $MainGroupTotalAmt = (float)$mainGroup['MainGroupTotalAmt'];
        if ($MainGroupTotalAmt == 0) {
          continue;
        }
      ...
      $rows = ['GROSS PROFIT B/F', '', '', '', '', '', '', number_format($GrossProfitC_F, 2)];
      $writer->writeSheetRow($sheet, $rows, [$title_css, '', '', '', '', '', '', $title_css]);

      // OTHER INCOME
      $rows = ['OTHER INCOME - add', '', '', '', '', '', '', number_format($OtherIncome->CurrentYear, 2)];
      $writer->writeSheetRow($sheet, $rows, [$title_css, '', '', '', '', '', '', $title_css]);
      foreach ($OtherIncome->nestedData as $OthKey => $OthVal) {
        $rows = ['', $OthVal["Group1Name"], '', '', '', '', number_format($OthVal["Group1ClsBal"], 2), ''];
        $writer->writeSheetRow($sheet, $rows, ['', $group1_css, '', '', '', '', $group1_css, '']);

        foreach ($OthVal["SubGroups2"] as $Oth2Key => $Oth2Val) {
          $rows = ['', '', $Oth2Val["SubActGroupID"], '', '', '', number_format($Oth2Val["Group2ClsBal"], 2), ''];
          $writer->writeSheetRow($sheet, $rows, ['', '', $group2_css, '', '', '', $group2_css, '']);
          foreach ($Oth2Val["Accounts"] as $OthActKey => $OthActVal) {
            $rows = ['', '', '', $OthActVal["AccountID"] . ' - ' . $OthActVal["AccountName"], '', '', number_format($OthActVal["AccountClsBal"], 2), ''];
            $writer->writeSheetRow($sheet, $rows, ['', '', '', $Item_css, '', '', $Item_css, '']);
          }
        }
      }
      ...
      $filename = 'Profit Loss Report-' . $from_date . ' To ' . $date . '.xlsx';
      $writer->writeToFile(str_replace($filename, TIMESHEETS_PATH_EXPORT_FILE . $filename, $filename));
      echo json_encode([
        'site_url'          => site_url(),
        'filename'          => TIMESHEETS_PATH_EXPORT_FILE . $filename,
  ...
  public function GetTransactionList(){
      ...
      "toDate"=>to_sql_date($this->input->post('toDate')).' 23:59:59'
		...
		$result = $this->ProfitLoss_model->GetTransactionList($filter_data);
    ...
    $totalQtySum = 0;$TotalItemAmt = 0;$TotalDiscAmt = 0;$TotalTaxableAmt = 0;$TotalCGSTAmt = 0;$TotalSGSTAmt = 0;$TotalIGSTAmt = 0;$TotalNetAmt = 0;
      ...
      if($TransactionType == "Sale"){
        ...
        // $redirectUrl = admin_url('KirtiOneOrder/AddEditDeliveryInvoice/').$valueue["ChallanID"];
      }else if($TransactionType == "Sale Return"){
        $redirectUrl = admin_url().'SaleReturn/AddEditSaleReturnInvoice/'.$value["OrderID"];
      ...
      $html .= '<td>' . number_format($value['OrderQty'], 2, '.', '') . '</td>';
      $html .= '<td>' . number_format($NetAmt, 2, '.', '') . '</td>';
      $html .= '<td>' . number_format($value['DiscAmt'], 2, '.', '') . '</td>';
      $html .= '<td>' . number_format($TaxableAmt, 2, '.', '') . '</td>';
      ...
      $TotalNetAmt += $NetAmt;
		...
		// $TotalBilledQty = 0;
		// $TotalItemAmt = 0;$TotalDiscAmt = 0;$TotalTaxableAmt = 0;$TotalCGSTAmt = 0; $TotalSGSTAmt = 0;$TotalIGSTAmt = 0;$TotalNetAmt = 0;
		// foreach($result as $key=>$val){
    //   if($TransactionType == "Sale"){
    //     $url = admin_url().'order/order/'.$val["OrderID"];
    //   }else if($TransactionType == "Sale Return"){
    //     $url = admin_url().'sale_return/edit/'.$val["OrderID"];
    //   }else if($TransactionType == "Purchase Return"){
    //     $url = admin_url().'purchase/purchaseRtn_list/'.$val["OrderID"];
    //   }else if($TransactionType == "Purchase"){
    //     $url = admin_url().'purchase/EditPurchaseEntry/'.$val["OrderID"];
    //   }else if($TransactionType == "Sale Credit"){
    //     $url = admin_url().'cd_notes/edit/'.$val["OrderID"];
    //   }else if($TransactionType == "Purchase Debit"){
    //     $url = admin_url().'cd_notes/edit/'.$val["OrderID"];
    //   }else{
    //     $url = "#";
    ...
    //   $html .= "<td style='text-align:center;'>".$val["TransID"]."</td>";
    //   $html .= "<td style='text-align:center;'>"._d(substr($val["TransDate2"],0,10))."</td>";
    //   //$html .= "<td style='text-align:center;'>".$val["BillID"]."</td>";
    //   $html .= "<td>".$val["company"]."</td>";
    //   $html .= "<td style='text-align:right;'>".number_format($val["BilledQty"], 2, '.', ',')."</td>";
    //   $TotalBilledQty += $val["BilledQty"];
    //   $html .= "<td style='text-align:center;'>".number_format($val["BasicRate"], 2, '.', ',')."</td>";
    //   $html .= "<td style='text-align:right;'>".number_format($val["TotalAmt"], 2, '.', ',')."</td>";
    //   $TotalItemAmt += $val["TotalAmt"];
    //   $html .= "<td style='text-align:center;'>". number_format($val["DiscPerc"], 2, '.', ',')."</td>";
    //   $html .= "<td style='text-align:right;'>". number_format($val["DiscAmt"], 2, '.', ',')."</td>";
    //   $TotalDiscAmt += $val["DiscAmt"];
    //   $TaxableAmt = $val["TotalAmt"] - $val["DiscAmt"];
    //   $TotalTaxableAmt += $TaxableAmt;
    //   $SGSTAmt = 0;$CGSTAmt = 0;$IGSTAmt = 0;$NetTAmt = 0;
    //   if($val["cgstamt"] > 0){
    //       $SGSTAmt = $TaxableAmt * (($val["GSTPer"]/2)/100);
    //       $CGSTAmt = $TaxableAmt * (($val["GSTPer"]/2)/100);
    //       $TotalSGSTAmt += $SGSTAmt;
    //       $TotalCGSTAmt += $CGSTAmt;
    //   }else{
    //       $IGSTAmt = $TaxableAmt * ($val["GSTPer"]/100);
    //       $TotalIGSTAmt += $IGSTAmt;
    //   }
    //   $NetTAmt = $TaxableAmt + $SGSTAmt + $CGSTAmt + $IGSTAmt;
    //   $TotalNetAmt += $NetTAmt;
    //   $html .= "<td style='text-align:right;'>".number_format($TaxableAmt, 2, '.', ',')."</td>";
    //   $html .= "<td style='text-align:center;'>".$val["GSTPer"]."</td>";
    ...
    //   $html .= "<td style='text-align:right;'>".number_format($NetTAmt, 2, '.', ',')."</td>";
    //   $html .= "</tr>";
    //   $SrNo++;
		// }
		  $html .= '<tr>';
        $html .= '<td colspan="9"><strong>Total</strong></td>';        
        $html .= '<td><strong>' . number_format($totalQtySum, 2, '.', '') . '</td>';
        $html .= '<td><strong>' . number_format($TotalItemAmt, 2, '.', '') . '</strong></td>';
        $html .= '<td><strong>' . number_format($TotalDiscAmt, 2, '.', '') . '</strong></td>';
        $html .= '<td><strong>' . number_format($TotalTaxableAmt, 2, '.', '') . '</strong></td>';
        $html .= '<td><strong>' . number_format($TotalCGSTAmt, 2, '.', '') . '</strong></td>';
        $html .= '<td><strong>' . number_format($TotalSGSTAmt, 2, '.', '') . '</strong></td>';
        $html .= '<td><strong>' . number_format($TotalIGSTAmt, 2, '.', '') . '</strong></td>';
        $html .= '<td><strong>' . number_format($TotalNetAmt, 2, '.', '') . '</strong></td>';
			$html .= '</tr>';
		$html .= "</tbody>";
		echo json_encode($html);
		die;
