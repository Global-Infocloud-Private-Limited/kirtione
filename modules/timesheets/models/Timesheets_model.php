...
public function calculate_attendance_timesheets($staffid, $time_in, $time_out){ 
    ...
    // Get IN Out Data for staff
    public function GetStaffByFilter($data){
        ...
        if ($by_staff) {
            $this->db->where(db_prefix() . 'staff.staffid', $by_staff);
        }
        
        if ($by_position) {
            $this->db->where(db_prefix() . 'staff.job_position', $by_position);
        }
        return $this->db->get()->result_array();    
    }
    // Get IN Data for staff
    public function GetTimesheetReport_check_in($data)
    {
        ...
        if ($by_staff) {
            $this->db->where(db_prefix() . 'check_in_out.staff_id', $by_staff);
        }
        if ($by_position) {
            $this->db->where(db_prefix() . 'staff.job_position', $by_position);
        }
    ...
    // Get Out Data for staff
    public function GetTimesheetReport_check_out($data)
    {
        $from_date = to_sql_date($data['from_date']).' 00:00:00';
        $to_date = to_sql_date($data['to_date']).' 23:59:59';
        $by_staff = $data['by_staff'];
    ...
    // Get Out Data for staff
    public function get_travel_detail_by_staff_id($staff_id, $fromDate, $toDate = null)
    {
        if($toDate) {
            $from_date = substr($fromDate, 0, 10) . ' 00:00:00';
            $to_date = substr($toDate, 0, 10) . ' 23:59:59';
        } else {
            $from_date = substr($fromDate, 0, 10) . ' 00:00:00';
            $to_date = substr($fromDate, 0, 10) . ' 23:59:59';
        }
    ...
    public function get_travel_points_by_staff_id($staff_id,$date) {
        $from_date = substr($date,0,10).' 00:00:00';
        $to_date = substr($date,0,10).' 23:59:59';
        
        // $tmpData = array($from_date,$to_date);
        
        $this->db->select('tblLocationTracking.latitude,tblLocationTracking.longitude');
        $this->db->from(db_prefix() . 'LocationTracking');
        $this->db->where(db_prefix() . 'LocationTracking.staffid', $staff_id);
        $this->db->where('TransDate >=', $from_date);
        $this->db->where('TransDate <=', $to_date);
