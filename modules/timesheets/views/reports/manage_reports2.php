               ...
               <div class="panel-body">
                  ...
                  <div class="row ">
                    ...
                    <div class="col-md-2 leads-filter-column">
                     <?php
                    $selected_date =  _d($this->uri->segment('4'));
                    ?>
                  ...
                  <hr class="hr-panel-heading" />
                    <div class="row">
                      <div class="col-md-7" style="overflow-y: scroll; height: 500px">
                        ...
                        <div class="col-md-5" id="map_container_parent">
                            <div id="map0"></div>
                        </div>
                    </div>
               </div>
...
<?php init_tail(); ?>
<script>
    function loadMap(polylinePoints_) {
        
        var tmpArray = JSON.parse(polylinePoints_);
        var travelPoints = [];
        for (let i = 0; i < tmpArray.length; i++) {
            travelPoints.push([tmpArray[i].latitude, tmpArray[i].longitude]);
        }
        document.getElementById('map_container_parent').innerHTML = "<div id='map0'></div>";
        var map = L.map('map0').setView(travelPoints[0], 14);
        L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
            maxZoom: 19,
            attribution: 'S'
        }).addTo(map);
        
        var polyline = L.polyline(travelPoints, { weight: 3, opacity: 1, color: 'black' }).addTo(map);
    }

    function getTravelPoints() {
        var staffID = $('#staff').val();
        var _date = $('#report_date').val();
        let [day, month, year] = _date.split('/');
        const dateStr = year +"-"+month+"-"+day;
        $.ajax({
            url: admin_url + 'timesheets/travel_points',
            method:"POST",
            data:{staff_id:staffID,date: dateStr},
