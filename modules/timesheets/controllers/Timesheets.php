...
class timesheets extends AdminController
{
	...
	{
	    if (!has_permission_new('timesheets_setting', '', 'view')) {
...
public function get_data_additional_timesheets($id){
	...
	*/
		...
		if ($this->input->is_ajax_request()) {
			if($this->input->post()){
				...
				foreach ($rResult as $aRow) {
					...
					$option = '';
					if (has_permission('timesheets_shift_type', '', 'edit') || is_admin()){
					    ...
					    $option .= '<a href="#" class="btn btn-default btn-icon disabled" disabled onclick="edit_shift_type(this); return false;" data-id="'.$aRow['id'].'" data-shift_type_name="'.$aRow['shift_type_name'].'" data-color="'.$aRow['color'].'" data-time_start="'.$aRow['time_start'].'" data-time_end="'.$aRow['time_end'].'" data-time_start_work="'.$aRow['time_start_work'].'" data-time_end_work="'.$aRow['time_end_work'].'" data-start_lunch_break_time="'.$aRow['start_lunch_break_time'].'" data-end_lunch_break_time="'.$aRow['end_lunch_break_time'].'" data-description="'.$aRow['description'].'" >';
						$option .= '<i class="fa fa-edit"></i>';
						$option .= '</a>';
					}
					if (has_permission('timesheets_shift_type', '', 'delete') || is_admin()){
					    ...
					    $option .= '<a href="' . admin_url('timesheets/delete_shift_type/' . $aRow['id']) . '" class="btn btn-danger btn-icon _delete disabled" disabled>';
						$option .= '<i class="fa fa-remove"></i>';
						$option .= '</a>';
					}
					$row[] = $option; 
	...
	public function manage_shift_type(){
		if (!(has_permission('timesheets_shift_type', '', 'view_own') || has_permission('timesheets_shift_type', '', 'view') || is_admin())) {          
			access_denied('timekeeping');
		}
		...
		if ($this->input->is_ajax_request()) {
			if($this->input->post()){
				...
				foreach ($rResult as $aRow) {
					...
					$option = '';
					if (has_permission('table_shiftwork_management', '', 'edit') || is_admin()){
					    ...
					    $option .= '<a href="' . admin_url('timesheets/add_allocation_shiftwork/' . $aRow['id']) . '" class="btn btn-default btn-icon disabled" disabled>';
						$option .= '<i class="fa fa-pencil-square-o"></i>';
						$option .= '</a>';
					}
					if (has_permission('table_shiftwork_management', '', 'delete') || is_admin()){
						$option .= '<a href="' . admin_url('timesheets/delete_shift/' . $aRow['id']) . '" class="btn btn-danger btn-icon _delete">';
						$option .= '<i class="fa fa-remove"></i>';
						$option .= '</a>';
					}else{
					    $option .= '<a href="' . admin_url('timesheets/delete_shift/' . $aRow['id']) . '" class="btn btn-danger btn-icon _delete disabled" disabled>';
						$option .= '<i class="fa fa-remove"></i>';
						$option .= '</a>';
					}
...
public function history_check_in_out_report3(){
 ...
 /**
	...
	*/
	...
	/*  Accounts Head List */
    public function TimesheetReport($accountId = '')
    {
        if (!has_permission_new('timesheet_report', '', 'view')) {
            access_denied('Invoice Items');
        }
        
        $data['title'] = "In Out Time list";
        $data['AllStaff'] = $this->timesheets_model->get_staff();
        $data['departments'] = $this->departments_model->get();
		$data['roles']         = $this->roles_model->get();
		$data['job_position'] = $this->timesheets_model->get_job_position();
        
        $this->load->view('timekeeping/TimesheetReport', $data); 
    }
    //load table in here
    public function GetTimesheetReport()
    {
        $Check_in_data =$this->timesheets_model->GetTimesheetReport_check_in($this->input->post());
        $Check_out_data =$this->timesheets_model->GetTimesheetReport_check_out($this->input->post());
        $GetStaffFilter = $this->timesheets_model->GetStaffByFilter($this->input->post());
        $by_staff = $this->input->post('by_staff');
        $by_dept = $this->input->post('by_dept');
        $by_position = $this->input->post('by_position');
        /*echo json_encode($Check_out_data);
        die;*/
        $html ='';
        foreach($Check_in_data as $value){
            ...
            $travel_detail = $this->timesheets_model->get_travel_detail_by_staff_id($value['staff_id'],$value['date']);
            $j = 1;
            $dist_cal_new = 0.00;
            $pi80 = M_PI / 180; 
            $r = 6372.797; // mean radius of Earth in km
            ...
            $html.= '<td>'._d(substr($value['date'],0,10)).'</td>'; 
            $html.= '<td>'._d($value['date']).'</td>'; 
            $html.= '<td>'._d($OutTime).'</td>'; 
            $html.= '<td>'.round($dist_cal_new, 2).'</td>'; 
            $html.= '<td><a href="' . admin_url('timesheets/TravelDetails/' . substr($value['date'],0,10).'/' . $value['staff_id']) . '" target="_blank">read more..</a></td>'; 
            $html.= '<tr>';
        }
        $data_array =array('html'=>$html,'by_staff'=>$by_staff,'by_dept'=>$by_dept,'by_position'=>$by_position);
    ...
    {
        $this->app->get_table_data('TravelDetails');
    }
    
    public function export_timesheetReport()
    {
        if (!class_exists('XLSXReader_fin')) {
            require_once(module_dir_path(TIMESHEETS_MODULE_NAME) . '/assets/plugins/XLSXReader/XLSXReader.php');
        }
        ...
        $set_col_tk = [];
            ...
            $set_col_tk["In Time"] = 'In Time';
            $set_col_tk["Out Time"] = 'Out Time';
            $set_col_tk["Km. Travel"] = 'Km. Travel';
            $writer_header = $set_col_tk;
        ...
        foreach ($Check_in_data as $value) {
            ...
            $list_add = [];
                ...
                $list_add[] = _d(substr($value['date'], 0, 10));
                $list_add[] = _d($value['date']);
                $list_add[] = _d($OutTime);
                $list_add[] = round($dist_cal_new, 2);
    ...
    public function travel_points() {
        $data = $this->input->post();
        // echo json_encode($data);
        echo json_encode($this->timesheets_model->get_travel_points_by_staff_id($data['staff_id'],$data['date']));
    }
