...
//============= Mandi Purchase Order punch =====================================
    public function mandi_pur_order($id = '')
    {
        if (!has_permission_new('mandi_purchase_order', '', 'view')) {
        ...
        if ($this->input->post()) {
            ...
            if ($id == '') {
                if (!has_permission_new('mandi_purchase_order', '', 'create')) {
                    access_denied('purchase_order');
                }
                $id = $this->purchase_model->add_mandi_pur_order($pur_order_data);
                if ($id) {
                    set_alert('success', _l('added_successfully', _l('pur_order')));
                    redirect(admin_url('purchase/mandi_pur_order'));
                }
            }else{
                if (!has_permission_new('mandi_purchase_order', '', 'edit')) {
                    access_denied('purchase_order');
                }
                $id = $this->purchase_model->edit_mandi_pur_order($pur_order_data);
            }
        }

        $title = "Mandi Purchase Order";
        $this->load->model('currencies_model');
        $data['base_currency'] = $this->currencies_model->get_base_currency();
        ...
        if($id){
            $OrderDetails = $this->purchase_model->GetMandiOrderDetails($id);
            $data['OrderDetails'] = $OrderDetails;
            $data['pur_Details'] = json_encode($OrderDetails->details);
            $data['WHListByCenterID'] = $this->purchase_model->WHListByCenterID($OrderDetails->CenterID);
        }
        ...
        $data['PaymentCycleList'] = $this->purchase_model->GetPaymentCycleList();
        $data['TDSList'] = $this->purchase_model->GetTDSList();
        $data['title'] = $title;
        $this->load->view('purchase_order/Mandi_pur_order', $data);
    }
    
    public function GetAccountDetails($AccountID)
    {
        $value = $this->purchase_model->GetAccountDetails($AccountID);
        echo json_encode([
            'value' => $value
        ]);
    }
    
    public function GetTdsPercentage()
    {
        $TDSCode = $this->input->post('TDSCode');
        $value = $this->purchase_model->GetTdsPercentage($TDSCode);
        echo json_encode($value);
    }

//============= Direct Purchase Order punch ====================================
    // Get Account Group 2 List
    public function GetLedgerGroupList()
    {
        $PurchaseType = $this->input->post('PurchaseType');
        $value = $this->purchase_model->GetLedgerGroupList($PurchaseType);
        echo json_encode($value);
    }
    ...
    {
        $LedgerGrp = $this->input->post('LedgerGrp');
        $value = $this->purchase_model->GetLedgerList($LedgerGrp);
        echo json_encode($value);
    }
    // Direct Purchase Order Create From ERP (Direct Purchase)
    public function pur_order($PONumber = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if ($PONumber == '') {
                ...
                $id = $this->purchase_model->AddDirectPurchase($pur_order_data);
                if ($id) {
                    set_alert('success', _l('added_successfully', _l('pur_order')));
                ...
                $id = $this->purchase_model->UpdateDirectPurchase($pur_order_data,$PONumber);
                if ($id) {
                    set_alert('success', _l('added_successfully', _l('pur_order')));
                    redirect(admin_url('purchase/pur_order'));
                }
        ...
        if ($PONumber == '') {
            ...
            $PurchaseDetails = $this->purchase_model->GetPurchaseDetails($PONumber);
            $data['purchase_details'] = $PurchaseDetails;
            $PurchaseItemList = $this->purchase_model->GetPurchaseItemList($PONumber);
            $data['pur_order_detail'] = json_encode($PurchaseItemList);
            $data['WHListByCenterID'] = $this->purchase_model->GetWHListByCenterID($PurchaseDetails->CenterID);
            $data['TDSRateBySection'] = $this->purchase_model->GetTDSRateBySection($PurchaseDetails->TDS);
            $data['LedgerDetails'] = $this->purchase_model->GetLedgerDetails($PurchaseDetails->LedgerEffect);
...
/**
    ...
    public function order_list($id="")
    {
        if (!has_permission_new('purchase-order', '', 'view')) {
         ...
         if ($this->input->post()) {
            ...
            $idd = $this->purchase_model->update_purchase_order_new($pur_order_data,$id);
             if ($idd) {
                    set_alert('success', _l('updated_successfully', _l('pur_order')));
                }else{
        ...
        $title = "Edit Purchase Order";
        $data['purchase_details'] = $this->purchase_model->GetPurchaseDetails($id);
        $data['purchase_history'] = $this->purchase_model->GetPurchaseItemList($id);
        $data['vendors'] = $this->purchase_model->get_vendor_data();
        $data['units'] = $this->purchase_model->get_units();
        $data['items'] = $this->purchase_model->get_items();
        $data['item_code'] = $this->purchase_model->get_items_code();
    ...
    public function load_mandi_purchase_list()
    {
        $data = array(
           'from_date' => $this->input->post('from_date'),
           'to_date'  => $this->input->post('to_date'),
        );
        $data = $this->purchase_model->load_mandi_purchase_list($data);
        echo json_encode($data);
    }
    // Load Purchase Data
    public function load_data_for_purchase()
    {
        ...
        $PurchaseList = $this->purchase_model->load_data_for_purchase($data);
        $html = "";
        $TotalPurchAmt = 0;
        $TotalDiscAmt = 0;
        $TotalFrtAmt = 0;
        ...
        foreach($PurchaseList as $key=>$val){
            $url = admin_url()."purchase/pur_order/".$val["PurchID"];
            $html .= '<tr onclick="window.open('."'".$url."'".')">';
            $html .= '<td style="text-align:center;">'.$val["BT"].'</td>';
            ...
            $html .= '<td style="text-align:center;">'.$val["Invoiceno"].'</td>';
            $html .= '<td style="text-align:center;">'._d(substr($val["Invoicedate"],0,10)).'</td>';
            $html .= '<td style="text-align:right;">'.$val["Purchamt"].'</td>';
            $html .= '<td style="text-align:right;">'.$val["Discamt"].'</td>';
            $html .= '<td style="text-align:right;">'.$val["Frtamt"].'</td>';
            $html .= '<td style="text-align:right;">'.$val["Othamt"].'</td>';
        ...
        $html .= '<td style="text-align:right;"><b>'.number_format($TotalFrtAmt, 2, '.', ',').'</b></td>';
        $html .= '<td style="text-align:right;"><b>'.number_format($TotalOthAmt, 2, '.', ',').'</b></td>';
        ...
        $html .= '<td style="text-align:right;"><b>'.number_format($TotalTdsAmt, 2, '.', ',').'</b></td>';
    ...
    public function pur_return(){
        ...
        if (!has_permission_new('purchase_return', '', 'view')) { 
            access_denied('purchase return');
        }
    ...
    public function purchaseRtn_list($id=""){
         if ($this->input->post()) {
            ...
            $idd = $this->purchase_model->update_purchaseRtn_order_new($purRtn_order_data,$id);
             if ($idd) {
                    set_alert('success', _l('updated_successfully', _l('pur_order')));
                }else{
                  set_alert('error', _l('Some thing went wrong', _l('pur_order'))); 
