...
{   
    ...
    public function GetVendorList($id = '')
    {
     ...
     $selected_company = $this->session->userdata('root_company');
      $year = $_SESSION['finacial_year'];
       $this->db->select(db_prefix() . 'clients.*,'.db_prefix() . 'accountbalances.BAL1,'.db_prefix() . 'xx_citylist.city_name,'.db_prefix() . 'contacts.FLNO1,'.db_prefix() . 'contacts.email,'.db_prefix() . 'contacts.Pan,'.db_prefix() . 'contacts.aadhaar_number');
        ...
        $this->db->join(db_prefix() . 'xx_citylist', db_prefix() . 'xx_citylist.id = ' . db_prefix() . 'clients.dist', 'left');
        $this->db->join(db_prefix() . 'xx_statelist', db_prefix() . 'xx_statelist.short_name = ' . db_prefix() . 'clients.state', 'left');
        $this->db->join(db_prefix() . 'accountbalances', db_prefix() . 'accountbalances.AccountID = ' . db_prefix() . 'clients.AccountID AND '.db_prefix() . 'accountbalances.PlantID = ' . db_prefix() . 'clients.PlantID AND '.db_prefix() . 'accountbalances.FY ="'.$year.'"', 'left');
       $this->db->where(db_prefix() . 'clients.PlantID', $selected_company);
    ...
    {
        $selected_company = $this->session->userdata('root_company');
        $cust_type = array('1','3','4');
        $payable_receible = array('1000006','1000016');
        $this->db->select(' CONCAT(AccountID," - " ,company) AS label,'.db_prefix() . 'clients.AccountID AS id,');
        $this->db->where_in(db_prefix() . 'clients.CustomerType', $cust_type);
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID', $payable_receible);
        $this->db->where(db_prefix() . 'clients.PlantID', $selected_company);
        $this->db->where(db_prefix() . 'clients.KYCStatus', '6');
        $this->db->order_by('company', 'asc');
        return $this->db->get(db_prefix() . 'clients')->result_array();
    }
    ...
    {
        $this->db->select(' CONCAT(CycleID," - " ,CycleName) AS label,'.db_prefix() . 'PaymentCycle.CycleID AS id,');
        $this->db->order_by('CycleName', 'asc');
        return $this->db->get(db_prefix() . 'PaymentCycle')->result_array();
    }
    ...
    public function GetAccountDetails($AccountID)
    {
        ...
        $cust_type = array('1','3','4');
        $payable_receible = array('1000006','1000016');
        $this->db->select( db_prefix() . 'clients.company AS label,'.db_prefix() . 'clients.AccountID AS id,');
        $this->db->where_in(db_prefix() . 'clients.CustomerType', $cust_type);
        $this->db->where_in(db_prefix() . 'clients.SubActGroupID', $payable_receible);
        $this->db->where(db_prefix() . 'clients.PlantID', $selected_company);
        $this->db->where(db_prefix() . 'clients.KYCStatus', '6');
    ...
    public function GetTdsPercentage($TDSCode)
    {
        $this->db->select('tblTDSDetails.TDSCode,tblTDSDetails.rate');
        $this->db->where(db_prefix() . 'TDSDetails.TDSCode', $TDSCode);
        return $this->db->get(db_prefix() . 'TDSDetails')->result_array();
    }
    ...
    public function GetTDSList()
    {
        ...
        $this->db->select( db_prefix() . 'TDSMaster.TDSName,'.db_prefix() . 'TDSMaster.TDSCode');
        $this->db->where(db_prefix() . 'TDSMaster.Blocked', 'N');
        $this->db->order_by('id', 'ASC');
        return $this->db->get(db_prefix() . 'TDSMaster')->result_array();
    }
    // Get Ledger SubGroup 2 List BY Sub Group 1
    public function GetLedgerGroupList($PurchaseType)
    {
        $this->db->select('tblaccountgroupssub.SubActGroupID,tblaccountgroupssub.SubActGroupName');
        $this->db->where(db_prefix() . 'accountgroupssub.SubActGroupID1', $PurchaseType);
        return $this->db->get(db_prefix() . 'accountgroupssub')->result_array();
    }
    // Get Ledger List By Ledger Group 2
    public function GetLedgerList($LedgerGrp)
    {
        $this->db->select('tblclients.AccountID,tblclients.company');
        $this->db->where(db_prefix() . 'clients.SubActGroupID', $LedgerGrp);
        return $this->db->get(db_prefix() . 'clients')->result_array();
    }
    ...
    public function get_items(){
         $selected_company = $this->session->userdata('root_company');
    //   $year = $_SESSION['finacial_year'];
       return $this->db->query('select id as id, CONCAT(ItemID," - " ,ItemName) as label from '.db_prefix().'items where PlantID = '.$selected_company)->result_array();
    }
    
    public function get_items_code(){
         $selected_company = $this->session->userdata('root_company');
    //   $year = $_SESSION['finacial_year'];
       return $this->db->query('select ItemID as id, CONCAT(ItemID," - ",ItemName) as label,ItemID from '.db_prefix().'items where PlantID = '.$selected_company)->result_array();
    }
    
    public function get_items_code_purReturn(){
         $selected_company = $this->session->userdata('root_company');
    //   $year = $_SESSION['finacial_year'];
       return $this->db->query('select ItemID as item_code, CONCAT(ItemID," - ",ItemName) as label,ItemID from '.db_prefix().'items where PlantID = '.$selected_company)->result_array();
    }
    public function get_items_for_purchRtn(){
         $selected_company = $this->session->userdata('root_company');
       return $this->db->query('select ItemID as id, CONCAT(ItemID," - " ,ItemName) as label from '.db_prefix().'items where PlantID = '.$selected_company)->result_array();
    }
    /**
    ...
    public function get_items_by_vendor($vendor){
       return $this->db->query('select id as id, CONCAT(ItemID," - " ,ItemName) as label from '.db_prefix().'items where id IN ( select items from '.db_prefix().'pur_vendor_items where vendor = '.$vendor.' )')->result_array();
    }
    public function get_items_by_vendor_data($item,$vendor)
    {
       return $this->db->query('select ItemID as id, CONCAT(ItemID," - " ,ItemName) as label from '.db_prefix().'items where ItemID IN ( select items from '.db_prefix().'pur_vendor_items where vendor = '.$vendor.' and items ='.$item.' )')->result_array();
    }
    ...
    // Get Item details By ItemID Using AJAX
    public function GetItemDetails($ItemID)
    {
        $this->db->select('tblitems.*,tblitems_sub_groups.*,tbltaxes.taxrate');
    ...
    public function get_p_order_detail($pur_request)
    {
        $selected_company = $this->session->userdata('root_company');
        $year = $_SESSION['finacial_year'];
        $this->db->select( db_prefix() . 'items_sub_groups.main_group_id,'.db_prefix() . 'history.*,'.db_prefix() . 'items.ItemID AS id,tblitems.ItemName,'.db_prefix() . 'items_main_groups.name');
    ...
    public function AddDirectPurchase($data)
    {
        ...
        $purchase_orderNumbar = get_option('next_purchase_number_for_kirti');
        $ItemDetails =  $this->db->get_where(db_prefix().'items',array('id'=>$ItemID,'PlantID'=>$PlantID))->row();
        $ItemID = $ItemDetails->ItemID;
        $AccountLedger = $data['Ledger'];
        /*echo "<pre>";
        echo $AccountLedger;
        print_r($data);
        die;*/
        ...
        $invoiceAmt = $data['vendor_invoice_amt'];
       ...
       die;*/
        ...
        if($this->db->affected_rows() > 0){
            ...
            $narrations = 'By PO no.'.$new_purchase_orderNumbar.' / against Vendor invoice no '.$Invoiceno;
            $ledger_credit = array(
                "PlantID" => $PlantID,
                "FY" => $FY,
                "Transdate" => $Transdate,
            ...
            if($tdsAmt > 0){
                ...
                $this->db->insert(db_prefix() . 'accountledger',$ledger_tdsAct);
                $ord_n++;
                
                // Party Ledger Debit against TDS Account ledger
                $ledger_tdsAct = array(
                    "PlantID"    => $PlantID,
                    "Transdate"  => $Transdate,
                    "TransDate2" => date('Y-m-d H:i:s'),
        ...
        // Debit ledger for selected account
            ...
            foreach($es_detail as $value){
                ...
                $SaleRate = $value['PurchRate'] + ($value['PurchRate'] * ($value['GST'] / 100));
    ...
    public function edit_mandi_pur_order($data)
    {
        if(isset($data['pur_order_detail'])){
          $pur_order_detail = json_decode($data['pur_order_detail']);
        ...
        $MandiPurchMaster = array(
            ...
            'ItemID'=>$ItemID,
            'OrderQty'=>$total_qty_in_qtl,
            'PurchAmt'=>$Total_value,
            'BrokerAmt'=>$total_brokerage,
            'MrktLevyAmt'=> $total_mrkt_levy,
            'GrossAmt'=>$total_gross_value,
            'TDS'=>$TDSCode,
            'tds_per'=>$tds_rate,
            'tdsAmt'=>$total_tds,
            'InvAmt'=>$total_net_value,
            'ItCount'=>$ItCount,
            "UserID2" => $_SESSION['username'],
            'Lupdate' =>date('Y-m-d H:i:s'),
        );
        $this->db->where('PurchID', $MPurchID);
        $this->db->update(db_prefix() . 'mandipurchasemaster',$MandiPurchMaster);
        if($this->db->affected_rows() > 0){
            foreach($es_detail as $value){
                $MandiPurchdetails = array(
                    'ItemID'=>$ItemID,
                    'TransDate' =>$Transdate,
                    'TransDate2' =>date('Y-m-d H:i:s'),
                    'AccountID' =>$value['VendorCode'],
                    'Payment_term' =>$value['PaymentTerm'],
                    'bag' =>$value['Bag'],
                    'wt_per_bag'=>$value['Weight_per_bag'],
                    'loose_in_kg'=>$value['Loose_in_kg'],
                    'OrderQty'=>$value['Qty_in_quintal'],
                    'PurchRate'=> $value['Rate_per_quintal'],
                    'OrderAmt'=>$value['Value'],
                    'BrokerAmt'=>$value['Brokerage'],
                    'MrtLevyAmt'=>$value['MarketLevy'],
                    'Round_off'=>$value['Round_off'],
                    'GrossAmt'=>$value['Gross'],
                    'TDSAmt'=>$value['TDS_Amt'],
                    'NetOrderAmt'=>$value['Net_Amt']
                );
                if($value['TransID'] == ""){
                    $MandiPurchdetails["PlantID"] = $PlantID;
                    $MandiPurchdetails["FY"] = $FY;
                    $MandiPurchdetails["PurchID"] = $MPurchID;
                    $MandiPurchdetails["UserID"] = $_SESSION['username'];
                    $this->db->insert(db_prefix() . 'mandipurchaseDetails',$MandiPurchdetails);
                    $last_mandi_details_id = $this->db->insert_id();
                }else{
                    $MandiPurchdetails["UserID2"] = $_SESSION['username'];
                    $MandiPurchdetails["LupDate"] = date('Y-m-d H:i:s');
                    $this->db->where('TransID', $value['TransID']);
                    $this->db->update(db_prefix() . 'mandipurchaseDetails',$MandiPurchdetails);
                }
                if($this->db->affected_rows() > 0){
                    $purchaseMaster = array(
                        'CenterID' =>$CenterID,
                        'WHID' =>$GodownID,
                        'PartyID'=>$PartyID,
                        'Transdate' =>$Transdate,
                        'AccountID'=>$value['VendorCode'],
                        'Purchamt'=> $value['Gross'],
                        'BrokerAmt'=> $value['Brokerage'],
                        'MrtLevyAmt'=> $value['MarketLevy'],
                        'Invamt'=>$value['Net_Amt'],
                        'ItCount'=>1,
                        'RoundOffAmt'=>$value['Round_off'],
                        'TDS'=>$TDSCode,
                        'tds_per'=>$tds_rate,
                        'tdsAmt'=>$value['TDS_Amt'],
                    ...
                    if($value['TransID'] == ""){
                        $PO = get_option('next_purchase_number_for_kirti');
                        $new_po = 'PO'.$FY.$PO; 
                        $purchaseMaster["PlantID"] = $PlantID;
                        $purchaseMaster["FY"] = $FY;
                        $purchaseMaster["PurchType"] = "M";
                        $purchaseMaster["BT"] = "B";
                        $purchaseMaster["TransID"] = $MPurchID;
                        $purchaseMaster["PurchID"] = $new_po;
                        $purchaseMaster["Userid"] = $_SESSION['username'];
                        $this->db->insert(db_prefix() . 'purchasemaster',$purchaseMaster);
                    }else{
                        $purchaseMaster["UserID2"] = $_SESSION['username'];
                        $purchaseMaster["Lupdate"] = date('Y-m-d H:i:s');
                        $this->db->where('PurchID', $value['TransID']);
                        $this->db->update(db_prefix() . 'purchasemaster',$purchaseMaster);
                    }
                    if($this->db->affected_rows() > 0){
                        $data_array_result = array(
                            'PartyID'=>$PartyID,
                            ...
                            'final_rate'=>$value['PurchRate'],
                            'SuppliedIn'=>1,
                            'OrderQty'=>$value['OrderQty']/10,
                            'BilledQty'=>$value['OrderQty']/10,
                            'OrderAmt'=>$value['OrderAmt'],
                            'ChallanAmt'=>$value['OrderAmt'],
                            'NetOrderAmt'=>$value['OrderAmt'],
                            'NetChallanAmt'=>$value['OrderAmt']
                        );
                        if($value['TransID'] == ""){
                            $this->increment_next_purchase_number();
                            // update Mandi Purchase Details table with Original purchase order
                            $this->db->where('PlantID', $PlantID);
                            $this->db->where('FY', $FY);
                            $this->db->where('id', $last_mandi_details_id);
                            $this->db->update(db_prefix() . 'mandipurchaseDetails', [
                                'DocNo' => $new_po,
                                'TransID' => $new_po,
                            ]);
                            $data_array_result["PlantID"] = $PlantID;
                            $data_array_result["FY"] = $FY;
                            $data_array_result["OrderID"] = $MPurchID;
                            $data_array_result["BillID"] = $MPurchID;
                            $data_array_result["TransID"] = $new_po;
                            $data_array_result["UserID"] = $_SESSION['username'];
                            $data_array_result["TType"] = "P";
                            $data_array_result["TType2"] = "Purchase";
                            $data_array_result["TypeID"] = "SP";
                            $this->db->insert(db_prefix() . 'history',$data_array_result);
                        }else{
                            $data_array_result["UserID2"] = $_SESSION['username'];
                            $data_array_result["Lupdate"] = date('Y-m-d H:i:s');
                            $this->db->where('OrderID', $MPurchID);
                            $this->db->where('TransID', $value['TransID']);
                            $this->db->update(db_prefix() . 'history',$data_array_result);
                            // delete Previous ledger
                            $GetPreviousLedgerEntry = $this->GetPreviousLedgerEntry($value['TransID']);
                            foreach($GetPreviousLedgerEntry as $LKey=>$LVal){
                                $ledger_audit = array(
            						"PlantID"=>$LVal["PlantID"],
            						"FY"=>$LVal["FY"],
            						"Transdate"=>$LVal["Transdate"],
            						"TransDate2"=>$LVal["TransDate2"],
            						"VoucherID"=>$LVal["VoucherID"],
            						"PartyID"=>$LVal["PartyID"],
            						"AccountID"=>$LVal["AccountID"],
            						"CenterID"=>$LVal["CenterID"],
            						"CommodityID"=>$LVal["CommodityID"],
            						"EntryFor"=>$LVal["EntryFor"],
            						"TType"=>$LVal["TType"],
            						"Amount"=>$LVal["Amount"],
            						"Narration"=>$LVal["Narration"],
            						"PassedFrom"=>$LVal["PassedFrom"],
            						"OrdinalNo"=>$LVal["OrdinalNo"],
            						"UserID"=>$LVal["UserID"],
            						"Lupdate"=>date('Y-m-d H:i:s'),
            						"UserID2"=>$_SESSION['username'],
            					);
                            ...
                            $this->db->LIKE('VoucherID', $value['TransID']);
    ...
    public function GetPreviousLedgerEntry($VoucherID)
    {
        $selected_company = $this->session->userdata('root_company');
    ...
    public function add_mandi_pur_order($data)
    {
        if(isset($data['pur_order_detail'])){
            $pur_order_detail = json_decode($data['pur_order_detail']);
            ...
            $header[] = 'PaymentTerm';
            $header[] = 'Bag';
            $header[] = 'Weight_per_bag';
            $header[] = 'Loose_in_kg';
            $header[] = 'Qty_in_quintal';
            $header[] = 'Rate_per_quintal';
            $header[] = 'Value';
            $header[] = 'Brokerage';
            $header[] = 'MarketLevy';
            $header[] = 'Round_off';
        ...
        $CenterID = $data['CenterID'];
        $vehicle_no = $data['vehicle_no'];
        $purchase_orderNumbar = get_option('next_mandi_purchase_number_for_kirti');
        $new_purchase_orderNumbar = 'MPO'.$FY.$purchase_orderNumbar;   
        $ItCount = count($es_detail);
        $Transdate =  to_sql_date($data['prd_date'])." ".date('H:i:s');
        $tds_rate =  $data['tds_rate_avl'];
        $TDSCode = $data['TDSCode'];
        ...
        $this->db->insert(db_prefix() . 'mandipurchasemaster',$MandiPurchMaster);
        if($this->db->affected_rows() > 0){
            $this->increment_next_mandi_purchase_number();
                ...
                if($this->db->affected_rows() > 0){
                    ...
                    if($this->db->affected_rows() > 0){
                        ...
                        $data_array_result = array(
                            ...
                            'PartyID'=>$PartyID,
                            'PurchRate'=>$value['Rate_per_quintal'],
                            'SaleRate'=>$value['Rate_per_quintal'],
                            'BasicRate'=>$value['Rate_per_quintal'],
                            'final_rate'=>$value['Rate_per_quintal'],
                            'SuppliedIn'=>1,
                            'OrderQty'=>$value['Qty_in_quintal']/10,
                            'BilledQty'=>$value['Qty_in_quintal']/10,
                            'DiscAmt'=>0.00,
                            'cgst'=>0.00,
                            'sgst'=>0.00,
                            'igst'=>0.00,
                        ...
                        // Ledger entry code
                        $ord_n = 1;
                        $narrations = 'By Mandi Purchase no.'.$new_purchase_orderNumbar.' / against PO No.'.$new_po;
                        // Credit to Vendor
                        $ledger_credit = array(
                            "PlantID" => $PlantID,
                            "Transdate" => $Transdate,
                        ...
                        if($value['MarketLevy'] > 0){
                            // Debit to Market Levy Account
                            $ledger_debit = array(
                                "PlantID" => $PlantID,
                                "Transdate" => $Transdate,
                                "TransDate2" => date('Y-m-d H:i:s'),
                        ...
                        if($value['Round_off'] > 0 || $value['Round_off'] < 0){
                            // Debit to round off Account
                            $ledger_debit = array(
                                "PlantID" => $PlantID,
                                "Transdate" => $Transdate,
                                "TransDate2" => date('Y-m-d H:i:s'),
    ...
    public function increment_next_mandi_purchase_number()
    {
        $FY = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        
        $this->db->where('name', 'next_mandi_purchase_number_for_kirti');
        $this->db->set('value', 'value+1', false);
        $this->db->WHERE('FY', $FY);
        $this->db->update(db_prefix() . 'options');
    }
...
/**
  ...
  public function get_commodity_group_add_commodity_data()
    ...
    public function load_mandi_purchase_list($data)
    {  
        ...
        $sql1 = '('.db_prefix().'mandipurchasemaster.TransDate BETWEEN "'.$from_date.' 00:00:00" AND "'.$to_date.' 23:59:59") 
        AND '.db_prefix().'mandipurchasemaster.FY = "'.$fy.'" 
        AND '.db_prefix().'mandipurchasemaster.PlantID = "'.$selected_company.'" ';
        
        $sql1 .= ' ORDER BY tblmandipurchasemaster.TransDate ASC';
        $sql ='SELECT '.db_prefix().'mandipurchasemaster.*,tblCenterMaster.CenterName,tblPlantMaster.PlantName,tblitems.ItemName
        FROM '.db_prefix().'mandipurchasemaster 
        INNER JOIN  tblCenterMaster ON tblCenterMaster.CenterID = tblmandipurchasemaster.CenterID 
        INNER JOIN  tblPlantMaster ON tblPlantMaster.PlantID = tblmandipurchasemaster.PartyID 
        INNER JOIN  tblitems ON tblitems.ItemID = tblmandipurchasemaster.ItemID AND tblitems.PlantID = tblmandipurchasemaster.PlantID
        WHERE '.$sql1;
        $result = $this->db->query($sql)->result_array();
        return $result;
    }
    
    public function GetMandiOrderDetails($MadiOrderID)
    {  
        $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
      
        $sql1 = ' tblmandipurchasemaster.PurchID = "'.$MadiOrderID.'"
        AND '.db_prefix().'mandipurchasemaster.FY = "'.$fy.'" 
        AND '.db_prefix().'mandipurchasemaster.PlantID = "'.$selected_company.'" ';
        
        $sql1 .= ' ORDER BY tblmandipurchasemaster.TransDate ASC';
        $sql ='SELECT '.db_prefix().'mandipurchasemaster.*
        FROM '.db_prefix().'mandipurchasemaster 
        WHERE '.$sql1;
        $result = $this->db->query($sql)->row();
        if($result){
            $sql1 = ' tblmandipurchaseDetails.PurchID = "'.$MadiOrderID.'"
            AND '.db_prefix().'mandipurchaseDetails.FY = "'.$fy.'" 
            AND '.db_prefix().'mandipurchaseDetails.PlantID = "'.$selected_company.'" ';
            
            $sql1 .= ' ORDER BY tblmandipurchaseDetails.TransID ASC';
            $sql ='SELECT '.db_prefix().'mandipurchaseDetails.*
            FROM '.db_prefix().'mandipurchaseDetails 
            WHERE '.$sql1;
    ...
    public function WHListByCenterID($CenterID)
    {  
        $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
      
        $sql1 = ' tblwarehouse.center = "'.$CenterID.'" ';
        $sql1 .= ' ORDER BY tblwarehouse.w_name ASC';
        $sql ='SELECT '.db_prefix().'warehouse.*
    ...
    {  
        ...
        $sql1 = '('.db_prefix().'purchasemaster.Transdate BETWEEN "'.$from_date.' 00:00:00" AND "'.$to_date.' 23:59:59") 
        AND tblpurchasemaster.FY = "'.$fy.'" 
        AND tblpurchasemaster.PlantID = "'.$selected_company.'" 
        AND tblpurchasemaster.PurchType = "'.$POType.'"
        ORDER BY PurchID ASC';
    ...
    public function GetPurchaseDetails($PONumber)
    {
        $selected_company = $this->session->userdata('root_company');
        $year = $this->session->userdata('finacial_year');
        $this->db->select('tblpurchasemaster.*,tblclients.company,tblclients.state,tblxx_statelist.state_name,tblGstRecord.gstin');
        $this->db->from(db_prefix() . 'purchasemaster');
        $this->db->join(db_prefix() . 'clients','tblclients.AccountID = tblpurchasemaster.AccountID AND tblclients.PlantID = tblpurchasemaster.PlantID');
        $this->db->join(db_prefix() . 'GstRecord','tblGstRecord.AccountID = tblclients.AccountID AND IsPrimary = "1"',"LEFT");
        $this->db->join(db_prefix() . 'xx_statelist', db_prefix() . 'xx_statelist.short_name = ' . db_prefix() . 'clients.state', 'left');
        $this->db->where(db_prefix() . 'purchasemaster.PurchID', $PONumber);
        $this->db->where(db_prefix() . 'purchasemaster.PlantID', $selected_company);
        $this->db->where(db_prefix() . 'purchasemaster.FY', $year);
       return $this->db->get()->row();
    }
    ...
    public function GetTDSRateBySection($TDSSection)
    {
        $this->db->select('tblTDSDetails.*');
        $this->db->from(db_prefix() . 'TDSDetails');
        $this->db->where(db_prefix() . 'TDSDetails.TDSCode', $TDSSection);
       return $this->db->get()->result_array();
    }
    
    public function GetLedgerDetails($LedgerEffect)
    {
        $this->db->select('tblclients.AccountID,tblclients.ActGroupID,tblclients.SubActGroupID1,tblclients.SubActGroupID');
        $this->db->from(db_prefix() . 'clients');
        $this->db->where(db_prefix() . 'clients.AccountID', $LedgerEffect);
        $LedgerDetails = $this->db->get()->row();
        if($LedgerDetails){
            // Get Account Group2 List By Account Group 1
            $this->db->select('tblaccountgroupssub.SubActGroupID,SubActGroupName');
            $this->db->from(db_prefix() . 'accountgroupssub');
            $this->db->where(db_prefix() . 'accountgroupssub.SubActGroupID1', $LedgerDetails->SubActGroupID1);
            $LedgerSubGroup2List = $this->db->get()->result_array();
            $LedgerDetails->ActSubGrp2List = $LedgerSubGroup2List;
            
            // Get Account List By Account Group 2
            $this->db->select('tblclients.AccountID,company');
            $this->db->from(db_prefix() . 'clients');
            $this->db->where(db_prefix() . 'clients.SubActGroupID', $LedgerDetails->SubActGroupID);
            $LedgerList = $this->db->get()->result_array();
            $LedgerDetails->LedgerList = $LedgerList;
        }
    ...
    public function GetPurchaseItemList($id)
    {
        $selected_company = $this->session->userdata('root_company');
        $fy = $this->session->userdata('finacial_year');
        $this->db->select('tblhistory.*,tblitems.ItemName,tblitems.ItemID AS id,tblitems_sub_groups.main_group_id,tblitems_main_groups.name,tbltaxes.taxrate AS gst');
        $this->db->from(db_prefix() . 'history');
        $this->db->join(db_prefix() . 'items', 'tblitems.ItemID = tblhistory.ItemID AND tblitems.PlantID = tblhistory.PlantID');
        $this->db->join(db_prefix() . 'taxes', 'tbltaxes.id = tblitems.tax');
        $this->db->join(db_prefix() . 'items_sub_groups', db_prefix() . 'items_sub_groups.id = ' . db_prefix() . 'items.subgroup_id', 'left');
        $this->db->join(db_prefix() . 'items_main_groups', db_prefix() . 'items_main_groups.id = ' . db_prefix() . 'items_sub_groups.main_group_id', 'left');
    ...
    public function UpdateDirectPurchase($data,$id)
    {
        $selected_company = $this->session->userdata('root_company');
        $fy = $this->session->userdata('finacial_year');
        ...
        $TDSCode =  $data['TDSCode'];
        $tds_rate_avl =  $data['tds_rate_avl'];
        $tdsAmt =  $data['tdsAmt'];
        $RoundOffAmt =  $data['Round_OFF'];
        $Invamt =  str_replace(",","",$data['Invoice_amt']);
        $ItCount = count($es_detail);
        ...
        if($this->db->affected_rows() > 0){
            $old_pur_details = $this->purchase_model->get_purchase_detail($PurchID);
            // Move record from tblhistory to tblhistory_audit
            foreach ($old_pur_details as $key => $value) {
                $old_data = array(
                    ...
                    'UserID'=>$value["UserID"],
                    'Lupdate'=>date('Y-m-s H:i:s'),
                    'UserID2'=>$_SESSION['username']
                );
                $this->db->insert(db_prefix() . 'history_audit',$old_data);
            }
            ...
            $GetLedgerList = $this->GetLedgerListByVoucher($PurchID);
            ...
            $this->db->insert(db_prefix() . 'accountledger',$ledger_data);
            $ord_no++;
            
            // Debit Ledger Entry to Purchase Account 
            $ledger_data = array(
                "PlantID"=>$selected_company,
                "FY"=>$fy,
                "Transdate"=>$new_date,
            ...
            // Credit DISC Account Ledger entry 
            $ledger_data = array(
                "PlantID"=>$selected_company,
                "FY"=>$fy,
                "Transdate"=>$new_date,
            ...
            // Debit GST Account Ledger entry 
            if($igstamt>0){
                $ledger_data = array(
                    "PlantID"=>$selected_company,
                    "FY"=>$fy,
  ...
  function itemlist($postData){
    ...
    $where_items = '';
     if(isset($postData['search']) ){
       ...
       $where_items .= '(	ItemID LIKE "%' . $q . '%" ESCAPE \'!\' OR ItemName LIKE "%' . $q . '%" ESCAPE \'!\') AND ' . db_prefix() . 'items.isactive = "Y" ';
   ...
   public function getallstate()
    ...
    //   print_r($data);die;
        ...
        $SQL = '';
       $SQL.= 'SELECT tblclients.AccountID as AccountID,tblaccountgroupssub.SubActGroupName,
...
AND tblclients.SubActGroupID IN("50003002")';
       if($states != ''){
...
$SQL.= ' ORDER BY `tblclients`.`AccountID` ASC';
    ...
    public function add_pur_return_order($data){
        ...
        foreach($es_detail as $value){
            ...
            $get_purch_stock = $this->get_purch_stock($value['ItemID']);
    ...
    public function get_pReturn_order_detail($purchRtn_id){
     ...
     public function update_purchaseRtn_order_new($data,$id){
        $selected_company = $this->session->userdata('root_company');
        $fy = $this->session->userdata('finacial_year');
      ...
      $get_pre_credit_amt33 = $this->get_pre_ledger_amt('ROUNDOFF',$PurchRtnID);
        ...
        if($acc_id->AccountID == $data["vendor_code"]){
            ...
            foreach($es_detail as $value){
                ...
                if (in_array($value['ItemID'], $old_item_code)){
