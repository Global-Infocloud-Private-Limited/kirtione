...
class App
{
    ...
    public function get_number($CeterID,$Type)
    {

        $val  = '';
        $name = trim($CeterID);

        if (!isset($this->options[$CeterID])) {
            $CI = & get_instance();
            $this->ci->db->select('Number');
            $this->ci->db->where('CenterID', $CeterID);
             $this->ci->db->where('TType', $Type);
            $row = $this->ci->db->get(db_prefix() . 'numberformat')->row();
            if ($row) {
                $val = $row->Number;
            }
        } else {
            $val = $this->options[$CeterID];
        }

        return hooks()->apply_filters('get_option', $val, $CeterID);
    }
