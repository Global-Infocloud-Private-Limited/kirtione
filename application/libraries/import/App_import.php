...
//========================== Trader/Farmer Rate Master Download Sample ================
    public function RateMasterSample($RateType)
    {
        if($RateType == "T"){
            $file_name = 'TraderKirtiPurchaseRateMaster.csv';
        }elseif($RateType == "F"){
            $file_name = 'FarmerKirtiPurchaseRateMaster.csv';
        }elseif($RateType == "TS"){
            $file_name = 'TraderKirtiSaleRateMaster.csv';
        }elseif($RateType == "M"){
            $file_name = 'MandiRateRateMaster.csv';
        }else{
            $file_name = "RateMaster.csv";
        }
        $totalSampleFields = 0;
        $dbFieldKeys       = [];
        ...
        $GetTraderKirtiPurchaseRate = $this->GetKirtiRateMaster($RateType);
        $totalSampleRows = count($GetTraderKirtiPurchaseRate);
        $row = 0;
        foreach($GetTraderKirtiPurchaseRate as $key=>$val) {
...
//===================== Get Trader/Farmer rate for kirti purchase =====================
    public function GetKirtiRateMaster($RateType)
    {
        if($RateType == "TS"){
            // Kirti Sale Rate Master For Trader
            $this->ci->db->select('IFNULL(tblSaleRateMaster.Rate, "0.00") AS ExRate,tblitems.ItemName,tblCenterMaster.CenterName,
            tblCenter_wise_item.*');
            $this->ci->db->join(db_prefix() . 'items', '' . db_prefix() . 'items.ItemID = ' . db_prefix() . 'Center_wise_item.ItemID');
            $this->ci->db->join(db_prefix() . 'CenterMaster', '' . db_prefix() . 'CenterMaster.CenterID = ' . db_prefix() . 'Center_wise_item.CenterID');
            $this->ci->db->join(db_prefix() . 'SaleRateMaster', 'tblSaleRateMaster.ItemID = tblCenter_wise_item.ItemID AND
            tblSaleRateMaster.CenterID = tblCenter_wise_item.CenterID AND tblSaleRateMaster.IsActive = "Y" AND tblSaleRateMaster.KeyID = "C01"',"LEFT");
            $this->ci->db->where('tblCenterMaster.status', 'Y');
            $this->ci->db->order_by('tblCenter_wise_item.CenterID',"ASC");
            return $this->ci->db->get('tblCenter_wise_item')->result_array();
        }elseif($RateType == "M"){
            // Mandi Rate
            // Center Wise Items
            $this->ci->db->select('tblitems.ItemName,tblCenterMaster.CenterName,tblCenterMaster.MandiIDs,
            tblCenter_wise_item.*');
            $this->ci->db->join(db_prefix() . 'items', '' . db_prefix() . 'items.ItemID = ' . db_prefix() . 'Center_wise_item.ItemID');
            $this->ci->db->join(db_prefix() . 'CenterMaster', '' . db_prefix() . 'CenterMaster.CenterID = ' . db_prefix() . 'Center_wise_item.CenterID');
            $this->ci->db->where('tblCenterMaster.status', 'Y');
            $this->ci->db->order_by('tblCenter_wise_item.CenterID',"ASC");
            $CenteWiseItemList = $this->ci->db->get('tblCenter_wise_item')->result_array();
            // Get All Mandi List
            $this->ci->db->select('tblCompetitorMaster.*');
            $this->ci->db->where('tblCompetitorMaster.Type', 'M');
            $this->ci->db->order_by('tblCompetitorMaster.id',"ASC");
            $CompetitorList = $this->ci->db->get('tblCompetitorMaster')->result_array();
            
            $result = array();
            foreach($CenteWiseItemList as $key=>$val){
                $Competitor_string = $val["MandiIDs"];
                $Competitor_array1 = explode(',', $Competitor_string);
                foreach($Competitor_array1 as $Ckey=>$CVal){
                    $CompName = "";
                    foreach($CompetitorList as $CompID=>$CompVal){
                        if($CompVal["CompetitorID"] == $CVal){
                            $CompName = $CompVal["Competitor"];
                        }
                    }
                    $new_array = array(
            ...
            $this->ci->db->select('IFNULL(tblRateMaster.Rate, "0.00") AS ExRate,tblitems.ItemName,tblCenterMaster.CenterName,
            tblCenter_wise_item.*');
            $this->ci->db->join(db_prefix() . 'items', '' . db_prefix() . 'items.ItemID = ' . db_prefix() . 'Center_wise_item.ItemID');
            $this->ci->db->join(db_prefix() . 'CenterMaster', '' . db_prefix() . 'CenterMaster.CenterID = ' . db_prefix() . 'Center_wise_item.CenterID');
            $this->ci->db->join(db_prefix() . 'RateMaster', 'tblRateMaster.ItemID = tblCenter_wise_item.ItemID AND
            tblRateMaster.CenterID = tblCenter_wise_item.CenterID AND tblRateMaster.IsActive = "Y" AND tblRateMaster.KeyID = "C01" AND tblRateMaster.Type = "'.$RateType.'"',"LEFT");
            $this->ci->db->where('tblCenterMaster.status', 'Y');
            $this->ci->db->order_by('tblCenter_wise_item.CenterID',"ASC");
            return $this->ci->db->get('tblCenter_wise_item')->result_array();
        }
...
//====================== Comman Download Sample ================================
    ...
    private function setDefaultImportGuidelinesInfo()
    {
        ...
        $this->addImportGuidelinesInfo('In Downloaded sample file shows three column like CenterID,ItemID and existing basic rate. Change only rate column if we want to change rate respective Item wise center wise');
