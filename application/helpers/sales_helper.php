<?php
	...
	*/
	...
	function get_order_amt_for_cd_notes($hsncode,$TransID,$FY,$PlantID)
	{
		...
		$CI->db->join(db_prefix() . 'items', '' . db_prefix() . 'items.ItemID = ' . db_prefix() . 'history.ItemID AND ' . db_prefix() . 'items.PlantID = ' . db_prefix() . 'history.PlantID');
	...
	function GetAsnDetailsFrGateIn($BookingID){
		$CI = & get_instance();
		$CI->db->select('*');
		$CI->db->from('tblGateMaster');
		$CI->db->where('BookingID', $BookingID);
		return $CI->db->get()->row();
	}
	...
	function GetItemDetailsFrK1Gatepass($OrderID)
	{
		$CI = & get_instance();
		$fy = $CI->session->userdata('finacial_year');
		$selected_company = $CI->session->userdata("root_company");
		$amt = 0.00;
		$CI->db->select('tblK1history.*,BatchExp.ExpDate,tblproduct.*,tbltaxes.*,tblbrands.*');
		$CI->db->from(db_prefix() . 'K1history');
		$CI->db->join('(SELECT BatchNo, MAX(ExpDate) AS ExpDate FROM ' . db_prefix() . 'K1history WHERE TType2 = "Purchase" GROUP BY BatchNo) AS BatchExp', 
               'tblK1history.BatchNo = BatchExp.BatchNo', 
               'left');
		$CI->db->join(db_prefix() . 'product', 'tblproduct.ProductID = tblK1history.ItemID AND tblproduct.PlantID = tblK1history.PlantID');
		$CI->db->join(db_prefix() . 'taxes', 'tbltaxes.id = tblproduct.gst'); 		
		$CI->db->join(db_prefix() . 'brands', 'tblbrands.id = tblproduct.BrandId');   		
		$CI->db->where(db_prefix() . 'K1history.OrderID', $OrderID);
		$CI->db->where(db_prefix() . 'K1history.PlantID', $selected_company);
		$CI->db->where(db_prefix() . 'K1history.FY', $fy);
		$CI->db->where(db_prefix() . 'K1history.BillID IS NOT NULL');
		...
		foreach ($results as &$row) {
			...
			if($row['SuppliedIn'] == 'Boxs'){
				$row['BasicRate'] = round(($row['PackingQty'] * $row['BasicRate']), 2);
				$row['PurchRate'] = round(($row['PackingQty'] * $row['PurchRate']), 2);
				$row['SaleRate'] = round(($row['PackingQty'] * $row['SaleRate']), 2);
			}
		
			if ($row['PackingQty'] == $row['CaseQty']) {	
				$row['SaleUnit'] = $row['Measuredin'];
				$row['OrderQty'] = $row['OrderQty'] / $row['PackingQty'];
				$row['Discount'] = number_format($row['Discount']/$row['OrderQty'],2);
			}else{
				$row['SaleUnit'] = 'Loose';
				$row['Discount'] = number_format(($row['Discount']/$row['OrderQty'])*$row['PackingQty'],2);
			}
		}
		return $results;
	}
	function GetItemDetailsFrK1GatepassBYChallan($ChallanID)
	{
		$CI = & get_instance();
		$fy = $CI->session->userdata('finacial_year');
		$selected_company = $CI->session->userdata("root_company");
		...
		$CI->db->join(db_prefix() . 'taxes', 'tbltaxes.id = tblproduct.gst'); 		
		$CI->db->join(db_prefix() . 'brands', 'tblbrands.id = tblproduct.BrandId');   		
		$CI->db->where(db_prefix() . 'K1history.BillID', $ChallanID);
		$CI->db->where(db_prefix() . 'K1history.PlantID', $selected_company);
		$CI->db->where(db_prefix() . 'K1history.FY', $fy);
		$CI->db->where(db_prefix() . 'K1history.BillID IS NOT NULL');
		$results = $CI->db->get()->result_array();
	...
	function get_prod_hsn_byitem_id($item_code)
	{
		//$fy = date("y");
		$CI = & get_instance();
		$selected_company = $CI->session->userdata("root_company");
		$CI->db->where('ProductID', $item_code);
		$CI->db->where('PlantID', $selected_company);
		//$CI->db->where('FY', $fy);
		return $CI->db->get(db_prefix() . 'product')->row();
	}
	...
	function get_client_detail($AccountID)
	{
		...
		$CI->db->select(db_prefix() .'clients.*,'.db_prefix() . 'contacts.phonenumber AS cmobile, GROUP_CONCAT(DISTINCT CONCAT(tblclients.house, ", ", tblclients.street, ", ", tblclients.loc, ", ", tblclients.vtc, ", ", tblxx_statelist.state_name, " - ", tblxx_citylist.city_name)) AS VendorAddress');
		...
		$CI->db->join('tblxx_citylist', 'tblxx_citylist.id = tblclients.dist', 'LEFT');
	...
	function get_cd_details($GateINID)
	{
		$CI = &get_instance();
		$selected_company = $CI->session->userdata("root_company");
		
		$CI->db->select(db_prefix() .'cdnote.*,tblpurchasemaster.PurchID,tblpurchasemaster.Purchamt');
		$CI->db->from(db_prefix() .'cdnote');
		$CI->db->join(db_prefix() .'purchasemaster', db_prefix() .'purchasemaster.TransID = '.db_prefix() .'cdnote.TransID','LEFT');
		$CI->db->where(db_prefix() .'cdnote.TransID', $GateINID);
		$CI->db->where(db_prefix() .'cdnote.plantid', $selected_company);
		return $CI->db->get()->row();
	}
	
	
	function get_control_details($GateINID)
	{
		$CI = &get_instance();
		$selected_company = $CI->session->userdata("root_company");
		
		$CI->db->select(db_prefix() .'GateMaster.*,tblitems.hsn_code,tblitems.ItemID,tblitems.ItemName,tbltaxes.taxrate,
		tblclients.company,tblclients.CustomerType,tblclients.state,tblCenterMaster.CenterName,tblwarehouse.w_name');
		$CI->db->from(db_prefix() .'GateMaster');
		$CI->db->join('tblclients','tblclients.AccountID = tblGateMaster.AccountID');
		$CI->db->join('tblCenterMaster','tblCenterMaster.CenterID = tblGateMaster.CenterID');
		$CI->db->join('tblwarehouse','tblwarehouse.AccountID = tblGateMaster.GodownID',"LEFT");
		$CI->db->join(db_prefix() .'items', db_prefix() .'items.ItemID = '.db_prefix() .'GateMaster.ItemID','LEFT');
		$CI->db->join(db_prefix() .'taxes', db_prefix() .'taxes.id = '.db_prefix() .'items.tax','LEFT');
		$CI->db->where(db_prefix() .'GateMaster.Gate_in_ID', $GateINID);
		return $CI->db->get()->row();
	}
	
	function get_bag_details($BookingID,$GateINID)
	{
		$CI = &get_instance();
		$selected_company = $CI->session->userdata("root_company");
		
		$CI->db->select('SUM(tblLayerMaster.qty) AS Quentity,tblLayerMaster.unit');
		$CI->db->from(db_prefix() .'LayerMaster');
		$CI->db->where(db_prefix() .'LayerMaster.BookingID', $BookingID);
		$CI->db->where(db_prefix() .'LayerMaster.Gate_in_ID', $GateINID);
		$CI->db->group_by('tblLayerMaster.unit');
		return $CI->db->get()->result_array();
	}
	...
	function K1get_sales_details($challan_id,$order_id)
	{
		...
		$CI->db->where('FY', $fy);
		$CI->db->where('PlantID', $selected_company);
		return $CI->db->get(db_prefix() . 'K1salesmaster')->row();
	...
	function get_k1gst_details($order_id)
	{
		
		$CI = & get_instance();
		$selected_company = $CI->session->userdata("root_company");
		$fy = $CI->session->userdata('finacial_year');
        $CI->db->select(db_prefix() .'K1history.cgst');
        $CI->db->distinct();
        $CI->db->from(db_prefix() .'K1history');
        $CI->db->where(db_prefix() .'K1history.OrderID', $order_id);
        $CI->db->where(db_prefix() .'K1history.PlantID', $selected_company);
        $CI->db->where(db_prefix() .'K1history.FY', $fy);
        return $CI->db->get()->result_array();
	}
	...
	function get_k1igst_details($order_id)
	{
		$CI = & get_instance();
		$fy = $CI->session->userdata('finacial_year');
		$selected_company = $CI->session->userdata("root_company");
		
        $CI->db->select(db_prefix() .'K1history.igst');
        $CI->db->distinct();
        $CI->db->from(db_prefix() .'K1history');
        $CI->db->where(db_prefix() .'K1history.OrderID', $order_id);
        $CI->db->where(db_prefix() .'K1history.BillID Is NOT NULL');
	...
	function get_gst_taxable_amt($order_id,$gst_per)
	{
        ...
        $CI->db->select_sum(db_prefix() .'K1history.OrderAmt');
        
        $CI->db->from(db_prefix() .'K1history');
        $CI->db->where(db_prefix() .'K1history.OrderID', $order_id);
        $CI->db->where(db_prefix() .'K1history.cgst', $gst_per);
        $CI->db->where(db_prefix() .'K1history.PlantID', $selected_company);
        $CI->db->where(db_prefix() .'K1history.FY', $fy);
        $result = $CI->db->get()->row();
        return $result->OrderAmt;
	}
	function get_k1gst_taxable_amt($order_id,$gst_per)
	{
	...
	function get_k1igst_taxable_amt($order_id,$gst_per)
	{
		$CI = & get_instance();
		$fy = $CI->session->userdata('finacial_year');
		$selected_company = $CI->session->userdata("root_company");
        ...
        $CI->db->from(db_prefix() .'K1history');
        $CI->db->where(db_prefix() .'K1history.OrderID', $order_id);
        $CI->db->where(db_prefix() .'K1history.igst', $gst_per);
        $CI->db->where(db_prefix() .'K1history.PlantID', $selected_company);
        $CI->db->where(db_prefix() .'K1history.FY', $fy);
        $result = $CI->db->get()->row();
        return $result->OrderAmt;
	...
	function get_k1_is_tcs($sale_id)
	{
        ...
        $CI->db->from(db_prefix() .'K1salesmaster');
        $CI->db->where(db_prefix() .'K1salesmaster.SalesID', $sale_id);
        //$CI->db->where(db_prefix() .'K1salesmaster.PlantID', $selected_company);
        //$CI->db->where(db_prefix() .'K1salesmaster.FY', $fy);
        $result = $CI->db->get()->row();
        return $result;
	}
	...
	function get_k1gst_amt($order_id,$gst_per)
	{
		$CI = & get_instance();
		$fy = $CI->session->userdata('finacial_year');
		$selected_company = $CI->session->userdata("root_company");
		
        $CI->db->select_sum(db_prefix() .'K1history.cgstamt');
	...
	function get_k1igst_amt($order_id,$gst_per)
	{
		$CI = & get_instance();
		$fy = $CI->session->userdata('finacial_year');
		$selected_company = $CI->session->userdata("root_company");
		
        $CI->db->select_sum(db_prefix() .'K1history.igstamt');
	...
	function get_k1gst_item_count($order_id,$gst_per)
	{
		$CI = & get_instance();
		$fy = $CI->session->userdata('finacial_year');
		$selected_company = $CI->session->userdata("root_company");
        ...
        $CI->db->where(db_prefix() .'K1history.FY', $fy);
        $CI->db->where(db_prefix() .'K1history.BillID Is NOT NULL');
        $CI->db->where(db_prefix() .'K1history.NetChallanAmt !=' , $amt);
        $result = $CI->db->get()->result_array();
        return $result;
	}
	...
	function get_k1igst_item_count($order_id,$gst_per)
	{
		$CI = & get_instance();
		$fy = $CI->session->userdata('finacial_year');
		$selected_company = $CI->session->userdata("root_company");
	...
	*/
	...
	function k1get_order_list($OrderID)
	{
		...
		$CI->db->join(db_prefix() . 'xx_statelist', db_prefix() . 'xx_statelist.short_name = ' . db_prefix() . 'clients.state', 'left');
		$CI->db->join(db_prefix() . 'CenterMaster', db_prefix() . 'CenterMaster.CenterID = ' . db_prefix() . 'K1ordermaster.CenterID', 'left');
		
		$CI->db->join(db_prefix() . 'K1history', 'tblK1history.OrderID = tblK1ordermaster.OrderID', 'left');
		// $this->db->join(db_prefix() . 'accountledger', 'tblaccountledger.AccountID = tblK1ordermaster.OrderID', 'left');
		
		$CI->db->where(db_prefix() . 'K1ordermaster.OrderID', $OrderID);
		$CI->db->where(db_prefix() . 'K1ordermaster.PlantID', $selected_company);
		$CI->db->where(db_prefix() . 'K1ordermaster.FY', $fy);
		
		return $CI->db->get()->result_array();
	}
	function K1get_shipping_details($id)
	{
		...
		$CI->db->select("tblShippingDetails.*,tblxx_statelist.state_name,tblxx_citylist.city_name,GROUP_CONCAT(CONCAT(IFNULL(House, ''), ', ',IFNULL(Street, ''), ', ',IFNULL(Locality, ''), ', ',IFNULL(Block, ''), ', (',IFNULL(state_name, ''), ' - ',IFNULL(city_name, ''), ', ',IFNULL(Pincode, ''), ')') SEPARATOR '\n') AS address");
		$CI->db->from(db_prefix() . 'ShippingDetails');	
		$CI->db->join('tblxx_statelist', 'tblxx_statelist.short_name = tblShippingDetails.State', 'LEFT');	
		$CI->db->join('tblxx_citylist', 'tblxx_citylist.id = tblShippingDetails.District', 'LEFT');	
		$CI->db->where(db_prefix() . 'ShippingDetails.id', $id);
