<?php
defined('BASEPATH') or exit('No direct script access allowed');
/**
 * @since  2.3.3
 * Get available staff permissions, modules can use the filter too to hook permissions
 * @return array
 */
function get_available_staff_permissions($data = [])
{
    $viewGlobalName = _l('permission_view') . '(' . _l('permission_global') . ')';
    $v_a_e_d = [
        'view'     => _l('permission_view'),
        'create'   => _l('permission_create'),
        'edit'     => _l('permission_edit'),
        'delete'   => ['not_applicable' => true, 'name' => _l('permission_delete')],
        // 'delete'   => _l('permission_delete'),
        'print' => ['not_applicable' => true, 'name' => 'Print'],
        'export' => ['not_applicable' => true, 'name' => 'Export'],
    ];
    $allPermissionsArray = [
        /*'view_own' => _l('permission_view_own'),*/
        'view'     => $viewGlobalName,
        'create'   => _l('permission_create'),
        'edit'     => _l('permission_edit'),
        'delete'   => _l('permission_delete'),
        'print' => "Print",
        'export' => "Export",
    ];
    $withoutViewOwnPermissionsArray = [
        /*'view_own' => ['not_applicable' => true, 'name' => _l('permission_view_own')],*/
        'view'   => $viewGlobalName,
        'create' => _l('permission_create'),
        'edit'   => _l('permission_edit'),
        'delete' => ['not_applicable' => true, 'name' => _l('permission_delete')],
        // 'delete' => _l('permission_delete'),
        'print' => "Print",
        'export' => "Export",
    ];
    $oneoptionPermissionsArray = [
        /*'view_own' => ['not_applicable' => true, 'name' => _l('permission_view_own')],*/
        'view'   => $viewGlobalName,
        'create' => ['not_applicable' => true, 'name' => _l('permission_create')],
        'update' => "update",
        'delete' => ['not_applicable' => true, 'name' => _l('permission_delete')],
        'print' => "Print",
        'export' => "Export",
    ];
    $onlyviewPrintExport = [
        'view'     => $viewGlobalName,
        'create' => ['not_applicable' => true, 'name' => _l('permission_create')],
        'edit' => ['not_applicable' => true, 'name' => _l('permission_edit')],
        'delete' => ['not_applicable' => true, 'name' => _l('permission_delete')],
        'print' => "Print",
        'export' => "Export",
    ];
    $Onlyview_edit = [
        'view'     => $viewGlobalName,
        'create' => ['not_applicable' => true, 'name' => _l('permission_create')],
        'edit' => _l('permission_edit'),
        'delete' => ['not_applicable' => true, 'name' => _l('permission_delete')],
        'print' => ['not_applicable' => true, 'name' => 'Print'],
        'export' => ['not_applicable' => true, 'name' => 'Print'],
    ];
    $view_edit_delete = [
        /*'view_own' => ['not_applicable' => true, 'name' => _l('permission_view_own')],*/
        'view'     => $viewGlobalName,
        'create' => ['not_applicable' => true, 'name' => _l('permission_create')],
        'edit' => _l('permission_edit'),
        'delete' => _l('permission_delete'),
        'print' => "Print",
        'export' => "Export",
    ];
    $onlycreate = [
        /*'view_own' => ['not_applicable' => true, 'name' => _l('permission_view_own')],*/
        'view'     => $viewGlobalName,
        'create' => _l('permission_create'),
        'edit' => ['not_applicable' => true, 'name' => _l('permission_edit')],
        'delete' => ['not_applicable' => true, 'name' => _l('permission_delete')],
        'print' => "Print",
        'export' => "Export",
    ];
    $createview = [
        /*'view_own' => ['not_applicable' => true, 'name' => _l('permission_view_own')],*/
        'view'     => $viewGlobalName,
        'create' => _l('permission_create'),
        'edit' => ['not_applicable' => true, 'name' => _l('permission_edit')],
        'delete' => ['not_applicable' => true, 'name' => _l('permission_delete')],
        'print' => ['not_applicable' => true, 'name' => 'Print'],
        'export' => ['not_applicable' => true, 'name' => 'Export'],
    ];
    $view_edit_print_export = [
        /*'view_own' => ['not_applicable' => true, 'name' => _l('permission_view_own')],*/
        'view'     => $viewGlobalName,
        'create' => ['not_applicable' => true, 'name' => _l('permission_create')],
        'edit' => _l('permission_edit'),
        'delete' => ['not_applicable' => true, 'name' => _l('permission_delete')],
        'print' => "Print",
        'export' => "Export",
    ];
    $view_create = [
        /*'view_own' => ['not_applicable' => true, 'name' => _l('permission_view_own')],*/
        'view'     => $viewGlobalName,
        'create' => _l('permission_create'),
        'edit' => ['not_applicable' => true, 'name' => _l('permission_edit')],
        'delete' => ['not_applicable' => true, 'name' => _l('permission_delete')],
        'print' => "Print",
        'export' => ['not_applicable' => true, 'name' => 'Export'],
    ];
    $create_edit = [
        /*'view_own' => ['not_applicable' => true, 'name' => _l('permission_view_own')],*/
        'view'     =>  $viewGlobalName,
        'create' => _l('permission_create'),
        'edit' => _l('permission_edit'),
        'delete' => ['not_applicable' => true, 'name' => _l('permission_delete')],
        'print' => "Print",
        'export' => "Export",
    ];
    $create_edit_view = [
        'view'     =>  $viewGlobalName,
        'create' => _l('permission_create'),
        'edit' => _l('permission_edit'),
        'delete' => ['not_applicable' => true, 'name' => _l('permission_delete')],
        'print' => "Print",
        'export' => "Export",
    ];
    $view_create_edit = [
        'view'     =>  $viewGlobalName,
        'create' => _l('permission_create'),
        'edit' => _l('permission_edit'),
        'delete' => ['not_applicable' => true, 'name' => _l('permission_delete')],
        'print' => ['not_applicable' => true, 'name' => _l('permission_delete')],
        'export' => ['not_applicable' => true, 'name' => _l('permission_delete')],
    ];
    $view_print_export = [
        'view'     =>  $viewGlobalName,
        'create' => ['not_applicable' => true, 'name' => _l('permission_delete')],
        'edit' => ['not_applicable' => true, 'name' => _l('permission_delete')],
        'delete' => ['not_applicable' => true, 'name' => _l('permission_delete')],
        'print' => "Print",
        'export' => "Export",
    ];
    //$withNotApplicableViewOwn = array_merge(['view_own' => ['not_applicable' => true, 'name' => _l('permission_view_own')]], $withoutViewOwnPermissionsArray);
    $withNotApplicableViewOwn = $withoutViewOwnPermissionsArray;
    $corePermissions = [
        /*'bulk_pdf_exporter' => [
            'name'         => _l('bulk_pdf_exporter'),
            'capabilities' => [
                'view' => $viewGlobalName,
            ],
        ],*/
        /*'contracts' => [
            'name'         => _l('contracts'),
            'capabilities' => $allPermissionsArray,
        ],*/
        'Fpo_Rate' => [
            'name'         => "Fpo Rate Master",
            'main_menu'    => 'Masters',
            'capabilities' => $onlycreate,
            'help'         => [
                'view_own' => _l('permission_customers_based_on_admins'),
            ],
        ],
        'TraderMaster' => [
            'name'         => "Trader Master",
            'main_menu'    => 'Masters',
            'capabilities' => $withNotApplicableViewOwn,
            'help'         => [
                'view_own' => _l('permission_customers_based_on_admins'),
            ],
        ],
        'BrokerMaster' => [
            'name'         => "Broker Master",
            'main_menu'    => 'Masters',
            'capabilities' => $withNotApplicableViewOwn,
            'help'         => [
                'view_own' => _l('permission_customers_based_on_admins'),
            ],
        ],
        'BlockUnblock_ledger' => [
            'name'         => "Block Unblock Ledger",
            'main_menu'    => 'Masters',
            'capabilities' => $Onlyview_edit,
            'help'         => [
                'view_own' => _l('permission_customers_based_on_admins'),
            ],
        ],
        'FarmarMaster' => [
            'name'         => "Farmar Master",
            'main_menu'    => 'Masters',
            'capabilities' => $v_a_e_d,
            'help'         => [
                'view_own' => _l('permission_customers_based_on_admins'),
            ],
        ],
        'ChangeMobileNo' => [
            'name'         => "Change Mobile No",
            'main_menu'    => 'Masters',
            'capabilities' => $Onlyview_edit,
            'help'         => [
                'view_own' => _l('permission_customers_based_on_admins'),
            ],
        ],
        /*'other_staff_master' => [
            'name'         => "other staff master",
            'main_menu'    => 'Masters',
            'capabilities' => $withoutViewOwnPermissionsArray,
        ],*/
        /*'routemaster' => [
            'name'         => "Route Master",
            'main_menu'    => 'Masters',
            'capabilities' => $withoutViewOwnPermissionsArray,
        ],*/
        //Admin Access
        /*'account_group' => [
            'name'         => "Account Group",
            'main_menu'    => 'Masters',
            'capabilities' => $withoutViewOwnPermissionsArray,
        ],*/
        /*'vehiclemaster' => [
            'name'         => "Vehicle Master",
            'main_menu'    => 'Masters',
            'capabilities' => $withoutViewOwnPermissionsArray,
        ],*/
        'hsnmaster' => [
            'name'         => "HSN Master",
            'main_menu'    => 'Masters',
            'capabilities' => $withoutViewOwnPermissionsArray,
        ],
        'tcsmaster' => [
            'name'         => "TCS Master",
            'main_menu'    => 'Masters',
            'capabilities' => $withoutViewOwnPermissionsArray,
        ],
        'tdsmaster' => [
            'name'         => "TDS Master",
            'main_menu'    => 'Masters',
            'capabilities' => $withoutViewOwnPermissionsArray,
            // 'capabilities' => $v_a_e_d,
        ],
        'items' => [
            'name'         => "Item Master",
            'main_menu'    => 'Masters',
            'capabilities' => $withoutViewOwnPermissionsArray,
        ],
        /*'itemsdivision' => [
            'name'         => "Item Division",
            'main_menu'    => 'Masters',
            'capabilities' => $withoutViewOwnPermissionsArray,
        ],*/
        'itemsparameters' => [
            'name'         => "Item QA Parameters",
            'main_menu'    => 'Masters',
            'capabilities' => $v_a_e_d,
            // 'capabilities' => $withoutViewOwnPermissionsArray,
        ],
        'itemsmaingrp' => [
            'name'         => "ItemMain Group",
            'main_menu'    => 'Masters',
            'capabilities' => $v_a_e_d,
            // 'capabilities' => $withoutViewOwnPermissionsArray,
        ],
        'itemssubgrp' => [
            'name'         => "Item Group",
            'main_menu'    => 'Masters',
            'capabilities' => $v_a_e_d,
            // 'capabilities' => $withoutViewOwnPermissionsArray,
        ],
        /*'hierarchy' => [
            'name'         => "Hierarchy",
            'main_menu'    => 'Masters',
            'capabilities' => $oneoptionPermissionsArray,
        ],
        'salesperassign' => [
            'name'         => "Attach SalesTeam to Parties",
            'main_menu'    => 'Masters',
            'capabilities' => $oneoptionPermissionsArray,
        ],*/
        /*'enquiry' => [
            'name'         => "Enquiry",
            'main_menu'    => 'Masters',
            'capabilities' => $allPermissionsArray,
        ],
        'tour' => [
            'name'         => "Tour Plan",
            'main_menu'    => 'Masters',
            'capabilities' => $allPermissionsArray,
        ],
        'year_transfer' => [
            'name'         => "Year Transfer",
            'main_menu'    => 'Masters',
            'capabilities' => $oneoptionPermissionsArray,
        ],*/
        'WarehouseMaster' => [
            'name'         => "Warehouse Master",
            'main_menu'    => 'Masters',
            'capabilities' => $withNotApplicableViewOwn,
        ],
        /*'GodownMaster' => [
            'name'         => "Godown Master",
            'main_menu'    => 'Masters',
            'capabilities' => $withNotApplicableViewOwn,
        ],*/
        'ClusterMaster' => [
            'name'         => "Cluster Master",
            'main_menu'    => 'Masters',
            'capabilities' => $v_a_e_d,
            // 'capabilities' => $withNotApplicableViewOwn,
        ],
        'RegionMaster' => [
            'name'         => "Region Master",
            'main_menu'    => 'Masters',
            'capabilities' => $v_a_e_d,
            // 'capabilities' => $withNotApplicableViewOwn,
        ],
        'CenterMaster' => [
            'name'         => "Center Master",
            'main_menu'    => 'Masters',
            'capabilities' => $v_a_e_d,
            // 'capabilities' => $withNotApplicableViewOwn,
        ],
        'Competitor' => [
            'name'         => "Competitor Master",
            'main_menu'    => 'Masters',
            'capabilities' => $withNotApplicableViewOwn,
        ],
        'MSPRate' => [
            'name'         => "MSP Rate Master",
            'main_menu'    => 'Masters',
            'capabilities' => $view_edit_print_export,
        ],
        'DailyTraderRate' => [
            'name'         => "Kirti Purchase Rate(Trader)",
            'main_menu'    => 'Masters',
            'capabilities' => $view_edit_print_export,
        ],
        'CityWiseCommodityRate' => [
            'name'         => "City Wise Commodity Rate Update",
            'main_menu'    => 'Masters',
            'capabilities' => $view_edit_print_export,
        ],
        'CityWiseFarmerCommodityRate' => [
            'name'         => "City Wise Farmer Commodity Rate Update",
            'main_menu'    => 'Masters',
            'capabilities' => $view_edit_print_export,
        ],
        'DailyFarmerRate' => [
            'name'         => "Kirti Purchase Rate(Farmer)",
            'main_menu'    => 'Masters',
            'capabilities' => $view_edit_print_export,
        ],
        'DailyCompetitorRate' => [
            'name'         => "Kirti Competitor Rate",
            'main_menu'    => 'Masters',
            'capabilities' => $view_edit_print_export,
        ],
        'DailyMandiRate' => [
            'name'         => "Mandi Rate Master",
            'main_menu'    => 'Masters',
            'capabilities' => $view_edit_print_export,
        ],
        'SellRateMaster' => [
            'name'         => "Sell Rate Master",
            'main_menu'    => 'Masters',
            'capabilities' => $withNotApplicableViewOwn,
        ],
        'Charges' => [
            'name'         => "Charges Master",
            'main_menu'    => 'Masters',
            'capabilities' => $withNotApplicableViewOwn,
        ],
        'DeductionMatrix' => [
            'name'         => "Deduction Matrix",
            'main_menu'    => 'Masters',
            'capabilities' => $withNotApplicableViewOwn,
        ],
        'PaymentCycle' => [
            'name'         => "Payment Cycle Master",
            'main_menu'    => 'Masters',
            'capabilities' => $v_a_e_d,
            // 'capabilities' => $withNotApplicableViewOwn,
        ],
        'Locking' => [
            'name'         => "Locking Period Master",
            'main_menu'    => 'Masters',
            'capabilities' => $withNotApplicableViewOwn,
        ],
         'CompanyMaster' => [
            'name'         => "Company Master",
            'main_menu'    => 'Masters',
            'capabilities' => $withNotApplicableViewOwn,
        ],
        'TransportMaster' => [
            'name'         => "Transport Master",
            'main_menu'    => 'Masters',
            'capabilities' => $withNotApplicableViewOwn,
        ],
        /*'salaryComponents' => [
            'name'         => "Salary Components",
            'main_menu'    => 'Masters',
            'capabilities' => $withNotApplicableViewOwn,
        ],
        'salarymaster' => [
            'name'         => "Salary Master",
            'main_menu'    => 'Masters',
            'capabilities' => $view_edit_print_export,
        ],*/
        /*'Staff_payroll' => [
            'name'         => "Staff Payroll",
            'main_menu'    => 'Masters',
            'capabilities' => $view_edit_print_export,
        ],*/
        'BrokerInitiateRequest' => [
            'name'         => "Broker Initiate",
            'main_menu'    => 'Masters',
            'capabilities' => $withNotApplicableViewOwn,
        ],
        'AddEditCard' => [
            'name'         => "MemberShip Card Master",
            'main_menu'    => 'Kirti One',
            'capabilities' => $view_create_edit,
        ],
        'CardAllotment' =>[
            'name'         => "Farmerwise Card Allocation",
            'main_menu'    => 'Kirti One',
            'capabilities' => $view_create_edit,
        ],
        'CardRequestList' =>[
            'name'         => "Farmerwise Card Request",
            'main_menu'    => 'Kirti One',
            'capabilities' => $view_print_export,
        ],
        'CardIssueList' =>[
            'name'         => "Farmerwise Card List",
            'main_menu'    => 'Kirti One',
            'capabilities' => $view_print_export,
        ],
        'CardLedger' =>[
            'name'         => "Card Point Ledger",
            'main_menu'    => 'Kirti One',
            'capabilities' => $view_print_export,
        ],
        'SoilTestRequest' =>[
            'name'         => "Soil Test Request",
            'main_menu'    => 'Kirti One',
            'capabilities' => $view_edit_print_export,
        ],
        'K1Dashboard' =>[
            'name'         => "Kirti One Dashboard",
            'main_menu'    => 'K1 Master',
            'capabilities' => $onlyviewPrintExport,
        ],
        'VillageChart' =>[
            'name'         => "Village Chart",
            'main_menu'    => 'K1 Master',
            'capabilities' => $onlyviewPrintExport,
        ],
        'BrandMaster' =>[
            'name'         => "Brand Master",
            'main_menu'    => 'K1 Master',
            'capabilities' => $view_create_edit,
        ],
        'ItemCategoryMaster' =>[
            'name'         => "Item Category Master",
            'main_menu'    => 'K1 Master',
            'capabilities' => $view_create_edit,
        ],
        'ItemSubCategoryMaster' =>[
            'name'         => "Item Sub Category Master",
            'main_menu'    => 'K1 Master',
            'capabilities' => $view_create_edit,
        ],
        'ItemMaster' => [
            'name'         => "Kirti One Item Master",
            'main_menu'    => 'K1 Master',
            'capabilities' => $withoutViewOwnPermissionsArray,
        ],
        'ItemWiseRateMaster' => [
            'name'         => "Item Wise Rate Master",
            'main_menu'    => 'K1 Master',
            'capabilities' => $view_create,
        ],
        'CropMaster' =>[
            'name'         => "Crop Master",
            'main_menu'    => 'K1 Master',
            'capabilities' => $view_create_edit,
        ],
        'FertilizerMaster' =>[
            'name'         => "Fertilizer Master",
            'main_menu'    => 'K1 Master',
            'capabilities' => $view_create_edit,
        ],
        'PesticideMaster' =>[
            'name'         => "Pesticide Master",
            'main_menu'    => 'K1 Master',
            'capabilities' => $view_create_edit,
        ],
        'SeedsMaster' =>[
            'name'         => "Seeds Master",
            'main_menu'    => 'K1 Master',
            'capabilities' => $view_create_edit,
        ],
        'PincodeMaster' =>[
            'name'         => "Pincode Master",
            'main_menu'    => 'K1 Master',
            'capabilities' => $view_create_edit,
        ],
        'VehicleTypeMaster' =>[
            'name'         => "Vehicle Type Master",
            'main_menu'    => 'K1 Master',
            'capabilities' => $view_create_edit,
        ],
        'VillageMaster' =>[
            'name'         => "Village Master",
            'main_menu'    => 'K1 Master',
            'capabilities' => $view_create_edit,
        ],
        'VillageReport' =>[
            'name'         => "Village Report",
            'main_menu'    => 'K1 Master',
            'capabilities' => $onlyviewPrintExport,
        ],
        'CommsionMaster' =>[
            'name'         => "Commision Master",
            'main_menu'    => 'K1 Master',
            'capabilities' => $onlycreate,
        ],
        'TargetvsAttunement' =>[
            'name'         => "Target Master",
            'main_menu'    => 'K1 Master',
            'capabilities' => $v_a_e_d,
        ],
		'KirtiOneInward' =>[
            'name'         => "Vendor Stock Inward",
            'main_menu'    => 'K1 Inventory',
            'capabilities' => $view_create_edit,
        ],
        'KirtiOneStockTransfer' =>[
            'name'         => "Stock Transfer",
            'main_menu'    => 'K1 Inventory',
            'capabilities' => $view_create_edit,
        ],
        'KirtiOneGodownStockTransfer' =>[
            'name'         => "Wholesale Retail Transfer",
            'main_menu'    => 'K1 Inventory',
            'capabilities' => $view_create_edit,
        ],
        'KirtiOneStockTransferList' =>[
            'name'         => "Stock Transfer List",
            'main_menu'    => 'K1 Inventory',
            'capabilities' => $onlyviewPrintExport,
        ],
        'KirtiOneStockAdjustment' =>[
            'name'         => "Stock Adjustment",
            'main_menu'    => 'K1 Inventory',
            'capabilities' => $view_create_edit,
        ],
        'K1StockPosition' =>[
            'name'         => "Stock Position",
            'main_menu'    => 'K1 Inventory',
            'capabilities' => $onlyviewPrintExport,
        ],
        'K1ItemWiseStockPosition' =>[
            'name'         => "Itemwise Stock Position",
            'main_menu'    => 'K1 Inventory',
            'capabilities' => $onlyviewPrintExport,
        ],
        'K1AsOnStockPosition' =>[
            'name'         => "As On Date Stock",
            'main_menu'    => 'K1 Inventory',
            'capabilities' => $onlyviewPrintExport,
        ],
        'K1ExpairyReport' =>[
            'name'         => "Expairy Report",
            'main_menu'    => 'K1 Inventory',
            'capabilities' => $onlyviewPrintExport,
        ],
        'K1SalableReport' =>[
            'name'         => "Most Salable Report",
            'main_menu'    => 'K1 Inventory',
            'capabilities' => $onlyviewPrintExport,
        ],
        'k1ProfitableReport' =>[
            'name'         => "Most Profitable Report",
            'main_menu'    => 'K1 Inventory',
            'capabilities' => $onlyviewPrintExport,
        ],
        'K1RateMaster' => [
            'name'         => 'K1 Rate Master',
            'main_menu'    => 'K1 Inventory',
            'capabilities' => $create_edit_view,
        ],
        'PurchaseRequest' =>[
            'name'         => "Purchase Request",
            'main_menu'    => 'K1 Purchase',
            'capabilities' => $create_edit_view,
        ],
        'PurchaseOrder' =>[
            'name'         => "Purchase Order",
            'main_menu'    => 'K1 Purchase',
            'capabilities' => $create_edit_view,
        ],
        'PurchaseOrderApprove' =>[
            'name'         => "Purchase Order Approve",
            'main_menu'    => 'K1 Purchase',
            'capabilities' => $create_edit_view,
        ],
        'PurchaseInvoice' =>[
            'name'         => "Purchase Invoice",
            'main_menu'    => 'K1 Purchase',
            'capabilities' => $create_edit_view,
        ],
        'PurchaseInvoiceLedger' =>[
            'name'         => "Purchase Payment",
            'main_menu'    => 'K1 Purchase',
            'capabilities' => $create_edit_view,
        ],
        'PurchOrderList' =>[
            'name'         => "Purchase List",
            'main_menu'    => 'K1 Purchase',
            'capabilities' => $onlyviewPrintExport,
        ],
        'PurchaseReturnInvoice' =>[
            'name'         => "Purchase Return Invoice",
            'main_menu'    => 'K1 Purchase',
            'capabilities' => $withoutViewOwnPermissionsArray,
        ],
        'DemandList' =>[
            'name'         => "Add Demand List",
            'main_menu'    => 'K1 Purchase',
            'capabilities' => $create_edit_view,
        ],
        'DemandListReport' =>[
            'name'         => "Demand List",
            'main_menu'    => 'K1 Purchase',
            'capabilities' => $onlyviewPrintExport,
        ],
        'PurchaseOrderReminderReport' =>[
            'name'         => "PO Reminder Report",
            'main_menu'    => 'K1 Purchase',
            'capabilities' => $onlyviewPrintExport,
        ],
        'SaleOrder' =>[
            'name'         => "Sale Order",
            'main_menu'    => 'K1 Sale',
            'capabilities' => $create_edit_view,
        ],
        'DeliveryOrder' =>[
            'name'         => "Delivery Order",
            'main_menu'    => 'K1 Sale',
            'capabilities' => $create_edit_view,
        ],
        'DeliveryInvoice' =>[
            'name'         => "Delivery Invoice",
            'main_menu'    => 'K1 Sale',
            'capabilities' => $create_edit_view,
        ],
		'OrderMaster' =>[
            'name'         => "Direct Sale Order",
            'main_menu'    => 'K1 Sale',
            'capabilities' => $create_edit_view,
        ],
        /*'DirectSale' =>[
            'name'         => "Direct Sale Order",
            'main_menu'    => 'K1 Sale',
            'capabilities' => $create_edit_view,
        ],*/
        'OrderList' =>[
            'name'         => "Sale List",
            'main_menu'    => 'K1 Sale',
            'capabilities' => $create_edit_view,
        ],
        'HsnWiseSale' =>[
            'name'         => "Hsn Wise Sale",
            'main_menu'    => 'K1 Sale',
            'capabilities' => $onlyviewPrintExport,
        ],
        'SaleReturnInvoice' =>[
            'name'         => "Sale Return Invoice",
            'main_menu'    => 'K1 Sale',
            'capabilities' => $withoutViewOwnPermissionsArray,
        ],
        'gatepass' =>[
            'name'         => "Gatepass",
            'main_menu'    => 'K1 Sale',
            'capabilities' => $create_edit_view,
        ],
        'WHspacemgmt' => [
            'name'         => "WH Chamber Mgmt",
            'main_menu'    => 'Warehouse',
            'capabilities' => $create_edit_view,
        ],
        'WHStackMgmt' => [
            'name'         => "WH Stack Mgmt",
            'main_menu'    => 'Warehouse',
            'capabilities' => $create_edit_view,
        ],
        'WHLotMgmt' => [
            'name'         => "WH Lot Mgmt",
            'main_menu'    => 'Warehouse',
            'capabilities' => $create_edit_view,
        ],
        'WarehouseReports' => [
            'name'         => "Warehouse Reports",
            'main_menu'    => 'Warehouse',
            'capabilities' => $onlyviewPrintExport,
        ],
        'StorageTradeList' => [
            'name'         => "Storage Trade List",
            'main_menu'    => 'Warehouse',
            'capabilities' => $onlyviewPrintExport,
        ],
        'StorageCharges' => [
            'name'         => "Storage Charges View",
            'main_menu'    => 'Warehouse',
            'capabilities' => $onlyviewPrintExport,
        ],
        'Fpo_Order_Form' => [
            'name'         => "FPO Order",
            'main_menu'    => 'Transactions',
            'capabilities' => $view_create_edit,
        ],
        'FpoOrder_Report' => [
            'name'         => "FPO Order Report",
            'main_menu'    => 'Transactions',
            'capabilities' => $onlyviewPrintExport,
        ],
        'FpoOrder_Payment'=> [
            'name'         => "FPO Order Payment",
            'main_menu'    => 'Transactions',
            'capabilities' => $createview,
        ],
        'FpoDispatch_Report' => [
            'name'         => "FPO Dispatch Report",
            'main_menu'    => 'Transactions',
            'capabilities' => $onlyviewPrintExport,
        ],
        'Bag_ledger' => [
            'name'         => "Bag Ledger",
            'main_menu'    => 'Transactions',
            'capabilities' => $allPermissionsArray,
        ],
        'FpoOrder_Dispatch'=> [
            'name'         => "FPO Dispatch Order",
            'main_menu'    => 'Transactions',
            'capabilities' => $view_create_edit,
        ],
        'Fpo_Inward'=> [
            'name'         => "FPO Inward",
            'main_menu'    => 'Transactions',
            'capabilities' => $view_create_edit,
        ],
        'Purchasetrade_punch' => [
            'name'         => "Purchase Trade Punch",
            'main_menu'    => 'Transactions',
            'capabilities' => $createview,
        ],
        'PurchasetradeRateChange' => [
            'name'         => "Purchase Trade Rate Change",
            'main_menu'    => 'Transactions',
            'capabilities' => $createview,
        ],
        'Purchase_Booking' => [
            'name'         => "Daily Purchase Trade",
            'main_menu'    => 'Transactions',
            'capabilities' => $onlyviewPrintExport,
        ],
        'DepositeTradePunch' => [
            'name'         => "Deposit Trade Punch",
            'main_menu'    => 'Transactions',
            'capabilities' => $createview,
        ],
        'Deposit_Booking' => [
            'name'         => "Daily Deposit Trade",
            'main_menu'    => 'Transactions',
            'capabilities' => $onlyviewPrintExport,
        ],
        'WithdrawalTradePunch' => [
            'name'         => "Withdrawal Trade Punch",
            'main_menu'    => 'Transactions',
            'capabilities' => $createview,
        ],
        'Withdrawal_Booking' => [
            'name'         => "Daily Withdrawal Trade",
            'main_menu'    => 'Transactions',
            'capabilities' => $onlyviewPrintExport,
        ],
        'Sell_Booking' => [
            'name'         => "Daily Sell Trade",
            'main_menu'    => 'Transactions',
            'capabilities' => $onlyviewPrintExport,
        ],
        'Anamat_Booking' => [
            'name'         => "Daily Anamat Trade",
            'main_menu'    => 'Transactions',
            'capabilities' => $onlyviewPrintExport,
        ],
        'Trade_Finance_Booking' => [
            'name'         => "Daily Trade Finance Trade",
            'main_menu'    => 'Transactions',
            'capabilities' => $onlyviewPrintExport,
        ],
        'Booking_list' => [
            'name'         => "All Trade List",
            'main_menu'    => 'Transactions',
            'capabilities' => $view_edit_print_export,
        ],
        'Ganerate_asn' => [
            'name'         => "Ganerate ASN",
            'main_menu'    => 'Transactions',
            'capabilities' => $v_a_e_d,
        ],
        'Ganerate_gatein' => [
            'name'         => "Generate GATEIN",
            'main_menu'    => 'Transactions',
            'capabilities' => $createview,
            // 'capabilities' => $createview,
        ],
        'GateControl_Reports' => [
            'name'         => "Gate Control",
            'main_menu'    => 'Transactions',
            'capabilities' => $onlyviewPrintExport,
        ],
        'AdvancePayment_List' => [
            'name'         => "Advance Payment List",
            'main_menu'    => 'Transactions',
            'capabilities' => $onlyviewPrintExport,
        ],
        'AdvancePayment' => [
            'name'         => "Advance Payment",
            'main_menu'    => 'Transactions',
            'capabilities' => $createview,
        ],
        'Booking_settlement' => [
            'name'         => "Trade Settlement",
            'main_menu'    => 'Transactions',
            'capabilities' => $createview,
        ],
        'SettledList' => [
            'name'         => "Trade Settled List",
            'main_menu'    => 'Transactions',
            'capabilities' => $onlyviewPrintExport,
        ],
        'CenterWiseTradeQty' => [
            'name'         => "Center Wise Trade Quantity",
            'main_menu'    => 'Transactions',
            'capabilities' => $onlyviewPrintExport,
        ],
        'PurchasePaymentList' => [
            'name'         => "Purchase Payment List",
            'main_menu'    => 'Transactions',
            'capabilities' => $onlyviewPrintExport,
            // 'capabilities' => $withNotApplicableViewOwn,
        ],
        'InvoiceList' => [
            'name'         => "Invoice List",
            'main_menu'    => 'Transactions',
            'capabilities' => $onlyviewPrintExport,
        ],
        'StockTransfer' => [
            'name'         => "Stock Transfer",
            'main_menu'    => 'Transactions',
            'capabilities' => $v_a_e_d,
            // 'capabilities' => $withNotApplicableViewOwn,
        ],
        /*'sale_list' => [
            'name'         => "Sale List",
            'main_menu'    => 'Transactions',
            'capabilities' => $onlyview,
        ],
        'challan' => [
            'name'         => "Challan",
            'main_menu'    => 'Transactions',
            'capabilities' => $onlycreate,
        ],
        'challan_list' => [
            'name'         => "Challan List",
            'main_menu'    => 'Transactions',
            'capabilities' => $view_edit_delete,
        ],*/
        /*
        'gatepass' => [
            'name'         => "Gatepass",
            'main_menu'    => 'Transactions',
            'capabilities' => $createview,
        ],
        'pending_orders' => [
            'name'         => "Pending Orders",
            'main_menu'    => 'Transactions',
            'capabilities' => $view_edit_print_export,
        ],
        'vehicle_return' => [
            'name'         => "Vehicle Return",
            'main_menu'    => 'Transactions',
            'capabilities' => $allPermissionsArray,
        ],*/
        'orders' => [
            'name'         => "Direct Sale",
            'main_menu'    => 'Transactions',
            'capabilities' => $createview,
        ],
        'sale_return' => [
            'name'         => "Sale Return",
            'main_menu'    => 'Transactions',
            'capabilities' => $view_create,
        ],
        'cd_notes' => [
            'name'         => "Credit/Debit Notes",
            'main_menu'    => 'Transactions',
            'capabilities' => $view_create,
            // 'capabilities' => $allPermissionsArray,
        ],
        'ItemIssue' => [
            'name'         => "Item Issue",
            'main_menu'    => 'Transactions',
            'capabilities' => $createview,
        ],
        'einvoice' => [
            'name'         => "E-invoice",
            'main_menu'    => 'Transactions',
            'capabilities' => $createview,
        ],
        /*
        'stock_adjustment' => [
            'name'         => "Stock Adjustment",
            'main_menu'    => 'Transactions',
            'capabilities' => $create_edit_view,
        ],*/
        /*'staff_target' => [
            'name'         => "Staff Target",
            'main_menu'    => 'Transactions',
            'capabilities' => $view_edit_print_export,
        ],
        'damage_entry' => [
            'name'         => "Damage Entry",
            'main_menu'    => 'Transactions',
            'capabilities' => $create_edit_view,
        ],
        'SplDisc' => [
            'name'         => "Special Discount",
            'main_menu'    => 'Transactions',
            'capabilities' => $create_edit_view,
        ],
        'StockTransfer' => [
            'name'         => "Stock Transfer",
            'main_menu'    => 'Transactions',
            'capabilities' => $withNotApplicableViewOwn,
        ],
        'SchemeMaster' => [
            'name'         => "Scheme Master",
            'main_menu'    => 'Transactions',
            'capabilities' => $create_edit_view,
        ],
        'WHBooking' => [
            'name'         => "WH Booking",
            'main_menu'    => 'Transactions',
            'capabilities' => $view_edit_print_export,
        ],*/
        /*'recipe' => [
            'name'         => "Recipe",
            'main_menu'    => 'Production',
            'capabilities' => $withNotApplicableViewOwn,
        ],
        'production' => [
            'name'         => "Production",
            'main_menu'    => 'Production',
            'capabilities' => $create_edit,
        ],
        'production_list' => [
            'name'         => "Production List",
            'main_menu'    => 'Production',
            'capabilities' => $onlyviewPrintExport,
        ],
        'production_reports' => [
            'name'         => "Production Report",
            'main_menu'    => 'Production',
            'capabilities' => $onlyviewPrintExport,
        ],
        'production_order_report' => [
            'name'         => "Production Order Report",
            'main_menu'    => 'Production',
            'capabilities' => $onlyviewPrintExport,
        ],
        'cost_report' => [
            'name'         => "Cost Report",
            'main_menu'    => 'Production',
            'capabilities' => $onlyviewPrintExport,
        ],*/
        'daily_sale' => [
            'name'         => "Daily Sales Report",
            'main_menu'    => 'Sales Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        /*'cummulatives_sale' => [
            'name'         => "Cummulatives Sales Report",
            'main_menu'    => 'Sales Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        'target_vs_achivements' => [
            'name'         => "Target Vs Achievement",
            'main_menu'    => 'Sales Reports',
            'capabilities' => $onlyviewPrintExport,
        ],*/
        'SaleRtn' => [
            'name'         => "Sales Return - Report",
            'main_menu'    => 'Sales Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        'saleVsSaleRtn' => [
            'name'         => "Sales Vs SalesReturn",
            'main_menu'    => 'Sales Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        /*'PartyItemWiseReport' => [
            'name'         => "Party ItemWise Report",
            'main_menu'    => 'Sales Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        'OrderVsDispatch' => [
            'name'         => "Order Vs Dispatch",
            'main_menu'    => 'Sales Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        'OrderVsDispatchItemWise' => [
            'name'         => "OrderVsDispatch ItemWise",
            'main_menu'    => 'Sales Reports',
            'capabilities' => $onlyviewPrintExport,
        ],*/
        'MISDashboard' => [
            'name'         => "MIS Dashboard",
            'main_menu'    => 'Misc. Reports',
            // 'main_menu'    => 'Pur. Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        'purchase_register' => [
            'name'         => "Purchase Register",
            'main_menu'    => 'Misc. Reports',
            // 'main_menu'    => 'Pur. Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        /*'account_list' => [
            'name'         => "Account List",
            'main_menu'    => 'Misc. Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        'item_rate_list' => [
            'name'         => "Item Rate List",
            'main_menu'    => 'Misc. Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        'market_outstanding' => [
            'name'         => "Market Outstanding",
            'main_menu'    => 'Misc. Reports',
            'capabilities' => $onlyviewPrintExport,
        ],*/
        'stock_position' => [
            'name'         => "Stock Position",
            'main_menu'    => 'Misc. Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        'stockCummulative' => [
            'name'         => "stock Cummulative",
            'main_menu'    => 'Misc. Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        'ItemWiseStockReport' => [
            'name'         => "ItemWise Stock Report",
            'main_menu'    => 'Misc. Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        'ItemList' => [
            'name'         => "ItemList",
            'main_menu'    => 'Misc. Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        'AcccountGroupList' => [
            'name'         => "AcccountGroupList",
            'main_menu'    => 'Misc. Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        // 'AcccountSubGroupList' => [
        //     'name'         => "AcccountSubGroupList",
        //     'main_menu'    => 'Misc. Reports',
        //     'capabilities' => $onlyviewPrintExport,
        // ],
        'AccountHeadList' => [
            'name'         => "AccountHeadList",
            'main_menu'    => 'Misc. Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        'CustomerList' => [
            'name'         => "CustomerList",
            'main_menu'    => 'Misc. Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        'survey_report' => [
            'name'         => "Survey Report",
            'main_menu'    => 'Misc. Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        'survey_reportChart' => [
            'name'         => "Survey Report Chart",
            'main_menu'    => 'Misc. Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        'rate_report' => [
            'name'         => "Rate Report",
            'main_menu'    => 'Misc. Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        'breakenen_report' => [
            'name'         => "Breakenen sheet report",
            'main_menu'    => 'Misc. Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        'traderbroker_report' => [
            'name'         => "Trader Broker report",
            'main_menu'    => 'Misc. Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        'customer_enquiry' => [
            'name'         => "Customers Enquiry",
            'main_menu'    => 'Misc. Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        'expense_list' => [
            'name'         => "Expense List",
            'main_menu'    => 'Misc. Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        'outstandingcalculation' => [
            'name'         => "OutStanding Calculation List",
            'main_menu'    => 'Misc. Reports',
            'capabilities' => $onlyviewPrintExport,
        ],
        'hrm_hr_records' => [
            'name'         => "Add Staff",
            'main_menu'    => 'HR',
            'capabilities' => $v_a_e_d,
            // 'capabilities' => $withNotApplicableViewOwn,
        ],
        'hrm_hr_sub_admin' => [
            'name'         => "Add Sub-Admin",
            'main_menu'    => 'HR',
            'capabilities' => $v_a_e_d,
            // 'capabilities' => $withNotApplicableViewOwn,
        ],
        'hrm_hr_recordslist' => [
            'name'         => "Staff List",
            'main_menu'    => 'HR',
            'capabilities' => $onlyviewPrintExport,
        ],
        'cliam_expenses' => [
            'name'         => "Claim Expenses",
            'main_menu'    => 'HR',
            'capabilities' => $onlyviewPrintExport,
        ],
        'staffmanage_job_position' => [
            'name'         => "Job Designation",
            'main_menu'    => 'HR',
            'capabilities' => $withNotApplicableViewOwn,
        ],
        'attendance_management' => [
            'name'         => "Attendance Sheet",
            'main_menu'    => 'HR',
            'capabilities' => $onlyviewPrintExport,
            // 'capabilities' => $withNotApplicableViewOwn,
        ],
        'leave_management' => [
            'name'         => "Leave",
            'main_menu'    => 'HR',
            'capabilities' => $withNotApplicableViewOwn,
        ],
        'timesheet_report' => [
            'name'         => "Timesheet Reports",
            'main_menu'    => 'HR',
            'capabilities' => $onlyviewPrintExport,
        ],
        'timesheets_table_shiftwork' => [
            'name'         => _l('shiftwork'),
            'main_menu'    => 'HR',
            'capabilities' => $createview,
        ],
        'table_shiftwork_management' => [
            'name'         => _l('shift_management'),
            'main_menu'    => 'HR',
            'capabilities' => $allPermissionsArray,
        ],
        'timesheets_shift_type' => [
            'name'         => _l('shift_type'),
            'main_menu'    => 'HR',
            'capabilities' => $allPermissionsArray,
        ],
        'GSTR_purchase' => [
            'name'         => "GSTR Purchase",
            'main_menu'    => 'E-Filling',
            'capabilities' => $onlyviewPrintExport,
        ],
        'GSTR_sales' => [
            'name'         => "GSTR Sales",
            'main_menu'    => 'E-Filling',
            'capabilities' => $onlyviewPrintExport,
        ],
        'GGSTR_1' => [
            'name'         => "GGSTR-1",
            'main_menu'    => 'E-Filling',
            'capabilities' => $onlyviewPrintExport,
        ],
        'GSTR_3B' => [
            'name'         => "GSTR-3B",
            'main_menu'    => 'E-Filling',
            'capabilities' => $onlyviewPrintExport,
        ],
        'k1GSTR_purchase' => [
            'name'         => "K1 GSTR Purchase",
            'main_menu'    => 'E-Filling',
            'capabilities' => $onlyviewPrintExport,
        ],
        'k1GSTR_sales' => [
            'name'         => "K1 GSTR Sales",
            'main_menu'    => 'E-Filling',
            'capabilities' => $onlyviewPrintExport,
        ],
        'k1GSTR1' => [
            'name'         => "K1 GSTR 1",
            'main_menu'    => 'E-Filling',
            'capabilities' => $onlyviewPrintExport,
        ],
        'K1GSTR3B' => [
            'name'         => "K1 GSTR 3B",
            'main_menu'    => 'E-Filling',
            'capabilities' => $onlyviewPrintExport,
        ],
        'user_master' => [
            'name'         => "User Master",
            'main_menu'    => 'Admin',
            'capabilities' => $view_create,
        ],
        'user_rights' => [
            'name'         => "User Rights",
            'main_menu'    => 'Admin',
            'capabilities' => $view_create,
        ],
        'sell_dashboard' => [
            'name'         => "Sell Dashboard",
            'main_menu'    => 'Admin',
            'capabilities' => $onlyviewPrintExport,
        ],
        'no_show' => [
            'name'         => "No Show Accounts",
            'main_menu'    => 'Admin',
            'capabilities' => $view_edit_print_export,
        ],
        'distributor_type' => [
            'name'         => "Distributor Type",
            'main_menu'    => 'Admin',
            'capabilities' => $allPermissionsArray,
        ],
        'AccountIDMerge' => [
            'name'         => "Merge AccountID",
            'main_menu'    => 'Admin',
            'capabilities' => $oneoptionPermissionsArray,
        ],
        'ItemIDMerge' => [
            'name'         => "Merge ItemID",
            'main_menu'    => 'Admin',
            'capabilities' => $oneoptionPermissionsArray,
        ],
        'roles' => [
            'name'         => "Staff Roles",
            'main_menu'    => 'Admin',
            'capabilities' => $allPermissionsArray,
        ],
        'hrm_salary_type' => [
            'name'         => "Hr Record Setting",
            // 'name'         => "HR Records Salary Type",
            'main_menu'    => 'Admin',
            'capabilities' => $allPermissionsArray,
        ],
        'hrm_allowance_type' => [
            'name'         => "HR Payroll Setting",
            // 'name'         => "HR Records Allowance Type",
            'main_menu'    => 'Admin',
            'capabilities' => $allPermissionsArray,
        ],
        'hrm_hedquarter' => [
            'name'         => "HR Records Hedquarter",
            'main_menu'    => 'Admin',
            'capabilities' => $allPermissionsArray,
        ],
        // 'timesheets_setting' => [
        //     'name'         => "Timesheets Setting",
        //     'main_menu'    => 'Admin',
        //     'capabilities' => $allPermissionsArray,
        // ],
        // 'hrm_hedquarter' => [
        //     'name'         => "HR Records Hedquarter",
        //     'main_menu'    => 'Admin',
        //     'capabilities' => $view_create,
        // ],
        'hrm_gen_setting' => [
            'name'         => "HR Records General Setting",
            'main_menu'    => 'Admin',
            'capabilities' => $view_edit_print_export,
        ],
        'news' => [
            'name'         => "News",
            'main_menu'    => 'Admin',
            'capabilities' => $withNotApplicableViewOwn,
        ],
        'departments' => [
            'name'         => "Departments",
            'main_menu'    => 'Admin',
            'capabilities' => $allPermissionsArray,
        ],
        // 'hrm_salary_type' => [
        //     'name'         => "Hr Record Setting",
        //     'main_menu'    => 'Admin',
        //     'capabilities' => $withNotApplicableViewOwn,
        // ],
        /*'weigh_bridge' => [
            'name'         => "Weigh Bridge",
            'main_menu'    => 'Staff Application',
            'capabilities' => $create_edit,
        ],
        'vehicle_unloading' => [
            'name'         => "Vehicle Unloading",
            'main_menu'    => 'Staff Application',
            'capabilities' => $create_edit,
        ],
        'vehicle_loading' => [
            'name'         => "Vehicle loading",
            'main_menu'    => 'Staff Application',
            'capabilities' => $create_edit,
        ],
        'layer_wise_qc' => [
            'name'         => "Layer Wise QC",
            'main_menu'    => 'Staff Application',
            'capabilities' => $create_edit,
        ],
        'Cleaning' => [
            'name'         => "Cleaning",
            'main_menu'    => 'Staff Application',
            'capabilities' => $create_edit,
        ],*/
        /*'email_templates' => [
            'name'         => _l('email_templates'),
            'capabilities' => [
                'view' => $viewGlobalName,
                'edit' => _l('permission_edit'),
            ],
        ],
        'estimates' => [
            'name'         => _l('estimates'),
            'capabilities' => $allPermissionsArray,
        ],*/
        /*'expenses' => [
            'name'         => _l('expenses'),
            'capabilities' => $allPermissionsArray,
        ],*/
        /*'knowledge_base' => [
            'name'         => _l('knowledge_base'),
            'capabilities' => $withoutViewOwnPermissionsArray,
        ],*/
        /*'payments' => [
            'name'         => _l('payments'),
            'capabilities' => $withNotApplicableViewOwn,
            'help'         => [
                'view_own' => _l('permission_payments_based_on_invoices'),
            ],
        ],*/
        /*'projects' => [
            'name'         => _l('projects'),
            'capabilities' => $withNotApplicableViewOwn,
            'help'         => [
                'view'     => _l('help_project_permissions'),
                'view_own' => _l('permission_projects_based_on_assignee'),
            ],
        ],
        'proposals' => [
            'name'         => _l('proposals'),
            'capabilities' => $allPermissionsArray,
        ],*/
        /*'reports' => [
            'name'         => _l('reports'),
            'capabilities' => [
                'view' => $viewGlobalName,
            ],
        ],*/
        //admin Menu
        /*'roles' => [
            'name'         => _l('roles'),
            'main_menu'    => 'Transactions',
            'capabilities' => $withoutViewOwnPermissionsArray,
        ],*/
        /*'view_own' => ['not_applicable' => true, 'name' => _l('permission_view_own')],
        'view'     => $viewGlobalName,
        'create' => ['not_applicable' => true, 'name' => _l('permission_create')],
        'edit' => ['not_applicable' => true, 'name' => _l('permission_edit')],
        'delete' => ['not_applicable' => true, 'name' => _l('permission_delete')]*/
        // admin menu
        /*'settings' => [
            'name'         => _l('settings'),
            'main_menu'    => 'Transactions',
            'capabilities' => [
                'view_own' => ['not_applicable' => true, 'name' => _l('permission_view_own')],
                'view' => $viewGlobalName,
                'create' => ['not_applicable' => true, 'name' => _l('permission_create')],
                'edit' => _l('permission_edit'),
                'delete' => ['not_applicable' => true, 'name' => _l('permission_delete')]
            ],
        ],*/
        /*'staff' => [
            'name'         => _l('staff'),
            'capabilities' => $withoutViewOwnPermissionsArray,
        ],*/
        /*'subscriptions' => [
            'name'         => _l('subscriptions'),
            'capabilities' => $allPermissionsArray,
        ],
        'tasks' => [
            'name'         => _l('tasks'),
            'capabilities' => $withNotApplicableViewOwn,
             'help'        => [
                'view'     => _l('help_tasks_permissions'),
                'view_own' => _l('permission_tasks_based_on_assignee'),
            ],
        ],
        'checklist_templates' => [
            'name'         => _l('checklist_templates'),
            'capabilities' => [
                'create' => _l('permission_create'),
                'delete' => _l('permission_delete'),
            ],
        ],*/
    ];
    /*$addLeadsPermission = true;
    if (isset($data['staff_id']) && $data['staff_id']) {
        $is_staff_member = is_staff_member($data['staff_id']);
        if (!$is_staff_member) {
            $addLeadsPermission = false;
        }
    }
    if ($addLeadsPermission) {
        $corePermissions['leads'] = [
            'name'         => _l('leads'),
            'capabilities' => [
                'view'   => $viewGlobalName,
                'delete' => _l('permission_delete'),
            ],
            'help' => [
                'view' => _l('help_leads_permission_view'),
            ],
        ];
    }*/
    return hooks()->apply_filters('staff_permissions', $corePermissions, $data);
}
function get_dist_name($id)
{
    $CI = &get_instance();
    $CI->db->select('company')->from(db_prefix() . 'clients')->where('userid', $id);
    return $CI->db->get()->row()->company;
}
/**
 * Get staff by ID or current logged in staff
 * @param  mixed $id staff id
 * @return mixed
 */
function get_staff($id = null)
{
    if (empty($id) && isset($GLOBALS['current_user'])) {
        return $GLOBALS['current_user'];
    }
    // Staff not logged in
    if (empty($id)) {
        return null;
    }
    if (!class_exists('staff_model', false)) {
        get_instance()->load->model('staff_model');
    }
    return get_instance()->staff_model->get($id);
}
function get_staff_permission($id)
{
    if (empty($id) && isset($GLOBALS['current_user'])) {
        return $GLOBALS['current_user'];
    }
    // Staff not logged in
    if (empty($id)) {
        return null;
    }
    $CI = & get_instance();
    $CI->db->select('*');
    $CI->db->from(db_prefix() . 'staff');
    $CI->db->where('staffid', $id);
    $staff_details =  $CI->db->get()->row();
    $CI->db->select(db_prefix() . 'setup.FIRMNAME,'.db_prefix() . 'setup.YEARFROM,'.db_prefix() . 'setup.YEARTO,'.db_prefix() . 'setup.PlantID');
    $CI->db->from(db_prefix() . 'setup');
    $CI->db->where(db_prefix() . 'setup.Status', 'Y');
    $CI->db->order_by(db_prefix() . 'setup.FY,'.db_prefix() . 'setup.PlantID',"desc");
    if($staff_details->admin == "1"){
    }else{
        $CI->db->join(db_prefix() . 'staff_permissions', '' . db_prefix() . 'staff_permissions.plant_id = ' . db_prefix() . 'setup.PlantID AND '. db_prefix() . 'staff_permissions.year = ' . db_prefix() . 'setup.FY');
        $CI->db->where(db_prefix() . 'staff_permissions.staff_id', $id);
        $CI->db->group_by(db_prefix() . 'staff_permissions.year,'.db_prefix() . 'staff_permissions.plant_id');
    }
    return $CI->db->get()->result_array();
}
/**
 * Return staff profile image url
 * @param  mixed $staff_id
 * @param  string $type
 * @return string
 */
function staff_profile_image_url($staff_id, $type = 'small')
{
    $url = base_url('assets/images/user-placeholder.jpg');
    if ((string) $staff_id === (string) get_staff_user_id() && isset($GLOBALS['current_user'])) {
        $staff = $GLOBALS['current_user'];
    } else {
        $CI = & get_instance();
        $CI->db->select('profile_image')
        ->where('staffid', $staff_id);
        $staff = $CI->db->get(db_prefix() . 'staff')->row();
    }
    if ($staff) {
        if (!empty($staff->profile_image)) {
            $profileImagePath = 'uploads/staff_profile_images/' . $staff_id . '/' . $type . '_' . $staff->profile_image;
            if (file_exists($profileImagePath)) {
                $url = base_url($profileImagePath);
            }
        }
    }
    return $url;
}
/**
 * Staff profile image with href
 * @param  boolean $id        staff id
 * @param  array   $classes   image classes
 * @param  string  $type
 * @param  array   $img_attrs additional <img /> attributes
 * @return string
 */
function staff_profile_image($id, $classes = ['staff-profile-image'], $type = 'small', $img_attrs = [])
{
    $url = base_url('assets/images/user-placeholder.jpg');
    $id = trim($id);
    $_attributes = '';
    foreach ($img_attrs as $key => $val) {
        $_attributes .= $key . '=' . '"' . html_escape($val) . '" ';
    }
    $blankImageFormatted = '<img src="' . $url . '" ' . $_attributes . ' class="' . implode(' ', $classes) . '" />';
    if ((string) $id === (string) get_staff_user_id() && isset($GLOBALS['current_user'])) {
        $result = $GLOBALS['current_user'];
    } else {
        $CI     = & get_instance();
        $result = $CI->app_object_cache->get('staff-profile-image-data-' . $id);
        if (!$result) {
            $CI->db->select('profile_image,firstname,lastname');
            $CI->db->where('staffid', $id);
            $result = $CI->db->get(db_prefix() . 'staff')->row();
            $CI->app_object_cache->add('staff-profile-image-data-' . $id, $result);
        }
    }
    if (!$result) {
        return $blankImageFormatted;
    }
    if ($result && $result->profile_image !== null) {
        $profileImagePath = 'uploads/staff_profile_images/' . $id . '/' . $type . '_' . $result->profile_image;
        if (file_exists($profileImagePath)) {
            $profile_image = '<img ' . $_attributes . ' src="' . base_url($profileImagePath) . '" class="' . implode(' ', $classes) . '" />';
        } else {
            return $blankImageFormatted;
        }
    } else {
        $profile_image = '<img src="' . $url . '" ' . $_attributes . ' class="' . implode(' ', $classes) . '" />';
    }
    return $profile_image;
}
/**
 * Get staff full name
 * @param  string $userid Optional
 * @return string Firstname and Lastname
 */
function get_staff_full_name($userid = '')
{
    $tmpStaffUserId = get_staff_user_id();
    if ($userid == '' || $userid == $tmpStaffUserId) {
        if (isset($GLOBALS['current_user'])) {
            return $GLOBALS['current_user']->firstname . ' ' . $GLOBALS['current_user']->lastname;
        }
        $userid = $tmpStaffUserId;
    }
    $CI = & get_instance();
    $staff = $CI->app_object_cache->get('staff-full-name-data-' . $userid);
    if (!$staff) {
        $CI->db->where('staffid', $userid);
        $staff = $CI->db->select('firstname,lastname')->from(db_prefix() . 'staff')->get()->row();
        $CI->app_object_cache->add('staff-full-name-data-' . $userid, $staff);
    }
    return html_escape($staff ? $staff->firstname . ' ' . $staff->lastname : '');
}
/**
 * Get Root Company name
 */
function get_root_company_name($compid = '')
{
    $CI = & get_instance();
    $selected_year = $CI->session->userdata("finacial_year");
    $CI->db->select('FIRMNAME,FY');
    $CI->db->from(db_prefix() . 'setup');
    $CI->db->where('PlantID', $compid);
    $CI->db->where('FY', $selected_year);
    $company_data = $CI->db->get()->row();
    if ($company_data) {
        return $company_data->FIRMNAME."(".$company_data->FY.")";
    }
}
function get_days_new($feature_name = '')
{
    $CI = & get_instance();
    $selected_year = $CI->session->userdata("finacial_year");
    $selected_company = $CI->session->userdata("root_company");
    $curr_user = $GLOBALS['current_user'];
    $CI->db->select('days');
    //$CI->db->from(db_prefix() . 'staff_permissions');
    $CI->db->LIKE('feature', $feature_name);
    $CI->db->where('staff_id', $curr_user);
    $CI->db->where('plant_id', $selected_company);
    $CI->db->where('year', $selected_year);
     return $CI->db->get(db_prefix() . 'staff_permissions')->row();
}
function get_route_name($routeid = '',$selected_company)
{
    $CI = & get_instance();
    $selected_company = $CI->session->userdata("root_company");
    $CI->db->where('PlantID', $selected_company);
    $CI->db->where('RouteID', $routeid);
    $route_data = $CI->db->get(db_prefix() . 'route')->row();
    if ($route_data) {
        return $route_data->name;
    }
}
function get_account_details($ChallanID = '',$selected_company)
{
    $CI = & get_instance();
    $CI->db->select('AccountID');
    $CI->db->from(db_prefix() . 'ordermaster');
    $CI->db->where('ChallanID', $ChallanID);
    $CI->db->where('PlantID', $selected_company);
    $acc_data = $CI->db->get()->result_array();
    return $acc_data;
}
function get_travel_detail_by_staff_id($staff_id = '',$date)
{
    $CI = & get_instance();
    $CI->db->select('*');
    $CI->db->from(db_prefix() . 'travel_report');
    $CI->db->where('staff_id', $staff_id);
    $CI->db->where('date', $date);
    $travel_data = $CI->db->get()->result_array();
    return $travel_data;
}
function get_party_detail($AccountID = '',$selected_company)
{
    $CI = & get_instance();
    $CI->db->select('company,state,StationName,city');
    $CI->db->from(db_prefix() . 'clients');
    $CI->db->where('AccountID', $AccountID);
    $CI->db->where('PlantID', $selected_company);
    $acc_details = $CI->db->get()->row();
    return $acc_details;
    /*if ($route_data) {
        return $route_data->name;
    }*/
}
/**
 * Get Root Company name
 */
function get_all_root_company($compid = '')
{
    $CI = & get_instance();
    $CI->db->select('company_name,id');
    $CI->db->from(db_prefix() . 'rootcompany');
    //$CI->db->where('id', $compid);
    return $CI->db->get()->result_array();
    /*if ($company_data) {
        return $company_data;
    }*/
}
/**
 * Get staff default language
 * @param  mixed $staffid
 * @return mixed
 */
function get_staff_default_language($staffid = '')
{
    if (!is_numeric($staffid)) {
        // checking for current user if is admin
        if (isset($GLOBALS['current_user'])) {
            return $GLOBALS['current_user']->default_language;
        }
        $staffid = get_staff_user_id();
    }
    $CI = & get_instance();
    $CI->db->select('default_language');
    $CI->db->from(db_prefix() . 'staff');
    $CI->db->where('staffid', $staffid);
    $staff = $CI->db->get()->row();
    if ($staff) {
        return $staff->default_language;
    }
    return '';
}
function get_staff_recent_search_history($staff_id = null)
{
    $recentSearches = get_staff_meta($staff_id ? $staff_id : get_staff_user_id(), 'recent_searches');
    if ($recentSearches == '') {
        $recentSearches = [];
    } else {
        $recentSearches = json_decode($recentSearches);
    }
    return $recentSearches;
}
function update_staff_recent_search_history($history, $staff_id = null)
{
    $totalRecentSearches = hooks()->apply_filters('total_recent_searches', 5);
    $history             = array_reverse($history);
    $history             = array_unique($history);
    $history             = array_splice($history, 0, $totalRecentSearches);
    update_staff_meta($staff_id ? $staff_id : get_staff_user_id(), 'recent_searches', json_encode($history));
    return $history;
}
/**
 * Check if user is staff member
 * In the staff profile there is option to check IS NOT STAFF MEMBER eq like contractor
 * Some features are disabled when user is not staff member
 * @param  string  $staff_id staff id
 * @return boolean
 */
function is_staff_member($staff_id = '')
{
    $CI = & get_instance();
    if ($staff_id == '') {
        if (isset($GLOBALS['current_user'])) {
            return $GLOBALS['current_user']->is_not_staff === '0';
        }
        $staff_id = get_staff_user_id();
    }
    $CI->db->where('staffid', $staff_id)
    ->where('is_not_staff', 0);
    return $CI->db->count_all_results(db_prefix() . 'staff') > 0 ? true : false;
}
