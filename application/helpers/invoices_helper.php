...
function GetAllTrade($Type)
{
    $CI = &get_instance();
    $start_date = date('Y-m-d').' 00:00:00';
    $end_date = date('Y-m-d').' 23:59:59';
    $CI->db->select('COUNT(tbllead_master.id) AS TotalTrade');
    //$CI->db->where("(TransDate BETWEEN '$start_date' and '$end_date')");
    $CI->db->where('TType',$Type);
    $TotalTrade = $CI->db->get(db_prefix() . 'lead_master')->row();
    
    $percent                 = ($TotalTrade->TotalTrade > 0 ? number_format(($TotalTrade->TotalTrade * 100) / $TotalTrade->TotalTrade, 2) : 0);
    $data['total_by_status'] = $TotalTrade->TotalTrade;
    $data['percent']         = $percent;
    $data['total']           = $TotalTrade->TotalTrade;
    return $data;
}
function GetTradeByStatus($status,$IsApprove='',$Type)
{
    $CI = &get_instance();
    $start_date = date('Y-m-d').' 00:00:00';
    $end_date = date('Y-m-d').' 23:59:59';
    ...
    // Trade By status
    $CI->db->select('COUNT(tbllead_master.BookingID) AS TradeByStatus');
    //$CI->db->where("(TransDate BETWEEN '$start_date' and '$end_date')");
    $CI->db->where('TType',$Type);
    $CI->db->where('status',$status);
    $CI->db->where('IsApprove',$IsApprove);
    $TradeByStatus = $CI->db->get(db_prefix() . 'lead_master')->row();
    
    $percent                 = ($TotalTrade->TotalTrade > 0 ? number_format(($TradeByStatus->TradeByStatus * 100) / $TotalTrade->TotalTrade, 2) : 0);
    $data['total_by_status'] = $TradeByStatus->TradeByStatus;
    $data['percent']         = $percent;
    $data['total']           = $TotalTrade->TotalTrade;
    return $data;
...
function GetAllInward($Type)
{
    $CI = &get_instance();
    $start_date = date('Y-m-d').' 00:00:00';
    $end_date = date('Y-m-d').' 23:59:59';
    $CI->db->select('COUNT(tblGateMaster.id) AS TotalTrade');
    //$CI->db->where("(gate_in_date BETWEEN '$start_date' and '$end_date')");
    $CI->db->where('TType',$Type);
    $CI->db->where('ASNID IS NOT NULL');
    $TotalTrade = $CI->db->get(db_prefix() . 'GateMaster')->row();
...
function GetInwardByStatus($status,$Type)
{
    $CI = &get_instance();
    $start_date = date('Y-m-d').' 00:00:00';
    $end_date = date('Y-m-d').' 23:59:59';
    ...
    // Trade By status
    $CI->db->select('COUNT(tblGateMaster.BookingID) AS TradeByStatus');
    //$CI->db->where("(gate_in_date BETWEEN '$start_date' and '$end_date')");
    $CI->db->where('TType',$Type);
    $CI->db->where('status',$status);
    $TradeByStatus = $CI->db->get(db_prefix() . 'GateMaster')->row();
...
function GateInGenerated($Type)
{
    ...
    //$CI->db->where("(gate_in_date BETWEEN '$start_date' and '$end_date')");
    $CI->db->where('TType',$Type);
    $CI->db->where('Gate_in_ID IS NOT NULL');
    $TradeByStatus = $CI->db->get(db_prefix() . 'GateMaster')->row();
...
function TareWeightGenerated($Type)
{
    $CI = &get_instance();
    $start_date = date('Y-m-d').' 00:00:00';
    $end_date = date('Y-m-d').' 23:59:59';
    ...
    //$CI->db->where("(gate_in_date BETWEEN '$start_date' and '$end_date')");
    $CI->db->where('TType',$Type);
    $CI->db->where('TareWeight IS NOT NULL');
    $TradeByStatus = $CI->db->get(db_prefix() . 'GateMaster')->row();
...
function GateOutGenerated($Type)
{
    $CI = &get_instance();
    $start_date = date('Y-m-d').' 00:00:00';
    $end_date = date('Y-m-d').' 23:59:59';
    ...
    //$CI->db->where("(gate_in_date BETWEEN '$start_date' and '$end_date')");
    $CI->db->where('TType',$Type);
    $CI->db->where('gate_out_by IS NOT NULL');
    $TradeByStatus = $CI->db->get(db_prefix() . 'GateMaster')->row();
...
function GetAccountListByType($Type)
{
    $CI = &get_instance();
    
    // All Account By Type
    $CI->db->select('COUNT(tblclients.AccountID) AS TotalAccounts');
    $CI->db->where('CustomerType',$Type);
    $AllAccounts = $CI->db->get(db_prefix() . 'clients')->row();
    
    // Account List for KYC Completed
    $CI->db->select('COUNT(tblclients.AccountID) AS KYCDoneAccount');
    $CI->db->where('CustomerType',$Type);
    $CI->db->where('KYCStatus',6);
    $KYCDoneAaccount = $CI->db->get(db_prefix() . 'clients')->row();
    
    $percent                 = ($AllAccounts->TotalAccounts > 0 ? number_format(($KYCDoneAaccount->KYCDoneAccount * 100) / $AllAccounts->TotalAccounts, 2) : 0);
    $data['total_by_status'] = $KYCDoneAaccount->KYCDoneAccount;
    $data['percent']         = $percent;
    $data['total']           = $AllAccounts->TotalAccounts;
    return $data;
}
