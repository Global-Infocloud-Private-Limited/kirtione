...
class Authentication extends ClientsController
{
    ...
    public function sendOTP(){
        ...
        $this->load->model('Authentication_model');
        $result = $this->Authentication_model->checkNumberExists($phoneNumber);
        if($result){
            $curl = curl_init();
            $authKey = '406120AG34t47jvSiB6513fd24P1';
            $templateId = '6517b0acd6fc052cd8393cc1';
        }else{
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://control.msg91.com/api/v5/otp?mobile=91' . $phoneNumber . '&otp_length=6&template_id='. $templateId .'',
    ...
    public function verifyOTP(){
        ...
        if($response["type"] == 'success'){
            ...
            $this->db->where(db_prefix() . 'contacts.AccountID', $phoneNumber);
            $user = $this->db->get($table)->row();
            $user_data = [
                'client_user_id'   => $user->id,
                'contact_user_id'  => $user->id,
                'root_company'     =>$user->PlantID,
