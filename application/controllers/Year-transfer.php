...
class Year_transfer extends AdminController
{
    ...
    public function KirtiOne($id='')
    {
        if (!has_permission_new('year_transfer', '', 'edit')) {
            access_denied('invoices');
        }
        $data['title'] = _l('Kirti One Year Transfer');
         if ($this->input->post()) {
            if (!has_permission_new('year_transfer', '', 'edit')) {
                access_denied('invoices');
            }
            $data = $this->input->post();
            $success = $this->year_transfer_model->KirtiOne_transfer_year($data);
            if ($success = true) {
                set_alert('success', "Transfer Successfully");
            }
            redirect(admin_url('year_transfer'));
        ...
        $this->load->view('admin/year-transfer/KirtiOne', $data);
    }
