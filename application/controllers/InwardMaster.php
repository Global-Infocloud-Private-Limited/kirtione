...
class InwardMaster extends ClientsController
{
...
//====================== Load Inward List page =================================
    public function index()
    {
		...
		$data['title'] = "Purchase Inward Master";
        $this->data($data);
        $this->view('InwardMaster/InwardList');
        $this->layout();
    }
//=================== Get Vendor Inward List ===================================
    public function GetInwardDetailsList()
	{
		 $data = array(
        ...
        $result = $this->PurchaseModel->GetInwardData($data); 
    	...
    	{
    	        $url = base_url()."InwardMaster/InwardDetails/".$value["PurchID"];
	...
	public function InwardDetails($id)
	{
	    $LogInUser = $this->session->userdata('AccountID');
		$data['AccountID'] = $LogInUser;
        $data['company_detail'] = $this->ItemModel->get_company_detail();      
		...
		$data['InwardData'] = $this->PurchaseModel->GetInwardNoWiseData($id);	
		...
		$data['title'] = "Purchase Inward Details";
        $this->data($data);
        $this->view('InwardMaster/InwardDetails');
        $this->layout();
	}
	
	public function GetInwardItemDetail()
	{
	    $ID = $this->input->post('ID');
	    $result = $this->PurchaseModel->GethistoryDetails($ID); 
        ...
        $html .= '<th style="text-align:center;">Exp.Date</th>';
        $html .= '</tr>';
        $html .= '</thead>';
        ...
        foreach($result as $key=>$value)
    	{
                ...
                $html .= '<td style="text-align:center;">'.$value["BatchNo"].'</td>';	
                $html .= '<td style="text-align:center;">'.$value["ExpDate"].'</td>';         
