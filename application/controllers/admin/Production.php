...
class Production extends AdminController
{
  ...
  public function create_order()
    ...
    if($selected_company == 1){
     ...
     $date = to_sql_date($data["start_date"])." ".date('H:i:s');
  ...
  public function load_data_for_recipe()
    ...
    public function production_order()
    {
                ...
                for($j=1; $j<$count1; $j++) {
                    ...
                    //if($selected_company == "3"){
                        ...
                        if($PRDDetails->status_count == "0"){
                            $PRDItemStocks = $this->production_model->GetPRDItemStock($proId);
                            $PRDItemOQty = $this->production_model->GetPRDItemOQty($proId);
        ...
        $data['ItemStocks'] = $this->production_model->GetPRDItemStockNew($PrdItem);
        $data['OQtyItems'] = $this->production_model->GetPRDItemOQty($pro_orid);
        $data['GodownData'] = $this->production_model->GetGodownData();
      ...
      if(count($data) >0){
          $minutes = 0;
            ...
            $html1.= '<tr onclick=location.href='.$url.'>';  
             ...
             $html1.= '<td align="center">'.$value['pro_order_id'].'</td>';
             
             $html1.= '<td align="left">'. _d(substr($value["TransDate"],0,10)).'</td>';
             $html1.= '<td align="right">'.strtoupper($value['recipeID']).'</td>';
             ...
             $dateTimeObject1 = date_create($value['p_start_time']); 
                    ...
                    $minutes += $difference->i;
                     $html1.= '<td style="text-align:center;">'.$minutes.'</td>';
        //       if($value['production_time'] != null){
        //       $production_time = ($value['production_time'])*60;
        // $html1.= '<td style="text-align:center;">'.$minutes.'</td>';
        //  }else{
        //     $html1.= '<td style="text-align:center;">0</td>';  
        //  }
         if($value['contractor_name'] == null){
          ...
          $html1.= '<td  style="text-align:left;">'.$AccoutName.'</td>';
          $html1.= '<td  style="text-align:center;">'.$value['production_status'].'</td>';
