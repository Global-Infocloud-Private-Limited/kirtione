...
class Sale_reports extends AdminController
{
    ...
    public function ItemWiseStockReport()
    {
        ...
        $data['company_detail'] = $this->sale_reports_model->get_company_detail();
        $data['GetActiveItemList'] = $this->sale_reports_model->GetActiveItemList();
    ...
    public function GetItemWiseStockReport()
    {
        ...
        foreach ($period as $key => $value) {
            ...
            foreach ($body_data as $key1 => $value1) {
                if(substr($value1['TransDate2'],0,10) == $date2){
                    if($value1["TType"] == "P" && $value1["TType2"] == "Purchase"){
                        $PurchQty = $value1['TotalQty'];
                    }
                    if($value1["TType"] == "S" && $value1["TType2"] == "Sale"){
                        $SaleQty = $value1['TotalQty'];
                    }
                }
            }
            ...
            $TotalInQty += $InQty;
            $TotalOutQty += $OutQty;
            $TotalClsQty += $OpnQty + $PurchQty - $SaleQty + $InQty - $OutQty;
            $TotalClsQty += $ClsQty;
            $html .= '<tr>';
            ...
            $html .= '<td align="center">'.number_format($TotalClsQty, 2, '.', '').'</td>';
            $html .= '</tr>';
        }
        
        $html .= '<tr>';
        $html .= '<td align="center" colspan="2"><b>Total</b></td>';
        $html .= '<td align="center"><b>'.number_format($TotalPurchQty, 2, '.', '').'</b></td>';
        $html .= '<td align="center"><b>'.number_format($TotalSaleQty, 2, '.', '').'</b></td>';
        $html .= '<td align="center"><b>'.number_format($TotalInQty, 2, '.', '').'</b></td>';
        $html .= '<td align="center"><b>'.number_format($TotalOutQty, 2, '.', '').'</b></td>';
        $html .= '<td align="center"><b>'.number_format($TotalClsQty, 2, '.', '').'</b></td>';
        $html .= '</tr>';
        $html .= '</tbody>';
        $html .= '</table>';
