...
class Invoice_items extends AdminController
{
    ...
    public function index()
    {
        ...
        $data['parameters'] = $this->invoice_items_model->get_parameters_data();
        
        $this->load->view('admin/invoice_items/AddEditItemMaster', $data);
    }
    
    public function AddParameter(){
        $data = array(
                'ItemID' => $this->input->post('item_code1'),
                'ItemParameterID' => $this->input->post('fill_parameter'),
                'MinValue' => $this->input->post('fill_MinValue'),
                'BaseValue' => $this->input->post('fill_MaxValue'),
                'MaxValue' => $this->input->post('fill_BaseValue'),
                'UserID' => $this->session->userdata('username'),
                'TransDate' => date('Y-m-d H:i:s')
            );
        $result = $this->invoice_items_model->AddParameterDB($data);
    ...
    public function ItemParameters(){
        ...
        $data['table_data'] = $this->invoice_items_model->get_ItemParameters_data();
        $data['lastId'] = $this->invoice_items_model->get_last_recordItemParameters();
        $data['title'] = 'ItemParameters';
        $this->load->view('admin/item_parameter/item_parameter', $data);
    }
    
    public function SaveItemParameters(){
        $data = array(
            'ItemParameterID'=>$this->input->post('ItemParameterID'),
            'ItemParameterName'=>$this->input->post('ItemParameterName'),
            'TransDate'=>date('Y-m-d H:i:s'),
            'UserID'=>$this->session->userdata('username')
        );
        $result  = $this->invoice_items_model->SaveItemParameter($data);
        echo json_encode($result);
    }
    
    public function UpdateItemParameters(){
        $data = array(
            'ItemParameterID'=>$this->input->post('ItemParameterID'),
            'ItemParameterName'=>$this->input->post('ItemParameterName'),
        );
        $result  = $this->invoice_items_model->UpdateItemParameter($data);
        echo json_encode($result);
    }
...
//===================== Get Item Details By ItemID =============================
    public function GetItemDetailByID()
    {
        ...
        $item = $this->invoice_items_model->GetItemDetails($ItemID);
        echo json_encode($item);
    }
//===================== Get Item List ==========================================
    public function GetItemList()
    {
        $searchisactive = $this->input->post('searchisactive');
        $ItemList = $this->invoice_items_model->GetItemList($searchisactive);
        $html = '';
        foreach($ItemList as $key=>$value){
            $html .= '<tr class="get_ItemID" data-id="'.$value["ItemID"].'">';
            $html .= '<td>'.$value['ItemID'].'</td>';
            $html .= '<td>'.$value['ItemName'].'</td>';
            $html .= '<td>'.$value["unit"].'</td>';
            $html .= '<td>'.$value["hsn_code"].'</td>';
            $html .= '<td>'.$value["MainName"].'</td>';
            $html .= '<td>'.$value["SubName"].'</td>';
    ...
    public function GetItemParametersDetailByID()
    {
        $ItemParameterID = $this->input->post('ItemParameterID');
        $result  = $this->invoice_items_model->getitemParametersDetails($ItemParameterID);
        echo json_encode($result);
    }
    ...
    public function SaveItemID()
    {
        ...
        $data = array(
            'ItemID'=>strtoupper($this->input->post('item_code')),
            'ItemName'=>$this->input->post('description'),
            'tax'=>$this->input->post('tax'),
            'unit'=>$this->input->post('unit'),
            'subgroup_id'=>$this->input->post('subgroup_id'),
            'DivisionID'=>$this->input->post('group_id'),
            'monitorstock'=>$this->input->post('monitorstock'),
            'cd_applicable'=>$this->input->post('cd_applicable'),
            'cd_percentage'=>$this->input->post('cd_percentage'),
        ...
        $item  = $this->invoice_items_model->SaveItemID($data);
        echo json_encode($item);
    }
    ...
    {
        if (!has_permission_new('itemsmaingrp', '', 'create')) {
            access_denied('Invoice Items');
        }
        $data = array(
    ...
    {
        if (!has_permission_new('itemssubgrp', '', 'create')) {
            access_denied('Invoice Items');
        }
        ...
        if (isset($_FILES['item_image']) && !empty($_FILES['item_image'])) {
            ...
            $directory = 'uploads/ItemGroupImages';
    ...
    public function UpdateItemID()
    {
        ...
        $data = array(
            'ItemID'=>$this->input->post('item_code'),
            'ItemName'=>$this->input->post('description'),
            'tax'=>$this->input->post('tax'),
            'unit'=>$this->input->post('unit'),
            'subgroup_id'=>$this->input->post('subgroup_id'),
            ...
            'cd_applicable'=>$this->input->post('cd_applicable'),
            'cd_percentage'=>$this->input->post('cd_percentage'),
            'base_value'=>$this->input->post('base_value'),
            'opening_stock'=>$this->input->post('opening_stock'),
            'paradataArraylength'=>$this->input->post('paradataArraylength'),
            'paradataSerializedArr'=>$this->input->post('paradataSerializedArr'),
        );
        $item  = $this->invoice_items_model->UpdateItemID($data);
        echo json_encode($item);
    }
    ...
    {
        if (!has_permission_new('itemssubgrp', '', 'edit')) {
            access_denied('Invoice Items');
        }
        ...
        if (isset($_FILES['item_image']) && !empty($_FILES['item_image'])) {
            $randdom = round(microtime(time() * 1000)) . rand(000, 999);
            $file_extension1 = pathinfo($_FILES['item_image']["name"], PATHINFO_EXTENSION);
            $file_name1 = $this->input->post('ItemGroupName').$randdom . '.' . $file_extension1;
            $directory = 'uploads/ItemGroupImages';
            if ($_FILES['item_image']["error"] <= 0) {
                move_uploaded_file($_FILES['item_image']['tmp_name'], $directory . '/' . $file_name1);
                $resultString =  $file_name1;
    ...
    public function export_ItemMaster(){
        ...
        if($this->input->post()){
            ...
            $result =$this->invoice_items_model->get_table_data();
            $writer = new XLSXWriter();
