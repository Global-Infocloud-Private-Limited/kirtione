...
class Staff extends AdminController
{
    ...
    public function edit_profile()
    {
        ...
        $data['departments']       = $this->departments_model->get();
        $data['staff_departments'] = $this->departments_model->get_staff_departments($member->staffid);
        $data['staffItem'] = $this->rate_master_model->GetItem_Staff_wise();
        $data['getCenter'] = $this->rate_master_model->getCenter();
        $data['staffItemPriority'] = $this->rate_master_model->GetItem_Staff_wisePriority();
        $data['staffCenterPriority'] = $this->rate_master_model->GetCenter_Staff_wisePriority();
        /*echo "<pre>";
        print_r($data['staffItem']);
        die;*/
    ...
    /* When staff change his assigned item priority */
    public function SetItemPriority()
    {
        if ($this->input->post()) {
            $response = $this->staff_model->change_item_priority($this->input->post(null, false), get_staff_user_id());
            if ($response == true) {
                set_alert('success', "Item Priority has been set");
            } else {
    ...
    /* When staff change his assigned Center priority */
    public function SetCenterPriority()
    {
        if ($this->input->post()) {
            $response = $this->staff_model->change_center_priority($this->input->post(null, false), get_staff_user_id());
            if ($response == true) {
                set_alert('success', "Center Priority has been set");
            } else {
                set_alert('warning', "something went wrong please try again");
            }
            redirect(admin_url('staff/edit_profile'));
