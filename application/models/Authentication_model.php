...
class Authentication_model extends App_Model
{
    ...
    public function login_admin($email, $password, $remember, $staff)
    {
        if ((!empty($email)) and (!empty($password))) {
    ...
    function checkNumberExists($phoneNumber)
    {
        ...
        $this->db->where(db_prefix() . 'clients.phonenumber', $phoneNumber);
        //$this->db->where(db_prefix() . 'clients.CustomerType != ', 1);
        $contact = $this->db->get(db_prefix() . 'clients')->row();
