...

{
...

	{
		...
		if ($data['report_type'] == "1") {
			...
			$this->db->select('tblclients.AccountID,tblclients.CustomerType,tblclients.company,tblCustomerType.Name as CustomerTypeName,tblcontacts.aadhaar_number as adharno , tblcontacts.Pan as panno,

				tblAadharDetails.house,tblAadharDetails.street,tblAadharDetails.loc,tblAadharDetails.po,tblAadharDetails.subdist,tblAadharDetails.dist,tblAadharDetails.state,tblAadharDetails.pincode,

				tblclients.address AS RegAddress,tblclients.datecreated,

				tblGstRecord.business_name,tblGstRecord.state AS GSTState,tblGstRecord.address AS GstAddress,tblGstRecord.gstin,

				tblstaff.firstname,tblstaff.lastname,tblxx_statelist.state_name,tblxx_citylist.city_name');

			$this->db->join('tblAadharDetails', 'tblAadharDetails.AccountID = tblclients.AccountID AND tblAadharDetails.Type = "1"', "LEFT");

			$this->db->join('tblGstRecord', 'tblGstRecord.AccountID = tblclients.AccountID AND tblGstRecord.IsPrimary="1"', 'LEFT');
		} else {

			$this->db->select('COUNT(tblclients.AccountID) AS TotalCount,tblclients.datecreated');

			$this->db->group_by("DATE(tblclients.datecreated)");
		}
		...
		$this->db->join('tblstaff', 'tblstaff.AccountID = tblclients.ref_by', "LEFT");
		...
		$this->db->where("tblclients.datecreated BETWEEN '$from_date' AND '$to_date'");

		if ($data['client_type'] != "") {

			$this->db->where('tblclients.CustomerType', $data['client_type']);
		}

		if ($data['Account_state'] != "") {

			$this->db->where('tblclients.state', $data['Account_state']);
		}

		$this->db->order_by("tblclients.datecreated ", "ASC");
...

	{
		...
		return $this->db->get(db_prefix() . 'Region')->result_array();
	}
...

	{
		...
		$ClientArray = array(
			...
			'IsKirtiOneAccess' => $Data["IsKirtiOneAccess"],
		...
		if ($this->db->affected_rows() > 0) {
			...
			$AllItemDivision = $this->get_itemDivision();

			foreach ($AllItemDivision as $key => $value) {
...

	{
		...
		if ($contacts->Pan) {
			...
			$ContactsArray['Pan'] = $Data["Pan"];

			$ContactsArray['pan_verified_date'] = date('Y-m-d H:i:s');

			$ContactsArray['panVerifiedID'] = $LogID;
		}
		...
		if ($CheckGstRecord) {
			...
			$GstArray = array('IsPrimary' => 0);



			$this->db->where('AccountID', $AccountID);

			$this->db->update(db_prefix() . 'GstRecord', $GstArray);

			// GST Update Primary

			$GstArrayUpdate = array(
				...
				'business_name' => $Data["AccoountName"],

				'constitution_of_business' => $Data["AccoountName"],

				'address' => $Data["house"] . ' ' . $Data["street"] . ' ' . $Data["loc"] . ' ' . $Data["po"] . ' ' . $Data["subdist"] . ' ' . $Data["city"] . ' ' . $Data["state"] . ' ' . $Data["zip"],

			);

			$this->db->where('gstin', strtoupper($Data["vat"]));

			$this->db->where('AccountID', $AccountID);

			$this->db->update(db_prefix() . 'GstRecord', $GstArrayUpdate);
		} else {
			$GstArray = array(
				'AccountID' => $AccountID,
				'gstin' => strtoupper($Data["vat"]),
				'state_code' => $Data["state"],
				'state' => $Data["state"],
				'business_name' => $Data["AccoountName"],
				'constitution_of_business' => $Data["AccoountName"],
			...
			$this->db->insert(db_prefix() . 'GstRecord', $GstArray);
		}

		$isprimaryGst = $Data['isprimaryGst'] ?? '';
		if (!empty($isprimaryGst)) {
			$this->db->where('AccountID', $AccountID);
			$this->db->update(db_prefix() . 'GstRecord', ['IsPrimary' => 0]);

			$this->db->where('AccountID', $AccountID);
			$this->db->where('id', $isprimaryGst);
			$this->db->update(db_prefix() . 'GstRecord', ['IsPrimary' => 1]);
		}
	...
	public function GetTDSSections()
	{

		$this->db->select('tblTDSMaster.*');

		$this->db->where('Blocked', 'N');
	...
	public function GetTDSSectionRate($tds_code)
	{

		// $this->db->select('t1.*');

		// $this->db->where('t1.TDSCode', $tds_code);

		// $this->db->where('t1.effective_date = (select max(t2.effective_date) from tblTDSDetails t2 where t2.TDSCode = "' . $tds_code . '")');

		// return $this->db->get(db_prefix() . 'TDSDetails t1')->row();



		$this->db->select('tblTDSDetails.*');

		$this->db->where('TDSCode', $tds_code);
	...
	public function SaveFarmarDetails($Data)

	{
		...
		if ($Data["is_manual"] == true) {
			...
			$full_name = $Data['aadharFullname'];
		}



		$state = $this->getstatedetail($Data["state"]);

		$city = $this->GetCityByid($Data["city"]);

		$subdist = $this->GetTalukaByid($Data["subdist"]);
		...
		if ($Data["village"] == "new") {
			...
			$insertvillage = array(
				...
				'TalukaId' => $Data['subdist'],

				'DistrictId' => $Data['city'],
			...
			$this->db->insert(db_prefix() . 'villagedetails', $insertvillage);
		...
		if ($Data["aadharDOB"]) {
			...
			$dob = to_sql_date($Data["dob"]);
		}

		if ($Data["aadharGender"]) {

			$gender = $Data["aadharGender"];
		} else {
		...
		if ($Data['is_manual'] == "true") {
			...
			$this->db->insert(db_prefix() . 'AadharDetails', $AadharDetails2);

			// Current address

			$this->db->insert(db_prefix() . 'AadharDetails', $AadharDetails3);
		} else {

			// add as per aadhaar address

			$this->db->insert(db_prefix() . 'AadharDetails', $AadharDetails1);
	...
	public function GetBankDetails($AccountID)

	{
		...
		$bank_details = $this->db->get(db_prefix() . 'BankDetails')->result_array();
	...
	public function UpdateFarmarDetails($Data)
	{
		$AccountID = $Data["AccountID"];
		$selected_company = $this->session->userdata('root_company');
		$FY = $this->session->userdata('finacial_year');
		$LogID = $this->session->userdata('username');
		$GetBankDetails = $this->GetBankDetails($AccountID);
		if ($GetBankDetails) {
			$KYCStatus = 6;
		} else {
			$KYCStatus = 5;
		...
		if ($Data["existing_aadhar"] != $Data["Aadhaarno"]) {
			if ($Data["loc"] == '') {
				$loc = $Data['aadharLoc'];
			}else{
				$loc = $Data["loc"];
		...
		$ClientArray = array(
			'company' => $Data["firstname"] . " " . $Data["middlename"] . " " . $Data["lastname"],
			'phonenumber' => $Data["phonenumber"],
			'state' => $Data["state"],
			'dist' => $Data["city"],
			'house' => $house,
		...
		if ($Data["existing_aadhar"] == $Data["Aadhaarno"]) {
			...
			$this->db->update(db_prefix() . 'AadharDetails', $addr_detail);
		} else {
			$this->db->where('AccountID', $AccountID);
			if ($this->db->delete(db_prefix() . 'AadharDetails')) {
				// Add As per Aadhaar address
				$AadharDetails1 = array(
					'AccountID' => strtoupper($Data["AccountID"]),
					'Type' => 1,
				...
				$this->db->insert(db_prefix() . 'AadharDetails', $AadharDetails1);
				// Current address same as aadhaar address
				$AadharDetails2 = array(
					'AccountID' => strtoupper($Data["AccountID"]),
					'Type' => 3,
					'TransDate' => date('Y-m-d H:i:s'),
		...
		if ($Data["aadhaar_verified_date_val"] == null || $Data["aadhaar_verified_date_val"] == "") {

			// $ContactsArray['aadhaar_number'] = $Data["Aadhaarno"];

			$ContactsArray['aadhaar_verified_date'] = date('Y-m-d H:i:s');
		}
	...
	public function UpdateDefaultBankAccount($id, $accountID)
	{
		...
		$BankDetailsArray = array('IsPrimary' => 1);

		$this->db->where('AccountID', $accountID);

		$this->db->where('id', $id);

		$rowsCount = $this->db->update(db_prefix() . 'BankDetails', $BankDetailsArray);
	...
	public function GetAccountBankDetails($AccountID)

	{

		$this->db->join('tblclients', 'tblclients.AccountID = tblBankDetails.AccountID');

		$this->db->where('tblBankDetails.AccountID', $AccountID);

		return $this->db->get('tblBankDetails')->result_array();
	}



	public function GetPanWiseDetails($Pan)

	{

		$this->db->select('tblcontacts.*,tblclients.company,tblclients.active,tblclients.CustomerType,tblGstRecord.gstin');

		$this->db->join('tblclients', 'tblclients.AccountID = tblcontacts.AccountID');

		$this->db->join('tblGstRecord', 'tblGstRecord.AccountID = tblcontacts.AccountID AND tblGstRecord.active_status="Active"', 'left');
	...
	public function UpdateStatusPan($Pan, $Status)

	{

		$this->db->select('tblclients.AccountID');
		...
		if (!empty($result)) {
			...
			return $this->db->update('tblclients', ['active' => $Status]);
		}
	...
	public function GetImageDetails($AccountID, $Type)

	{
		...
		if ($Type == 'bank_cheque') {
			...
			return $this->db->get('tblBankDetails')->row_array();
		} else if ($Type == 'farm_image') {

			$this->db->where('id', $AccountID);

			return $this->db->get('tblfarm_details')->row_array();
	...
	public function getCityIDdb($city)
	...
	public function getTalukaIDdb($taluka)

	{

		$this->db->where('TalukaName', $taluka);

		return $this->db->get('tblTalukaMaster')->row();
	}



	public function GetDeafultGst($AccountID)

	{

		$this->db->join('tblclients', 'tblclients.AccountID = tblGstRecord.AccountID');

		$this->db->where('tblGstRecord.AccountID', $AccountID);

		$this->db->where('IsPrimary', '1');

		return $this->db->get('tblGstRecord')->row();
	}



	public function GetAccountGstDetails($AccountID)

	{

		$this->db->where('AccountID', $AccountID);

		$result = $this->db->get('tblGstRecord')->result_array();

		return $result;
	}

	public function GetAccountFirmDetails($AccountID)

	{

		$this->db->select('tblfarm_details.id,tblfarm_details.farm_name,tblfarm_details.survey_number,tblfarm_details.farm_area,tblfarm_details.farm_unit,tblfarm_details.village,

			tblfarm_details.pincode,tblfarm_details.saatbara_image,tblfarm_details.Address,tblfarm_details.latitude,tblfarm_details.longitude,tblxx_statelist.state_name,tbl_xx_city.city,tblTalukaMaster.TalukaName');

		$this->db->where('tblfarm_details.AccountID', $AccountID);

		$this->db->join('tblxx_statelist', 'tblxx_statelist.id = tblfarm_details.state');

		$this->db->join('tbl_xx_city', 'tbl_xx_city.id = tblfarm_details.district');

		$this->db->join('tblTalukaMaster', 'tblTalukaMaster.id = tblfarm_details.taluka');

		$result = $this->db->get('tblfarm_details')->result_array();
	...
	public function GetAccountCropList($AccountID)

	{

		$this->db->select('tblCropMaster.CropArea,tblCropMaster.AreaUnit,tblCropMaster.season,tblCropMaster.ItemID,

			tblfarm_details.farm_name,tblitems.ItemName');

		$this->db->where('tblCropMaster.AccountID', $AccountID);
		...
		$result = $this->db->get('tblCropMaster')->result_array();
...

	{
		...
		$this->db->where(db_prefix() . '_xx_city.state', $StateID);

		$this->db->order_by(db_prefix() . '_xx_city.city', 'ASC');

		return $this->db->get('tbl_xx_city')->result_array();
	}

	public function GetCityListByShortCode($StateCode)

	{

		$this->db->where(db_prefix() . 'xx_citylist.state_id', $StateCode);
	...
	public function GetCityListForCompanyMaster($StateID)
...

	{
		...
		$this->db->where(db_prefix() . 'xx_citylist.id', $cityid);

		$this->db->order_by(db_prefix() . 'xx_citylist.city_name', 'ASC');

		return $this->db->get('tblxx_citylist')->row();
	}
	...
	public function GetVillageList($zip)

	{

		$this->db->select(db_prefix() . 'villagedetails.id,villagedetails.VillageName,villagedetails.Pincode');

		$this->db->where(db_prefix() . 'villagedetails.Pincode', $zip);

		$this->db->order_by(db_prefix() . 'villagedetails.VillageName', 'ASC');
	...
	public function GetTalukaByid($Talukaid)

	{

		$this->db->select(db_prefix() . 'TalukaMaster.*');

		$this->db->where(db_prefix() . 'TalukaMaster.id', $Talukaid);
	...
	public function getstatedetail($ShortCode)

	{

		$this->db->where('country_id', '1');

		$this->db->where('short_name', $ShortCode);

		$this->db->order_by('state_name', 'ASE');

		return $this->db->get(db_prefix() . 'xx_statelist')->row();
	}
...

	{
		...
		return $this->db->get(db_prefix() . 'CustomerType')->result_array();
	}
...

	{
		...
		$this->db->where('tblclients.CustomerType', '2');
	...
	// public function GetFarmarList($search = '')

	// {

	//     $this->db->select('tblclients.AccountID,tblcontacts.firstname,tblcontacts.lastname,tblclients.phonenumber,tblxx_statelist.state_name,tblxx_citylist.city_name,tblcontacts.aadhaar_number as adharno');
	...
	//     $this->db->where('(tblclients.accountID LIKE "%' . $search . '%" OR tblcontacts.firstname LIKE "%' . $search . '%" OR tblcontacts.lastname LIKE "%' . $search . '%" OR tblcontacts.aadhaar_number LIKE "%' . $search . '%")');

	//     $this->db->where(db_prefix() . 'clients.CustomerType', 1);
...

	{
		...
		$this->db->select('COALESCE(tblcontacts.firstname, "") as firstname,

			COALESCE(tblcontacts.lastname, "") as lastname,

			COALESCE(tblcontacts.accountID, "") as AccountID,

			COALESCE(tblcontacts.phonenumber, "") as phonenumber,

			COALESCE(tblxx_statelist.state_name, "") as state_name,

			COALESCE(tblxx_citylist.city_name, "") as city_name,

			COALESCE(tblcontacts.aadhaar_number, "") as adharno');
		...
		$this->db->like('tblcontacts.accountID', $search);

		$this->db->or_like('tblcontacts.firstname', $search);

		$this->db->or_like('tblcontacts.lastname', $search);

		$this->db->or_like('tblcontacts.aadhaar_number', $search);

		// $this->db->or_like('tblxx_statelist.state_name', $search);

		// $this->db->or_like('tblxx_citylist.city_name', $search);
	...
	public function GetAadhar($acc_id)
...

	{
		...
		$this->db->select('tblclients.*,tblcontacts.Pan,' . db_prefix() . 'contacts.aadhaar_number,

			' . db_prefix() . 'contacts.istcs,' . db_prefix() . 'contacts.istds,' . db_prefix() . 'contacts.TcsStartDate,' . db_prefix() . 'contacts.phonenumber as altnumber,
...

	{
		...
		$this->db->select('tblclients.*,tblcontacts.firstname,tblcontacts.middlename,tblcontacts.lastname,tblcontacts.gender,tblcontacts.dob,tblcontacts.email,

			tblcontacts.Pan,tblcontacts.aadhaar_number,tblcontacts.aadhaar_verified_date,tblcontacts.veryfiedUserID,tblcontacts.TcsStartDate,tblcontacts.istcs,tblcontacts.istds,tblcontacts.TdsSection,tblcontacts.TdsPercentage,tblcontacts.TdsPercentage as rate,tblcontacts.aadhaar_verified_date,tblcontacts.gender,tblcontacts.dob,

			tblAadharDetails.state AS Astate,tblAadharDetails.dist AS Adist,tblAadharDetails.subdist AS Asubdist,tblAadharDetails.loc AS Aloc,tblAadharDetails.pincode AS Apincode,');
		...
		$this->db->where('tblxx_statelist.short_name', $result->state);
		...
		if ($result) {
			...
			$CityList = $this->GetCityListByShortCode($stateID->short_name);

			$TDSList = $this->GetTDSSectionRate($result->TdsSection);

			$TalukaList = $this->GetTaluka($result->dist);

			$VillageList = $this->GetVillageList($result->zip);

			$result->CityList = $CityList;

			$result->TDSList = $TDSList;
	...
	public function checkAadhar($aadharNo)

	{

		$this->db->where('aadhaar_number', $aadharNo);
	...
	public function VerifyAadharDb($data)

	{

		$this->db->where('aadhaar_number', $data['aadhaar_number']);

		$this->db->where('AccountID', $data['AccountID']);

		return $this->db->update('tblcontacts', $data);
	}



	public function VerifyPanDb($data)

	{

		$this->db->where('Pan', $data['Pan']);
...

	{
		...
		return $this->db->get('tblBankDetails')->row();
	}



	public function CheckPanExist($Pan)
...

	{
		...
		//$this->db->where('SubActGroupID2', '60002');
	...
	public function SaveBrokerDetails($Data)
	...
	public function UpdateBrokerDetails($Data)
...

	{
		...
		$sql = 'SELECT state_name FROM tblxx_statelist WHERE short_name = "' . $state . '"';
