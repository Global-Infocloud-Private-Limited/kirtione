...
class E_filling_model extends App_Model
{
  ...
  public function get_Account_Details($postData)
    ...
    public function get_GstTypeWiseValueP($data){
        ...
        $this->db->from(db_prefix() . 'history');
        $this->db->join(db_prefix() . 'purchasemaster', db_prefix() . 'purchasemaster.PurchID = ' . db_prefix() . 'history.TransID AND  '.db_prefix() . 'purchasemaster.PlantID = ' . db_prefix() . 'history.PlantID AND '.db_prefix() . 'purchasemaster.FY = ' . db_prefix() . 'history.FY');
       ...
       $this->db->where(db_prefix() . 'salesmaster.Transdate BETWEEN "'.$from_date.'" AND "'.$to_date.'"');
        ...
        if(!empty($CDNRSaleID)){
            ...
            if(!empty($CDNRSaleIDS)){
                ...
                $this->db->where(db_prefix() . 'purchasemaster.BT', "N");
        ...
        $this->db->where(db_prefix() . 'history.TType', 'R');
        $this->db->where(db_prefix() . 'history.Transdate BETWEEN "'.$from_date.'" AND "'.$to_date.'"');
        $this->db->group_by(db_prefix() . 'history.TransID,'.db_prefix() . 'history.TransID');
    ...
    public function get_data_for_B2B_CD($data){
        ...
        $sql = 'SELECT tbltaxes.taxrate,SUM(tblcdnotehistory.rate) AS taxableAmt,SUM(tblcdnotehistory.amount) AS InvAmt,tblcdnotehistory.billno,tblordermaster.GSTNO,tblcdnotehistory.transdate, tblclients.state,tblxx_statelist.state_name FROM `tblcdnotehistory` 
                JOIN tblitems ON tblitems.ItemID = tblcdnotehistory.itemid AND tblitems.PlantID = '.$selected_company.'
    ...
    public function get_data_for_B2CS($data){
        ...
        $sql = 'SELECT tbltaxes.taxrate,SUM(ChallanAmt) AS taxableAmt, tblclients.state,tblxx_statelist.state_name FROM `tblhistory` 
                JOIN tblitems ON tblitems.ItemID = tblhistory.ItemID AND tblitems.PlantID = '.$selected_company.'
                JOIN tbltaxes ON tbltaxes.id = tblitems.tax 
...
GROUP BY tblitems.hsn_code,tblhistory.cgst,tblhistory.sgst,tblhistory.igst';
    ...
    public function get_data_for_HSNCD($data){
        ...
        $sql = 'SELECT tblcdnotehistory.hsncode,tblcdnotehistory.cgst,SUM(tblcdnotehistory.cgstamt) AS CGSTSUM, tblcdnotehistory.sgst, SUM(tblcdnotehistory.sgstamt) SGSTSUM,tblcdnotehistory.igst, 
        ...
        FROM `tblcdnotehistory` WHERE plantid = '.$selected_company.' AND fy = "'.$year.'" AND transdate BETWEEN "'.$from_date.'" AND "'.$to_date.'" AND tblcdnotehistory.ttype = "C"
        GROUP BY tblcdnotehistory.hsncode,tblcdnotehistory.cgst,tblcdnotehistory.sgst,tblcdnotehistory.igst';
        $result = $this->db->query($sql)->result_array();
        return $result;
    }
    ...
    public function get_data_for_HSNDD($data){
        ...
        $sql = 'SELECT tblcdnotehistory.hsncode,tblcdnotehistory.cgst,SUM(tblcdnotehistory.cgstamt) AS CGSTSUM, tblcdnotehistory.sgst, SUM(tblcdnotehistory.sgstamt) SGSTSUM,tblcdnotehistory.igst, 
        SUM(tblcdnotehistory.igstamt) AS IGSTSUM, SUM(tblcdnotehistory.rate) AS TaxableAmt , SUM(tblcdnotehistory.amount) AS BillAmt,SUM(tblcdnotehistory.qty) AS BilledQtySum  
        FROM `tblcdnotehistory` WHERE plantid = '.$selected_company.' AND fy = "'.$year.'" AND transdate BETWEEN "'.$from_date.'" AND "'.$to_date.'" AND tblcdnotehistory.ttype = "D"
        GROUP BY tblcdnotehistory.hsncode,tblcdnotehistory.cgst,tblcdnotehistory.sgst,tblcdnotehistory.igst';
        $result = $this->db->query($sql)->result_array();
        return $result;
    }
    ...
    public function get_data_for_gstr_3_1_a($data){
        ...
        $sqlMFirst = 'SELECT SUM(tblsalesmaster.SaleAmt) AS TaxableAmt, SUM(tblsalesmaster.sgstamt) AS SAmt, 
        SUM(tblsalesmaster.cgstamt) AS CAMT, SUM(tblsalesmaster.igstamt) AS IAmt FROM `tblsalesmaster` 
        WHERE tblsalesmaster.PlantID = '.$selected_company.' AND tblsalesmaster.FY LIKE "'.$year.'" AND tblsalesmaster.BT IN("T","M","C") AND tblsalesmaster.Transdate BETWEEN "'.$from_date.'" AND "'.$to_date.'" ';
        $resultMFirst = $this->db->query($sqlMFirst)->row();
...
INNER JOIN tblsalesmaster ON tblsalesmaster.SalesID = tblcdnotehistory.TransID AND tblsalesmaster.PlantID = tblcdnotehistory.plantid 
WHERE tblcdnotehistory.plantid = '.$selected_company.' AND tblcdnotehistory.fy = "'.$year.'" AND tblcdnotehistory.transdate BETWEEN "'.$from_date.'" AND "'.$to_date.'" AND tblsalesmaster.BT IN("T","M","C") AND tblcdnotehistory.ttype = "C" ';
...
INNER JOIN tblsalesmaster ON tblsalesmaster.SalesID = tblcdnotehistory.TransID AND tblsalesmaster.PlantID = tblcdnotehistory.plantid
WHERE tblcdnotehistory.plantid = '.$selected_company.' AND tblcdnotehistory.fy = "'.$year.'" AND tblcdnotehistory.transdate BETWEEN "'.$from_date.'" AND "'.$to_date.'" AND tblsalesmaster.BT IN("T","M","C") AND tblcdnotehistory.ttype = "D" ';
        ...
        WHERE tblhistory.PlantID = '.$selected_company.' AND tblhistory.FY = "'.$year.'"  AND tblhistory.TType ="R" AND tblhistory.Transdate BETWEEN "'.$from_date.'" AND "'.$to_date.'" ';
