  ...
  public function get_Account_Details_salesman($postData)
    ...
    */
	...
	{	
		...
		$this->db->select('K1salesmaster.SalesID,K1salesmaster.CenterID,tblxx_citylist.city_name,K1salesmaster.GSTIN AS gstno,tblclients.company,tblclients.phonenumber,
		tblclients.house,tblclients.zip,tblxx_statelist.id As StateId,K1salesmaster.SaleAmt,K1salesmaster.DiscAmt,K1salesmaster.cgstamt,K1salesmaster.sgstamt,K1salesmaster.igstamt,K1salesmaster.RndAmt,K1salesmaster.BillAmt,
		tblcontacts.email,K1salesmaster.vehicleno,K1salesmaster.ewaybill_no');
		$this->db->from(db_prefix() . 'K1salesmaster');
		//$this->db->join(db_prefix() . 'K1challanmaster', db_prefix() . 'K1salesmaster.ChallanID = '.db_prefix() . 'K1challanmaster.ChallanID');
		...
		$this->db->join(db_prefix() . 'contacts', db_prefix() . 'clients.AccountID = '.db_prefix() . 'contacts.AccountID AND '.db_prefix() . 'clients.PlantID = '.db_prefix() . 'contacts.PlantID',"LEFT");
		$this->db->join(db_prefix() . 'xx_citylist', db_prefix() . 'clients.dist = '.db_prefix() . 'xx_citylist.id','LEFT');
		$this->db->join(db_prefix() . 'xx_statelist', db_prefix() . 'clients.state = '.db_prefix() . 'xx_statelist.short_name',"LEFT");
	...
	public function fetchCenterDetails($CenterID)
	{
		$this->db->select(db_prefix() . 'CenterMaster.*,tblxx_citylist.city_name,tblxx_statelist.id AS statecode');
		$this->db->from(db_prefix() . 'CenterMaster');
		$this->db->join(db_prefix() . 'xx_citylist', db_prefix() . 'CenterMaster.city = '.db_prefix() . 'xx_citylist.id','LEFT');
		$this->db->join(db_prefix() . 'xx_statelist', db_prefix() . 'CenterMaster.state = '.db_prefix() . 'xx_statelist.short_name',"LEFT");
	...
	public function fetchItemDetails($TransId)
	{
		...
		$this->db->select(db_prefix() . 'K1history.*,'.db_prefix() . 'product.hsn_code,'.db_prefix() . 'product.unit,'.db_prefix() . 'product.ProductName');
		$this->db->from(db_prefix() . 'K1history');
		$this->db->join(db_prefix() . 'product', db_prefix() . 'K1history.ItemID = '.db_prefix() . 'product.ProductID AND '.db_prefix() . 'K1history.PlantID = '.db_prefix() . 'product.PlantID');
