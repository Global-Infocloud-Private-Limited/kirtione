   ...
   return  $query->row();
    ...
    $where_clients = '';
     ...
     if(isset($postData['search']) ){
       ...
       $q = $postData['search'];
       $this->db->select(db_prefix() . 'clients.*,'.db_prefix() . 'accountroutes.RouteID,'.db_prefix() . 'route.name,'.db_prefix() . 'xx_statelist.state_name,'.db_prefix() . 'customers_groups.name AS aname,'.db_prefix() . 'customers_groups.id AS customers_groups_id,'.db_prefix() . 'accountlocations.LocationTypeID');
       ...
       foreach($records as $row ){
          $response[] = array("label"=>$row->company,"value"=>$row->AccountID,"address"=>$row->address,"address2"=>$row->Address3,"state"=>$row->state,"station"=>$row->StationName,"gst"=>$row->vat,"route"=>$row->RouteID,"route_name"=>$row->name,"state_name"=>$row->state_name,"account_type"=>$row->DistributorType,"account_type_name"=>$row->aname,"customers_groups_id"=>$row->customers_groups_id,"city"=>$row->city,"LocationTypeID"=>$row->LocationTypeID);
       }
