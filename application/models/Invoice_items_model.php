...
class Invoice_items_model extends App_Model
{
    ...
    public function get($id)
    {
        ...
        $this->db->join('tblitems_groups','tblitems_groups.id = tblitems.DivisionID');
        $this->db->join('tblitems_sub_groups','tblitems_sub_groups.id = tblitems.subgroup_id');
        //$this->db->join('tblstockmaster','tblstockmaster.ItemID = tblitems.ItemID');
        $this->db->order_by('tblitems.ItemID', 'ASC');
        if (($id != null) || ($id != '')) {
            $this->db->where('tblitems.ItemID', $id);
    ...
    public function GetItemList($searchisactive)
    {
        $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        
        $this->db->select('tblitems.*,tblitems_sub_groups.name AS SubName,tblitems_main_groups.name As MainName');
        $this->db->join('tblitems_sub_groups','tblitems_sub_groups.id = tblitems.subgroup_id');
        $this->db->join('tblitems_main_groups','tblitems_main_groups.id = tblitems_sub_groups.main_group_id');
        $this->db->where('tblitems.PlantID', $selected_company);
        if($searchisactive){
            $this->db->where('tblitems.isactive', $searchisactive);
        }
    ...
    function AddParameterDB($data){
        $records = $this->db->insert('tblItemQCParameter',$data);
        return $records;
    }
    ...
    function getItemStatus($id){
        ...
        $this->db->where(db_prefix() . 'items.ItemID', $id);
        $this->db->order_by('PlantID', 'ASC');
  ...
  function getItemDetailsByID($postData){
    ...
    public function SaveItemID($data)
    {
		...
		$GetGroupCode = $this->getItemGroupDetails($data["subgroup_id"]);
		$data['GroupCode'] = $GetGroupCode->ShortCode;
        ...
        if($INSERT > 0){
            foreach ($ParameterArray as $value) {
                ...
                $this->db->insert(db_prefix() . 'ItemQCParameter', $insertArray);
            }
            return true;
        }else{
            return false;
    ...
    public function SaveItemParameter($data)
    {
        return $this->db->insert('tblItemParameter', $data);
    }
    
    public function UpdateItemParameter($data)
    {   
        $this->db->where('ItemParameterID',$data['ItemParameterID']);
        $this->db->set('ItemParameterName',$data['ItemParameterName']);
        $this->db->set('Lupdate',date('Y-m-d H:i:s'));
        $this->db->set('UserID2',$this->session->userdata('username'));
        return $this->db->update('tblItemParameter');
    }
    ...
    public function UpdateItemID($data)
    {
        ...
        if($this->db->update(db_prefix() . 'items', $data)){
            
            $checkStockRecord = $this->ChkStockRecord($ItemID,$selected_company,$FY);
            if(empty($checkStockRecord)){
                ...
                $this->db->where('PlantID', $selected_company);
                $this->db->where('FY', $FY);
                $this->db->update(db_prefix() . 'stockmaster', $stockdata);
            }
            
            // Insert / Update Parameter
			for($k=0; $k<$ParameterAssignArraylen; $k++) {
				$ItemParameterID = $ParameterAssignArray[$k][0];
				$MinValue = $ParameterAssignArray[$k][1];
				$MaxValue = $ParameterAssignArray[$k][2];
				$BaseValue = $ParameterAssignArray[$k][3];
				$addtblid = $ParameterAssignArray[$k][4];
				$status = $ParameterAssignArray[$k][5];
				...
				if(!empty($addtblid))
				{
					...
					$this->db->update(db_prefix() . 'ItemQCParameter', $UpdateAddress);
				}else{
					$InsAddress = array(
    					'ItemID' =>$ItemID,
    					'ItemParameterID' =>$ItemParameterID,
					...
					$this->db->insert(db_prefix() . 'ItemQCParameter',$InsAddress);
				}
			}
			return true;
        }else{
...
*/
    ...
    public function get_ItemParameters_data(){
        $this->db->order_by('ItemParameterID', 'ASC');
        return $this->db->get('tblItemParameter')->result_array();
    }
    ...
    public function getitemParametersDetails($ItemParameterID){
        $this->db->where('ItemParameterID', $ItemParameterID);
        return $this->db->get('tblItemParameter')->row();
    }
    ...
    public function get_last_recordItemParameters(){
        $this->db->select('*');
        $this->db->from('tblItemParameter');
        $this->db->order_by('ItemParameterID', 'DESC'); // 'created_at' is the column name of the date on which the record has stored in the database.
        $ItemParametersRecord =  $this->db->get()->row();
        return $ItemParametersRecord->ItemParameterID;
    }
