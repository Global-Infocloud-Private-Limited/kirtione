    ...
    class Cron_model extends App_Model
    {
        ...
        public function POOrder()
        {
            ...
            $OrderDetails = $this->KirtiOneOrderModel->GetLeanMarkEntries();
            $PoOrderArray = [];
            foreach($OrderDetails as $Value)
            {
                $Rate = $Value['rate'];
                $CommissionPercent = $Value['CommissionPercent'];
                $NetRate = $Rate * ($CommissionPercent / 100);
                $PoOrderArray[] = array(
                    'CenterID'=>$Value['CenterID'],
                    'ItemID'=>$Value['ItemID'],
                    'ItemName'=>$Value['ProductName'],
                    'AccountID'=>$Value['PartyID'],
                    'BilledQty'=>$Value['TotalQty'],
                    'Rate'=>$Rate,
                    'Commission'=>$CommissionPercent,
                    'taxrate'=>$Value['taxrate'],
                    'unit'=>$Value['unit'],
                    'state'=>$Value['state'],
                    'PackingQty'=>$Value['PackingQty'],
                    'NetRate'=>$NetRate
                );
            }
            $grouped = [];
            foreach ($PoOrderArray as $row) {
                $key = $row['AccountID'] . '_' . $row['CenterID'];
            ...
            {
                $ItCount = count($value['Items']);
                $prefix = 'PR';
                $purchase_orderNumbar = get_option('next_purchase_request_number_for_kirtione');
                $new_purchase_orderNumbar = $prefix.$FY."1".$purchase_orderNumbar;
                //Insert PR Request
                $KirtiOnePurchMaster = array(
                    'PlantID'=>$PlantID,
                    'FY'=>$FY,
                ...
                $this->db->insert(db_prefix() . 'K1purchase_request_master',$KirtiOnePurchMaster);
                if($this->db->affected_rows() > 0)
                {
                    $this->increment_next_number('next_purchase_request_number_for_kirtione');
                    $i =1;
                    $TotalCgstAmt = 0;
                    $TotalSgstAmt = 0;
                    $TotalIgstAmt = 0;
                    ...
                    {
                        $PurchRate = $item['Rate'] - ($item['Rate'] * $item['Commission'] / 100);
                        $SaleRate = ($PurchRate * $item['taxrate'] / 100) + $PurchRate;
                        $taxableAmount = $PurchRate * $item['BilledQty'];
                        $cgst = 0;$sgst=0;$igst0;$cgstAmt =0;$sgstAmt=0;$igstAmt=0;
                        if($item['state'] == 'MH')
                        {
                            $cgst = $item['taxrate']/2;
                            $sgst = $item['taxrate']/2;
                            $cgstAmt = ($taxableAmount * ($cgst/100));
                            $sgstAmt = ($taxableAmount * ($sgst/100));
                        }else{
                            $igst = $item['taxrate'];
                            $igstAmt = ($taxableAmount * ($igst/100));
                        }
                        $NetOrderAmt = $taxableAmount + $cgstAmt + $sgstAmt + $igstAmt;
                        //$TotalRate = $item
                        $data_array_result = array(
                            ...
                            'igst'=>$igst,
                            'igstamt'=>$igstAmt,
                            'CaseQty'=>$item['PackingQty'],
                            'Cases'=>0.00,
                            'OrderAmt'=>$taxableAmount,
                            'ChallanAmt'=>$taxableAmount,
                        ...
                        $TotalOrderAmt += $taxableAmount;
                        $TotalInvAmt += $NetOrderAmt;
                        $i++;
                    }
                    $Invamt = floor($TotalInvAmt);
                    $RoundOffAmt = $TotalInvAmt - $Invamt;
                    $Invamt = number_format($Invamt, 2, '.', '');
                    $RoundOffAmt = number_format($RoundOffAmt, 2, '.', '');
                    //update purchase request master
                    $Update_Purchase_Request = array(
                            'Purchamt'=>$TotalOrderAmt,
                    ...
                    $this->db->update(db_prefix() . 'K1purchase_request_master',$Update_Purchase_Request);
                    $insertedOrders[] = $new_purchase_orderNumbar;
                }
            }
            ...
            {
                $prefixs = 'PO';
                $purchase_orderNumbars = get_option('next_purchase_number_for_kirtione');
                $purch_orderNumbar = $prefixs.$FY."1".$purchase_orderNumbars;
                $KirtiOnePurchOrder = array(
                    'PlantID'=>$PlantID,
                    'FY'=>$FY,
                    'PurchID' =>$purch_orderNumbar,
                    'Transdate' =>date('Y-m-d H:i:s'),
                    'OrderStatus' =>'P',
                    'PartyID'=>"KASPL",
                    'Flag'=>"Y",
                    'Pr_no' =>$insertedOrders[$index],
                    'CenterID' =>$value['CenterID'],
                    'AccountID' =>$value['AccountID'],
                    'Purchamt'=>0.00,
                    'Discamt'=>0.00,
                ...
                $this->db->insert(db_prefix() . 'K1purchasemaster',$KirtiOnePurchOrder);
                if($this->db->affected_rows() > 0)
                {
                $this->increment_next_number('next_purchase_number_for_kirtione');
                    ...
                    foreach($value['Items'] as $item)
                    {
                        ...
                        $NetOrderAmt = $taxableAmount + $cgstAmt + $sgstAmt + $igstAmt;
                        $data_array_results = array(
                            'PlantID'=>$PlantID,
                            'FY'=>$FY,
                            'OrderID' =>$purch_orderNumbar,
                            'BillID' =>$insertedOrders[$index],
                            'TransDate' =>date('Y-m-d H:i:s'),
                            'TransDate2'=>date('Y-m-d H:i:s'),
                            'TType'=>'P',
                            'TType2'=> 'Purchase',
                        ...
                        $this->db->insert(db_prefix() . 'K1history',$data_array_results);
                        $AllCgstAmt += $cgstAmt;
                        $AllSgstAmt += $sgstAmt;
                        $AllIgstAmt += $igstAmt;
                        $TotalPurchAmt += $taxableAmount;
                        $TotalInvAmt += $NetOrderAmt;
                        $j++;
                    }
                    $Invamt = floor($TotalInvAmt);
                    ...
                    $this->db->where('PurchID', $purch_orderNumbar);
                    $this->db->where('FY', $FY);
                    $this->db->where('PlantID', $PlantID);
                    $this->db->update(db_prefix() . 'K1purchasemaster',$Update_PurchaseMaster);
                    //purchase invoice
                    $pre = 'PI';
                    $purchase_Inv_Numbar = get_option('next_purchase_invoice_number_for_kirtione');
                    $new_purchase_Inv_Number = $pre.$FY."1".$purchase_Inv_Numbar;
                    $KirtiOneInvoiceMaster = [
                    ...
                    $this->db->where('PurchID', $purch_orderNumbar);
                    $this->db->update(db_prefix() . 'K1purchasemaster',$KirtiOneInvoiceMaster);
                    if($this->db->affected_rows() > 0)
                    {
                        ...
                        foreach($value['Items'] as $item)
                        {
                            ...
                            $this->db->where('OrderID', $purch_orderNumbar);
                            $this->db->where('Ordinalno', $ordinal);
                            $this->db->where('ItemID', $item['ItemID']);
                            $this->db->where('AccountID', $value['AccountID']);
                            $this->db->update(db_prefix() . 'K1history',$data_array_results);
                            $ordinal++;
                        }
                    }
                }
            ...
            $totalOrders = count($grouped);
            $insertedCount = count($insertedOrders);
            if ($insertedCount === $totalOrders) {
                echo "<script>alert('All purchase orders inserted successfully.');</script>";
            } elseif ($insertedCount > 0) {
                echo "<script>alert('Some purchase orders were inserted successfully.');</script>";
            } else {
                echo "<script>alert('No purchase orders were inserted.');</script>";
            }
        }
        ...
        private function purchase_order_reminders()
        {
