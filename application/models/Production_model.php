...
class Production_model extends App_Model
{
  ...
  public function load_data_for_production($data)
     {  
        ...
        $sql ='SELECT '.db_prefix().'production.*,  
        ...
        FROM '.db_prefix().'production WHERE '.$sql1;
  ...
  public function get_managername($id = '')
    ...
    public function GetPRDItemStock($pro_orid)
    {
        $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
    ...
    public function GetPRDItemStockNew($PrdItem)
    {
        $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
    ...
    public function GetPRDItemOQty($pro_orid)
    {
        $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        if($selected_company == "1"){
  ...
  public function pro_cost_report_details($data){
    ...
    //   print_r($pro_order_id_data);die;
      if($pro_order_id_data ==''){
          return true;
      }
      
		    $this->db->select('tblproduction_details.item_name,tblproduction_details.item_id,SUM(tblproduction_details.production_req_qty) as production_req_qty,SUM(tblproduction_details.return_req_qty) as return_req_qty,SUM(tblproduction_details.ExtraQty) as ExtraQty,'.db_prefix() .'taxes.taxrate');
