...
class Claim_expenses_model extends App_Model
{
   ...
   public function get_data_table(){
        ...
        $selected_company = $this->session->userdata('root_company');
           $this->db->select(db_prefix() . 'claimexpense.*,'.db_prefix() .'staff.*,'.db_prefix() . 'claimexpense.id as cliam_id,'.db_prefix() . 'claimexpense.date as date_claim,'.db_prefix() .'timesheets_timesheet.*,'.db_prefix() .'check_in_out.*,'.db_prefix() .'staff.staffid,'.db_prefix() .'headquarter.name,'.db_prefix() .'staff.firstname,'.db_prefix() .'staff.lastname');
      $this->db->from(db_prefix() . 'claimexpense');
        $this->db->join(db_prefix() .'staff', db_prefix() .'claimexpense.UserID = '.db_prefix() .'staff.AccountID');
      $this->db->join(db_prefix() .'check_in_out', db_prefix() .'staff.staffid = '.db_prefix() .'check_in_out.staff_id AND '.db_prefix() .'claimexpense.date = '.db_prefix() .'check_in_out.date','left');
     ...
     if($month <= 03){
        ...
        $selected_company = $this->session->userdata('root_company');
           $this->db->select(db_prefix() . 'claimexpense.*,'.db_prefix() . 'staff_departments.*,'.db_prefix() .'staff.*,'.db_prefix() . 'claimexpense.id as cliam_id,'.db_prefix() . 'claimexpense.date as date_claim,'.db_prefix() .'timesheets_timesheet.*,'.db_prefix() .'check_in_out.*,'.db_prefix() .'staff.staffid,'.db_prefix() .'headquarter.name,'.db_prefix() .'staff.firstname,'.db_prefix() .'staff.lastname');
		    ...
		    $this->db->group_by(db_prefix() . 'claimexpense.date');
