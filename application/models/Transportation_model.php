...
class Transportation_model extends App_Model
{
    ...
    {
        $this->db->order_by('short_name', 'ASE');
        $query = $this->db->get(db_prefix() . 'countries')->result_array();
        return $query;
    }

    public function getallcity()
    {
        $this->db->order_by('city_name', 'ASE');
        $query = $this->db->get(db_prefix() . 'xx_citylist')->result_array();
        return $query;
    }
    ...
    {
        $this->db->join('tblxx_citylist', 'tblTransportMaster.city = tblxx_citylist.id', 'LEFT');
        $this->db->select('tblxx_citylist.city_name,tblTransportMaster.*');
        $AccountLedger = $this->db->get('tblTransportMaster')->result_array();
        //print_r($AccountLedger);die;
    ...
    {
        $this->db->select(db_prefix() . 'TransportMaster.*');
        $this->db->where(db_prefix() . 'TransportMaster.id', $AccountID);
        $AccountDetails = $this->db->get(db_prefix() . 'TransportMaster')->row();
        if ($AccountDetails) {
            $stateID = $AccountDetails->state;
            $this->db->select('tblxx_citylist.*');
            $this->db->where(db_prefix() . 'xx_citylist.state_id', $stateID);
            $CityList = $this->db->get(db_prefix() . 'xx_citylist')->result_array();
            $AccountDetails->CityList = $CityList;
            $TransportID = $AccountDetails->TransportID;
            $this->db->select('tblTransportServices.StateID');
            $this->db->where(db_prefix() . 'TransportServices.TransportID', $TransportID);
            $state_list = $this->db->get(db_prefix() . 'TransportServices')->result_array();
            $service_state = array();
            foreach ($state_list as $vl) {
                array_push($service_state, $vl["StateID"]);
            }
            $AccountDetails->state_list = $service_state;
        }
    ...
    {
        $TransportID = strtoupper($data['TransportID']);
        unset($data['TransportID']);

        $stateList = isset($data['state_list']) ? $data['state_list'] : [];
        unset($data['state_list']);

        $this->db->where('TransportID', $TransportID);
        $this->db->delete(db_prefix() . 'TransportServices');

        $this->db->where('TransportID', $TransportID);
        $this->db->update(db_prefix() . 'TransportMaster', $data);

        if (!empty($stateList)) {
            foreach (explode(',', $stateList) as $val) {
                $val = trim($val);
                $service_array = array(
                    "UserID" => $data['UserID'],
                    "TransDate" => date('Y-m-d H:i:s'),
                    "TransportID" => strtoupper($TransportID),
                    "StateID" => $val
                );

                if (!$this->db->insert(db_prefix() . 'TransportServices', $service_array)) {
                    return false;
                }
            }
        }
    ...
    public function SaveAccountID($data)
    {
        $Client_data = array(
            ...
            'gst_certification_image' => $data['gst_certification_image'],
            'aadhaar_image' => $data['aadhaar_image'],
            'shop_act_image' => $data['shop_act_image'],
            'transport_permit' => $data['transport_permit'],
            'cancel_cheque' => $data['cancel_cheque'],
            'ownership_photo' => $data['ownership_photo'],
            'address_proof' => $data['address_proof'],
        );
        if ($this->db->insert(db_prefix() . 'TransportMaster', $Client_data)) {
            $insert_id = $this->db->insert_id();

            // Check if state_list is not empty and is an array
            if (!empty($data['state_list'])) {
                $stateList = explode(',', $data["state_list"]);
                foreach ($stateList as $val) {
                    $val = trim($val);
                    $service_array = array(
                        "UserID" => $data['UserID'], // Adding UserID
                        "TransDate" => date('Y-m-d H:i:s'), // Adding TransDate
                        "TransportID" => strtoupper($data['TransportID']),
                        "StateID" => $val
                    );

                    // Insert each state into TransportServices table
                    if (!$this->db->insert(db_prefix() . 'TransportServices', $service_array)) {
                        return false;
                    }
                }
