...
class Staff_model extends App_Model
{
    ...
    public function update_profile($data, $id)
    {
        ...
        if (empty($data['password'])) {
            ...
            $data['password_erp']             = app_hash_password($data['password']);
            $data['last_password_change'] = date('Y-m-d H:i:s');
        }
    ...
    public function change_password($data, $userid)
    {
        ...
        if (!app_hasher()->CheckPassword($data['oldpassword'], $member->password_erp)) {
            return [
                [
        ...
        $this->db->update(db_prefix() . 'staff', [
            'password_erp'             => $data['newpasswordr'],
    ...
    public function change_item_priority($data)
    {
        $UserID = $this->session->userdata('username');
        $this->db->where('staff_id', $UserID);
        $this->db->delete(db_prefix() . 'Itemwise_staff_priority');
        $insert = 0;
        foreach($data as $Key=>$val){
            ...
            if($this->db->insert(db_prefix() . 'Itemwise_staff_priority', $insert)){
                $insert++;
            }
        }
        if($insert > 0){
    ...
    public function change_center_priority($data)
    {
        $UserID = $this->session->userdata('username');
        $this->db->where('staff_id', $UserID);
        $this->db->delete(db_prefix() . 'Centerwise_staff_priority');
        $insert = 0;
        foreach($data as $Key=>$val){
            $insert = array(
                "staff_id"=>$UserID,
            ...
            if($this->db->insert(db_prefix() . 'Centerwise_staff_priority', $insert)){
                $insert++;
            }
        }
        if($insert > 0){
