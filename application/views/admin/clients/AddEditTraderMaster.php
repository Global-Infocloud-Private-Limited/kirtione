					...
					<div class="panel-body">
					    <nav aria-label="breadcrumb" >
    						<ol class="breadcrumb custombreadcrumb" style="background-color:#fff !important; margin-Bottom:0px !important;">
    							...
    							<li class="breadcrumb-item active" aria-current="page"><b>Trader Master</b></li>
							</ol>
						</nav>
    					<hr class="hr_style">
						...
						<div class="row">
							...
							<!--<div class="col-md-3">
								...
								<select name="AccountType" id="AccountType" class="selectpicker form-control" data-width="100%" data-none-selected-text="Non Selected" data-live-search="true">
							...
							<div class="col-md-3">
								<div class="form-group" app-field-wrapper="Pan"> 
									...
									<!--<span class="input-group-btn">
										<button id="verifyPann" class="btn btn-primary" type="button">
										Verify
										</button>
									</span>-->
							...
							<div class="col-md-3">
								<div class="form-group">
									...
									<select class="selectpicker form-control" id="subdist" name="subdist" data-none-selected-text="Non Selected" data-live-search="true">
									</select>
								</div>
							</div>
							...
							<div class="col-md-2">
								...
								<select name="isactive" id="isactive" class="selectpicker form-control tcs_type" data-none-selected-text="None selected" data-live-search = "true" tabindex="-98">
							...
							<div class="col-md-2">
								<label for="isapprove">Is Approve</label>
								<select name="isapprove" id="isapprove" class="selectpicker form-control" data-none-selected-text="None selected" data-live-search = "true" tabindex="-98">
							...
							<div class="col-md-2" id="div_tds_section">
							...
							<div class="row">
								<div class="col-md-12">
									<?php if (has_permission_new('TraderMaster', '', 'create')) {
									?>
									...
									<?php if (has_permission_new('TraderMaster', '', 'edit')) {
									?>
								...
								<div class="modal-dialog modal-lg" role="document">
									<div class="modal-content">
										...
										<div class="modal-body" style="padding:0px 5px !important">
											<div class="col-md-5">
												<?php if (has_permission_new('TraderMaster', '', 'export')) {
												?>
												...
												<?php if (has_permission_new('TraderMaster', '', 'print')) {
												?>
	...
	<script>
		$(document).ready(function(){
						...
						if(data == null){
							...
							if(PlantID ==data.PlantID ){
								if(data.CustomerType == "3"){
									...
									$('select[name=isapprove]').val(data.is_approve);
									$('.selectpicker').selectpicker('refresh');
									
								    $('select[name=isactive]').val(data.active);
									$('.selectpicker').selectpicker('refresh');
									...
									alert('This Account is use for as Broker account');
									$('#AccountID').val('');
									$('#phonenumber').val('');
								}
								}else{
			...
			$("#AccountID").dblclick(function(){
				...
				var AccountID = "";
				$.ajax({
					url:"<?php echo admin_url(); ?>clients/AccountListPopUp",
					//dataType:"JSON",
					method:"POST",
					cache: false,
					data:{AccountID:AccountID,},
		...
		if(AccountID == ''){
			...
			$.ajax({
				...
				data:{AccountID:AccountID,AccoountName:AccoountName,firstname:firstname,lastname:lastname,phonenumber:phonenumber,IsKirtiOneAccess:IsKirtiOneAccess,
					...
					zip:zip,Pan:Pan,istds:istds,tds_section:tds_section,tds_rate:tds_rate,TcsStartDate1:TcsStartDate1,active:active,StartDate:StartDate,
					groups_in:groups_in,location_type:location_type,route:route,bill_till_bal:bill_till_bal,ActSalestype:ActSalestype,CustomerType:CustomerType,isapprove:isapprove,isactive:isactive
				},
				beforeSend: function () {
...
//======================= Update Exiting Trader ================================
	$('.updateBtn').on('click',function(){ 
		...
		isapprove = $('#isapprove').val();
		isactive = $('#isactive').val();
		isprimaryGst = $('input[name=IsPrimary]:checked').val();
		...
		if(AccountID == ''){
			...
			alert('Enter valid Mobile number');
			$('#phonenumber').focus();
		}else if(!$('#Pan').val().match('[a-zA-Z]{5}[0-9]{4}[a-zA-Z]{1}') && $('#Pan').val() !== "" && isValidPan == 'N'){
			alert('Enter valid PAN number');
			$('#Pan').focus();
		}else {
			$.ajax({
				...
				street:street,loc:loc,vtc:vtc,po:po,altphonenumber:altphonenumber,email:email,state:state,city:city,taluka:taluka,house:house,IsKirtiOneAccess:IsKirtiOneAccess,
					zip:zip,Pan:Pan,istds:istds,tds_section:tds_section,tds_rate:tds_rate,TcsStartDate1:TcsStartDate1,active:active,StartDate:StartDate,isapprove:isapprove,isactive:isactive, isprimaryGst:isprimaryGst
				},
				beforeSend: function () {
