...
<script>
	...
	$(document).ready(function() {
		$('#Account_district').change(function() {
			$("#Account_taluka").val('').selectpicker('refresh');
		...
		function villagereportcharts_filterwise(id, value, title_c){
			...
			$.ajax({
				url:"<?php echo admin_url(); ?>VillageMaster/village_wise_chart",
				dataType:"JSON",
				method:"POST",
				data:{from_date:from_date,to_date:to_date,District:District,Taluka:Taluka,ReportFor:ReportFor,Staff_Id:Staff_Id,ChartType:ChartType,GroupBy:GroupBy},
				success:function(response){
				console.log(response.ChartData);
						...
						tooltip: {
							pointFormat: 'Staff Wise Village Count : <b>{point.y:.1f} % </b>'
						},
						series: [{
							// colors: [ '#691af3',
