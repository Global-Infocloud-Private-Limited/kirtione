	...
	*/
			...
			{			 
				...
				if(prop == 'id')
				{
					...
					if(newValue == null)
					{
						...
						hot.setDataAtCell(row,7, '0'); // qty
						hot.setDataAtCell(row,8, ''); // rate
						hot.setDataAtCell(row,9, '0'); // discount
						hot.setDataAtCell(row,10, '0'); // gst
						hot.setDataAtCell(row,11, '0'); // cgst
						hot.setDataAtCell(row,12, '0'); // sgst
						hot.setDataAtCell(row,13, '0'); // igst
					...
					{         
						...
						hot.setDataAtCell(row, 11, parseFloat(cgstamt));  
						hot.setDataAtCell(row, 12, parseFloat(sgstamt));  
						hot.setDataAtCell(row, 13, parseFloat(igstamt));
						hot.setDataAtCell(row, 14, parseFloat(netAmount));
					} 								
				}	
	...
	$(document).off('submit', '#pur_order-form').on('submit', '#pur_order-form', function(e) {
		...
		for (let i = 0; i < data.length; i++) {
			...
			let batchQty = data[i][7]; // BatchNo column
