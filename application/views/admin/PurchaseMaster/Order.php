											...
											<div class="col-md-2">
												<div class="form-group" app-field-wrapper="AccountID">
													...
													<select name="ordstat" id="ordstat" class="selectpicker form-control" data-none-selected-text="None selected" data-live-search="true" disabled>
														...
														<option value="I"<?php echo ($value == "I") ? 'selected' : ''; ?>>In Progress</option>
...
<script type="text/javascript" language="javascript" >
	...
	$(document).ready(function(){
		...
		{
			$.ajax({
				url:"<?php echo admin_url(); ?>PurchaseMaster/filter_data_for_purchase_order",
				method:"POST",
				dataType:"JSON",
				...
				success: function(response) {
						...
						if (response.status == 'success') {
								...
								$.each(response.data, function(index, val) {
										...
										var url = admin_url + 'PurchaseMaster/Order/' + val.PurchID;
										...
										tbody.append(`
												<tr onclick="window.location.href='${url}'" style="cursor:pointer;">
														...
														<td style="text-align:right;">${parseFloat(val.Purchamt).toFixed(2)}</td>
														<td style="text-align:right;">${parseFloat(val.Discamt).toFixed(2)}</td>
														<td style="text-align:right;">${parseFloat(val.cgstamt).toFixed(2)}</td>
														<td style="text-align:right;">${parseFloat(val.sgstamt).toFixed(2)}</td>
														<td style="text-align:right;">${parseFloat(val.igstamt).toFixed(2)}</td>
														<td style="text-align:right;">${parseFloat(val.Invamt).toFixed(2)}</td>
												</tr>
										`);
								...
								tbody.append(`
										<tr>
												...
												<td style="text-align:right;"><b>${TotalPurchAmt.toLocaleString('en-IN',{minimumFractionDigits:2})}</b></td>
												<td style="text-align:right;"><b>${TotalDiscAmt.toLocaleString('en-IN',{minimumFractionDigits:2})}</b></td>
												<td style="text-align:right;"><b>${TotalCgstAmt.toLocaleString('en-IN',{minimumFractionDigits:2})}</b></td>
												<td style="text-align:right;"><b>${TotalSgstAmt.toLocaleString('en-IN',{minimumFractionDigits:2})}</b></td>
												<td style="text-align:right;"><b>${TotalIgstAmt.toLocaleString('en-IN',{minimumFractionDigits:2})}</b></td>
												<td style="text-align:right;"><b>${TotalInvAmt.toLocaleString('en-IN',{minimumFractionDigits:2})}</b></td>
										</tr>
								`);
		...
		$('.Approve-new-order').on('click', function()
		{
			...
			{
				$.ajax({
					url: "<?php echo admin_url(); ?>PurchaseMaster/ApprovePO",
					type: 'POST',
					data: {poId:poId},
					dataType: 'json',
					success: function(response){
...
<script>
    $(document).ready(function()
	{
        $('#resetbutton').on('click', function(e) {
            e.preventDefault();
			window.location.href = '<?php echo admin_url(); ?>PurchaseMaster/Order';
		});
	});
</script>
<script type="text/javascript" language="javascript" >
