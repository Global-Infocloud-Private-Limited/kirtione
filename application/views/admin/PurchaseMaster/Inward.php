			...
			<div class="row" style="display:none;">
				<div class="col-md-12">
					<table id="print_table">
						...
						<tbody>
							<tr>
								<td colspan="12"><center><b>Purchase Invoice</b></center></td>
							</tr>
							<tr>
								...
								<td colspan="4"> <b>Inv No :</b> <?php echo $purchase_details->Inv_No ; ?></td>
							...
							<?php
								...
								if(isset($items)){
									...
									<tr style=" white-space: nowrap;">
										...
										<?php $GST = ($value["cgstamt"]+$value["sgstamt"]+$value["igstamt"]);?>
										<td align="right"><?php echo number_format($GST, 2, '.', ''); ?></td>
							...
							<div class="panel-body">
								<nav aria-label="breadcrumb" >
									<ol class="breadcrumb custombreadcrumb" style="background-color:#fff !important; margin-Bottom:0px !important;">
										...
										<li class="breadcrumb-item active" aria-current="page"><b>Purchase Inward</b></li>
									</ol>
								</nav>
								<hr class="hr_style">
										...
										<div class="row">
											<div class="col-md-2">
												<?php
													...
													if(isset($purchase_details)){
														$PONumber = substr($purchase_details->Inv_No,4);
														$isedit = "Y";
													}else{
														$new_purchase_Inv_Number = get_option('next_purchase_invoice_number_for_kirtione');
												...
												<div class="form-group">
													<label for="pro_orderid">PI.No.</label>
											...
											<div class="col-md-2">
												...
												<?php
													...
													echo render_date_input('prd_date','PI Date',$value,$attr);
												?>
											</div>
											<div class="col-md-2">
											...
											<div class="col-md-2">
												...
												<div class="form-group" app-field-wrapper="PurchID">
													<label for="PurchID">Purch Order No</label>
													<select name="PurchID" id="PurchID" onchange="GetPODetails(this.value)"  class="selectpicker" data-live-search="true" data-width="100%" data-none-selected-text="Non Selected" <?php echo $Isdisabled; ?>>
														<option value=""></option>
														<?php if ($Isdisabled){ ?>
											...
											<!-- <div class="col-md-2">
												<div class="form-group" app-field-wrapper="InvoiceNo">
											...
											<div class="col-md-2">
												<div class="form-group" app-field-wrapper="DeliveryChallanNo">
													...
													<input type="text" name="DeliveryChallanNo" id="DeliveryChallanNo" class="form-control" value = "<?php echo $purchase_details->DeliveryChallanNo ?? ''; ?>" data-isedit="<?php echo $isedit; ?>" data-original-number="<?php echo $purchase_details->DeliveryChallanNo ?? ''; ?>">
												</div>
											</div>
											<div class="col-md-2">
												<?php
											...
											<div class="col-md-2" style="display: none;">
											    <?php $value = (isset($purchase_details) ? $purchase_details->Othamt : '0'); ?>
											...
											<div class="col-md-2" style="display: none;">
												<?php $value = (isset($purchase_details) ? $purchase_details->OthAmtEffectOn : ''); ?>
												<div class="form-group">
											...
											<div class="col-md-8" style="margin-top: 25px;color: red;">
										        <?php
										        ...
										        if(isset($purchase_details) && $purchase_details->IsSale > 0 && $value == 'N' && !empty($purchase_details->Inv_No)){
										            ?>
										            <span><b>Note : </b> Some or all quantities from this purchase batch have already been sold, so we cannot update this purchase inward entry.</span>
										            <?php
										        }elseif(isset($purchase_details) && $value == 'Y' && !empty($purchase_details->Inv_No)){
										            ?>
										                <span><b>Note : </b> A purchase invoice has already been generated against this inward entry, so it cannot be updated.</span>
										            <?php
										        }
						...
						<div class="col-md-12 mtop15">
							<div class="panel-body bottom-transaction">
								...
								<div class="btn-bottom-toolbar text-right" style="width: 100%; display: flex; justify-content: flex-end; align-items: center;">
									...
									?>
									<div class="col-md-1" style="margin-left: 10px;">
										<button type="button"  class="btn btn-default hidden" id="qrcodebtn" onclick="generateQR();"><i class="fa fa-qrcode"></i> Generate QR</button>
									</div>
									<div class="col-md-1" style="margin-left: 10px;">
									...
									<?php if (has_permission_new('PurchaseInvoice', '', 'edit')){
										$value = (isset($purchase_details) ? $purchase_details->Is_Ledger : '');
										if(isset($purchase_details) && $value == 'N' && !empty($purchase_details->Inv_No) && $purchase_details->IsSale <= 0){
										?>
...
<script type="text/javascript">
	function generateQR(){
		$.ajax({
			url: '<?php echo admin_url('PurchaseMaster/generateQR'); ?>',
			method: 'POST',
			dataType: 'json',
			data : {
			...
			success: function (data) {
				if(data.success){
						...
						window.location.href = '<?php echo admin_url("PurchaseMaster/downloadQRPDF/".$purchase_details->Inv_No); ?>';
				} else {
					...
					*/
					if (purchaseOrderItemData.length > 0){
						...
						*/
						...
						|--------------------------------------------------------------------------
						*/
						// purchaseOrderItemData.forEach(function(orderItem, rowIndex)
						// {
						// 	applyBatchListToRow(
						// 		rowIndex,
						// 		orderItem.BatchList,
...
<?php require 'kirtione_pur_inward_js.php';?>