<script>
    ...
    var hotSettings = {
		...
		'<?php echo _l('Purch Unit'); ?>',
        '<?php echo _l('PI Qty(loose)'); ?>',
        '<?php echo _l('Avl Stock'); ?>',
        '<?php echo _l('Rtn Qty(loose)'); ?>',
        '<?php echo _l('Rate'); ?>',
			...
			{			 
				...
				{	
					if(prop == 'ReturnOrderQty'){
						...
						let AvlStockQty = hot.getDataAtCell(row, 8); 
						
						if (AvlStockQty == 0 && hot.getDataAtCell(row, 9) > 0) {
                            hot.setDataAtCell(row, 9, 0); 
                            alert('Available stock is 0. Return quantity should be 0.');
                            return;
                        }
						
						if(hot.getDataAtCell(row, 9) > PIQty){
							hot.setDataAtCell(row, 9, oldValue);
							alert('Qty Should Be Less Then Or Equals To PI Qty');
							return;
						}else if(hot.getDataAtCell(row, 9) > AvlStockQty){
						    hot.setDataAtCell(row, 9, oldValue);
						    alert('Qty Should Be Less Then Or Equals To Avl Stock');
							return;
						}
					}
	...
	function calaulateSubTotal()
	{
		...
		for (var i = 0; i < hot.countRows(); i++) {
			var qty = hot.getDataAtCell(i, 9); 
			var amountval = hot.getDataAtCell(i, 10);             
	...
	function calculateTotalDiscount()
	{
		...
		for (var i = 0; i < hot.countRows(); i++) {
			var qty = hot.getDataAtCell(i, 9); 
			var amountval = hot.getDataAtCell(i, 10);
			var disc = hot.getDataAtCell(i, 11);
	...
	function calculateTotalValue()
	{
		...
		for (var i = 0; i < hot.countRows(); i++) {
			var qty = hot.getDataAtCell(i, 9);  
			var amountval = hot.getDataAtCell(i, 10);   
			var discount = hot.getDataAtCell(i, 11) * qty;   
	...
	{	
		var totalCgstAmt = 0;
		for (var i = 0; i < hot.countRows(); i++) {
			var cgstamt = hot.getDataAtCell(i, 13); 
	...
	function calculateTotalSgstAmt()
	{
		var totalSgstAmt = 0;
		for (var i = 0; i < hot.countRows(); i++) {
			var sgstamt = hot.getDataAtCell(i, 14);
	...
	function calculateTotalIgstAmt()
	{
		var totalIgstAmt = 0;
		for (var i = 0; i < hot.countRows(); i++) {
			var Igst = hot.getDataAtCell(i, 15);
			if (Igst && !isNaN(Igst)) {
	...
	function calculateTotalNetAmount()
	{
		...
		{
			var qty = hot.getDataAtCell(i, 9);  
			var rateval = hot.getDataAtCell(i, 10); 
			var discount = hot.getDataAtCell(i, 11); 
			var gst = hot.getDataAtCell(i, 12);    
	...
	$(document).off('submit', '#pur_order-form').on('submit', '#pur_order-form', function(e) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            var returnQty = data[i][9];
            var batchNo = data[i][17];
            var expDate = data[i][18];
    
            if(returnQty === null || returnQty === '') {
                alert(`Return Qty cannot be empty in row ${i+1}.`);
                isValid = false;
                break;
            }
