    ...
    function loadHandsontable(dynamicCols = [], dynamicHeaders = [])
    {
        ...
        const staticColsBefore = [
          ...
          const staticColsAfter = [
            ...
            { data: 'NetRate', type: 'numeric', numericFormat: { pattern: '0,0' }, readOnly: true, width: 60 },
            { data: 'tenweight', type: 'numeric', numericFormat: { pattern: '0,00' }, readOnly: true, width: 60 },
        ...
        if (QcDetails && Array.isArray(QcDetails) && QcDetails.length > 0) {
            ...
            for (let rowIndex = 0; rowIndex < rowCount; rowIndex++) {
                const rowId = hot.getDataAtRowProp(rowIndex, 'id'); // get row's id
        
                if (!rowId) continue; // skip if no id present
        
                // Filter QC data matching this row id (assuming AccountID matches row id)
                ...
                filteredQc.forEach(qc => {
                    ...
                    // Set QC value if column exists
                    if (window.dynamicPropKeys.includes(param)) {
                        hot.setDataAtRowProp(rowIndex, param, val);
                    }
        
                    // Set QC amount in the appropriate _Amt or ' Amt' column if exists
        ...
        hot.addHook('afterChange', function(changes, src) {
        	...
        	if(changes !== null){
        	    changes.forEach(([row, prop, oldValue, newValue]) => {
                    ...
                    if (['Rate', 'NetWeight','Bag'].includes(prop)) {
                        const rate = parseFloat(hot.getDataAtRowProp(row, 'Rate')) || 0;
                        const weight = parseFloat(hot.getDataAtRowProp(row, 'NetWeight')) || 0;
