<script>
	...
	$(function(){
		...
		function validate_purorder_form(selector) {
			...
			appValidateForm($(selector), {
				...
				TranportName: 'required',
				/*ShippingHouse: {
					required: function(element) {
						return $('#ShippingID').val() === 'new';
					}
				},*/
				/*ShippingStreet: {
					required: function(element) {
    ...
    var hotSettings = {
		...
		maxRows: 40,
		rowHeaders: true,
		colWidths: [100,80, 60, 60, 70,70,50,50,50,70,50, 60, 50,50,50,50,50,70],
		colHeaders: [
	...
	hot.addHook('afterChange', function(changes, src) {
		if(changes !== null){
			...
			{			 
				...
				if(prop == 'DOQty' || prop == 'PurchRate')
				{				
					let qtyValue = hot.getDataAtCell(row, 9);  
					let rate = hot.getDataAtCell(row, 11);     
					let discount = hot.getDataAtCell(row, 12); 				
					...
					let gst = hot.getDataAtCell(row, 13);
					
					let statsid = $("#CenterState").val();
					let vendorstate = $('#state').val();
					
					let DOQty = hot.getDataAtCell(row, 10); 
					let StockQty = hot.getDataAtCell(row, 8);
					let OrderQty = hot.getDataAtCell(row, 9);
					...
					if(parseFloat(DOQty) > parseFloat(OrderQty)){
						hot.setDataAtCell(row, 10, OrderQty); 
					...
					{         
						...
						if (unit !== saleunit && packingqty) {
							let newRate = (rate / packingqty) * DOQty;
							rate= newRate  
							amount =rate;                                   
						} else {
							amount = rate * DOQty;      
						...
						hot.setDataAtCell(row, 14, parseFloat(cgstamt));  
						hot.setDataAtCell(row, 15, parseFloat(sgstamt));  
						hot.setDataAtCell(row, 16, parseFloat(igstamt));
						hot.setDataAtCell(row, 17, parseFloat(netAmount));
					} 								
				}	
	...
	function customRenderers(instance, td, row, col, prop, value, cellProperties) {
		"use strict";
		Handsontable.renderers.TextRenderer.apply(this, arguments);
	...
	function calaulateSubTotal()
	{
		...
		for (var i = 0; i < hot.countRows(); i++) {
			var qty = hot.getDataAtCell(i, 9); 
			var amountval = hot.getDataAtCell(i, 11);             
	...
	function calculateTotalDiscount()
	{
		...
		for (var i = 0; i < hot.countRows(); i++) {
			var qty = hot.getDataAtCell(i, 9); 
			var amountval = hot.getDataAtCell(i, 11);
			var disc = hot.getDataAtCell(i, 12);
	...
	function calculateTotalValue()
	{
		...
		for (var i = 0; i < hot.countRows(); i++) {
			var qty = hot.getDataAtCell(i, 9);  
			var amountval = hot.getDataAtCell(i, 11);   
			var discount = hot.getDataAtCell(i, 12) * qty;   
	...
	{	
		var totalCgstAmt = 0;
		for (var i = 0; i < hot.countRows(); i++) {
			var cgstamt = hot.getDataAtCell(i, 14); 
	...
	function calculateTotalSgstAmt()
	{
		var totalSgstAmt = 0;
		for (var i = 0; i < hot.countRows(); i++) {
			var sgstamt = hot.getDataAtCell(i, 15);
	...
	function calculateTotalIgstAmt()
	{
		var totalIgstAmt = 0;
		for (var i = 0; i < hot.countRows(); i++) {
			var Igst = hot.getDataAtCell(i, 16);
			if (Igst && !isNaN(Igst)) {
	...
	function calculateTotalNetAmount()
	{
		...
		{
			var qty = hot.getDataAtCell(i, 9);  
			var rateval = hot.getDataAtCell(i, 11); 
			var discount = hot.getDataAtCell(i, 12); 
			var gst = hot.getDataAtCell(i, 13);    
