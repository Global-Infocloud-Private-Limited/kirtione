		    ...
		    <div class="_buttons">
		        <div class="col-md-12 text-centerr"  >
					<nav aria-label="breadcrumb" >
						<ol class="breadcrumb custombreadcrumb" style="background-color:#fff !important; margin-Bottom:0px !important;">
							...
							<li class="breadcrumb-item active" aria-current="page"><b>Advance Payment List</b></li>
                ...
                <div class="col-md-3" >
                    <div class="form-group" app-field-wrapper="PartyID">
                        ...
                        <select name="PartyID" id="PartyID" class="selectpicker form-control" data-live-search="true">
                            <option value="">All</option>
                            <?php
...
$(document).ready(function(){
    ...
    $('#search_data').on('click',function(){
	    ...
	    var PartyID = $("#PartyID :selected").val();
	   
	    $.ajax({
            url:"<?php echo admin_url(); ?>GateControl/GetAdvancePaymentData",
            method:"POST",
            data:{from_date:from_date, to_date:to_date,CenterID:CenterID,PartyID:PartyID},
            beforeSend: function () {
...
<script>
    ...
    function ReSendTradeToPcSoft(ID)
    {
        $.ajax({
            url: "<?php echo admin_url('GateControl/SendAdvancePaymentData'); ?>",
            type: "POST",
            dataType: "json",
            data: { ID: ID },
            success: function(response) {
                if (response == true) {
                    ...
                    alert("Error sending data to Pc Soft");
                }
            },
...
<script type="text/javascript">
    function printPage()
    {
        var from_date = $("#from_date").val();
	    var to_date = $("#to_date").val();
	    var CenterID = $("#Center :selected").text(); 
        var PartyID = $("#PartyID :selected").text(); 
        ...
        heading_data += '<tr>';
        heading_data += '<td style="text-align:center;"colspan="3">Advance Payment List</td>';
        heading_data += '</tr>';
        heading_data += '<tr><td colspan="3"><strong>Filters:</strong> From: ' + from_date + ' , To: ' + to_date +
        ' , Center Name: ' + CenterID + ' , Party Name: ' + PartyID + '</td></tr>';
...
<script>
    $("#caexcel").click(function(){
	    ...
	    var partytext = $("#PartyID option:selected").text();
	    
        $.ajax({
            url:"<?php echo admin_url(); ?>GateControl/export_AdvancePaymentList",
            method:"POST",
            data:{from_date:from_date, to_date:to_date,CenterID:CenterID,PartyID:PartyID,centertext:centertext,partytext:partytext
            },
            beforeSend: function () {
