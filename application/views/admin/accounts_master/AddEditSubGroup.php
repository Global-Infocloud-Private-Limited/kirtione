					...
					<div class="panel-body">
						...
						<nav aria-label="breadcrumb" >
    						<ol class="breadcrumb custombreadcrumb" style="background-color:#fff !important; margin-Bottom:0px !important;">
    							...
    							<li class="breadcrumb-item active" aria-current="page"><b>Account SubGroups</b></li>
    						</ol>
    					</nav>
    					<hr class="hr_style">
						<div class="row">
							...
							<input type="hidden" name="OldAccountSubGroupID3" id="OldAccountSubGroupID3" value="<?php echo str_pad($NextAccountGroupID3->SubActGroupID+1, 7, '0', STR_PAD_RIGHT);?>">
							<div class="col-md-4">
								<div class="form-group">
									<label for="subgroup_code">AccountSubGroupId2</label>
									<input type="text" name="subgroup_code" id="subgroup_code" class="form-control" value="<?php echo str_pad($NextAccountGroupID3->SubActGroupID+1, 7, '0', STR_PAD_RIGHT);?>" onkeypress="return isNumber(event)">
									
								</div>
							</div>
							<div class="col-md-6">
								<div class="form-group">
									<label for="subgroup_name">AccountSubGroupId2 Name</label>
							...
							<div class="modal-dialog modal-lg" role="document">
								<div class="modal-content">
									<div class="modal-header" style="padding:5px 10px;">
										...
										<h4 class="modal-title">Account SubGroup 2 </h4>
									</div>
										...
										<div class="table-AccountSubgroup tableFixHead2">
											<table class="tree table table-striped table-bordered table-AccountSubgroup tableFixHead2" id="table_AccountSubgroup" width="100%">
												<thead>
													...
													<tr>
														<th id="sl">SubAccountGroupID2</th>
														<th>SubAccountGroupName2</th>
														<th>SubAccountGroupID1</th>
														<th>SubAccountGroupName1</th>
													</tr>
												</thead>
												<tbody>
													<?php
...
<script type="text/javascript" language="javascript" >
	$(document).ready(function(){
		...
		$('#subgroup_code').on('focus',function(){
			...
			$.ajax({
				...
				success:function(data){
					...
					$('#subgroup_code').val(data.NextAccountSubGroupID);
					$('#subgroup_name').val('');
					$("#AccountSubGroupID1").children().remove();
    			    var len = data.AccountSubGroupID1.length;
    			    for (var i = 0; i < len; i++) {
    					$("#AccountSubGroupID1").append('<option value="'+data.AccountSubGroupID1[i]["SubActGroupID1"]+'">'+data.AccountSubGroupID1[i]["SubActGroupName"]+'</option>');
    				}
    			    $('.selectpicker').selectpicker('refresh');
					$('#form_mode').val('add');
					$('.saveBtn').show();
            ...
            $.ajax({
				...
				success:function(data){
                    if(empty(data)){
                        ...
                        $('#subgroup_name').val(data.SubActGroupName);
                        $('select[name=main_group]').val(data.ActGroupID);
						GetChangedData(data.SubActGroupID)
                        $('.selectpicker').selectpicker('refresh');
                        $('#form_mode').val('edit');
                        $('.saveBtn').hide();
                        $('.updateBtn').show();
        ...
        $('.saveBtn').on('click',function(){ 
            ...
            SubGroupID1 = $('#AccountSubGroupID1').val();
            
            $.ajax({
                url:"<?php echo admin_url(); ?>accounts_master/SaveSubGroup",
                dataType:"JSON",
                method:"POST",
                data:{SubGroupID:SubGroupID,SubGroupName:SubGroupName,SubGroupID1:SubGroupID1
				},
                beforeSend: function () {
        ...
        $('.updateBtn').on('click',function(){ 
            ...
            $.ajax({
                ...
                dataType:"JSON",
                method:"POST",
                data:{SubGroupID:SubGroupID,SubGroupName:SubGroupName,AccountSubGroupID1:AccountSubGroupID1
				},
                beforeSend: function () {
