                            ...
                            <div class="row"> 
                                <nav aria-label="breadcrumb" >
                                    <ol class="breadcrumb custombreadcrumb" style="background-color:#fff !important; margin-Bottom:0px !important;">
                                        ...
                                        <li class="breadcrumb-item active" aria-current="page"><b>Crop Master</b></li>
                                    </ol>
                                </nav>
                                <hr class="hr_style">
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">                                      
                                        <label for="CropID">CropID</label>
                                        <input type="text" name="CropID" id="CropID" class="form-control" value="" readonly>   
                                        <input type="hidden" name="ORGCropID" id="ORGCropID"  value="">   
                                    </div>
                                </div>         
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="CropName" id="CropName" class="form-control" value="">
                                    </div>
                                </div>	
                            </div>	                             
                         ...
                         <div class="modal fade Item_List" id="Item_List" tabindex="-1" role="dialog" data-keyboard="false" data-backdrop="static">
                                <div class="modal-dialog modal-lg" role="document">
                                    ...
                                    <div class="modal-header" style="padding:5px 10px;">
                                        ...
                                        <h4 class="modal-title">Crops List</h4>
                                    </div>
                                    <div class="modal-body" style="padding:0px 5px !important">                                            
                                        <div class="table_Crop_List tableFixHead2">
                                            <table class="tree table table-striped table-bordered table_Crop_List tableFixHead2" id="table_Crop_List" width="100%">
                                                <thead>
                                                    ...
                                                    <tr>
                                                        <th id="sl" style="text-align:left; width: 25%;">CropId</th>                                                        
                                                        <th style="text-align:left; width: 75%;">Crop Name</th>                                                       
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <?php
...
<script>
    ...
    {
        $.ajax({
            url:  "<?php echo admin_url(); ?>CropMaster/Crop_table_data",
            type: "GET", 
            dataType: "json", 
            success: function(data) {             
               
                var tableBody = $("#table_Crop_List tbody"); 
                tableBody.empty();                 

                $.each(data, function(index, value) {
                    var newRow = $("<tr class='get_ItemID' data-id='" + value.id + "'>");
                    newRow.append("<td>" + value.id + "</td>");
                    newRow.append("<td>" + value.CropName + "</td>");                    
...
{    
    var maxCropId = <?php echo isset($maxCropId) ? $maxCropId : 'null'; ?>;
    if (maxCropId == null || maxCropId == undefined || maxCropId == '') 
    {
        $('#CropID').val(1);
        $('#ORGCropID').val(1);
    }else
    {
        $('#CropID').val(maxCropId + 1);  
        $('#ORGCropID').val(maxCropId + 1);  
    }    
    ...
    $('.saveBtn').on('click',function() 
    {
        ...
        {
            $.ajax({
                url: "<?php echo admin_url(); ?>CropMaster/insertCropDetails", 
                type: 'POST', 
                ...
                success: function(response) {
                    if (response.success) {                   
                        ...
                        var maxCropId = response.newMaxCropId;  
                        $('#ORGCropID').val(maxCropId + 1); 
                        ResetForm();                            
                    } else {                    
...
{
    CropId = $(this).attr("data-id");
    $.ajax({
			url:"<?php echo admin_url(); ?>CropMaster/GetCropDetailsbyID",
			dataType:"JSON",
			method:"POST",
			data:{CropId:CropId},
			beforeSend: function () {
    ...
    {        
        ...
        $.ajax({
                url: "<?php echo admin_url(); ?>CropMaster/UpdateCropDetails", 
                type: 'POST', 
                data: {cropid:cropid,cropname:cropname}, 
                dataType: 'json',
                success: function(response) {
                    if (response.success) {                   
                        alert_float('success', 'Record Updated Successfully...');
                        var crops = response.crops;
                        var lastCrop = crops[crops.length - 1];  
                        var lastCropId = parseInt(lastCrop.id);
                        var newCropId = lastCropId + 1;                  
                        $('#ORGCropID').val(newCropId);
                        ResetForm();  
                    }else{
                        alert_float('warning', 'Something went wrong...'); 
                    }
...
<script>
    ...
    {
        $('#CropName').val(''); 
        var ORGCropID = $('#ORGCropID').val();  
        $('#CropID').val(ORGCropID);   
        $('.saveBtn').show();
        $('.updateBtn').hide();
        $('.saveBtn2').show();
        $('.updateBtn2').hide();
...
<script>
    function myFunction2() 
    {
        ...
        table = document.getElementById("table_Crop_List");
...
<style>	
    #table_Crop_List td:hover {
        cursor: pointer;
    }
    #table_Crop_List tr:hover {
        background-color: #ccc;
    }
    .table_Crop_List          { overflow: auto;max-height: 65vh;width:100%;position:relative;top: 0px; }
    .table_Crop_List thead th { position: sticky; top: 0; z-index: 1; }
    .table_Crop_List tbody th { position: sticky; left: 0; }
