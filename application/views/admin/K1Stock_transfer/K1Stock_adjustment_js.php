		...
		if(changes !== null){
			...
			{			 
				...
				{   
					if(newValue == null)
					{
						...
						if(CenterID == ""){
							...
							count++;					
							$.post(admin_url + 'KirtiOneOrder/GetItemDetailsForDirectSale/'+newValue+ '/'+CenterID+ '/'+AccountID).done(function(response){
				...
				}else if(prop == 'BatchNo')
				{
					...
					{
						$.post(admin_url + 'KirtiOneOrder/GetItemBatchStock/',{ItemID:itemID,BatchID:newValue,CenterID : CenterID}).done(function(response){
							response = JSON.parse(response);
