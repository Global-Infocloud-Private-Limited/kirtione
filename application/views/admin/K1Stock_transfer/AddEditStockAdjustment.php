				...
				<div class="panel_s accounting-template estimate">
							...
							<div class="panel-body">
								<nav aria-label="breadcrumb" >
									<ol class="breadcrumb custombreadcrumb" style="background-color:#fff !important; margin-Bottom:0px !important;">
										...
										<li class="breadcrumb-item active" aria-current="page"><b>Kirti One Stock Adjustment</b></li>
									</ol>
								</nav>
								<hr class="hr_style">
								<div class="tab-content">
									<div role="tabpanel" class="tab-pane active" id="general_infor">
										<div class="row">
											<div class="col-md-2">
												<?php
													...
													if(isset($stock_details)){
														$AdjNumber = substr($stock_details->AdjustmentID,5);
														$isedit = "Y";
														}else{
														$new_stockadjustmentNumbar = get_option('next_K1Stockadjustment_number_for_kirti');
														$new_stockadjustmentNumbar = '1' . $new_stockadjustmentNumbar;
														$__number = $new_stockadjustmentNumbar;
														$AdjNumber = str_pad($__number, get_option('number_padding_prefixes'), '0', STR_PAD_LEFT);
														$isedit = '';
													}
												?>
												<div class="form-group">
													<label for="transferid">Adjustment No.</label>
													<div class="input-group">
														<span class="input-group-addon">
															<?php echo $prefix; ?>
														</span>
														<input type="text" name="transferid" id="transferid" class="form-control receiptsid" value="<?php echo $AdjNumber; ?>" data-isedit="<?php echo $isedit; ?>" data-original-number="<?php echo $stock_details->AdjustmentID; ?>" <?php echo ($isedit) ? 'disabled' : '' ?>>
													</div>
												</div>
											</div>
											<div class="col-md-2">
												...
												?>
												<?php
													$value = (isset($stock_details) ? _d(substr($stock_details->AdjustmentDate,0,10)) : _d($date));
													echo render_date_input('trf_date','Adjustment Date',$value);
												?>
											</div>
											<div class="col-md-2">
												<?php $value = (isset($stock_details) ? $stock_details->CenterID : ''); ?>
											...
											<div class="col-md-2">
												<div class="form-group" app-field-wrapper="AdjType">
													<?php $value = (isset($stock_details) ? $stock_details->Type : ''); ?>
													<label for="AdjType" class="control-label">Adjustment Type</label>
													<select name="AdjType" id="AdjType" class="selectpicker form-control" data-none-selected-text="Non Selected" data-live-search="true">
														<option value=""></option>
														<option value="MARKETING" <?php echo ($value == "MARKETING") ? 'selected' : ''; ?>>Marketing</option>
														<option value="FREE DISTRIBUTION" <?php echo ($value == "FREE DISTRIBUTION") ? 'selected' : ''; ?>>Free Distribution</option>
														<option value="STOCK ADJUSTMENT" <?php echo ($value == "STOCK ADJUSTMENT") ? 'selected' : ''; ?>>Stock Adjustment</option>
													</select>
												</div>
											</div>
										</div>
					...
					<div class="panel-body mtop10">
						<div class="row col-md-12">
							<p class="bold p_style">Stock Adjustment Detail</p>
							<hr class="hr_style"/>
						...
						<div class="col-md-12 mtop15">
							<div class="panel-body bottom-transaction">
								...
								<div class="btn-bottom-toolbar text-right" style="width: 100%; display: flex; justify-content: flex-end; align-items: center;">
									...
									<?php if (has_permission_new('KirtiOneStockAdjustment', '', 'create')){
									?>
							...
							<table class="tree table table-striped table-bordered table_purchase_report" id="table_purchase_report" width="100%">
								<thead>
									...
									<tr>
										...
										<th style="width:8% ">Adj Date</th>
										<th style="width:15% text-align:left;">Center Name</th>
...
<script type="text/javascript" language="javascript" >
	...
	$(document).ready(function(){
		...
		var regex = /\/AddEditStockAdjustment\/([^\/?#]+)/;
		if (url.match(regex)) {
		...
		{
			$.ajax({
				url:"<?php echo admin_url(); ?>K1Stock_transfer/load_data_for_stockadjustment",
				method:"POST",
		...
		$('.cancel-new-order').on('click', function()
		{
			...
			var AdjId = segments[segments.length - 1].split('#')[0];
			var userConfirmed = confirm("Are you sure you want to cancel the stock adjustment?");
			if (userConfirmed)
			{
				$.ajax({
					url: "<?php echo admin_url(); ?>K1Stock_transfer/CancelItemsStockAdjOrderWise",
					type: 'POST',
					data: {AdjId:AdjId},
					dataType: 'json',
...
<script>
    $(document).ready(function()
	{
        $('#resetbutton').on('click', function(e) {
            e.preventDefault();
			window.location.href = '<?php echo admin_url(); ?>K1Stock_transfer/AddEditStockAdjustment';
		});
	});
</script>
<script>
...
<?php require 'K1Stock_adjustment_js.php';?>
